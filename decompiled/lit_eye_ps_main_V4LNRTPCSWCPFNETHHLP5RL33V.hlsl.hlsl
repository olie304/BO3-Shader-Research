// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:35:50 2021

cbuffer _Globals : register(b0)
{
  float4 flagParams : packoffset(c0);
  float4 colorObjMin : packoffset(c1);
  float4 colorObjMax : packoffset(c2);
  float colorObjMinBaseBlend : packoffset(c3);
  float colorObjMaxBaseBlend : packoffset(c3.y);
  float2 uvScroll : packoffset(c3.z);
  float4 detailScale : packoffset(c4);
  float4 detailScale1 : packoffset(c5);
  float4 detailScale2 : packoffset(c6);
  float4 detailScale3 : packoffset(c7);
  float4 alphaRevealParms : packoffset(c8);
  float4 colorDetailScale : packoffset(c9);
  float3 specColorTint : packoffset(c10);
  float baseNormalHeight : packoffset(c10.w);
  float2 glossRange : packoffset(c11);
  float detailScaleHeight : packoffset(c11.z);
  float eyeIrisScale : packoffset(c11.w);
  float eyeLimbusScale : packoffset(c12);
  float eyeRedness : packoffset(c12.y);
  float eyePupilDilation : packoffset(c12.z);
}

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

cbuffer LightingGlobals : register(b2)
{
  uint numLights : packoffset(c0);
  uint numRefProbes : packoffset(c0.y);
  uint numRefProbeBlends : packoffset(c0.z);
  uint numDedicatedLights : packoffset(c0.w);
  float4 clearColor : packoffset(c1);
  float4 sssParams : packoffset(c2);

  struct
  {
    float4 fogColor;
    float4 sunFogColor;
    float K0;
    float skyK0;
    float expMul;
    float expAdd;
    float heightFalloff;
    float skyHeightFalloff;
    float K0b;
    float padding0;
    float skyK0b;
    float3 wldSunFogDir;
    float2 sunFogAngles;
    float atmospheresunstrength;
    float atmosphereMieSchlickK;
    float2 atmosphereskyfogdensityatcamera;
    float atmosphereExtinctionIntensity;
    float atmosphereInScatterIntensity;
    float3 atmosphereRayleighDensity;
    float atmospherehazebasedist;
    float3 atmosphereMieDensity;
    float atmospherehazefadedist;
    float3 atmosphereTotalDensity;
    float worldfogskysize;
    float3 atmosphereInScatterIntensityOverTotalDensity;
    float blendAmount;
    float2 atmosphereskyfogheightdensityscale;
    float2 atmospherefogdistanceoffset;
    float2 atmospherefogdistancedensityscale;
    float2 atmospherefogheightdensityscale;
    float2 atmospherefogdensityatcamera;
    float2 padding1;
  } fogConstants : packoffset(c3);


  struct
  {
    float3 wldDir;
    float splitDepthOffset;
    float3 color;
    float specScale;
    float globalProbeExposure;
    float3 avgGlobalProbeColor;
    float4 splitPinTransform[3];
    uint sunCookieIndex;
    float sunCookieIntensity;
    float sunVolumetricCookieIntensity;
    uint toolsGfxDisableSunShadow;
    float4 sunCookieTransform[2];
    float intensity;
    int splitArrayOffset;
    float2 pad0;

    struct
    {

      struct
      {
        float2 dimensionInTiles;
        float inchesPerTexel;
        float spanInInches;
      } constants;

      row_major float4x4 offToPinTransform;
      float coordScale;
      uint rootOffset;
      uint2 pad0;
    } sstLightingConstants;

  } sunConstants : packoffset(c17);


  struct
  {
    float2 sstToMinMaxScale;
    float2 pad0;
    float2 halfMinTexelSize;
    float rcpInchesDimLevel0;
    float shadowBiasInches;
  } minMaxConstants : packoffset(c33);


  struct
  {
    uint4 data[3];
  } globalProbeConstants : packoffset(c35);


  struct
  {
    float rain;
    float windSpeed;
    float windDirSin;
    float windDirCos;
    float weatherTile;
    float3 weatherVector;
    float3 weatherVector2;
    float padding0;
    float3 weatherTint;
    float padding1;
    float3 weatherTint2;
    float padding2;
  } weather : packoffset(c38);


  struct
  {

    struct
    {
      float2 dimensionInTiles;
      float inchesPerTexel;
      float spanInInches;
    } constants;

    row_major float4x4 offToPinTransform;
    float coordScale;
    uint rootOffset;
    uint2 pad0;
  } outdoorSSTConstants : packoffset(c43);

  float4 pinToWorldZ : packoffset(c49);

  struct
  {
    uint firstSpotLight;
    uint lastSpotLight;
    uint firstOmniLight;
    uint lastOmniLight;
    uint firstProbe;
    uint lastProbe;
    uint padding0;
    uint padding1;
  } volumetric : packoffset(c50);

  uint viewID : packoffset(c52);
  uint enableCountDebug : packoffset(c52.y);
  uint centerGroupIDx : packoffset(c52.z);
  uint centerGroupIDy : packoffset(c52.w);
  uint viewmodelSunShadowRaw : packoffset(c53);
  uint debugDrawOverlayType : packoffset(c53.y);
  float drawNumLighstScale : packoffset(c53.z);
  float drawNumLightsAlpha : packoffset(c53.w);
  uint probeDebug : packoffset(c54);
  float probeDebugRadius : packoffset(c54.y);
  uint showSunVis : packoffset(c54.z);
  uint enableAO : packoffset(c54.w);
  uint showAO : packoffset(c55);
  uint numGpuCullTilesWidth : packoffset(c55.y);
  uint numGpuCullTilesHeight : packoffset(c55.z);
  int permuteHighlight : packoffset(c55.w);
  uint overdrawOverlayMaxValue : packoffset(c56);
  float overdrawOverlayAlpha : packoffset(c56.y);
  uint overdrawDecals : packoffset(c56.z);
  uint overdrawDecalsLayers : packoffset(c56.w);
  uint permuteStride : packoffset(c57);
  uint numDecals : packoffset(c57.y);
  uint numStaticDecals : packoffset(c57.z);
  uint numDynamicDecals : packoffset(c57.w);
  uint numOverrideProbes : packoffset(c58);
  uint enableDitheredShadow : packoffset(c58.y);
  uint oitMaxEntries : packoffset(c58.z);
  uint numAttenuationVolumes : packoffset(c58.w);
}

SamplerState samp0_s : register(s0);
SamplerState colorSampler_s : register(s1);
SamplerState samplerLinear_s : register(s2);
SamplerState normalSampler_s : register(s3);
SamplerState samplerLinearClamp_s : register(s4);
SamplerState LinearSampler_s : register(s5);
SamplerComparisonState ShadowSamplerComparisonState_s : register(s13);
StructuredBuffer<shaderConstantSetBuffer> shaderConstantSetBuffer : register(t0);
StructuredBuffer<modelInstanceBuffer> modelInstanceBuffer : register(t4);
Texture2D<float2> gEnvBRDFGeneric : register(t5);
Texture2DArray<uint> visibleLights : register(t6);
Texture2DArray<uint> visibleProbes : register(t7);
TextureCubeArray<float3> gReflectionProbeArray : register(t8);
Texture2DArray<float> gSpotShadowmapArray : register(t9);
TextureCubeArray<float> gOmniShadowmapArray : register(t10);
StructuredBuffer<gCullConstants> gCullConstants : register(t11);
StructuredBuffer<lightConstants> lightConstants : register(t12);
Texture2DArray<float> gSunShadowmapArray : register(t13);
StructuredBuffer<refProbeConstants> refProbeConstants : register(t15);
StructuredBuffer<refProbeAttenuationConstants> refProbeAttenuationConstants : register(t16);
Texture3D<float3> gProbeXArray : register(t17);
Texture3D<float3> gProbeYArray : register(t18);
Texture3D<float3> gProbeZArray : register(t19);
Texture2DArray<float4> gCookieArray : register(t20);
Texture2D<float4> floatZSampler : register(t21);
Texture2D<float4> colorMap : register(t22);
StructuredBuffer<gSunShadowTree> gSunShadowTree : register(t23);
Texture2D<float4> detailMap : register(t24);
Texture2D<float4> causticMap : register(t25);
Texture2DArray<float> gTransShadowmapArray : register(t31);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float4 v0 : SV_POSITION0,
  float3 v1 : COLOR0,
  float w1 : COLOR1,
  float4 v2 : TEXCOORD0,
  nointerpolation uint v3 : TEXCOORD4,
  nointerpolation float3 w3 : TEXCOORD9,
  float4 v4 : TEXCOORD1,
  float4 v5 : TEXCOORD2,
  float3 v6 : TEXCOORD3,
  float3 v7 : OFFPOSITION0,
  nointerpolation float4 v8 : TEXCOORD10,
  nointerpolation float3 v9 : TEXCOORD11,
  uint v10 : SV_IsFrontFace0,
  out float3 o0 : SV_TARGET0,
  out float3 o1 : SV_TARGET1,
  out float4 o2 : SV_TARGET2)
{
  const float4 icb[] = { { -0.808114, 0.808114, 0, 0},
                              { 0, -1.000000, 0, 0},
                              { 0.606057, 0.606057, 0, 0},
                              { -0.714286, 0, 0, 0},
                              { 0.404114, -0.404114, 0, 0},
                              { 0, 0.428571, 0, 0},
                              { -0.202057, -0.202057, 0, 0},
                              { 0.142857, 0, 0, 0} };
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t0, 384
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t4, 24
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t11, 80
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t12, 240
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t15, 224
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t16, 96
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t23, 4
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15,r16,r17,r18,r19,r20,r21,r22,r23,r24,r25,r26,r27,r28,r29,r30,r31,r32,r33,r34,r35,r36,r37,r38,r39,r40,r41,r42,r43,r44,r45,r46,r47,r48,r49,r50,r51,r52,r53,r54,r55,r56,r57,r58,r59,r60;
  uint4 bitmask, uiDest;
  float4 fDest;

  float4 x0[4];
  float4 x1[4];
  float4 x2[4];
  float4 x3[4];
  float4 x4[4];
  float4 x5[4];
  r0.x = dot(v4.xyz, v4.xyz);
  r0.x = rsqrt(r0.x);
  r0.xyz = v4.xyz * r0.xxx;
  r0.w = dot(v5.xyz, v5.xyz);
  r0.w = rsqrt(r0.w);
  r1.xyz = v5.xyz * r0.www;
  r0.w = dot(-v7.xyz, -v7.xyz);
  r0.w = rsqrt(r0.w);
  r2.xyz = -v7.xyz * r0.www;
  r0.w = dot(v6.xyz, v6.xyz);
  r0.w = rsqrt(r0.w);
  r3.xyz = v6.xyz * r0.www;
  r0.w = rcp(eyeIrisScale);
  r4.xy = float2(-0.5,-0.5) + v2.xy;
  r1.w = dot(r4.xy, r4.xy);
  r1.w = sqrt(r1.w);
  r2.w = max(9.99999975e-06, r1.w);
  r2.w = rcp(r2.w);
  r4.xy = r4.xy * r2.ww;
  r0.w = r1.w * r0.w;
  r1.w = -eyeIrisScale * 0.5 + r1.w;
  r2.w = -eyeIrisScale * 0.5 + 0.5;
  r1.w = r1.w / r2.w;
  r2.w = cmp(r1.w < 0);
  r0.w = 0.439999998 * r0.w;
  r1.w = r1.w * 0.280000001 + 0.219999999;
  r0.w = r2.w ? r0.w : r1.w;
  r4.zw = r4.xy * r0.ww + float2(0.5,0.5);
  r1.w = eyeLimbusScale * 0.0500000007 + 0.0199999996;
  r2.w = r1.w + r0.w;
  r2.w = -0.200000003 + r2.w;
  r2.w = r2.w / r1.w;
  r2.w = -r2.w * 10 + 5;
  r2.w = 1.44269502 * r2.w;
  r2.w = exp2(r2.w);
  r2.w = 1 + r2.w;
  r2.w = 1.01357996 / r2.w;
  r2.w = saturate(-0.00669999979 + r2.w);
  r5.x = 1 + -r2.w;
  r2.w = dot(w3.xyz, w3.xyz);
  r2.w = rsqrt(r2.w);
  r6.xyz = w3.xyz * r2.www;
  r3.w = r0.w * r0.w;
  r3.w = saturate(-r3.w * 18.3999996 + 1);
  r3.w = 0.101000004 * r3.w;
  r5.w = dot(r0.xyz, r2.xyz);
  r7.xy = r5.ww * float2(0.726957142,0.726957142) + float2(-0.726957142,0.726957142);
  r6.w = r7.x * r7.y + 1;
  r6.w = sqrt(r6.w);
  r5.w = r5.w * 0.726957142 + -r6.w;
  r7.xyz = float3(0.726957142,0.726957142,0.726957142) * r2.xyz;
  r7.xyz = r5.www * r0.xyz + -r7.xyz;
  r5.w = dot(r6.xyz, -r7.xyz);
  r5.w = r3.w / r5.w;
  r7.xyz = r5.www * r7.xyz;
  r8.x = dot(r7.xyz, v8.xyz);
  r8.y = dot(r7.xyz, v9.xyz);
  r5.y = 0.741839767;
  r7.xy = r8.xy * r5.xy;
  r5.z = -r5.x;
  r5.yz = r5.yz * r7.xy + r4.zw;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r5.w, v3.x, l(4), t4.xxxx
r5.w = samp0[]..swiz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=384)(mixed,mixed,mixed,mixed) r5.w, r5.w, l(0), t0.xxxx
r5.w = samp0[]..swiz;
  r5.w = saturate(r5.w);
  r5.w = r5.w * 0.499000013 + 0.00100000005;
  r6.w = rcp(r5.w);
  r7.xy = float2(-0.5,-0.5) + r5.yz;
  r7.z = dot(r7.xy, r7.xy);
  r7.z = sqrt(r7.z);
  r7.w = max(9.99999975e-06, r7.z);
  r7.w = rcp(r7.w);
  r7.xy = r7.xy * r7.ww;
  r6.w = r7.z * r6.w;
  r7.z = -r5.w * 0.219999999 + r7.z;
  r5.w = -r5.w * 0.219999999 + 0.219999999;
  r5.w = r7.z / r5.w;
  r7.z = cmp(r5.w < 0);
  r6.w = 0.181818172 * r6.w;
  r5.w = r5.w * 0.180000007 + 0.0399999991;
  r5.w = r7.z ? r6.w : r5.w;
  r7.xy = r7.xy * r5.ww + float2(0.5,0.5);
  r4.xy = r1.ww * r4.xy + r4.zw;
  r4.xyz = colorMap.Sample(colorSampler_s, r4.xy).xyz;
  r8.xyz = colorMap.Sample(colorSampler_s, r7.xy).xyz;
  r0.w = eyeRedness * r0.w;
  r7.zw = float2(1.22000003,3.33333325) * r0.ww;
  r9.xyz = r7.zzz * r4.xyz;
  r4.xyw = r9.xyz * float3(0.299999952,-0.949999988,-0.949999988) + r4.xyz;
  r9.xyz = r4.xyw * r7.www;
  r9.xyz = float3(-0.300000012,-0.699999988,-0.699999988) * r9.xyz;
  r0.w = saturate(-r4.z * 2 + 1);
  r4.xyz = saturate(r0.www * r9.xyz + r4.xyw);
  r8.xyz = r8.xyz + -r4.xyz;
  o2.xyz = r5.xxx * r8.xyz + r4.xyz;
  o2.w = r5.x * -0.300000012 + 0.5;
  r4.xy = detailScale.xy * v2.xy;
  r4.xyz = detailMap.Sample(normalSampler_s, r4.xy).xyz;
  r8.xy = r4.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r0.w = dot(r8.xy, r8.xy);
  r0.w = 1 + -r0.w;
  r0.w = max(0, r0.w);
  r8.z = sqrt(r0.w);
  r0.w = r4.z * r4.z;
  r0.w = 0.333333343 * r0.w;
  r8.w = min(1, r0.w);
  r4.xyzw = float4(0,0,1,0) + -r8.xyzw;
  r4.xyzw = r5.xxxx * r4.xyzw + r8.xyzw;
  r0.w = dot(r4.xyzw, r4.xyzw);
  r0.w = rsqrt(r0.w);
  r4.xyz = r4.xyz * r0.www;
  r3.xyz = r4.yyy * r3.xyz;
  r1.xyz = r4.xxx * r1.xyz + r3.xyz;
  r1.xyz = r4.zzz * r0.xyz + r1.xyz;
  r0.w = r4.w * r0.w + 0.000471648847;
  r0.w = log2(r0.w);
  r0.w = saturate(-0.0588235296 * r0.w);
  r3.xyz = viewMatrix._m10_m11_m12 * r0.yyy;
  r3.xyz = r0.xxx * viewMatrix._m00_m01_m02 + r3.xyz;
  r3.xyz = r0.zzz * viewMatrix._m20_m21_m22 + r3.xyz;
  r1.w = dot(r3.xyz, r3.xyz);
  r1.w = rsqrt(r1.w);
  r3.xyz = r3.xyz * r1.www;
  r4.xyzw = renderTargetSize.zwzw * v0.xyxy;
  r7.zw = viewSpaceScaleBias.xy * r4.zw + -viewSpaceScaleBias.zw;
  r8.xy = v0.ww * r7.zw;
  r1.w = dot(v0.xy, float2(0.0671105608,0.00583714992));
  r1.w = frac(r1.w);
  r1.w = 52.9829178 * r1.w;
  r1.w = frac(r1.w);
  r5.w = 3.14159989 * r1.w;
  r5.w = gameTick.w * 0.5 + r5.w;
  sincos(r5.w, r9.x, r10.x);
  r11.xyzw = float4(4.16666698,4.16666698,16.6666679,16.6666679) * renderTargetSize.zwzw;
  r10.y = r9.x;
  r12.xyzw = r11.xyzw * r10.xyxy + r4.zwzw;
  r13.xyzw = renderTargetSize.xyxy * r12.xyzw;
  r14.xy = (int2)r13.xy;
  r14.zw = float2(0,0);
  r6.w = floatZSampler.Load(r14.xyz).x;
  r7.z = cmp(r6.w >= 0.984375);
  r7.w = 1.01587307 * r6.w;
  r6.w = r6.w * 64 + -63;
  r6.w = r7.z ? r6.w : r7.w;
  r6.w = max(9.99999994e-09, r6.w);
  r14.z = rcp(r6.w);
  r12.xyzw = viewSpaceScaleBias.xyxy * r12.xyzw + -viewSpaceScaleBias.zwzw;
  r14.xy = r14.zz * r12.xy;
  r8.z = v0.w;
  r9.yzw = r14.xyz + -r8.xyz;
  r6.w = dot(r9.yzw, r9.yzw);
  r6.w = rsqrt(r6.w);
  r6.w = -r9.w * r6.w;
  r6.w = max(-1, r6.w);
  r13.xy = (int2)r13.zw;
  r13.zw = float2(0,0);
  r7.z = floatZSampler.Load(r13.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r13.z = rcp(r7.z);
  r13.xy = r13.zz * r12.zw;
  r9.yzw = r13.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r6.w = max(r7.z, r6.w);
  r12.xyzw = float4(37.5,37.5,66.6666718,66.6666718) * renderTargetSize.zwzw;
  r13.xyzw = r12.xyzw * r10.xyxy + r4.zwzw;
  r14.xyzw = renderTargetSize.xyxy * r13.xyzw;
  r15.xy = (int2)r14.xy;
  r15.zw = float2(0,0);
  r7.z = floatZSampler.Load(r15.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r15.z = rcp(r7.z);
  r13.xyzw = viewSpaceScaleBias.xyxy * r13.xyzw + -viewSpaceScaleBias.zwzw;
  r15.xy = r15.zz * r13.xy;
  r9.yzw = r15.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r6.w = max(r7.z, r6.w);
  r14.xy = (int2)r14.zw;
  r14.zw = float2(0,0);
  r7.z = floatZSampler.Load(r14.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r14.z = rcp(r7.z);
  r14.xy = r14.zz * r13.zw;
  r9.yzw = r14.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r6.w = max(r7.z, r6.w);
  r13.xyzw = float4(104.166664,104.166664,150,150) * renderTargetSize.zwzw;
  r14.xyzw = r13.xyzw * r10.xyxy + r4.zwzw;
  r15.xyzw = renderTargetSize.xyxy * r14.xyzw;
  r16.xy = (int2)r15.xy;
  r16.zw = float2(0,0);
  r7.z = floatZSampler.Load(r16.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r16.z = rcp(r7.z);
  r14.xyzw = viewSpaceScaleBias.xyxy * r14.xyzw + -viewSpaceScaleBias.zwzw;
  r16.xy = r16.zz * r14.xy;
  r9.yzw = r16.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r6.w = max(r7.z, r6.w);
  r15.xy = (int2)r15.zw;
  r15.zw = float2(0,0);
  r7.z = floatZSampler.Load(r15.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r15.z = rcp(r7.z);
  r15.xy = r15.zz * r14.zw;
  r9.yzw = r15.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r6.w = max(r7.z, r6.w);
  r11.xyzw = r11.xyzw * -r10.xyxy + r4.zwzw;
  r14.xyzw = renderTargetSize.xyxy * r11.xyzw;
  r14.xyzw = (int4)r14.zwxy;
  r15.xy = r14.zw;
  r15.zw = float2(0,0);
  r7.z = floatZSampler.Load(r15.xyz).x;
  r7.w = cmp(r7.z >= 0.984375);
  r8.w = 1.01587307 * r7.z;
  r7.z = r7.z * 64 + -63;
  r7.z = r7.w ? r7.z : r8.w;
  r7.z = max(9.99999994e-09, r7.z);
  r15.z = rcp(r7.z);
  r11.xyzw = viewSpaceScaleBias.xyxy * r11.xyzw + -viewSpaceScaleBias.zwzw;
  r15.xy = r15.zz * r11.xy;
  r9.yzw = r15.xyz + -r8.xyz;
  r7.z = dot(r9.yzw, r9.yzw);
  r7.z = rsqrt(r7.z);
  r7.z = -r9.w * r7.z;
  r7.z = max(-1, r7.z);
  r14.zw = float2(0,0);
  r7.w = floatZSampler.Load(r14.xyz).x;
  r8.w = cmp(r7.w >= 0.984375);
  r9.y = 1.01587307 * r7.w;
  r7.w = r7.w * 64 + -63;
  r7.w = r8.w ? r7.w : r9.y;
  r7.w = max(9.99999994e-09, r7.w);
  r14.z = rcp(r7.w);
  r14.xy = r14.zz * r11.zw;
  r9.yzw = r14.xyz + -r8.xyz;
  r7.w = dot(r9.yzw, r9.yzw);
  r7.w = rsqrt(r7.w);
  r7.w = -r9.w * r7.w;
  r7.z = max(r7.z, r7.w);
  r11.xyzw = r12.xyzw * -r10.xyxy + r4.zwzw;
  r12.xyzw = renderTargetSize.xyxy * r11.xyzw;
  r12.xyzw = (int4)r12.zwxy;
  r14.xy = r12.zw;
  r14.zw = float2(0,0);
  r7.w = floatZSampler.Load(r14.xyz).x;
  r8.w = cmp(r7.w >= 0.984375);
  r9.y = 1.01587307 * r7.w;
  r7.w = r7.w * 64 + -63;
  r7.w = r8.w ? r7.w : r9.y;
  r7.w = max(9.99999994e-09, r7.w);
  r14.z = rcp(r7.w);
  r11.xyzw = viewSpaceScaleBias.xyxy * r11.xyzw + -viewSpaceScaleBias.zwzw;
  r14.xy = r14.zz * r11.xy;
  r9.yzw = r14.xyz + -r8.xyz;
  r7.w = dot(r9.yzw, r9.yzw);
  r7.w = rsqrt(r7.w);
  r7.w = -r9.w * r7.w;
  r7.z = max(r7.z, r7.w);
  r12.zw = float2(0,0);
  r7.w = floatZSampler.Load(r12.xyz).x;
  r8.w = cmp(r7.w >= 0.984375);
  r9.y = 1.01587307 * r7.w;
  r7.w = r7.w * 64 + -63;
  r7.w = r8.w ? r7.w : r9.y;
  r7.w = max(9.99999994e-09, r7.w);
  r12.z = rcp(r7.w);
  r12.xy = r12.zz * r11.zw;
  r9.yzw = r12.xyz + -r8.xyz;
  r7.w = dot(r9.yzw, r9.yzw);
  r7.w = rsqrt(r7.w);
  r7.w = -r9.w * r7.w;
  r7.z = max(r7.z, r7.w);
  r4.xyzw = r13.xyzw * -r10.xyxy + r4.xyzw;
  r11.xyzw = renderTargetSize.xyxy * r4.xyzw;
  r11.xyzw = (int4)r11.zwxy;
  r12.xy = r11.zw;
  r12.zw = float2(0,0);
  r7.w = floatZSampler.Load(r12.xyz).x;
  r8.w = cmp(r7.w >= 0.984375);
  r9.y = 1.01587307 * r7.w;
  r7.w = r7.w * 64 + -63;
  r7.w = r8.w ? r7.w : r9.y;
  r7.w = max(9.99999994e-09, r7.w);
  r12.z = rcp(r7.w);
  r4.xyzw = viewSpaceScaleBias.xyxy * r4.xyzw + -viewSpaceScaleBias.zwzw;
  r12.xy = r12.zz * r4.xy;
  r9.yzw = r12.xyz + -r8.xyz;
  r4.x = dot(r9.yzw, r9.yzw);
  r4.x = rsqrt(r4.x);
  r4.x = -r9.w * r4.x;
  r4.x = max(r7.z, r4.x);
  r11.zw = float2(0,0);
  r4.y = floatZSampler.Load(r11.xyz).x;
  r7.z = cmp(r4.y >= 0.984375);
  r7.w = 1.01587307 * r4.y;
  r4.y = r4.y * 64 + -63;
  r4.y = r7.z ? r4.y : r7.w;
  r4.y = max(9.99999994e-09, r4.y);
  r11.z = rcp(r4.y);
  r11.xy = r11.zz * r4.zw;
  r4.yzw = r11.xyz + -r8.xyz;
  r4.y = dot(r4.yzw, r4.yzw);
  r4.y = rsqrt(r4.y);
  r4.y = -r4.w * r4.y;
  r4.x = max(r4.x, r4.y);
  r4.yz = viewSpaceScaleBias.xy * r10.xy;
  r4.w = dot(r4.yz, r4.yz);
  r4.w = rsqrt(r4.w);
  r8.xy = r4.yz * r4.ww;
  r8.zw = -r8.yy;
  r4.y = dot(r3.yx, r8.xz);
  r3.xy = -r8.wx * r4.yy + r3.xy;
  r4.y = dot(r3.xyz, r3.xyz);
  r4.y = rsqrt(r4.y);
  r3.xyz = r4.yyy * r3.zxy;
  r3.y = dot(r8.xy, r3.yz);
  r3.y = cmp(r3.y >= 0);
  r3.y = r3.y ? -1 : 1;
  r3.x = saturate(-r3.x);
  r3.z = 1 + -r3.x;
  r3.z = sqrt(r3.z);
  r4.y = r3.x * -0.0187292993 + 0.0742610022;
  r4.y = r4.y * r3.x + -0.212114394;
  r3.x = r4.y * r3.x + 1.57072878;
  r3.x = r3.x * r3.z;
  r3.z = r3.y * r3.x;
  r4.y = 1 + -abs(r6.w);
  r4.y = sqrt(r4.y);
  r4.z = abs(r6.w) * -0.0187292993 + 0.0742610022;
  r4.z = r4.z * abs(r6.w) + -0.212114394;
  r4.z = r4.z * abs(r6.w) + 1.57072878;
  r4.w = r4.z * r4.y;
  r4.w = r4.w * -2 + 3.14159274;
  r6.w = cmp(r6.w < -r6.w);
  r4.w = r6.w ? r4.w : 0;
  r4.y = r4.z * r4.y + r4.w;
  r4.z = 1 + -abs(r4.x);
  r4.z = sqrt(r4.z);
  r4.w = abs(r4.x) * -0.0187292993 + 0.0742610022;
  r4.w = r4.w * abs(r4.x) + -0.212114394;
  r4.w = r4.w * abs(r4.x) + 1.57072878;
  r6.w = r4.w * r4.z;
  r6.w = r6.w * -2 + 3.14159274;
  r4.x = cmp(r4.x < -r4.x);
  r4.x = r4.x ? r6.w : 0;
  r4.x = r4.w * r4.z + r4.x;
  r4.y = -r3.y * r3.x + -r4.y;
  r4.y = max(-1.57079995, r4.y);
  r4.y = r3.y * r3.x + r4.y;
  r4.x = -r3.y * r3.x + r4.x;
  r4.x = min(1.57079995, r4.x);
  r4.x = r3.y * r3.x + r4.x;
  sincos(r3.z, r8.x, r11.x);
  r4.xy = r4.xy + r4.xy;
  r4.z = r4.y * r8.x + r11.x;
  r4.y = -r3.y * r3.x + r4.y;
  r4.y = cos(r4.y);
  r4.y = r4.z + -r4.y;
  r4.z = r4.x * r8.x + r11.x;
  r3.x = -r3.y * r3.x + r4.x;
  r3.x = cos(r3.x);
  r3.x = r4.z + -r3.x;
  r3.x = 0.25 * r3.x;
  r3.x = r4.y * 0.25 + r3.x;
  r3.y = r3.z * r8.x + r11.x;
  r3.x = r3.x / r3.y;
  r3.x = log2(abs(r3.x));
  r3.xyz = float3(0.600000024,0.850000024,0.870000005) * r3.xxx;
  r3.xyz = exp2(r3.xyz);
  r4.xy = r7.xy + r5.yz;
  r7.xyz = -eyeOffset.yyy * viewProjectionMatrix._m10_m11_m13;
  r7.xyz = -eyeOffset.xxx * viewProjectionMatrix._m00_m01_m03 + r7.xyz;
  r7.xyz = -eyeOffset.zzz * viewProjectionMatrix._m20_m21_m23 + r7.xyz;
  r7.xyz = viewProjectionMatrix._m30_m31_m33 + r7.xyz;
  r8.xyz = eyeOffset.xyz + v7.xyz;
  r4.z = dot(-r2.xyz, r0.xyz);
  r4.z = r4.z + r4.z;
  r2.xyz = r0.xyz * -r4.zzz + -r2.xyz;
  r4.z = dot(r2.xyz, r2.xyz);
  r4.z = rsqrt(r4.z);
  r2.xyz = r4.zzz * r2.xyz;
  r9.yzw = viewProjectionMatrix._m10_m11_m13 * r8.yyy;
  r9.yzw = r8.xxx * viewProjectionMatrix._m00_m01_m03 + r9.yzw;
  r9.yzw = r8.zzz * viewProjectionMatrix._m20_m21_m23 + r9.yzw;
  r9.yzw = r9.yzw + r7.xyz;
  r11.z = rcp(r9.w);
  r4.zw = r11.zz * r9.yz;
  r11.xy = r4.zw * float2(0.5,-0.5) + float2(0.5,0.5);
  r2.xyz = r2.xyz * float3(2.4000001,2.4000001,2.4000001) + r8.xyz;
  r8.xyz = viewProjectionMatrix._m10_m11_m13 * r2.yyy;
  r8.xyz = r2.xxx * viewProjectionMatrix._m00_m01_m03 + r8.xyz;
  r2.xyz = r2.zzz * viewProjectionMatrix._m20_m21_m23 + r8.xyz;
  r2.xyz = r2.xyz + r7.xyz;
  r2.xy = float2(0.5,-0.5) * r2.xy;
  r8.z = rcp(r2.z);
  r8.xy = r2.xy * r8.zz + float2(0.5,0.5);
  r2.xyz = r8.xyz + -r11.xyz;
  r4.z = ddx_coarse(r11.z);
  r4.w = ddy_coarse(r11.z);
  r8.xy = float2(-0.25,0.25) * r4.ww;
  r4.zw = r4.zz * float2(-0.25,0.25) + r8.xy;
  r8.xyz = ddx_coarse(r2.xyz);
  r10.yzw = ddy_coarse(r2.xyz);
  r12.xyz = float3(-0.25,-0.25,-0.25) * r10.yzw;
  r12.xyz = r8.xyz * float3(-0.25,-0.25,-0.25) + r12.xyz;
  r12.xyz = r12.xyz + r2.xyz;
  r13.xyzw = renderTargetSize.zwzw * float4(0.25,0.25,-0.25,-0.25) + r11.xyxy;
  r11.xy = r11.zz + r4.zw;
  r10.yzw = float3(0.25,0.25,0.25) * r10.yzw;
  r8.xyz = r8.xyz * float3(0.25,0.25,0.25) + r10.yzw;
  r2.xyz = r8.xyz + r2.xyz;
  r4.z = dot(renderTargetSize.zw, renderTargetSize.zw);
  r4.w = dot(r12.xy, r12.xy);
  r4.zw = sqrt(r4.zw);
  r4.w = rcp(r4.w);
  r4.w = r4.z * r4.w;
  x1[0].x = 0.00173611124;
  r11.zw = r13.zw;
  r8.xyz = r12.xyz * float3(0.00173611124,0.00173611124,0.00173611124) + r11.zwx;
  r9.yz = cmp(r4.ww < float2(0.00173611124,0.015625));
  r8.xy = renderTargetSize.xy * r8.xy;
  r14.xy = (int2)r8.xy;
  r14.zw = float2(0,0);
  r6.w = floatZSampler.Load(r14.xyz).x;
  r7.w = cmp(r6.w >= 0.984375);
  r8.x = 1.01587307 * r6.w;
  r6.w = r6.w * 64 + -63;
  r6.w = r7.w ? r6.w : r8.x;
  r6.w = max(9.99999994e-09, r6.w);
  r6.w = rcp(r6.w);
  x2[0].x = r6.w;
  r7.w = r8.z * r6.w;
  r7.w = cmp(1 < r7.w);
  r7.w = r7.w ? 1.000000 : 0;
  r7.w = r9.y ? r7.w : 1;
  x0[0].x = r7.w;
  x1[1].x = 0.015625;
  r13.z = r11.y;
  r8.xyz = r2.xyz * float3(0.015625,0.015625,0.015625) + r13.xyz;
  r8.xy = renderTargetSize.xy * r8.xy;
  r14.xy = (int2)r8.xy;
  r14.zw = float2(0,0);
  r8.x = floatZSampler.Load(r14.xyz).x;
  r8.y = cmp(r8.x >= 0.984375);
  r8.w = 1.01587307 * r8.x;
  r8.x = r8.x * 64 + -63;
  r8.x = r8.y ? r8.x : r8.w;
  r8.x = max(9.99999994e-09, r8.x);
  r8.x = rcp(r8.x);
  x2[1].x = r8.x;
  r8.y = r8.z * r8.x;
  r8.y = cmp(1 < r8.y);
  r8.y = r8.y ? 1.000000 : 0;
  r8.y = r9.z ? r8.y : 1;
  x0[1].x = r8.y;
  r8.z = r8.y + r7.w;
  r8.z = cmp(r8.z != 0.000000);
  if (r8.z != 0) {
    r8.z = r7.w * 0.0260416679 + 0.00173611124;
    x1[0].x = r8.z;
    r10.yzw = r12.xyz * r8.zzz + r11.zwx;
    r8.z = cmp(r4.w < r8.z);
    r9.yz = renderTargetSize.xy * r10.yz;
    r14.xy = (int2)r9.yz;
    r14.zw = float2(0,0);
    r8.w = floatZSampler.Load(r14.xyz).x;
    r9.y = cmp(r8.w >= 0.984375);
    r9.z = 1.01587307 * r8.w;
    r8.w = r8.w * 64 + -63;
    r8.w = r9.y ? r8.w : r9.z;
    r8.w = max(9.99999994e-09, r8.w);
    r8.w = rcp(r8.w);
    r6.w = r8.z ? r8.w : r6.w;
    x2[0].x = r6.w;
    r6.w = r10.w * r8.w;
    r6.w = cmp(1 < r6.w);
    r6.w = r6.w ? 1.000000 : 0;
    r6.w = r7.w * r6.w;
    r6.w = r8.z ? r6.w : r7.w;
    x0[0].x = r6.w;
    r7.w = r8.y * 0.046875 + 0.015625;
    x1[1].x = r7.w;
    r10.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.zw = renderTargetSize.xy * r10.yz;
    r14.xy = (int2)r8.zw;
    r14.zw = float2(0,0);
    r8.z = floatZSampler.Load(r14.xyz).x;
    r8.w = cmp(r8.z >= 0.984375);
    r9.y = 1.01587307 * r8.z;
    r8.z = r8.z * 64 + -63;
    r8.z = r8.w ? r8.z : r9.y;
    r8.z = max(9.99999994e-09, r8.z);
    r8.z = rcp(r8.z);
    r8.x = r7.w ? r8.z : r8.x;
    x2[1].x = r8.x;
    r8.x = r10.w * r8.z;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.y * r8.x;
    r7.w = r7.w ? r8.x : r8.y;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.0850694403 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    x1[0].x = r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r14.xy = (int2)r8.xy;
    r14.zw = float2(0,0);
    r8.x = floatZSampler.Load(r14.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r7.w = x1[1].x;
    r8.x = x0[1].x;
    r8.y = 0.140625 + -r7.w;
    r7.w = r8.x * r8.y + r7.w;
    x1[1].x = r7.w;
    r8.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.yz = renderTargetSize.xy * r8.yz;
    r14.xy = (int2)r8.yz;
    r14.zw = float2(0,0);
    r8.y = floatZSampler.Load(r14.xyz).x;
    r8.z = cmp(r8.y >= 0.984375);
    r9.y = 1.01587307 * r8.y;
    r8.y = r8.y * 64 + -63;
    r8.y = r8.z ? r8.y : r9.y;
    r8.y = max(9.99999994e-09, r8.y);
    r8.y = rcp(r8.y);
    r8.z = x2[1].x;
    r8.z = r7.w ? r8.y : r8.z;
    x2[1].x = r8.z;
    r8.y = r8.w * r8.y;
    r8.y = cmp(1 < r8.y);
    r8.y = r8.y ? 1.000000 : 0;
    r8.y = r8.x * r8.y;
    r7.w = r7.w ? r8.y : r8.x;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.173611104 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    x1[0].x = r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r14.xy = (int2)r8.xy;
    r14.zw = float2(0,0);
    r8.x = floatZSampler.Load(r14.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r7.w = x1[1].x;
    r8.x = x0[1].x;
    r8.y = 0.25 + -r7.w;
    r7.w = r8.x * r8.y + r7.w;
    x1[1].x = r7.w;
    r8.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.yz = renderTargetSize.xy * r8.yz;
    r14.xy = (int2)r8.yz;
    r14.zw = float2(0,0);
    r8.y = floatZSampler.Load(r14.xyz).x;
    r8.z = cmp(r8.y >= 0.984375);
    r9.y = 1.01587307 * r8.y;
    r8.y = r8.y * 64 + -63;
    r8.y = r8.z ? r8.y : r9.y;
    r8.y = max(9.99999994e-09, r8.y);
    r8.y = rcp(r8.y);
    r8.z = x2[1].x;
    r8.z = r7.w ? r8.y : r8.z;
    x2[1].x = r8.z;
    r8.y = r8.w * r8.y;
    r8.y = cmp(1 < r8.y);
    r8.y = r8.y ? 1.000000 : 0;
    r8.y = r8.x * r8.y;
    r7.w = r7.w ? r8.y : r8.x;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.293402791 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    x1[0].x = r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r14.xy = (int2)r8.xy;
    r14.zw = float2(0,0);
    r8.x = floatZSampler.Load(r14.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r7.w = x1[1].x;
    r8.x = x0[1].x;
    r8.y = 0.390625 + -r7.w;
    r7.w = r8.x * r8.y + r7.w;
    x1[1].x = r7.w;
    r8.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.yz = renderTargetSize.xy * r8.yz;
    r14.xy = (int2)r8.yz;
    r14.zw = float2(0,0);
    r8.y = floatZSampler.Load(r14.xyz).x;
    r8.z = cmp(r8.y >= 0.984375);
    r9.y = 1.01587307 * r8.y;
    r8.y = r8.y * 64 + -63;
    r8.y = r8.z ? r8.y : r9.y;
    r8.y = max(9.99999994e-09, r8.y);
    r8.y = rcp(r8.y);
    r8.z = x2[1].x;
    r8.z = r7.w ? r8.y : r8.z;
    x2[1].x = r8.z;
    r8.y = r8.w * r8.y;
    r8.y = cmp(1 < r8.y);
    r8.y = r8.y ? 1.000000 : 0;
    r8.y = r8.x * r8.y;
    r7.w = r7.w ? r8.y : r8.x;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.444444478 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    x1[0].x = r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r14.xy = (int2)r8.xy;
    r14.zw = float2(0,0);
    r8.x = floatZSampler.Load(r14.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r7.w = x1[1].x;
    r8.x = x0[1].x;
    r8.y = 0.5625 + -r7.w;
    r7.w = r8.x * r8.y + r7.w;
    x1[1].x = r7.w;
    r8.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.yz = renderTargetSize.xy * r8.yz;
    r14.xy = (int2)r8.yz;
    r14.zw = float2(0,0);
    r8.y = floatZSampler.Load(r14.xyz).x;
    r8.z = cmp(r8.y >= 0.984375);
    r9.y = 1.01587307 * r8.y;
    r8.y = r8.y * 64 + -63;
    r8.y = r8.z ? r8.y : r9.y;
    r8.y = max(9.99999994e-09, r8.y);
    r8.y = rcp(r8.y);
    r8.z = x2[1].x;
    r8.z = r7.w ? r8.y : r8.z;
    x2[1].x = r8.z;
    r8.y = r8.w * r8.y;
    r8.y = cmp(1 < r8.y);
    r8.y = r8.y ? 1.000000 : 0;
    r8.y = r8.x * r8.y;
    r7.w = r7.w ? r8.y : r8.x;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.626736164 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    x1[0].x = r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r14.xy = (int2)r8.xy;
    r14.zw = float2(0,0);
    r8.x = floatZSampler.Load(r14.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r7.w = x1[1].x;
    r8.x = x0[1].x;
    r8.y = 0.765625 + -r7.w;
    r7.w = r8.x * r8.y + r7.w;
    x1[1].x = r7.w;
    r8.yzw = r2.xyz * r7.www + r13.xyz;
    r7.w = cmp(r4.w < r7.w);
    r8.yz = renderTargetSize.xy * r8.yz;
    r14.xy = (int2)r8.yz;
    r14.zw = float2(0,0);
    r8.y = floatZSampler.Load(r14.xyz).x;
    r8.z = cmp(r8.y >= 0.984375);
    r9.y = 1.01587307 * r8.y;
    r8.y = r8.y * 64 + -63;
    r8.y = r8.z ? r8.y : r9.y;
    r8.y = max(9.99999994e-09, r8.y);
    r8.y = rcp(r8.y);
    r8.z = x2[1].x;
    r8.z = r7.w ? r8.y : r8.z;
    x2[1].x = r8.z;
    r8.y = r8.w * r8.y;
    r8.y = cmp(1 < r8.y);
    r8.y = r8.y ? 1.000000 : 0;
    r8.y = r8.x * r8.y;
    r7.w = r7.w ? r8.y : r8.x;
    x0[1].x = r7.w;
    r6.w = r7.w + r6.w;
    r6.w = cmp(r6.w == 0.000000);
  } else {
    r6.w = -1;
  }
  if (r6.w == 0) {
    r6.w = x1[0].x;
    r7.w = x0[0].x;
    r8.x = 0.840277791 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    r8.xyz = r12.xyz * r6.www + r11.zwx;
    r6.w = cmp(r4.w < r6.w);
    r8.xy = renderTargetSize.xy * r8.xy;
    r11.xy = (int2)r8.xy;
    r11.zw = float2(0,0);
    r8.x = floatZSampler.Load(r11.xyz).x;
    r8.y = cmp(r8.x >= 0.984375);
    r8.w = 1.01587307 * r8.x;
    r8.x = r8.x * 64 + -63;
    r8.x = r8.y ? r8.x : r8.w;
    r8.x = max(9.99999994e-09, r8.x);
    r8.x = rcp(r8.x);
    r8.y = x2[0].x;
    r8.y = r6.w ? r8.x : r8.y;
    x2[0].x = r8.y;
    r8.x = r8.z * r8.x;
    r8.x = cmp(1 < r8.x);
    r8.x = r8.x ? 1.000000 : 0;
    r8.x = r8.x * r7.w;
    r6.w = r6.w ? r8.x : r7.w;
    x0[0].x = r6.w;
    r6.w = x1[1].x;
    r7.w = x0[1].x;
    r8.x = 1 + -r6.w;
    r6.w = r7.w * r8.x + r6.w;
    r2.xyz = r2.xyz * r6.www + r13.xyz;
    r4.w = cmp(r4.w < r6.w);
    r2.xy = renderTargetSize.xy * r2.xy;
    r8.xy = (int2)r2.xy;
    r8.zw = float2(0,0);
    r2.x = floatZSampler.Load(r8.xyz).x;
    r2.y = cmp(r2.x >= 0.984375);
    r6.w = 1.01587307 * r2.x;
    r2.x = r2.x * 64 + -63;
    r2.x = r2.y ? r2.x : r6.w;
    r2.x = max(9.99999994e-09, r2.x);
    r2.x = rcp(r2.x);
    r2.y = x2[1].x;
    r2.y = r4.w ? r2.x : r2.y;
    x2[1].x = r2.y;
    r2.x = r2.z * r2.x;
    r2.x = cmp(1 < r2.x);
    r2.x = r2.x ? 1.000000 : 0;
    r2.x = r7.w * r2.x;
    r2.x = r4.w ? r2.x : r7.w;
    x0[1].x = r2.x;
  }
  r2.x = x2[0].x;
  r2.x = r2.x + -r9.w;
  r2.x = cmp(2.4000001 < abs(r2.x));
  r2.y = x0[0].x;
  r2.y = -r2.y * 0.5 + 1;
  r2.x = r2.x ? 0.5 : r2.y;
  r2.y = x2[1].x;
  r2.y = r2.y + -r9.w;
  r2.y = cmp(2.4000001 < abs(r2.y));
  r2.z = x0[1].x;
  r2.z = 0.5 * r2.z;
  r2.y = r2.y ? -0.5 : -r2.z;
  r2.x = r2.x + r2.y;
  r2.x = 1 + -r2.x;
  r2.yzw = w3.xyz * r2.www + -r0.xyz;
  r2.yzw = r5.xxx * r2.yzw + r0.xyz;
  r4.w = dot(r2.yzw, r2.yzw);
  r4.w = rsqrt(r4.w);
  r8.xyz = r4.www * r2.yzw;
  r2.yzw = sqrt(r3.xyz);
  r9.yzw = r3.www * r6.xyz;
  r9.yzw = r9.yzw * r5.xxx;
  r11.xyz = -r9.yzw * float3(0.5,0.5,0.5) + v7.xyz;
  r9.yz = (uint2)v0.xy;
  r3.w = cmp(v0.z >= 0.984375);
  r4.w = 1.01587307 * v0.z;
  r6.w = v0.z * 64 + -63;
  r3.w = r3.w ? r6.w : r4.w;
  r3.w = max(9.99999994e-09, r3.w);
  r3.w = rcp(r3.w);
  r4.w = cmp(isDepthHack != 0);
  r10.yz = (uint2)r9.zy;
  r6.w = dot(r10.zy, float2(0.0671105608,0.00583714992));
  r6.w = frac(r6.w);
  r6.w = 52.9829178 * r6.w;
  r6.w = frac(r6.w);
  r6.w = r6.w * 6.28318548 + gameTick.w;
  sincos(r6.w, r12.x, r13.x);
  r6.w = dot(r10.yz, float2(0.0671105608,0.00583714992));
  r6.w = frac(r6.w);
  r6.w = 52.9829178 * r6.w;
  r6.w = frac(r6.w);
  r6.w = r6.w * 6.28318548 + gameTick.w;
  sincos(r6.w, r14.x, r15.x);
  r6.w = 17 * r0.w;
  r6.w = exp2(r6.w);
  r7.w = 2 + r6.w;
  r7.w = 2 / r7.w;
  r9.w = dot(-r11.xyz, -r11.xyz);
  r9.w = rsqrt(r9.w);
  r10.yzw = -r11.xyz * r9.www;
  r16.xyz = eyeOffset.xyz + r11.xyz;
  r14.yzw = sunConstants.sstLightingConstants.offToPinTransform._m10_m11_m12 * r11.yyy;
  r14.yzw = r11.xxx * sunConstants.sstLightingConstants.offToPinTransform._m00_m01_m02 + r14.yzw;
  r14.yzw = r11.zzz * sunConstants.sstLightingConstants.offToPinTransform._m20_m21_m22 + r14.yzw;
  r14.yzw = sunConstants.sstLightingConstants.offToPinTransform._m30_m31_m32 + r14.yzw;
  r12.z = -sunConstants.splitDepthOffset + r14.w;
  r12.z = -r12.z * 6.10351563e-05 + 1;
  r12.w = saturate(r12.z);
  r12.w = cmp(r12.z == r12.w);
  if (r12.w != 0) {
    r12.w = 0;
    r13.w = 0;
    while (true) {
      r15.y = cmp(r12.w >= 3);
      if (r15.y != 0) break;
      r15.y = (uint)r12.w;
      r15.zw = -sunConstants.splitPinTransform[r15.y].xy + r14.yz;
      r15.z = max(abs(r15.z), abs(r15.w));
      r13.w = sunConstants.splitPinTransform[r15.y].z * r15.z;
      r15.y = cmp(r13.w < 1);
      if (r15.y != 0) {
        break;
      }
      r12.w = 1 + r12.w;
      r13.w = 0;
    }
  } else {
    r12.w = 3;
    r13.w = 0;
  }
  r15.y = cmp(r12.w >= 3);
  if (r15.y != 0) {
    r17.xyz = sunConstants.sstLightingConstants.constants.inchesPerTexel * float3(128,0.5,0.5);
    r18.xz = rcp(r17.xx);
    r18.y = -r18.z;
    r17.xyz = r14.yzy * r18.xyz + r17.yzy;
    r18.xyz = sunConstants.sstLightingConstants.constants.dimensionInTiles.xyx + float3(-0.0078125,-0.0078125,-0.0078125);
    r17.xyz = max(float3(0,0,0), r17.xyz);
    r17.xyz = min(r17.xyz, r18.xyz);
    r15.zw = sunConstants.sstLightingConstants.coordScale * r17.zy;
    r15.zw = floor(r15.zw);
    r15.w = sunConstants.sstLightingConstants.constants.dimensionInTiles.x * r15.w;
    r15.z = r15.w * sunConstants.sstLightingConstants.coordScale + r15.z;
    r15.z = (uint)r15.z;
    r15.z = (int)r15.z + (int10)sunConstants.sstLightingConstants.rootOffset;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r15.w, r15.z, l(0), t23.xxxx
  r15.w = samp0[]..swiz;
    r17.w = (int)r15.w & 0x40000000;
    r18.x = (uint)r15.w << 2;
    if (r17.w == 0) {
      r17.w = (int)r15.w & 0x01ffffff;
      r19.x = (int)r15.z + (int)r17.w;
      r15.z = (uint)r15.w >> 25;
      r15.z = (uint)r15.z;
      r17.xyz = r17.xyz * r15.zzz;
      r17.xyz = frac(r17.xyz);
      r17.xyz = float3(128,128,128) * r17.xyz;
      r17.xyz = (uint3)r17.xyz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r19.z, r19.x, l(0), t23.xxxx
    r19.z = samp0[]..swiz;
      r15.zw = (uint2)r17.zy >> int2(6,6);
      r17.w = (int)r19.z & 0xc0000000;
      r18.y = (int)r19.x + 1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.y, r18.y, l(0), t23.xxxx
    r18.y = samp0[]..swiz;
      r15.w = r15.w ? r19.z : r18.y;
      r18.y = (uint)r15.w >> 13;
      r15.z = r15.z ? r18.y : r15.w;
      r15.z = (int)r15.z & 8191;
      r20.x = (int)r15.z + (int)r19.x;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r20.z, r20.x, l(0), t23.xxxx
    r20.z = samp0[]..swiz;
      r19.y = 0;
      r20.y = 1;
      r18.yzw = r17.www ? r19.xyz : r20.xyz;
      r20.yz = r17.ww ? float2(8.40779079e-45,1.40129846e-45) : float2(7.00649232e-45,2.80259693e-45);
      r15.zw = (uint2)r17.zy >> (uint2)r20.yy;
      r15.zw = (int2)r15.zw & int2(1,1);
      r19.w = (int)r18.w & 0xc0000000;
      r20.y = (int)r18.y + 1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r20.y, r20.y, l(0), t23.xxxx
    r20.y = samp0[]..swiz;
      r15.w = r15.w ? r18.w : r20.y;
      r20.y = (uint)r15.w >> 13;
      r15.z = r15.z ? r20.y : r15.w;
      r15.z = (int)r15.z & 8191;
      r20.x = (int)r15.z + (int)r18.y;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r20.w, r20.x, l(0), t23.xxxx
    r20.w = samp0[]..swiz;
      r18.yzw = r19.www ? r18.yzw : r20.xzw;
      r18.yzw = r17.www ? r19.xyz : r18.yzw;
      r15.z = (int)r18.w & 0xc0000000;
      if (r15.z == 0) {
        r15.z = (int)-r18.z + 6;
        r15.zw = (uint2)r17.zy >> (uint2)r15.zz;
        r17.w = (int)r18.w | 0x40000000;
        bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r15.w = (((uint)r15.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
        bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r15.z = (((uint)r15.z << 0) & bitmask.z) | ((uint)r15.w & ~bitmask.z);
        r15.z = (int)r15.z * 10;
        r15.z = (uint)r17.w >> (uint)r15.z;
        r15.z = (int)r15.z & 1023;
        r19.x = (int)r15.z + (int)r18.y;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r19.w, r19.x, l(0), t23.xxxx
      r19.w = samp0[]..swiz;
        r19.yz = (int2)r18.zz + int2(1,2);
        r15.z = (int)-r19.y + 6;
        r15.zw = (uint2)r17.zy >> (uint2)r15.zz;
        r17.w = (int)r19.w & 0xc0000000;
        r18.z = (int)r19.w | 0x40000000;
        bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r15.w = (((uint)r15.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
        bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r15.z = (((uint)r15.z << 0) & bitmask.z) | ((uint)r15.w & ~bitmask.z);
        r15.z = (int)r15.z * 10;
        r15.z = (uint)r18.z >> (uint)r15.z;
        r15.z = (int)r15.z & 1023;
        r20.x = (int)r15.z + (int)r19.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r20.z, r20.x, l(0), t23.xxxx
      r20.z = samp0[]..swiz;
        r20.y = r19.z;
        r20.xyz = r17.www ? r19.xyw : r20.xyz;
        r15.z = (int)-r20.y + 6;
        r15.zw = (uint2)r17.zy >> (uint2)r15.zz;
        r18.z = (int)r20.z & 0xc0000000;
        r19.y = (int)r20.z | 0x40000000;
        bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r15.w = (((uint)r15.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
        bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r15.z = (((uint)r15.z << 0) & bitmask.z) | ((uint)r15.w & ~bitmask.z);
        r15.z = (int)r15.z * 10;
        r15.z = (uint)r19.y >> (uint)r15.z;
        r15.z = (int)r15.z & 1023;
        r21.x = (int)r15.z + (int)r20.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r21.y, r21.x, l(0), t23.xxxx
      r21.y = samp0[]..swiz;
        r15.zw = r18.zz ? r20.xz : r21.xy;
        r18.yw = r17.ww ? r19.xw : r15.zw;
      }
      r15.z = (int)r18.w & 0xc0000000;
      if (r15.z == 0) {
        if (14 == 0) r15.w = 0; else if (14+15 < 32) {         r15.w = (uint)r18.w << (32-(14 + 15)); r15.w = (uint)r15.w >> (32-14);        } else r15.w = (uint)r18.w >> 15;
        r15.w = (uint)r15.w;
        r15.w = sunConstants.sstLightingConstants.constants.spanInInches * r15.w;
        r15.w = 6.10388815e-05 * r15.w;
        r19.xy = (int2)r18.ww & int2(32767,536870912);
        r17.w = (uint)r19.x;
        r17.w = sunConstants.sstLightingConstants.constants.spanInInches * r17.w;
        r17.w = 3.05185094e-05 * r17.w;
        r18.z = (int)r17.y & 3;
        r18.z = (int)r18.z + (int)r18.y;
        r18.z = (int)r18.z + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.z, r18.z, l(0), t23.xxxx
      r18.z = samp0[]..swiz;
        bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r19.x = (((uint)r17.x << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
        bitmask.z = ((~(-1 << 2)) << 1) & 0xffffffff;  r19.z = (((uint)r17.y << 1) & bitmask.z) | ((uint)0 & ~bitmask.z);
        bitmask.w = ((~(-1 << 1)) << 4) & 0xffffffff;  r19.w = (((uint)r17.z << 4) & bitmask.w) | ((uint)0 & ~bitmask.w);
        r17.x = (uint)r18.z >> (uint)r19.x;
        r17.x = (int)r17.x & 255;
        r17.x = (uint)r17.x;
        r17.x = r17.x * r17.w;
        r17.x = r17.x * 0.00392156886 + r15.w;
        r17.y = (int)r19.z + 1;
        if (1 == 0) r17.z = 0; else if (1+1 < 32) {         r17.z = (uint)r17.z << (32-(1 + 1)); r17.z = (uint)r17.z >> (32-1);        } else r17.z = (uint)r17.z >> 1;
        r17.y = (int)r17.z + (int)r17.y;
        r17.y = (int)r17.y + (int)r18.y;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.y, r17.y, l(0), t23.xxxx
      r17.y = samp0[]..swiz;
        r17.y = (uint)r17.y >> (uint)r19.w;
        r17.y = (int)r17.y & 0x0000ffff;
        r17.y = (uint)r17.y;
        r17.y = r17.y * r17.w;
        r15.w = r17.y * 1.52590219e-05 + r15.w;
        r18.x = r19.y ? r17.x : r15.w;
      } else {
        r15.w = (int)r18.w & 0x80000000;
        r17.x = (int)r18.y + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.x, r17.x, l(0), t23.xxxx
      r17.x = samp0[]..swiz;
        r15.w = r15.w ? r17.x : 0;
        r17.x = (uint)r18.w << 2;
        r17.y = (uint)r15.w >> 16;
        r17.y = f16tof32(r17.y);
        r15.w = (int)r15.w & 0x0000ffff;
        r15.w = f16tof32(r15.w);
        r17.x = r14.y * r17.y + r17.x;
        r15.w = r14.z * r15.w + r17.x;
        r18.x = r15.z ? r15.w : r18.x;
      }
    }
    r14.w = -sunConstants.sstLightingConstants.constants.inchesPerTexel * 2 + r14.w;
    r14.w = cmp(r18.x < r14.w);
    r17.y = r14.w ? 0 : 1;
  }
  if (r15.y == 0) {
    if (enableDitheredShadow == 0) {
      r14.w = (uint)r12.w;
      r15.yz = -sunConstants.splitPinTransform[r14.w].xy + r14.yz;
      r15.yz = sunConstants.splitPinTransform[r14.w].zz * r15.yz;
      r18.xy = r15.yz * float2(0.5,-0.5) + float2(0.5,0.5);
      r14.w = (int16)sunConstants.splitArrayOffset;
      r18.z = r14.w + r12.w;
      r14.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r18.xyz, r12.z).x;
      r15.y = gTransShadowmapArray.SampleLevel(samp0_s, r18.xyz, 0).x;
      r14.w = r15.y + r14.w;
      r14.w = saturate(-1 + r14.w);
      r15.y = r14.w * r14.w;
      r17.y = r15.y * r14.w;
    }
    if (enableDitheredShadow != 0) {
      r18.x = -r12.x;
      r14.w = (uint)r12.w;
      r12.w = 1 + r12.w;
      r12.w = min(2, r12.w);
      r12.w = (uint)r12.w;
      r13.w = 1 + -r13.w;
      r13.w = 28 * r13.w;
      r13.w = (uint)r13.w;
      r15.yz = -sunConstants.splitPinTransform[r14.w].xy + r14.yz;
      r15.yz = sunConstants.splitPinTransform[r14.w].zz * r15.yz;
      r15.yz = r15.yz * float2(0.5,-0.5) + float2(0.5,0.5);
      r14.yz = -sunConstants.splitPinTransform[r12.w].xy + r14.yz;
      r14.yz = sunConstants.splitPinTransform[r12.w].zz * r14.yz;
      r14.yz = r14.yz * float2(0.5,-0.5) + float2(0.5,0.5);
      r18.y = r13.x;
      r18.z = r12.x;
      r15.w = 0;
      r17.z = 0;
      while (true) {
        r17.w = cmp((uint)r17.z >= 8);
        if (r17.w != 0) break;
        r17.w = cmp((uint)r13.w < (uint)r17.z);
        r19.xy = r17.ww ? r14.yz : r15.yz;
        r18.w = r17.w ? sunConstants.splitPinTransform[r12.w].w : sunConstants.splitPinTransform[r14.w].w;
        r17.w = r17.w ? r12.w : r14.w;
        r20.x = dot(icb[r17.z+0].yx, r18.xy);
        r20.y = dot(icb[r17.z+0].yx, r18.yz);
        r19.xy = r20.xy * r18.ww + r19.xy;
        r17.w = (int)r17.w + (int16)sunConstants.splitArrayOffset;
        r19.z = (uint)r17.w;
        r17.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r19.xyz, r12.z).x;
        r18.w = gTransShadowmapArray.SampleLevel(samp0_s, r19.xyz, 0).x;
        r17.w = r18.w + r17.w;
        r17.w = saturate(-1 + r17.w);
        r15.w = r17.w * 0.125 + r15.w;
        r17.z = (int)r17.z + 1;
      }
      r12.z = r15.w * r15.w;
      r17.y = r12.z * r15.w;
    }
  }
  r12.z = viewmodelSunShadowRaw >> 16;
  r12.w = cmp(0 < (uint)r12.z);
  r12.w = r4.w ? r12.w : 0;
  if (r12.w != 0) {
    r12.z = (int)r12.z + numLights;
    r12.z = (int)r12.z + -1;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.w, r12.z, l(52), t12.xxxx
  r12.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r12.z, l(68), t12.xyzw
  r18.x = samp0[]..swiz;
  r18.y = samp0[]..swiz;
  r18.z = samp0[]..swiz;
  r18.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r19.xyzw, r12.z, l(84), t12.xyzw
  r19.x = samp0[]..swiz;
  r19.y = samp0[]..swiz;
  r19.z = samp0[]..swiz;
  r19.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r20.xyzw, r12.z, l(132), t12.xyzw
  r20.x = samp0[]..swiz;
  r20.y = samp0[]..swiz;
  r20.z = samp0[]..swiz;
  r20.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r21.xyzw, r12.z, l(148), t12.xyzw
  r21.x = samp0[]..swiz;
  r21.y = samp0[]..swiz;
  r21.z = samp0[]..swiz;
  r21.w = samp0[]..swiz;
    r13.w = dot(sunConstants.wldDir.xyz, r6.xyz);
    r13.w = abs(r13.w) * -0.200000003 + 0.400000006;
    r22.xyz = r6.xyz * r13.www + r11.xyz;
    r22.w = 1;
    r18.x = dot(r18.xyzw, r22.xyzw);
    r18.y = dot(r19.xyzw, r22.xyzw);
    r14.yz = r18.xy * float2(0.5,-0.5) + float2(0.5,0.5);
    r14.yz = r14.yz + r21.zw;
    r14.yz = r14.yz * r21.xy;
    r15.yz = r12.ww / r20.xz;
    r17.zw = float2(1,1) + -r15.yz;
    r17.zw = cmp(r14.yz >= r17.zw);
    r15.yz = cmp(r15.yz >= r14.yz);
    r15.yz = (int2)r15.yz | (int2)r17.zw;
    r13.w = (int)r15.z | (int)r15.y;
    if (r13.w == 0) {
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.w, r12.z, l(28), t12.xxxx
    r13.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r12.z, l(100), t12.xyzw
    r18.x = samp0[]..swiz;
    r18.y = samp0[]..swiz;
    r18.z = samp0[]..swiz;
    r18.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.yz, r12.z, l(164), t12.xxyx
    r15.y = samp0[]..swiz;
    r15.z = samp0[]..swiz;
      r12.z = dot(r18.xyzw, r22.xyzw);
      r14.yz = saturate(r14.yz);
      r18.xy = r14.yz * r20.xz + r20.yw;
      r12.z = r12.z + r15.y;
      r12.z = r12.z / r15.z;
      r12.z = max(6.10351563e-05, r12.z);
      r13.w = (int)r13.w & 0x0000ffff;
      if (enableDitheredShadow != 0) {
        r19.x = -r12.x;
        r20.z = (uint)r13.w;
        r19.y = r13.x;
        r19.z = r12.x;
        r14.yz = float2(0,0);
        while (true) {
          r14.w = cmp((int)r14.z >= 8);
          if (r14.w != 0) break;
          r21.x = dot(icb[r14.z+0].yx, r19.xy);
          r21.y = dot(icb[r14.z+0].yx, r19.yz);
          r20.xy = r21.xy * r12.ww + r18.xy;
          r14.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r12.z).x;
          r14.y = r14.w * 0.125 + r14.y;
          r14.z = (int)r14.z + 1;
        }
      } else {
        r18.z = (uint)r13.w;
        r14.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r18.xyz, r12.z).x;
      }
      r12.z = r14.y * r14.y;
      r12.z = r12.z * r14.y;
    } else {
      r12.z = 1;
    }
    r17.y = r17.y * r12.z;
  } else {
    r12.z = viewmodelSunShadowRaw & 0x0000ffff;
    r12.w = cmp(0 < (uint)r12.z);
    r13.w = ~(int)r4.w;
    r12.w = r12.w ? r13.w : 0;
    if (r12.w != 0) {
      r12.z = (int)r12.z + numLights;
      r12.z = (int)r12.z + -1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.w, r12.z, l(52), t12.xxxx
    r12.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r12.z, l(68), t12.xyzw
    r18.x = samp0[]..swiz;
    r18.y = samp0[]..swiz;
    r18.z = samp0[]..swiz;
    r18.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r19.xyzw, r12.z, l(84), t12.xyzw
    r19.x = samp0[]..swiz;
    r19.y = samp0[]..swiz;
    r19.z = samp0[]..swiz;
    r19.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r20.xyzw, r12.z, l(132), t12.xyzw
    r20.x = samp0[]..swiz;
    r20.y = samp0[]..swiz;
    r20.z = samp0[]..swiz;
    r20.w = samp0[]..swiz;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r21.xyzw, r12.z, l(148), t12.xyzw
    r21.x = samp0[]..swiz;
    r21.y = samp0[]..swiz;
    r21.z = samp0[]..swiz;
    r21.w = samp0[]..swiz;
      r13.w = dot(sunConstants.wldDir.xyz, r6.xyz);
      r13.w = abs(r13.w) * -0.200000003 + 0.400000006;
      r22.xyz = r6.xyz * r13.www + r11.xyz;
      r22.w = 1;
      r18.x = dot(r18.xyzw, r22.xyzw);
      r18.y = dot(r19.xyzw, r22.xyzw);
      r14.yz = r18.xy * float2(0.5,-0.5) + float2(0.5,0.5);
      r14.yz = r14.yz + r21.zw;
      r14.yz = r14.yz * r21.xy;
      r15.yz = r12.ww / r20.xz;
      r17.zw = float2(1,1) + -r15.yz;
      r17.zw = cmp(r14.yz >= r17.zw);
      r15.yz = cmp(r15.yz >= r14.yz);
      r15.yz = (int2)r15.yz | (int2)r17.zw;
      r13.w = (int)r15.z | (int)r15.y;
      if (r13.w == 0) {
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.w, r12.z, l(28), t12.xxxx
      r13.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r12.z, l(100), t12.xyzw
      r18.x = samp0[]..swiz;
      r18.y = samp0[]..swiz;
      r18.z = samp0[]..swiz;
      r18.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.yz, r12.z, l(164), t12.xxyx
      r15.y = samp0[]..swiz;
      r15.z = samp0[]..swiz;
        r12.z = dot(r18.xyzw, r22.xyzw);
        r14.yz = saturate(r14.yz);
        r18.xy = r14.yz * r20.xz + r20.yw;
        r12.z = r12.z + r15.y;
        r12.z = r12.z / r15.z;
        r12.z = max(6.10351563e-05, r12.z);
        r13.w = (int)r13.w & 0x0000ffff;
        if (enableDitheredShadow != 0) {
          r19.x = -r12.x;
          r20.z = (uint)r13.w;
          r19.y = r13.x;
          r19.z = r12.x;
          r14.yz = float2(0,0);
          while (true) {
            r14.w = cmp((int)r14.z >= 8);
            if (r14.w != 0) break;
            r21.x = dot(icb[r14.z+0].yx, r19.xy);
            r21.y = dot(icb[r14.z+0].yx, r19.yz);
            r20.xy = r21.xy * r12.ww + r18.xy;
            r14.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r12.z).x;
            r14.y = r14.w * 0.125 + r14.y;
            r14.z = (int)r14.z + 1;
          }
        } else {
          r18.z = (uint)r13.w;
          r14.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r18.xyz, r12.z).x;
        }
        r12.z = r14.y * r14.y;
        r12.z = r12.z * r14.y;
      } else {
        r12.z = 1;
      }
      r17.y = r17.y * r12.z;
    }
  }
  r12.z = cmp(0 < r17.y);
  if (r12.z != 0) {
    r14.yzw = sunConstants.color.xyz * r2.yzw;
    r12.z = saturate(dot(r1.xyz, sunConstants.wldDir.xyz));
    if (sunConstants.sunCookieIndex != 0) {
      r16.w = 1;
      r12.w = dot(sunConstants.sunCookieTransform[0].xyzw, r16.xyzw);
      r13.w = dot(sunConstants.sunCookieTransform[1].xyzw, r16.xyzw);
      r18.x = frac(r12.w);
      r18.y = frac(r13.w);
      r12.w = -1 + (int14)sunConstants.sunCookieIndex;
      r18.z = (uint)r12.w;
      r15.yzw = gCookieArray.SampleLevel(samplerLinear_s, r18.xyz, 0).xyz;
      r15.yzw = float3(-1,-1,-1) + r15.yzw;
      r15.yzw = sunConstants.sunCookieIntensity * r15.yzw + float3(1,1,1);
      r14.yzw = r15.yzw * r14.yzw;
    }
    r12.w = dot(sunConstants.wldDir.xyz, r0.xyz);
    r12.w = 0.200000003 + r12.w;
    r12.w = saturate(0.833333313 * r12.w);
    r12.w = r12.w * r12.w;
    r13.w = 1.25 * r12.w;
    r15.yz = r4.xy * float2(0.795454562,0.795454562) + float2(-0.795454562,-0.795454562);
    r18.x = dot(sunConstants.wldDir.xyz, v8.xyz);
    r18.y = dot(sunConstants.wldDir.xyz, v9.xyz);
    r15.w = dot(r18.xy, r18.xy);
    r15.w = rsqrt(r15.w);
    r18.xy = r18.xy * r15.ww;
    r18.z = -r18.x;
    r19.x = dot(r15.zy, r18.yz);
    r19.y = dot(r15.zy, -r18.xy);
    r18.xy = r19.xy * float2(1,2) + float2(0.5,1);
    r15.y = dot(sunConstants.wldDir.xyz, r6.xyz);
    r15.y = r15.y * 0.689655125 + 0.310344785;
    r15.z = cmp(1 < r18.y);
    r15.w = 2 + -r18.y;
    r18.z = r15.z ? r15.w : r18.y;
    r15.z = 126.5 * r15.y;
    r15.w = floor(r15.z);
    r15.z = ceil(r15.z);
    r17.zw = float2(0.0078125,1) * r18.xz;
    r18.xy = r15.ww * float2(0.0078125,0) + r17.zw;
    r16.w = causticMap.SampleLevel(LinearSampler_s, r18.xy, 0).x;
    r17.zw = r15.zz * float2(0.0078125,0) + r17.zw;
    r15.z = causticMap.SampleLevel(LinearSampler_s, r17.zw, 0).x;
    r15.y = r15.y * 126.5 + -r15.w;
    r15.z = r15.z + -r16.w;
    r15.y = r15.y * r15.z + r16.w;
    r12.w = -r12.w * 1.25 + r15.y;
    r15.yzw = r5.xxx * r12.www + r13.www;
    r12.w = cmp(0 < r12.z);
    if (r12.w != 0) {
      r12.w = sunConstants.specScale * r7.w;
      r12.w = 0.25 * r12.w;
      r18.xyz = -r11.xyz * r9.www + sunConstants.wldDir.xyz;
      r13.w = dot(r18.xyz, r18.xyz);
      r13.w = rsqrt(r13.w);
      r18.xyz = r18.xyz * r13.www;
      r13.w = dot(r1.xyz, r18.xyz);
      r16.w = r6.w * 0.125 + 0.25;
      r13.w = log2(abs(r13.w));
      r13.w = r13.w * r6.w;
      r13.w = exp2(r13.w);
      r13.w = r16.w * r13.w;
      r12.z = r13.w * r12.z;
      r12.z = r12.z * r12.w;
      r12.w = 0.25 * r7.w;
      r19.xyz = r12.zzz / r12.www;
      r12.z = dot(r18.xyz, sunConstants.wldDir.xyz);
      r12.z = saturate(1 + -r12.z);
      r12.w = r12.z * r12.z;
      r12.w = r12.w * r12.w;
      r12.z = r12.z * r12.w;
      r12.z = r19.z * r12.z;
      r17.zw = r12.zz;
    } else {
      r17.zw = float2(0,0);
      r12.z = 0;
      r19.xyz = float3(0,0,0);
    }
    r12.w = dot(r14.yzw, float3(0.212599993,0.715200007,0.0722000003));
    r12.w = r17.y * r12.w;
    r17.x = 1;
    r18.xy = sunConstants.wldDir.xy;
    r13.w = sunConstants.wldDir.z;
  } else {
    r14.yzw = float3(0,0,0);
    r18.xy = float2(0,0);
    r15.yzw = float3(0,0,0);
    r13.w = 0;
    r19.xyz = float3(0,0,0);
    r12.zw = float2(0,0);
    r17.xyzw = float4(0,0,0,0);
  }
  r3.w = 0.0078125 * r3.w;
  r3.w = min(15, r3.w);
  r3.w = (uint)r3.w;
  r20.xy = (uint2)r9.yz >> int2(6,6);
  r20.z = (uint)r3.w << 4;
  r3.w = ~(int)r4.w;
  r21.x = -r12.x;
  r4.xy = r4.xy * float2(0.795454562,0.795454562) + float2(-0.795454562,-0.795454562);
  r9.y = r6.w * 0.125 + 0.25;
  r9.z = 0.25 * r7.w;
  r22.xy = float2(0,0);
  r23.w = 0;
  r11.w = 1;
  r24.w = 1;
  r25.w = 1;
  r26.z = 1;
  r27.w = 1;
  r21.z = r12.x;
  r21.yw = r13.xx;
  r12.y = r21.w;
  r13.y = r21.x;
  r13.z = r12.x;
  r28.x = r13.x;
  r28.y = r21.x;
  r28.z = r12.x;
  r29.xyz = r14.yzw;
  r30.xy = r18.xy;
  r30.z = r13.w;
  r31.xyz = r15.yzw;
  r32.xyz = r19.xyz;
  r33.x = r12.z;
  r33.yz = r17.zw;
  r34.xyz = float3(0,0,0);
  r35.xyz = float3(0,0,0);
  r36.xyz = float3(0,0,0);
  r37.x = enableDitheredShadow;
  r37.y = r12.w;
  r18.zw = r17.xy;
  r16.w = 0;
  while (true) {
    r19.w = cmp((uint)r16.w >= numLights);
    if (r19.w != 0) break;
    r22.z = (uint)r16.w >> 5;
    r23.xyz = (int3)r20.xyz + (int3)r22.xyz;
    r19.w = visibleLights.Load(r23.xyzw).x;
    r23.xyz = r29.xyz;
    r38.xyz = r30.xyz;
    r39.xyz = r31.xyz;
    r40.xyz = r32.xyz;
    r41.xyz = r33.xyz;
    r42.xyz = r34.xyz;
    r43.xyz = r35.xyz;
    r22.zw = r18.zw;
    r20.w = r37.x;
    r26.w = r37.y;
    r44.xyz = r36.xyz;
    r28.w = r19.w;
    while (true) {
      if (r28.w == 0) break;
      r29.w = firstbitlow((uint)r28.w);
      r30.w = 1 << (int)r29.w;
      r31.w = (int)r28.w & (int)r30.w;
      if (r31.w != 0) {
        r28.w = (int)r28.w ^ (int)r30.w;
        r29.w = (int)r16.w + (int)r29.w;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r45.xyzw, r29.w, l(0), t11.xyzw
      r45.x = samp0[]..swiz;
      r45.y = samp0[]..swiz;
      r45.z = samp0[]..swiz;
      r45.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r46.xy, r29.w, l(16), t11.xyxx
      r46.x = samp0[]..swiz;
      r46.y = samp0[]..swiz;
        r45.xyz = r45.xyz + -r11.xyz;
        r46.z = r45.w;
        r45.xyz = cmp(abs(r45.xyz) < r46.zxy);
        r30.w = r45.y ? r45.x : 0;
        r30.w = r45.z ? r30.w : 0;
        if (r30.w != 0) {
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r29.w, l(0), t12.xyzw
        r45.x = samp0[]..swiz;
        r45.y = samp0[]..swiz;
        r45.z = samp0[]..swiz;
        r45.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r46.xyzw, r29.w, l(16), t12.zxyw
        r46.x = samp0[]..swiz;
        r46.y = samp0[]..swiz;
        r46.z = samp0[]..swiz;
        r46.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r29.w, l(32), t12.xyzw
        r47.x = samp0[]..swiz;
        r47.y = samp0[]..swiz;
        r47.z = samp0[]..swiz;
        r47.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r48.xyzw, r29.w, l(48), t12.xyzw
        r48.x = samp0[]..swiz;
        r48.y = samp0[]..swiz;
        r48.z = samp0[]..swiz;
        r48.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r49.xyzw, r29.w, l(64), t12.yzwx
        r49.x = samp0[]..swiz;
        r49.y = samp0[]..swiz;
        r49.z = samp0[]..swiz;
        r49.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r50.xyzw, r29.w, l(80), t12.yzwx
        r50.x = samp0[]..swiz;
        r50.y = samp0[]..swiz;
        r50.z = samp0[]..swiz;
        r50.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r51.xyzw, r29.w, l(96), t12.yzwx
        r51.x = samp0[]..swiz;
        r51.y = samp0[]..swiz;
        r51.z = samp0[]..swiz;
        r51.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r52.xyzw, r29.w, l(112), t12.yzwx
        r52.x = samp0[]..swiz;
        r52.y = samp0[]..swiz;
        r52.z = samp0[]..swiz;
        r52.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r53.xyzw, r29.w, l(128), t12.xyzw
        r53.x = samp0[]..swiz;
        r53.y = samp0[]..swiz;
        r53.z = samp0[]..swiz;
        r53.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r54.xyzw, r29.w, l(172), t12.yzwx
        r54.x = samp0[]..swiz;
        r54.y = samp0[]..swiz;
        r54.z = samp0[]..swiz;
        r54.w = samp0[]..swiz;
          if (3 == 0) r30.w = 0; else if (3+24 < 32) {           r30.w = (uint)r46.w << (32-(3 + 24)); r30.w = (uint)r30.w >> (32-3);          } else r30.w = (uint)r46.w >> 24;
          r31.w = cmp((int)r30.w == 4);
          if (r31.w != 0) {
            r32.w = r46.x * r7.w;
            r32.w = 0.25 * r32.w;
            r33.w = cmp(0 < r51.x);
            r55.xy = r50.yz;
            r55.z = r51.w;
            r56.xyz = -r55.xyz * float3(0.5,0.5,0.5) + r45.xyz;
            r57.xyz = r56.xyz + -r11.xyz;
            r34.w = dot(r55.xyz, r57.xyz);
            r34.w = saturate(-r34.w / r51.x);
            r55.xyz = r34.www * r55.xyz + r56.xyz;
            r55.xyz = r33.www ? r55.xyz : r45.xyz;
            r56.xyz = r55.xyz + -r11.xyz;
            r33.w = dot(r56.xyz, r56.xyz);
            r33.w = rsqrt(r33.w);
            r56.xyz = r56.xyz * r33.www;
          } else {
            r33.w = cmp((int)r30.w == 1);
            r57.xy = r48.zw;
            r57.z = r49.w;
            r57.xyz = r57.xyz + -r11.xyz;
            r34.w = dot(r57.xyz, r57.xyz);
            r34.w = rsqrt(r34.w);
            r57.xyz = r57.xyz * r34.www;
            r56.xyz = r33.www ? r54.wxy : r57.xyz;
            r33.w = r46.x * r7.w;
            r32.w = 0.25 * r33.w;
            r55.xyz = r45.xyz;
          }
          switch (r30.w) {
            case 2 :            r58.xyz = r52.xyz;
            r58.w = r53.x;
            r35.w = dot(r58.xyzw, r11.xyzw);
            r36.w = cmp(r35.w < 1);
            if (r36.w != 0) {
              r57.xyz = float3(1,1,1);
              r33.w = 0;
              r34.w = -1;
              break;
            } else {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r58.xyzw, r29.w, l(188), t12.wxyz
            r58.x = samp0[]..swiz;
            r58.y = samp0[]..swiz;
            r58.z = samp0[]..swiz;
            r58.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r59.xyzw, r29.w, l(204), t12.xyzw
            r59.x = samp0[]..swiz;
            r59.y = samp0[]..swiz;
            r59.z = samp0[]..swiz;
            r59.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.w, r29.w, l(236), t12.xxxx
            r36.w = samp0[]..swiz;
              r45.xyz = r45.xyz + -r11.xyz;
              r37.z = r48.x * r48.x;
              r37.w = dot(r45.xyz, r45.xyz);
              r37.z = r37.z / r37.w;
              r37.z = min(1, r37.z);
              r45.xy = saturate(r35.ww * r47.xz + r47.yw);
              r48.zw = r45.xy * r45.xy;
              r45.xy = r45.xy * float2(-2,-2) + float2(3,3);
              r45.xy = r48.zw * r45.xy;
              r37.z = r45.x * r37.z;
              r37.z = r37.z * r45.y;
              r49.w = r50.w;
              r45.x = dot(r49.xyzw, r11.xyzw);
              r60.xyz = r50.xyz;
              r60.w = r51.w;
              r45.y = dot(r60.xyzw, r11.xyzw);
              r26.xy = r45.xy / r35.ww;
              r35.w = cmp(r54.w < 0.00048828125);
              if (r35.w != 0) {
                r58.y = r59.x;
                r45.xy = saturate(abs(r26.xy) * r58.zw + r58.xy);
                r48.zw = r45.xy * r45.xy;
                r45.xy = r45.xy * float2(-2,-2) + float2(3,3);
                r45.xy = r48.zw * r45.xy;
                r35.w = r45.x * r45.y;
              } else {
                r54.w = r58.y;
                r54.xyzw = saturate(r54.xyzw * abs(r26.yyxx));
                r54.xyzw = log2(r54.xyzw);
                r54.xyzw = r58.xxxx * r54.xyzw;
                r54.xyzw = exp2(r54.xyzw);
                r45.xy = r54.xy + r54.zw;
                r45.xy = log2(r45.xy);
                r45.xy = r59.xx * r45.xy;
                r45.xy = exp2(r45.xy);
                r37.w = r58.z * r45.x;
                r38.w = r58.w * r45.y + -1;
                r37.w = r58.w * r45.y + -r37.w;
                r37.w = saturate(r38.w / r37.w);
                r38.w = r37.w * r37.w;
                r37.w = r37.w * -2 + 3;
                r35.w = r38.w * r37.w;
              }
              r33.w = r37.z * r35.w;
              r35.w = r36.w ? 0.000000 : 0;
              if (r35.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyz, r29.w, l(220), t12.xyzx
              r45.x = samp0[]..swiz;
              r45.y = samp0[]..swiz;
              r45.z = samp0[]..swiz;
                r36.w = dot(r59.yzw, r26.xyz);
                r26.x = dot(r45.xyz, r26.xyz);
                r45.x = frac(r36.w);
                r45.y = frac(r26.x);
                r26.x = (int)r35.w + -1;
                r45.z = (uint)r26.x;
                r57.xyz = gCookieArray.SampleLevel(samplerLinear_s, r45.xyz, 0).xyz;
              } else {
                r57.xyz = float3(1,1,1);
              }
              r34.w = -1;
              break;
            }
            case 4 :            r45.xyz = r55.xyz + -r11.xyz;
            r26.x = dot(r45.xyz, r45.xyz);
            r26.y = sqrt(r26.x);
            r35.w = r48.x * r48.x;
            r26.x = r35.w / r26.x;
            r26.x = min(1, r26.x);
            r37.zw = saturate(r26.yy * r47.xz + r47.yw);
            r45.xy = r37.zw * r37.zw;
            r37.zw = r37.zw * float2(-2,-2) + float2(3,3);
            r37.zw = r45.xy * r37.zw;
            r26.x = r37.z * r26.x;
            r33.w = r26.x * r37.w;
            r57.xyz = float3(1,1,1);
            r34.w = -1;
            break;
            default :
            r57.xyz = float3(1,1,1);
            r34.w = 0;
            break;
          }
          r26.x = r34.w ? r33.w : 1;
          r26.y = cmp(0 < r26.x);
          if (r26.y != 0) {
            if (3 == 0) r37.z = 0; else if (3+27 < 32) {             r37.z = (uint)r46.w << (32-(3 + 27)); r37.z = (uint)r37.z >> (32-3);            } else r37.z = (uint)r46.w >> 27;
            if (4 == 0) r37.w = 0; else if (4+16 < 32) {             r37.w = (uint)r46.w << (32-(4 + 16)); r37.w = (uint)r37.w >> (32-4);            } else r37.w = (uint)r46.w >> 16;
            r26.y = cmp((int)r37.z != 1);
            if (r26.y != 0) {
              r26.y = dot(r56.xyz, r6.xyz);
              r26.y = abs(r26.y) * -0.200000003 + 0.400000006;
              r25.xyz = r6.xyz * r26.yyy + r11.xyz;
              r26.y = cmp((int)r30.w != 4);
              if (r26.y != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.y, r29.w, l(144), t12.xxxx
              r45.y = samp0[]..swiz;
                r49.w = r50.w;
                r47.x = dot(r49.xyzw, r25.xyzw);
                r50.w = r51.w;
                r47.y = dot(r50.xyzw, r25.xyzw);
                r51.w = r52.w;
                r26.y = dot(r51.xyzw, r25.xyzw);
                r52.w = r53.x;
                r29.w = dot(r52.xyzw, r25.xyzw);
                r30.w = cmp(r29.w < r26.y);
                r47.xy = r47.xy / r29.ww;
                r47.xy = saturate(r47.xy * float2(0.5,-0.5) + float2(0.5,0.5));
                r45.x = r53.z;
                r47.xy = r47.xy * r53.yw + r45.xy;
                r26.y = r26.y / r29.w;
              } else {
                r25.xyz = r25.xyz + -r49.xyz;
                r29.w = max(abs(r25.y), abs(r25.z));
                r29.w = max(r29.w, abs(r25.x));
                r29.w = r50.w / r29.w;
                r26.y = r29.w + r50.x;
                r29.w = dot(r25.xyz, r25.xyz);
                r29.w = rsqrt(r29.w);
                r47.xyz = r29.www * r25.xyz;
                r30.w = 0;
              }
              if (r30.w == 0) {
                r25.x = max(6.10351563e-05, r26.y);
                if (r31.w != 0) {
                  r25.y = (int)r46.w & 0x0000ffff;
                  r49.w = (uint)r25.y;
                  r25.yz = float2(0,0);
                  while (true) {
                    r26.y = cmp((int)r25.z >= 8);
                    if (r26.y != 0) break;
                    r45.y = dot(icb[r25.z+0].yx, r21.xy);
                    r45.z = dot(icb[r25.z+0].yx, r21.yz);
                    r50.yz = r45.yz * r48.yy;
                    r50.x = r50.y * r15.x;
                    r50.w = r50.y * r14.x;
                    r49.xyz = r50.xzw + r47.xyz;
                    r26.y = gOmniShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r49.xyzw, r25.x).x;
                    r25.y = r26.y * 0.125 + r25.y;
                    r25.z = (int)r25.z + 1;
                  }
                } else {
                  r25.z = (int)r46.w & 0x0000ffff;
                  if (r20.w != 0) {
                    r45.z = (uint)r25.z;
                    r26.y = 0;
                    r29.w = 0;
                    while (true) {
                      r30.w = cmp((int)r29.w >= 8);
                      if (r30.w != 0) break;
                      r49.x = dot(icb[r29.w+0].yx, r21.xw);
                      r49.y = dot(icb[r29.w+0].xy, r12.xy);
                      r45.xy = r49.xy * r48.yy + r47.xy;
                      r30.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r45.xyz, r25.x).x;
                      r26.y = r30.w * 0.125 + r26.y;
                      r29.w = (int)r29.w + 1;
                    }
                  } else {
                    r47.w = (uint)r25.z;
                    r26.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r47.xyw, r25.x).x;
                  }
                  r25.x = r26.y * r26.y;
                  r25.y = r25.x * r26.y;
                }
              } else {
                r25.y = 1;
              }
            } else {
              r25.y = 1;
            }
            r25.x = cmp(0 < (uint)r37.w);
            r25.x = r4.w ? r25.x : 0;
            if (r25.x != 0) {
              r25.x = (int)r37.w + numLights;
              r25.x = (int)r25.x + -1;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r25.z, r25.x, l(52), t12.xxxx
            r25.z = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r25.x, l(68), t12.xyzw
            r47.x = samp0[]..swiz;
            r47.y = samp0[]..swiz;
            r47.z = samp0[]..swiz;
            r47.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r48.xyzw, r25.x, l(84), t12.xyzw
            r48.x = samp0[]..swiz;
            r48.y = samp0[]..swiz;
            r48.z = samp0[]..swiz;
            r48.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r49.xyzw, r25.x, l(100), t12.xyzw
            r49.x = samp0[]..swiz;
            r49.y = samp0[]..swiz;
            r49.z = samp0[]..swiz;
            r49.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r50.xyzw, r25.x, l(116), t12.xyzw
            r50.x = samp0[]..swiz;
            r50.y = samp0[]..swiz;
            r50.z = samp0[]..swiz;
            r50.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r51.xyzw, r25.x, l(132), t12.xyzw
            r51.x = samp0[]..swiz;
            r51.y = samp0[]..swiz;
            r51.z = samp0[]..swiz;
            r51.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r52.xyzw, r25.x, l(148), t12.xyzw
            r52.x = samp0[]..swiz;
            r52.y = samp0[]..swiz;
            r52.z = samp0[]..swiz;
            r52.w = samp0[]..swiz;
              r29.w = dot(r56.xyz, r6.xyz);
              r29.w = abs(r29.w) * -0.200000003 + 0.400000006;
              r24.xyz = r6.xyz * r29.www + r11.xyz;
              r45.x = dot(r47.xyzw, r24.xyzw);
              r45.y = dot(r48.xyzw, r24.xyzw);
              r29.w = dot(r49.xyzw, r24.xyzw);
              r24.x = dot(r50.xyzw, r24.xyzw);
              r24.y = cmp(r24.x < r29.w);
              r37.zw = r45.xy / r24.xx;
              r37.zw = r37.zw * float2(0.5,-0.5) + float2(0.5,0.5);
              r37.zw = r37.zw + r52.zw;
              r37.zw = r37.zw * r52.xy;
              r45.xy = r25.zz / r51.xz;
              r47.xy = float2(1,1) + -r45.xy;
              r47.xy = cmp(r37.zw >= r47.xy);
              r45.xy = cmp(r45.xy >= r37.zw);
              r45.xy = (int2)r45.xy | (int2)r47.xy;
              r24.z = (int)r45.y | (int)r45.x;
              r24.y = (int)r24.y | (int)r24.z;
              if (r24.y == 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r24.y, r25.x, l(28), t12.xxxx
              r24.y = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xy, r25.x, l(164), t12.xyxx
              r45.x = samp0[]..swiz;
              r45.y = samp0[]..swiz;
                r37.zw = saturate(r37.zw);
                r47.xy = r37.zw * r51.xz + r51.yw;
                r24.z = r45.y * r24.x;
                r24.x = r45.x * r24.x + r29.w;
                r24.x = r24.x / r24.z;
                r24.x = max(6.10351563e-05, r24.x);
                r24.y = r24.y ? 0.000000 : 0;
                if (r20.w != 0) {
                  r45.z = (uint)r24.y;
                  r24.z = 0;
                  r25.x = 0;
                  while (true) {
                    r29.w = cmp((int)r25.x >= 8);
                    if (r29.w != 0) break;
                    r48.x = dot(icb[r25.x+0].xy, r13.xy);
                    r48.y = dot(icb[r25.x+0].yx, r13.xz);
                    r45.xy = r48.xy * r25.zz + r47.xy;
                    r29.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r45.xyz, r24.x).x;
                    r24.z = r29.w * 0.125 + r24.z;
                    r25.x = (int)r25.x + 1;
                  }
                } else {
                  r47.z = (uint)r24.y;
                  r24.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r47.xyz, r24.x).x;
                }
                r24.x = r24.z * r24.z;
                r24.x = r24.x * r24.z;
              } else {
                r24.x = 1;
              }
              r25.y = r25.y * r24.x;
            } else {
              if (4 == 0) r24.x = 0; else if (4+20 < 32) {               r24.x = (uint)r46.w << (32-(4 + 20)); r24.x = (uint)r24.x >> (32-4);              } else r24.x = (uint)r46.w >> 20;
              r24.y = cmp(0 < (uint)r24.x);
              r24.y = r24.y ? r3.w : 0;
              if (r24.y != 0) {
                r24.x = (int)r24.x + numLights;
                r24.x = (int)r24.x + -1;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r24.y, r24.x, l(52), t12.xxxx
              r24.y = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r24.x, l(68), t12.xyzw
              r47.x = samp0[]..swiz;
              r47.y = samp0[]..swiz;
              r47.z = samp0[]..swiz;
              r47.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r48.xyzw, r24.x, l(84), t12.xyzw
              r48.x = samp0[]..swiz;
              r48.y = samp0[]..swiz;
              r48.z = samp0[]..swiz;
              r48.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r49.xyzw, r24.x, l(100), t12.xyzw
              r49.x = samp0[]..swiz;
              r49.y = samp0[]..swiz;
              r49.z = samp0[]..swiz;
              r49.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r50.xyzw, r24.x, l(116), t12.xyzw
              r50.x = samp0[]..swiz;
              r50.y = samp0[]..swiz;
              r50.z = samp0[]..swiz;
              r50.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r51.xyzw, r24.x, l(132), t12.xyzw
              r51.x = samp0[]..swiz;
              r51.y = samp0[]..swiz;
              r51.z = samp0[]..swiz;
              r51.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r52.xyzw, r24.x, l(148), t12.xyzw
              r52.x = samp0[]..swiz;
              r52.y = samp0[]..swiz;
              r52.z = samp0[]..swiz;
              r52.w = samp0[]..swiz;
                r25.x = dot(r56.xyz, r6.xyz);
                r25.x = abs(r25.x) * -0.200000003 + 0.400000006;
                r27.xyz = r6.xyz * r25.xxx + r11.xyz;
                r45.x = dot(r47.xyzw, r27.xyzw);
                r45.y = dot(r48.xyzw, r27.xyzw);
                r25.x = dot(r49.xyzw, r27.xyzw);
                r25.z = dot(r50.xyzw, r27.xyzw);
                r27.x = cmp(r25.z < r25.x);
                r27.yz = r45.xy / r25.zz;
                r27.yz = r27.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r27.yz = r27.yz + r52.zw;
                r27.yz = r27.yz * r52.xy;
                r37.zw = r24.yy / r51.xz;
                r45.xy = float2(1,1) + -r37.zw;
                r45.xy = cmp(r27.yz >= r45.xy);
                r37.zw = cmp(r37.zw >= r27.yz);
                r37.zw = (int2)r37.zw | (int2)r45.xy;
                r29.w = (int)r37.w | (int)r37.z;
                r27.x = (int)r27.x | (int)r29.w;
                if (r27.x == 0) {
                // Known bad code for instruction (needs manual fix):
                                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r27.x, r24.x, l(28), t12.xxxx
                r27.x = samp0[]..swiz;
                // Known bad code for instruction (needs manual fix):
                                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.zw, r24.x, l(164), t12.xxxy
                r37.z = samp0[]..swiz;
                r37.w = samp0[]..swiz;
                  r27.yz = saturate(r27.yz);
                  r45.xy = r27.yz * r51.xz + r51.yw;
                  r24.x = r37.w * r25.z;
                  r25.x = r37.z * r25.z + r25.x;
                  r24.x = r25.x / r24.x;
                  r24.x = max(6.10351563e-05, r24.x);
                  r25.x = r27.x ? 0.000000 : 0;
                  if (r20.w != 0) {
                    r27.z = (uint)r25.x;
                    r25.z = 0;
                    r29.w = 0;
                    while (true) {
                      r30.w = cmp((int)r29.w >= 8);
                      if (r30.w != 0) break;
                      r47.x = dot(icb[r29.w+0].xy, r28.xy);
                      r47.y = dot(icb[r29.w+0].yx, r28.xz);
                      r27.xy = r47.xy * r24.yy + r45.xy;
                      r27.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r27.xyz, r24.x).x;
                      r25.z = r27.x * 0.125 + r25.z;
                      r29.w = (int)r29.w + 1;
                    }
                  } else {
                    r45.z = (uint)r25.x;
                    r25.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r45.xyz, r24.x).x;
                  }
                  r24.x = r25.z * r25.z;
                  r24.x = r24.x * r25.z;
                } else {
                  r24.x = 1;
                }
                r25.y = r25.y * r24.x;
              }
            }
            r24.x = r26.x * r25.y;
            r24.y = cmp(0 < r24.x);
            if (r24.y != 0) {
              r46.x = r45.w;
              r27.xyz = r46.xyz * r2.yzw;
              r24.y = saturate(dot(r1.xyz, r56.xyz));
              r27.xyz = r27.xyz * r57.xyz;
              r25.x = dot(r56.xyz, r0.xyz);
              r25.x = 0.200000003 + r25.x;
              r25.x = saturate(0.833333313 * r25.x);
              r25.x = r25.x * r25.x;
              r29.w = 1.25 * r25.x;
              r45.x = dot(r56.xyz, v8.xyz);
              r45.y = dot(r56.xyz, v9.xyz);
              r30.w = dot(r45.xy, r45.xy);
              r30.w = rsqrt(r30.w);
              r45.xy = r45.xy * r30.ww;
              r45.z = -r45.x;
              r46.x = dot(r4.yx, r45.yz);
              r46.y = dot(r4.yx, -r45.xy);
              r45.xy = r46.xy * float2(1,2) + float2(0.5,1);
              r30.w = dot(r56.xyz, r6.xyz);
              r30.w = r30.w * 0.689655125 + 0.310344785;
              r31.w = cmp(1 < r45.y);
              r33.w = 2 + -r45.y;
              r45.z = r31.w ? r33.w : r45.y;
              r31.w = 126.5 * r30.w;
              r33.w = floor(r31.w);
              r31.w = ceil(r31.w);
              r37.zw = float2(0.0078125,1) * r45.xz;
              r45.xy = r33.ww * float2(0.0078125,0) + r37.zw;
              r34.w = causticMap.SampleLevel(LinearSampler_s, r45.xy, 0).x;
              r37.zw = r31.ww * float2(0.0078125,0) + r37.zw;
              r31.w = causticMap.SampleLevel(LinearSampler_s, r37.zw, 0).x;
              r30.w = r30.w * 126.5 + -r33.w;
              r31.w = r31.w + -r34.w;
              r30.w = r30.w * r31.w + r34.w;
              r25.x = -r25.x * 1.25 + r30.w;
              r45.xyz = r5.xxx * r25.xxx + r29.www;
              r25.x = cmp(0 < r24.y);
              if (r25.x != 0) {
                r46.xyz = -r11.xyz * r9.www + r56.xyz;
                r25.x = dot(r46.xyz, r46.xyz);
                r25.x = rsqrt(r25.x);
                r46.xyz = r46.xyz * r25.xxx;
                r25.x = dot(r1.xyz, r46.xyz);
                r25.x = log2(abs(r25.x));
                r25.x = r25.x * r6.w;
                r25.x = exp2(r25.x);
                r25.x = r25.x * r9.y;
                r24.y = r25.x * r24.y;
                r24.y = r24.y * r32.w;
                r47.xyz = r24.yyy / r9.zzz;
                r24.y = dot(r46.xyz, r56.xyz);
                r24.y = saturate(1 + -r24.y);
                r25.x = r24.y * r24.y;
                r25.x = r25.x * r25.x;
                r24.y = r25.x * r24.y;
                r46.xyz = r47.zzz * r24.yyy;
              } else {
                r46.xyz = float3(0,0,0);
                r47.xyz = float3(0,0,0);
              }
              r24.y = dot(r27.xyz, float3(0.212599993,0.715200007,0.0722000003));
              r24.y = r24.x * r24.y;
              r25.x = cmp(r26.w < r24.y);
              if (r25.x != 0) {
                r25.x = r22.z * r22.w;
                r48.xyz = r25.xxx * r23.xyz;
                r42.xyz = r48.xyz * r39.xyz + r42.xyz;
                r44.xyz = r48.xyz * r40.xyz + r44.xyz;
                r43.xyz = r48.xyz * r41.xyz + r43.xyz;
                r23.xyz = r27.xyz;
                r38.xyz = r56.xyz;
                r39.xyz = r45.xyz;
                r40.xyz = r47.xyz;
                r41.xyz = r46.xyz;
                r26.w = r24.y;
                r22.z = r26.x;
                r22.w = r25.y;
              } else {
                r27.xyz = r27.xyz * r24.xxx;
                r42.xyz = r27.xyz * r45.zzz + r42.xyz;
                r44.xyz = r27.xyz * r47.xyz + r44.xyz;
                r43.xyz = r27.xyz * r46.xyz + r43.xyz;
              }
            }
          }
        }
      }
    }
    r29.xyz = r23.xyz;
    r30.xyz = r38.xyz;
    r31.xyz = r39.xyz;
    r32.xyz = r40.xyz;
    r33.xyz = r41.xyz;
    r34.xyz = r42.xyz;
    r35.xyz = r43.xyz;
    r18.zw = r22.zw;
    r37.y = r26.w;
    r36.xyz = r44.xyz;
    r16.w = (int)r16.w + 32;
  }
  r0.z = 0;
  r0.xy = float2(1,-1) * r30.yx;
  r2.yzw = r30.zxy * r0.yzx;
  r2.yzw = r30.yzx * r0.zxy + -r2.yzw;
  r4.xyw = viewProjectionMatrix._m10_m11_m13 * r16.yyy;
  r4.xyw = r16.xxx * viewProjectionMatrix._m00_m01_m03 + r4.xyw;
  r4.xyw = r16.zzz * viewProjectionMatrix._m20_m21_m23 + r4.xyw;
  r4.xyw = r4.xyw + r7.xyz;
  r6.z = rcp(r4.w);
  r4.xy = r6.zz * r4.xy;
  r6.xy = r4.xy * float2(0.5,-0.5) + float2(0.5,0.5);
  r9.yzw = r30.xyz * float3(2.4000001,2.4000001,2.4000001) + r16.xyz;
  r12.xyz = r9.xxx * r2.yzw;
  r12.xyz = r10.xxx * r0.xyz + r12.xyz;
  r12.xyz = r12.xyz * float3(0.119999997,0.119999997,0.119999997) + r9.yzw;
  r13.xyz = viewProjectionMatrix._m10_m11_m13 * r12.yyy;
  r12.xyw = r12.xxx * viewProjectionMatrix._m00_m01_m03 + r13.xyz;
  r12.xyz = r12.zzz * viewProjectionMatrix._m20_m21_m23 + r12.xyw;
  r12.xyz = r12.xyz + r7.xyz;
  r4.xy = float2(0.5,-0.5) * r12.xy;
  r12.z = rcp(r12.z);
  r12.xy = r4.xy * r12.zz + float2(0.5,0.5);
  r13.xyz = -r6.xyz;
  r12.xyz = r13.xyz + r12.xyz;
  r3.w = 3.14159989 + r5.w;
  sincos(r3.w, r4.x, r9.x);
  r2.yzw = r4.xxx * r2.yzw;
  r0.xyz = r9.xxx * r0.xyz + r2.yzw;
  r0.xyz = r0.xyz * float3(0.119999997,0.119999997,0.119999997) + r9.yzw;
  r2.yzw = viewProjectionMatrix._m10_m11_m13 * r0.yyy;
  r2.yzw = r0.xxx * viewProjectionMatrix._m00_m01_m03 + r2.yzw;
  r0.xyz = r0.zzz * viewProjectionMatrix._m20_m21_m23 + r2.yzw;
  r0.xyz = r0.xyz + r7.xyz;
  r0.xy = float2(0.5,-0.5) * r0.xy;
  r7.z = rcp(r0.z);
  r7.xy = r0.xy * r7.zz + r13.xy;
  r13.w = 0.5;
  r0.xyz = r7.xyz + r13.wwz;
  r2.y = dot(r12.xy, r12.xy);
  r2.y = sqrt(r2.y);
  r2.y = rcp(r2.y);
  r2.y = r4.z * r2.y;
  r2.zw = r1.ww * float2(0.5,0.5) + float2(0.333333343,1);
  r2.zw = float2(0.117647059,0.117647059) * r2.zw;
  r2.zw = r2.zw * r2.zw;
  x4[0].x = r2.z;
  r4.xyz = r12.xyz * r2.zzz + r6.xyz;
  r7.xy = cmp(r2.yy < r2.zw);
  r4.xy = renderTargetSize.xy * r4.xy;
  r9.xy = (int2)r4.xy;
  r9.zw = float2(0,0);
  r3.w = floatZSampler.Load(r9.xyz).x;
  r4.x = cmp(r3.w >= 0.984375);
  r4.y = 1.01587307 * r3.w;
  r3.w = r3.w * 64 + -63;
  r3.w = r4.x ? r3.w : r4.y;
  r3.w = max(9.99999994e-09, r3.w);
  r3.w = rcp(r3.w);
  x5[0].x = r3.w;
  r4.x = 0.0419999994 + r3.w;
  r4.x = r4.z * r4.x;
  r4.x = cmp(1 < r4.x);
  r4.x = r4.x ? 1.000000 : 0;
  r4.x = r7.x ? r4.x : 1;
  x3[0].x = r4.x;
  x4[1].x = r2.w;
  r7.xzw = r0.xyz * r2.www + r6.xyz;
  r4.yz = renderTargetSize.xy * r7.xz;
  r9.xy = (int2)r4.yz;
  r9.zw = float2(0,0);
  r4.y = floatZSampler.Load(r9.xyz).x;
  r4.z = cmp(r4.y >= 0.984375);
  r5.w = 1.01587307 * r4.y;
  r4.y = r4.y * 64 + -63;
  r4.y = r4.z ? r4.y : r5.w;
  r4.y = max(9.99999994e-09, r4.y);
  r4.y = rcp(r4.y);
  x5[1].x = r4.y;
  r4.z = 0.0419999994 + r4.y;
  r4.z = r7.w * r4.z;
  r4.z = cmp(1 < r4.z);
  r4.z = r4.z ? 1.000000 : 0;
  r4.z = r7.y ? r4.z : 1;
  x3[1].x = r4.z;
  r5.w = r4.x + r4.z;
  r5.w = cmp(r5.w != 0.000000);
  if (r5.w != 0) {
    r7.xy = r1.ww * float2(0.5,0.5) + float2(1.33333337,2);
    r7.xy = float2(0.117647059,0.117647059) * r7.xy;
    r7.xy = r7.xy * r7.xy + -r2.zw;
    r2.z = r4.x * r7.x + r2.z;
    x4[0].x = r2.z;
    r7.xzw = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r7.xz = renderTargetSize.xy * r7.xz;
    r9.xy = (int2)r7.xz;
    r9.zw = float2(0,0);
    r5.w = floatZSampler.Load(r9.xyz).x;
    r6.w = cmp(r5.w >= 0.984375);
    r7.x = 1.01587307 * r5.w;
    r5.w = r5.w * 64 + -63;
    r5.w = r6.w ? r5.w : r7.x;
    r5.w = max(9.99999994e-09, r5.w);
    r5.w = rcp(r5.w);
    r3.w = r2.z ? r5.w : r3.w;
    x5[0].x = r3.w;
    r3.w = 0.0419999994 + r5.w;
    r3.w = r7.w * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r2.w = r4.z * r7.y + r2.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r7.xy = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r7.xy;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.x = cmp(r3.w >= 0.984375);
    r5.w = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.x ? r3.w : r5.w;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.x = r2.w ? r3.w : r4.y;
    x5[1].x = r4.x;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.z * r3.w;
    r2.w = r2.w ? r3.w : r4.z;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(2.33333325,3);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r3.w = x4[0].x;
    r4.x = x3[0].x;
    r2.z = r2.z * r2.z + -r3.w;
    r2.z = r4.x * r2.z + r3.w;
    x4[0].x = r2.z;
    r7.xyz = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[0].x;
    r4.y = r2.z ? r3.w : r4.y;
    x5[0].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r3.w = x4[1].x;
    r4.x = x3[1].x;
    r2.w = r2.w * r2.w + -r3.w;
    r2.w = r4.x * r2.w + r3.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[1].x;
    r4.y = r2.w ? r3.w : r4.y;
    x5[1].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.w = r2.w ? r3.w : r4.x;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(3.33333325,4);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r3.w = x4[0].x;
    r4.x = x3[0].x;
    r2.z = r2.z * r2.z + -r3.w;
    r2.z = r4.x * r2.z + r3.w;
    x4[0].x = r2.z;
    r7.xyz = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[0].x;
    r4.y = r2.z ? r3.w : r4.y;
    x5[0].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r3.w = x4[1].x;
    r4.x = x3[1].x;
    r2.w = r2.w * r2.w + -r3.w;
    r2.w = r4.x * r2.w + r3.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[1].x;
    r4.y = r2.w ? r3.w : r4.y;
    x5[1].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.w = r2.w ? r3.w : r4.x;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(4.33333349,5);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r3.w = x4[0].x;
    r4.x = x3[0].x;
    r2.z = r2.z * r2.z + -r3.w;
    r2.z = r4.x * r2.z + r3.w;
    x4[0].x = r2.z;
    r7.xyz = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[0].x;
    r4.y = r2.z ? r3.w : r4.y;
    x5[0].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r3.w = x4[1].x;
    r4.x = x3[1].x;
    r2.w = r2.w * r2.w + -r3.w;
    r2.w = r4.x * r2.w + r3.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[1].x;
    r4.y = r2.w ? r3.w : r4.y;
    x5[1].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.w = r2.w ? r3.w : r4.x;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(5.33333349,6);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r3.w = x4[0].x;
    r4.x = x3[0].x;
    r2.z = r2.z * r2.z + -r3.w;
    r2.z = r4.x * r2.z + r3.w;
    x4[0].x = r2.z;
    r7.xyz = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[0].x;
    r4.y = r2.z ? r3.w : r4.y;
    x5[0].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r3.w = x4[1].x;
    r4.x = x3[1].x;
    r2.w = r2.w * r2.w + -r3.w;
    r2.w = r4.x * r2.w + r3.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[1].x;
    r4.y = r2.w ? r3.w : r4.y;
    x5[1].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.w = r2.w ? r3.w : r4.x;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(6.33333349,7);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r3.w = x4[0].x;
    r4.x = x3[0].x;
    r2.z = r2.z * r2.z + -r3.w;
    r2.z = r4.x * r2.z + r3.w;
    x4[0].x = r2.z;
    r7.xyz = r12.xyz * r2.zzz + r6.xyz;
    r2.z = cmp(r2.y < r2.z);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[0].x;
    r4.y = r2.z ? r3.w : r4.y;
    x5[0].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.z = r2.z ? r3.w : r4.x;
    x3[0].x = r2.z;
    r3.w = x4[1].x;
    r4.x = x3[1].x;
    r2.w = r2.w * r2.w + -r3.w;
    r2.w = r4.x * r2.w + r3.w;
    x4[1].x = r2.w;
    r7.xyz = r0.xyz * r2.www + r6.xyz;
    r2.w = cmp(r2.y < r2.w);
    r4.yz = renderTargetSize.xy * r7.xy;
    r9.xy = (int2)r4.yz;
    r9.zw = float2(0,0);
    r3.w = floatZSampler.Load(r9.xyz).x;
    r4.y = cmp(r3.w >= 0.984375);
    r4.z = 1.01587307 * r3.w;
    r3.w = r3.w * 64 + -63;
    r3.w = r4.y ? r3.w : r4.z;
    r3.w = max(9.99999994e-09, r3.w);
    r3.w = rcp(r3.w);
    r4.y = x5[1].x;
    r4.y = r2.w ? r3.w : r4.y;
    x5[1].x = r4.y;
    r3.w = 0.0419999994 + r3.w;
    r3.w = r7.z * r3.w;
    r3.w = cmp(1 < r3.w);
    r3.w = r3.w ? 1.000000 : 0;
    r3.w = r4.x * r3.w;
    r2.w = r2.w ? r3.w : r4.x;
    x3[1].x = r2.w;
    r2.z = r2.z + r2.w;
    r2.z = cmp(r2.z == 0.000000);
  } else {
    r2.z = -1;
  }
  if (r2.z == 0) {
    r2.zw = r1.ww * float2(0.5,0.5) + float2(7.33333349,8);
    r2.zw = float2(0.117647059,0.117647059) * r2.zw;
    r1.w = x4[0].x;
    r3.w = x3[0].x;
    r2.z = r2.z * r2.z + -r1.w;
    r1.w = r3.w * r2.z + r1.w;
    r4.xyz = r12.xyz * r1.www + r6.xyz;
    r1.w = cmp(r2.y < r1.w);
    r4.xy = renderTargetSize.xy * r4.xy;
    r7.xy = (int2)r4.xy;
    r7.zw = float2(0,0);
    r2.z = floatZSampler.Load(r7.xyz).x;
    r4.x = cmp(r2.z >= 0.984375);
    r4.y = 1.01587307 * r2.z;
    r2.z = r2.z * 64 + -63;
    r2.z = r4.x ? r2.z : r4.y;
    r2.z = max(9.99999994e-09, r2.z);
    r2.z = rcp(r2.z);
    r4.x = x5[0].x;
    r4.x = r1.w ? r2.z : r4.x;
    x5[0].x = r4.x;
    r2.z = 0.0419999994 + r2.z;
    r2.z = r4.z * r2.z;
    r2.z = cmp(1 < r2.z);
    r2.z = r2.z ? 1.000000 : 0;
    r2.z = r3.w * r2.z;
    r1.w = r1.w ? r2.z : r3.w;
    x3[0].x = r1.w;
    r1.w = x4[1].x;
    r2.z = x3[1].x;
    r2.w = r2.w * r2.w + -r1.w;
    r1.w = r2.z * r2.w + r1.w;
    r0.xyz = r0.xyz * r1.www + r6.xyz;
    r1.w = cmp(r2.y < r1.w);
    r0.xy = renderTargetSize.xy * r0.xy;
    r6.xy = (int2)r0.xy;
    r6.zw = float2(0,0);
    r0.x = floatZSampler.Load(r6.xyz).x;
    r0.y = cmp(r0.x >= 0.984375);
    r2.y = 1.01587307 * r0.x;
    r0.x = r0.x * 64 + -63;
    r0.x = r0.y ? r0.x : r2.y;
    r0.x = max(9.99999994e-09, r0.x);
    r0.x = rcp(r0.x);
    r0.y = x5[1].x;
    r0.y = r1.w ? r0.x : r0.y;
    x5[1].x = r0.y;
    r0.x = 0.0419999994 + r0.x;
    r0.x = r0.z * r0.x;
    r0.x = cmp(1 < r0.x);
    r0.x = r0.x ? 1.000000 : 0;
    r0.x = r2.z * r0.x;
    r0.x = r1.w ? r0.x : r2.z;
    x3[1].x = r0.x;
  }
  r0.x = x5[0].x;
  r0.x = r0.x + -r4.w;
  r0.x = cmp(2.4000001 < abs(r0.x));
  r0.y = x3[0].x;
  r0.y = -r0.y * 0.5 + 1;
  r0.x = r0.x ? 0.5 : r0.y;
  r0.y = x5[1].x;
  r0.y = r0.y + -r4.w;
  r0.y = cmp(2.4000001 < abs(r0.y));
  r0.z = x3[1].x;
  r0.z = 0.5 * r0.z;
  r0.y = r0.y ? -0.5 : -r0.z;
  r0.x = r0.x + r0.y;
  r0.x = saturate(1 + -r0.x);
  r0.y = r0.x * -2 + 3;
  r0.x = r0.x * r0.x;
  r0.x = r0.y * r0.x;
  r0.x = min(r18.w, r0.x);
  r0.x = r18.z * r0.x;
  r0.xyz = r0.xxx * r29.xyz;
  r2.yzw = r0.xyz * r31.xyz + r34.xyz;
  r4.xyz = r0.xyz * r32.xyz + r36.xyz;
  r0.xyz = r0.xyz * r33.xyz + r35.xyz;
  r1.w = saturate(dot(r8.xyz, r10.yzw));
  r6.y = 0.150000006 + r0.w;
  r0.w = 1 + -r6.y;
  r3.w = 5 * r0.w;
  r4.w = r0.w * 5 + -2.5;
  r4.w = 0.400000006 * r4.w;
  r4.w = max(0, r4.w);
  r4.w = 100 * r4.w;
  r6.zw = -r0.ww * float2(10,5) + float2(6.85740995,7.08500004);
  r5.w = exp2(r6.z);
  r5.w = r5.w * r1.w;
  r5.w = 9.1368103 * r5.w;
  r6.z = r3.w * r0.w;
  r6.z = -r6.z * 2.0159049 + r6.w;
  r6.z = exp2(r6.z);
  r1.w = r6.z * r1.w;
  r1.w = 9.70808983 * r1.w;
  r1.w = max(r5.w, r1.w);
  r1.w = max(1.26815999, r1.w);
  r5.w = dot(-r10.yzw, r1.xyz);
  r5.w = r5.w + r5.w;
  r7.xyz = r1.xyz * -r5.www + -r10.yzw;
  r5.w = numRefProbes + -numOverrideProbes;
  r6.z = (int)r5.w & -32;
  r6.w = (int)r5.w + (int)-r6.z;
  r9.x = numRefProbes & -32;
  r9.y = (int)-r9.x + numRefProbes;
  r12.xy = float2(0,0);
  r13.w = 0;
  r14.yz = float2(0,1);
  r15.xy = float2(0,0);
  r16.x = 1;
  r17.xy = float2(0,0);
  r18.xy = float2(0,0);
  r19.xyzw = float4(0,0,0,0);
  r21.yzw = float3(0,0,0);
  r9.z = r6.z;
  while (true) {
    r9.w = cmp((uint)r9.z >= numRefProbes);
    if (r9.w != 0) break;
    r12.z = (uint)r9.z >> 5;
    r13.xyz = (int3)r12.xyz + (int3)r20.xyz;
    r9.w = visibleProbes.Load(r13.xyzw).x;
    r10.x = cmp((int)r6.z == (int)r9.z);
    bitmask.w = ((~(-1 << r6.w)) << 0) & 0xffffffff;  r11.w = (((uint)0 << 0) & bitmask.w) | ((uint)r9.w & ~bitmask.w);
    r9.w = r10.x ? r11.w : r9.w;
    r10.x = cmp((int)r9.x == (int)r9.z);
    if (r9.y == 0) r11.w = 0; else if (r9.y+0 < 32) {     r11.w = (uint)r9.w << (32-(r9.y + 0)); r11.w = (uint)r11.w >> (32-r9.y);    } else r11.w = (uint)r9.w >> 0;
    r9.w = r10.x ? r11.w : r9.w;
    r10.x = (int)r9.z + numLights;
    r22.xyzw = r19.xyzw;
    r23.xyz = r21.yzw;
    r11.w = r9.w;
    while (true) {
      if (r11.w == 0) break;
      r12.z = firstbitlow((uint)r11.w);
      r12.w = 1 << (int)r12.z;
      r13.x = (int)r11.w & (int)r12.w;
      if (r13.x != 0) {
        r11.w = (int)r11.w ^ (int)r12.w;
        r12.w = (int)r10.x + (int)r12.z;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r24.xyzw, r12.w, l(0), t11.xyzw
      r24.x = samp0[]..swiz;
      r24.y = samp0[]..swiz;
      r24.z = samp0[]..swiz;
      r24.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r13.xy, r12.w, l(16), t11.xyxx
      r13.x = samp0[]..swiz;
      r13.y = samp0[]..swiz;
        r24.xyz = r24.xyz + -r11.xyz;
        r13.z = r24.w;
        r13.xyz = cmp(abs(r24.xyz) < r13.zxy);
        r12.w = r13.y ? r13.x : 0;
        r12.w = r13.z ? r12.w : 0;
        if (r12.w != 0) {
          r12.z = (int)r9.z + (int)r12.z;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r12.z, l(0), t15.wxyz
        r24.x = samp0[]..swiz;
        r24.y = samp0[]..swiz;
        r24.z = samp0[]..swiz;
        r24.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r13.xy, r12.z, l(96), t15.xyxx
        r13.x = samp0[]..swiz;
        r13.y = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r25.xyzw, r12.z, l(116), t15.zwxy
        r25.x = samp0[]..swiz;
        r25.y = samp0[]..swiz;
        r25.z = samp0[]..swiz;
        r25.w = samp0[]..swiz;
          r26.xyz = -r24.yzw + r11.xyz;
          r12.w = (int)r25.w & 0x0000ffff;
          if (6 == 0) r13.z = 0; else if (6+25 < 32) {           r13.z = (uint)r25.w << (32-(6 + 25)); r13.z = (uint)r13.z >> (32-6);          } else r13.z = (uint)r25.w >> 25;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r12.w, l(0), t16.xyzw
        r27.x = samp0[]..swiz;
        r27.y = samp0[]..swiz;
        r27.z = samp0[]..swiz;
        r27.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r12.w, l(16), t16.xyzw
        r28.x = samp0[]..swiz;
        r28.y = samp0[]..swiz;
        r28.z = samp0[]..swiz;
        r28.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r12.w, l(32), t16.xyzw
        r29.x = samp0[]..swiz;
        r29.y = samp0[]..swiz;
        r29.z = samp0[]..swiz;
        r29.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r12.w, l(48), t16.xyzw
        r30.x = samp0[]..swiz;
        r30.y = samp0[]..swiz;
        r30.z = samp0[]..swiz;
        r30.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r12.w, l(64), t16.xyzw
        r31.x = samp0[]..swiz;
        r31.y = samp0[]..swiz;
        r31.z = samp0[]..swiz;
        r31.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r12.w, l(80), t16.xyzw
        r32.x = samp0[]..swiz;
        r32.y = samp0[]..swiz;
        r32.z = samp0[]..swiz;
        r32.w = samp0[]..swiz;
          r14.w = dot(r27.xyz, r26.xyz);
          r14.w = saturate(r14.w + r27.w);
          r15.w = dot(r28.xyz, r26.xyz);
          r15.w = saturate(r15.w + r28.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r29.xyz, r26.xyz);
          r15.w = saturate(r15.w + r29.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r30.xyz, r26.xyz);
          r15.w = saturate(r15.w + r30.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r31.xyz, r26.xyz);
          r15.w = saturate(r15.w + r31.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r32.xyz, r26.xyz);
          r15.w = saturate(r15.w + r32.w);
          r14.x = r15.w * r14.w;
          r14.w = r13.x ? 0.000000 : 0;
          r14.xw = r14.ww ? r14.xy : r14.zx;
          r15.w = r13.x;
          r27.xy = r14.xw;
          r16.z = 1;
          while (true) {
            r16.w = cmp((int)r16.z >= (int)r13.z);
            if (r16.w != 0) break;
            r16.w = (int)r12.w + (int)r16.z;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r16.w, l(0), t16.xyzw
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r16.w, l(16), t16.xyzw
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r16.w, l(32), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r16.w, l(48), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r16.w, l(64), t16.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r16.w, l(80), t16.xyzw
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
            r16.w = dot(r28.xyz, r26.xyz);
            r16.w = saturate(r16.w + r28.w);
            r16.w = r27.x * r16.w;
            r17.w = dot(r29.xyz, r26.xyz);
            r17.w = saturate(r17.w + r29.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r30.xyz, r26.xyz);
            r17.w = saturate(r17.w + r30.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r31.xyz, r26.xyz);
            r17.w = saturate(r17.w + r31.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r32.xyz, r26.xyz);
            r17.w = saturate(r17.w + r32.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r33.xyz, r26.xyz);
            r17.w = saturate(r17.w + r33.w);
            r27.x = r17.w * r16.w;
            r18.w = (uint)r15.w >> 2;
            if (1 == 0) r20.w = 0; else if (1+2 < 32) {             r20.w = (uint)r15.w << (32-(1 + 2)); r20.w = (uint)r20.w >> (32-1);            } else r20.w = (uint)r15.w >> 2;
            r24.w = (int)r18.w & 2;
            r26.w = max(r27.y, r27.x);
            r16.w = -r16.w * r17.w + 1;
            r16.w = r27.y * r16.w;
            r16.y = r24.w ? r16.w : r26.w;
            r27.xy = r20.ww ? r27.xy : r16.xy;
            r16.z = (int)r16.z + 1;
            r15.w = r18.w;
          }
          r27.y = saturate(r27.y);
          r12.w = r27.y * r13.y;
          r13.x = cmp(0 < r12.w);
          if (r13.x != 0) {
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r28.xyzw, r12.z, l(16), t15.xyzw
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r29.xyzw, r12.z, l(32), t15.yxwz
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r12.z, l(48), t15.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r12.z, l(64), t15.zwxy
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r12.z, l(80), t15.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r12.z, l(132), t15.zwxy
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r12.z, l(148), t15.zwxy
          r34.x = samp0[]..swiz;
          r34.y = samp0[]..swiz;
          r34.z = samp0[]..swiz;
          r34.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r12.z, l(164), t15.zwxy
          r35.x = samp0[]..swiz;
          r35.y = samp0[]..swiz;
          r35.z = samp0[]..swiz;
          r35.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r12.z, l(180), t15.zwxy
          r36.x = samp0[]..swiz;
          r36.y = samp0[]..swiz;
          r36.z = samp0[]..swiz;
          r36.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xyzw, r12.z, l(196), t15.xyzw
          r37.x = samp0[]..swiz;
          r37.y = samp0[]..swiz;
          r37.z = samp0[]..swiz;
          r37.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r38.xy, r12.z, l(212), t15.xyxx
          r38.x = samp0[]..swiz;
          r38.y = samp0[]..swiz;
            r23.z = r27.y * r13.y + r23.z;
            r12.z = r12.w * r25.z;
            r25.z = r33.z;
            r12.w = dot(r7.xyz, r25.xyz);
            r13.x = dot(r26.xyz, r25.xyz);
            r13.x = r13.x + -r33.w;
            r13.y = cmp(r13.x >= 0);
            r13.x = max(abs(r13.x), r4.w);
            r13.x = r13.y ? r13.x : -r13.x;
            r12.w = max(1.00000001e-07, -r12.w);
            r12.w = r13.x / r12.w;
            r12.w = min(131072, abs(r12.w));
            r33.z = r34.z;
            r13.x = dot(r7.xyz, r33.xyz);
            r13.y = dot(r26.xyz, r33.xyz);
            r13.y = r13.y + -r34.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r4.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r34.z = r35.z;
            r13.x = dot(r7.xyz, r34.xyz);
            r13.y = dot(r26.xyz, r34.xyz);
            r13.y = r13.y + -r35.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r4.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r35.z = r36.z;
            r13.x = dot(r7.xyz, r35.xyz);
            r13.y = dot(r26.xyz, r35.xyz);
            r13.y = r13.y + -r36.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r4.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r36.z = r37.x;
            r13.x = dot(r7.xyz, r36.xyz);
            r13.y = dot(r26.xyz, r36.xyz);
            r13.y = r13.y + -r37.y;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r4.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r38.zw = r37.zw;
            r13.x = dot(r7.zxy, r38.xzw);
            r13.y = dot(r26.zxy, r38.xzw);
            r13.y = r13.y + -r38.y;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r4.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r13.x = r30.w;
            r13.yz = r31.zw;
            r13.xyz = r13.xyz + r26.xyz;
            r13.xyz = r7.xyz * r12.www + r13.xyz;
            r14.x = dot(r13.xyz, r13.xyz);
            r14.x = sqrt(r14.x);
            r12.w = r12.w / r14.x;
            r12.w = r12.w + r12.w;
            r12.w = sqrt(r12.w);
            r12.w = r0.w * 5 + r12.w;
            r12.w = -0.844799995 + r12.w;
            r24.y = r28.z;
            r24.z = r29.x;
            r33.x = dot(r13.xyz, r24.xyz);
            r25.xy = r28.xw;
            r25.z = r29.w;
            r33.y = dot(r13.xyz, r25.xyz);
            r29.x = r28.y;
            r33.z = dot(r13.xyz, r29.xyz);
            if (9 == 0) r13.x = 0; else if (9+16 < 32) {             r13.x = (uint)r25.w << (32-(9 + 16)); r13.x = (uint)r13.x >> (32-9);            } else r13.x = (uint)r25.w >> 16;
            r33.w = (uint)r13.x;
            r13.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r33.xyzw, r12.w).xyz;
            r28.x = dot(r26.xyz, r24.xyz);
            r28.y = dot(r26.xyz, r25.xyz);
            r28.z = dot(r26.xyz, r29.xyz);
            r16.yzw = saturate(r28.xyz * r30.xyz + float3(0.5,0.5,0.5));
            r31.z = r32.x;
            r16.yzw = r16.yzw * r31.xyz + r32.yzw;
            r33.x = dot(r8.xyz, r24.xyz);
            r33.y = dot(r8.xyz, r25.xyz);
            r33.z = dot(r8.xyz, r29.xyz);
            r24.xyz = cmp(float3(0,0,0) < r33.xyz);
            r15.z = r24.x ? 0 : 0.5;
            r25.xyz = r16.yzw + r15.xyz;
            r25.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r25.xyz, 0).xyz;
            r26.xyz = r33.xyz * r33.xyz;
            r26.xyz = r26.xyz * r12.zzz;
            r17.z = r24.y ? 0 : 0.5;
            r24.xyw = r17.xyz + r16.yzw;
            r24.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r24.xyw, 0).xyz;
            r24.xyw = r24.xyw * r26.yyy;
            r24.xyw = r25.xyz * r26.xxx + r24.xyw;
            r18.z = r24.z ? 0 : 0.5;
            r16.yzw = r18.xyz + r16.yzw;
            r16.yzw = gProbeZArray.SampleLevel(samplerLinearClamp_s, r16.yzw, 0).xyz;
            r16.yzw = r16.yzw * r26.zzz + r24.xyw;
            r24.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r33.xyzw, 6).xyz;
            r25.xyz = r24.xyz * r16.yzw;
            r12.w = dot(r25.xyz, float3(0.212599993,0.715200007,0.0722000003));
            r22.xyz = r16.yzw * r24.xyz + r22.xyz;
            r14.x = r12.w * r1.w;
            r13.xyz = r13.xyz * r12.zzz;
            r12.z = dot(r13.xyz, float3(0.212599993,0.715200007,0.0722000003));
            r12.z = r1.w * r12.w + r12.z;
            r12.z = r14.x / r12.z;
            r24.x = r22.w;
            r24.yz = r23.xy;
            r23.xyw = r13.yzx * r12.zzz + r24.yzx;
            r22.w = r23.w;
          }
        }
      }
    }
    r19.xyzw = r22.xyzw;
    r21.yzw = r23.xyz;
    r9.z = (int)r9.z + 32;
  }
  r9.x = cmp(r21.w < 1);
  if (r9.x != 0) {
    r9.xy = float2(0,0);
    r12.w = 0;
    r13.yz = float2(0,1);
    r14.xy = float2(0,0);
    r15.x = 1;
    r16.xy = float2(0,0);
    r17.xy = float2(0,0);
    r18.x = r19.w;
    r18.yzw = r21.yzw;
    r22.xyz = r19.xyz;
    r9.w = r21.w;
    r10.x = 0;
    while (true) {
      r11.w = cmp((uint)r10.x >= (uint)r5.w);
      if (r11.w != 0) break;
      r9.z = (uint)r10.x >> 5;
      r12.xyz = (int3)r9.xyz + (int3)r20.xyz;
      r9.z = visibleProbes.Load(r12.xyzw).x;
      r11.w = cmp((int)r6.z == (int)r10.x);
      if (r6.w == 0) r12.x = 0; else if (r6.w+0 < 32) {       r12.x = (uint)r9.z << (32-(r6.w + 0)); r12.x = (uint)r12.x >> (32-r6.w);      } else r12.x = (uint)r9.z >> 0;
      r9.z = r11.w ? r12.x : r9.z;
      r11.w = (int)r10.x + numLights;
      r23.xyzw = r18.xyzw;
      r12.xyz = r22.xyz;
      r13.w = r9.w;
      r14.w = r9.z;
      while (true) {
        if (r14.w == 0) break;
        r15.z = firstbitlow((uint)r14.w);
        r15.w = 1 << (int)r15.z;
        r16.w = (int)r14.w & (int)r15.w;
        if (r16.w != 0) {
          r14.w = (int)r14.w ^ (int)r15.w;
          r15.w = (int)r11.w + (int)r15.z;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r24.xyzw, r15.w, l(0), t11.xyzw
        r24.x = samp0[]..swiz;
        r24.y = samp0[]..swiz;
        r24.z = samp0[]..swiz;
        r24.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r25.xy, r15.w, l(16), t11.xyxx
        r25.x = samp0[]..swiz;
        r25.y = samp0[]..swiz;
          r24.xyz = r24.xyz + -r11.xyz;
          r25.z = r24.w;
          r24.xyz = cmp(abs(r24.xyz) < r25.zxy);
          r15.w = r24.y ? r24.x : 0;
          r15.w = r24.z ? r15.w : 0;
          if (r15.w != 0) {
            r15.z = (int)r10.x + (int)r15.z;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r15.z, l(0), t15.wxyz
          r24.x = samp0[]..swiz;
          r24.y = samp0[]..swiz;
          r24.z = samp0[]..swiz;
          r24.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r25.xy, r15.z, l(96), t15.xyxx
          r25.x = samp0[]..swiz;
          r25.y = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r26.xyzw, r15.z, l(116), t15.zwxy
          r26.x = samp0[]..swiz;
          r26.y = samp0[]..swiz;
          r26.z = samp0[]..swiz;
          r26.w = samp0[]..swiz;
            r27.xyz = -r24.yzw + r11.xyz;
            r15.w = (int)r26.w & 0x0000ffff;
            if (6 == 0) r16.w = 0; else if (6+25 < 32) {             r16.w = (uint)r26.w << (32-(6 + 25)); r16.w = (uint)r16.w >> (32-6);            } else r16.w = (uint)r26.w >> 25;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r15.w, l(0), t16.xyzw
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r15.w, l(16), t16.xyzw
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r15.w, l(32), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r15.w, l(48), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r15.w, l(64), t16.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r15.w, l(80), t16.xyzw
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
            r17.w = dot(r28.xyz, r27.xyz);
            r17.w = saturate(r17.w + r28.w);
            r20.w = dot(r29.xyz, r27.xyz);
            r20.w = saturate(r20.w + r29.w);
            r17.w = r20.w * r17.w;
            r20.w = dot(r30.xyz, r27.xyz);
            r20.w = saturate(r20.w + r30.w);
            r17.w = r20.w * r17.w;
            r20.w = dot(r31.xyz, r27.xyz);
            r20.w = saturate(r20.w + r31.w);
            r17.w = r20.w * r17.w;
            r20.w = dot(r32.xyz, r27.xyz);
            r20.w = saturate(r20.w + r32.w);
            r17.w = r20.w * r17.w;
            r20.w = dot(r33.xyz, r27.xyz);
            r20.w = saturate(r20.w + r33.w);
            r13.x = r20.w * r17.w;
            r17.w = (int)r25.x & 1;
            r25.zw = r17.ww ? r13.xy : r13.zx;
            r13.x = r25.x;
            r28.xy = r25.zw;
            r17.w = 1;
            while (true) {
              r20.w = cmp((int)r17.w >= (int)r16.w);
              if (r20.w != 0) break;
              r20.w = (int)r15.w + (int)r17.w;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r20.w, l(0), t16.xyzw
            r29.x = samp0[]..swiz;
            r29.y = samp0[]..swiz;
            r29.z = samp0[]..swiz;
            r29.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r20.w, l(16), t16.xyzw
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r20.w, l(32), t16.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r20.w, l(48), t16.xyzw
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r20.w, l(64), t16.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r20.w, l(80), t16.xyzw
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
              r20.w = dot(r29.xyz, r27.xyz);
              r20.w = saturate(r20.w + r29.w);
              r20.w = r28.x * r20.w;
              r22.w = dot(r30.xyz, r27.xyz);
              r22.w = saturate(r22.w + r30.w);
              r20.w = r22.w * r20.w;
              r22.w = dot(r31.xyz, r27.xyz);
              r22.w = saturate(r22.w + r31.w);
              r20.w = r22.w * r20.w;
              r22.w = dot(r32.xyz, r27.xyz);
              r22.w = saturate(r22.w + r32.w);
              r20.w = r22.w * r20.w;
              r22.w = dot(r33.xyz, r27.xyz);
              r22.w = saturate(r22.w + r33.w);
              r20.w = r22.w * r20.w;
              r22.w = dot(r34.xyz, r27.xyz);
              r22.w = saturate(r22.w + r34.w);
              r28.x = r22.w * r20.w;
              r24.w = (uint)r13.x >> 2;
              if (1 == 0) r27.w = 0; else if (1+2 < 32) {               r27.w = (uint)r13.x << (32-(1 + 2)); r27.w = (uint)r27.w >> (32-1);              } else r27.w = (uint)r13.x >> 2;
              r28.z = (int)r24.w & 2;
              r28.w = max(r28.y, r28.x);
              r20.w = -r20.w * r22.w + 1;
              r20.w = r28.y * r20.w;
              r15.y = r28.z ? r20.w : r28.w;
              r28.xy = r27.ww ? r28.xy : r15.xy;
              r17.w = (int)r17.w + 1;
              r13.x = r24.w;
            }
            r13.x = saturate(r28.y + -r13.w);
            r15.y = r13.x * r25.y;
            r15.w = cmp(0 < r15.y);
            if (r15.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r29.xyzw, r15.z, l(16), t15.xyzw
            r29.x = samp0[]..swiz;
            r29.y = samp0[]..swiz;
            r29.z = samp0[]..swiz;
            r29.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r15.z, l(32), t15.yxwz
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r15.z, l(48), t15.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r15.z, l(64), t15.zwxy
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r15.z, l(80), t15.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r15.z, l(132), t15.zwxy
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r15.z, l(148), t15.zwxy
            r35.x = samp0[]..swiz;
            r35.y = samp0[]..swiz;
            r35.z = samp0[]..swiz;
            r35.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r15.z, l(164), t15.zwxy
            r36.x = samp0[]..swiz;
            r36.y = samp0[]..swiz;
            r36.z = samp0[]..swiz;
            r36.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xyzw, r15.z, l(180), t15.zwxy
            r37.x = samp0[]..swiz;
            r37.y = samp0[]..swiz;
            r37.z = samp0[]..swiz;
            r37.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r38.xyzw, r15.z, l(196), t15.xyzw
            r38.x = samp0[]..swiz;
            r38.y = samp0[]..swiz;
            r38.z = samp0[]..swiz;
            r38.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r39.xy, r15.z, l(212), t15.xyxx
            r39.x = samp0[]..swiz;
            r39.y = samp0[]..swiz;
              r23.w = r13.x * r25.y + r23.w;
              r13.x = r15.y * r26.z;
              r26.z = r34.z;
              r15.y = dot(r7.xyz, r26.xyz);
              r15.z = dot(r27.xyz, r26.xyz);
              r15.z = r15.z + -r34.w;
              r15.w = cmp(r15.z >= 0);
              r15.z = max(abs(r15.z), r4.w);
              r15.z = r15.w ? r15.z : -r15.z;
              r15.y = max(1.00000001e-07, -r15.y);
              r15.y = r15.z / r15.y;
              r15.y = min(131072, abs(r15.y));
              r34.z = r35.z;
              r15.z = dot(r7.xyz, r34.xyz);
              r15.w = dot(r27.xyz, r34.xyz);
              r15.w = r15.w + -r35.w;
              r16.w = cmp(r15.w >= 0);
              r15.w = max(abs(r15.w), r4.w);
              r15.w = r16.w ? r15.w : -r15.w;
              r15.z = max(1.00000001e-07, -r15.z);
              r15.z = r15.w / r15.z;
              r15.y = min(r15.y, abs(r15.z));
              r35.z = r36.z;
              r15.z = dot(r7.xyz, r35.xyz);
              r15.w = dot(r27.xyz, r35.xyz);
              r15.w = r15.w + -r36.w;
              r16.w = cmp(r15.w >= 0);
              r15.w = max(abs(r15.w), r4.w);
              r15.w = r16.w ? r15.w : -r15.w;
              r15.z = max(1.00000001e-07, -r15.z);
              r15.z = r15.w / r15.z;
              r15.y = min(r15.y, abs(r15.z));
              r36.z = r37.z;
              r15.z = dot(r7.xyz, r36.xyz);
              r15.w = dot(r27.xyz, r36.xyz);
              r15.w = r15.w + -r37.w;
              r16.w = cmp(r15.w >= 0);
              r15.w = max(abs(r15.w), r4.w);
              r15.w = r16.w ? r15.w : -r15.w;
              r15.z = max(1.00000001e-07, -r15.z);
              r15.z = r15.w / r15.z;
              r15.y = min(r15.y, abs(r15.z));
              r37.z = r38.x;
              r15.z = dot(r7.xyz, r37.xyz);
              r15.w = dot(r27.xyz, r37.xyz);
              r15.w = r15.w + -r38.y;
              r16.w = cmp(r15.w >= 0);
              r15.w = max(abs(r15.w), r4.w);
              r15.w = r16.w ? r15.w : -r15.w;
              r15.z = max(1.00000001e-07, -r15.z);
              r15.z = r15.w / r15.z;
              r15.y = min(r15.y, abs(r15.z));
              r39.zw = r38.zw;
              r15.z = dot(r7.zxy, r39.xzw);
              r15.w = dot(r27.zxy, r39.xzw);
              r15.w = r15.w + -r39.y;
              r16.w = cmp(r15.w >= 0);
              r15.w = max(abs(r15.w), r4.w);
              r15.w = r16.w ? r15.w : -r15.w;
              r15.z = max(1.00000001e-07, -r15.z);
              r15.z = r15.w / r15.z;
              r15.y = min(r15.y, abs(r15.z));
              r25.x = r31.w;
              r25.yz = r32.zw;
              r25.xyz = r25.xyz + r27.xyz;
              r25.xyz = r7.xyz * r15.yyy + r25.xyz;
              r15.z = dot(r25.xyz, r25.xyz);
              r15.z = sqrt(r15.z);
              r15.y = r15.y / r15.z;
              r15.y = r15.y + r15.y;
              r15.y = sqrt(r15.y);
              r15.y = r0.w * 5 + r15.y;
              r15.y = -0.844799995 + r15.y;
              r24.y = r29.z;
              r24.z = r30.x;
              r34.x = dot(r25.xyz, r24.xyz);
              r26.xy = r29.xw;
              r26.z = r30.w;
              r34.y = dot(r25.xyz, r26.xyz);
              r30.x = r29.y;
              r34.z = dot(r25.xyz, r30.xyz);
              if (9 == 0) r15.z = 0; else if (9+16 < 32) {               r15.z = (uint)r26.w << (32-(9 + 16)); r15.z = (uint)r15.z >> (32-9);              } else r15.z = (uint)r26.w >> 16;
              r34.w = (uint)r15.z;
              r15.yzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r34.xyzw, r15.y).xyz;
              r25.x = dot(r27.xyz, r24.xyz);
              r25.y = dot(r27.xyz, r26.xyz);
              r25.z = dot(r27.xyz, r30.xyz);
              r25.xyz = saturate(r25.xyz * r31.xyz + float3(0.5,0.5,0.5));
              r32.z = r33.x;
              r25.xyz = r25.xyz * r32.xyz + r33.yzw;
              r34.x = dot(r8.xyz, r24.xyz);
              r34.y = dot(r8.xyz, r26.xyz);
              r34.z = dot(r8.xyz, r30.xyz);
              r24.xyz = cmp(float3(0,0,0) < r34.xyz);
              r14.z = r24.x ? 0 : 0.5;
              r26.xyz = r25.xyz + r14.xyz;
              r26.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r26.xyz, 0).xyz;
              r27.xyz = r34.xyz * r34.xyz;
              r27.xyz = r27.xyz * r13.xxx;
              r16.z = r24.y ? 0 : 0.5;
              r24.xyw = r25.xyz + r16.xyz;
              r24.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r24.xyw, 0).xyz;
              r24.xyw = r24.xyw * r27.yyy;
              r24.xyw = r26.xyz * r27.xxx + r24.xyw;
              r17.z = r24.z ? 0 : 0.5;
              r25.xyz = r25.xyz + r17.xyz;
              r25.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r25.xyz, 0).xyz;
              r24.xyz = r25.xyz * r27.zzz + r24.xyw;
              r25.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r34.xyzw, 6).xyz;
              r26.xyz = r25.xyz * r24.xyz;
              r14.z = dot(r26.xyz, float3(0.212599993,0.715200007,0.0722000003));
              r12.xyz = r24.xyz * r25.xyz + r12.xyz;
              r16.z = r14.z * r1.w;
              r15.yzw = r15.yzw * r13.xxx;
              r13.x = dot(r15.yzw, float3(0.212599993,0.715200007,0.0722000003));
              r13.x = r1.w * r14.z + r13.x;
              r13.x = r16.z / r13.x;
              r23.xyz = r15.yzw * r13.xxx + r23.xyz;
            }
          }
        }
      }
      r18.xyzw = r23.xyzw;
      r22.xyz = r12.xyz;
      r10.x = (int)r10.x + 32;
    }
    r21.xyzw = r18.zxyw;
    r19.xyz = r22.xyz;
    r21.xyzw = r21.yzxw;
  } else {
    r21.x = r19.w;
  }
  r0.w = max(1, r21.w);
  r0.w = rcp(r0.w);
  r21.w = saturate(r21.w);
  r9.xyz = r19.xyz * r0.www;
  r12.xyz = r21.xyz * r0.www;
  r1.w = cmp(r21.w < 0.99000001);
  if (r1.w != 0) {
    r1.w = 1 + -r21.w;
    r4.w = sunConstants.globalProbeExposure * r1.w;
    r11.xyz = -globalProbeConstants.data[0].xyz + r11.xyz;
    r13.x = globalProbeConstants.data[0].w * r11.x;
    r13.yz = globalProbeConstants.data[1].xy * r11.yz;
    r11.xyz = saturate(float3(0.5,0.5,0.5) + r13.xyz);
    r13.xy = globalProbeConstants.data[1].zw * r11.xy;
    r13.z = globalProbeConstants.data[2].x * r11.z;
    r11.xyz = globalProbeConstants.data[2].yzw + r13.xyz;
    r13.xyz = cmp(float3(0,0,0) < r8.xyz);
    r13.xyz = r13.xyz ? float3(0,0,0) : float3(0.5,0.5,0.5);
    r13.w = 0;
    r14.xyz = r13.wwx + r11.xyz;
    r14.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r14.xyz, 0).xyz;
    r15.xyz = r8.xyz * r8.xyz;
    r15.xyz = r15.xyz * r4.www;
    r16.xyz = r13.wwy + r11.xyz;
    r16.xyz = gProbeYArray.SampleLevel(samplerLinearClamp_s, r16.xyz, 0).xyz;
    r16.xyz = r16.xyz * r15.yyy;
    r14.xyz = r14.xyz * r15.xxx + r16.xyz;
    r11.xyz = r13.wwz + r11.xyz;
    r11.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r11.xyz, 0).xyz;
    r11.xyz = r11.xyz * r15.zzz + r14.xyz;
    r8.w = 0;
    r8.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r8.xyzw, 6).xyz;
    r8.xyz = r11.xyz * r8.xyz;
    r4.w = dot(r8.xyz, float3(0.212599993,0.715200007,0.0722000003));
    r9.xyz = r19.xyz * r0.www + r8.xyz;
    r7.w = 0;
    r7.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r7.xyzw, r3.w).xyz;
    r0.w = sunConstants.globalProbeExposure * r1.w + -r4.w;
    r0.w = r6.y * r0.w + r4.w;
    r12.xyz = r7.xyz * r0.www + r12.xyz;
  }
  r5.yz = r5.yz * float2(1.59090912,1.59090912) + float2(-0.795454562,-0.795454562);
  r7.xy = r5.yz * float2(1,2) + float2(0.5,1);
  r0.w = cmp(1 < r7.y);
  r1.w = 2 + -r7.y;
  r7.z = r0.w ? r1.w : r7.y;
  r5.yz = r7.xz * float2(0.0078125,1) + float2(0.9921875,0);
  r0.w = causticMap.SampleLevel(LinearSampler_s, r5.yz, 0).x;
  r0.w = -1 + r0.w;
  r0.w = r5.x * r0.w + 1;
  r5.xyz = r9.xyz * r0.www;
  r6.x = saturate(dot(r1.xyz, r10.yzw));
  r1.xyz = r5.xyz * r3.xyz + r2.yzw;
  r2.yz = r6.xy * float2(0.984375,0.984375) + float2(0.0078125,0.0078125);
  r2.yz = gEnvBRDFGeneric.SampleLevel(samplerLinear_s, r2.yz, 0).xy;
  r3.xyz = r12.xyz * r2.xxx;
  r0.xyz = r3.xyz * r2.yyy + r0.xyz;
  r2.xyz = r3.xyz * r2.zzz + r4.xyz;
  r0.xyz = float3(0.967999995,0.967999995,0.967999995) * r0.xyz;
  r0.xyz = r2.xyz * float3(0.0320000015,0.0320000015,0.0320000015) + r0.xyz;
  r0.w = cmp(0 < fogConstants.atmosphereExtinctionIntensity);
  if (r0.w != 0) {
    r1.w = dot(v7.xyz, v7.xyz);
    r1.w = sqrt(r1.w);
    r2.x = cmp(0 < fogConstants.blendAmount);
    if (r2.x != 0) {
      r2.xy = r1.ww * fogConstants.atmospherefogdistancedensityscale.xy + fogConstants.atmospherefogdistanceoffset.xy;
      r2.xy = fogConstants.atmospherefogdensityatcamera.xy * r2.xy;
      r2.z = cmp(0.00999999978 < abs(v7.z));
      r3.xy = fogConstants.atmospherefogheightdensityscale.xy * v7.zz;
      r3.zw = float2(-1.44269502,-1.44269502) * r3.xy;
      r3.zw = exp2(r3.zw);
      r3.zw = float2(1,1) + -r3.zw;
      r3.xy = r3.zw / r3.xy;
      r3.xy = r3.xy * r2.xy;
      r2.xy = r2.zz ? r3.xy : r2.xy;
      r2.xzw = fogConstants.atmosphereTotalDensity.xyz * r2.xxx;
      r2.xzw = exp2(r2.xzw);
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r2.yyy;
      r3.xyz = exp2(r3.xyz);
      r3.xyz = r3.xyz + -r2.xzw;
      r2.xyz = fogConstants.blendAmount * r3.xyz + r2.xzw;
    } else {
      r1.w = r1.w * fogConstants.atmospherefogdistancedensityscale.x + fogConstants.atmospherefogdistanceoffset.x;
      r1.w = fogConstants.atmospherefogdensityatcamera.x * r1.w;
      r2.w = cmp(0.00999999978 < abs(v7.z));
      r3.x = fogConstants.atmospherefogheightdensityscale.x * v7.z;
      r3.y = -1.44269502 * r3.x;
      r3.y = exp2(r3.y);
      r3.y = 1 + -r3.y;
      r3.x = r3.y / r3.x;
      r3.x = r3.x * r1.w;
      r1.w = r2.w ? r3.x : r1.w;
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.www;
      r2.xyz = exp2(r3.xyz);
    }
    r2.xyz = r2.xyz * fogConstants.atmosphereExtinctionIntensity + float3(1,1,1);
    r2.xyz = saturate(-fogConstants.atmosphereExtinctionIntensity + r2.xyz);
    r2.xyz = r2.xyz * r1.xyz;
  } else {
    r1.w = fogConstants.heightFalloff * v7.z;
    r2.w = fogConstants.heightFalloff * v7.z + fogConstants.K0;
    r3.x = cmp(abs(r1.w) < 9.99999975e-05);
    r3.y = min(64, r2.w);
    r3.y = 1.44269502 * r3.y;
    r3.y = exp2(r3.y);
    r3.z = saturate(fogConstants.K0b);
    r3.w = cmp(r2.w < 0);
    r2.w = 1 + r2.w;
    r2.w = r3.w ? r3.y : r2.w;
    r2.w = -fogConstants.K0b + r2.w;
    r1.w = r3.x ? 1 : r1.w;
    r1.w = r2.w / r1.w;
    r1.w = r3.x ? r3.z : r1.w;
    r1.w = fogConstants.expMul * r1.w;
    r2.w = dot(v7.xyz, v7.xyz);
    r3.x = sqrt(r2.w);
    r1.w = r1.w * r3.x + fogConstants.expAdd;
    r1.w = exp2(r1.w);
    r1.w = min(1, r1.w);
    r1.w = 1 + -r1.w;
    r2.w = rsqrt(r2.w);
    r3.xyz = v7.xyz * r2.www;
    r2.w = dot(fogConstants.wldSunFogDir.xyz, r3.xyz);
    r2.w = saturate(fogConstants.sunFogAngles.y * r2.w + fogConstants.sunFogAngles.x);
    r3.x = -fogConstants.sunFogColor.w + fogConstants.fogColor.w;
    r2.w = r2.w * r3.x + fogConstants.sunFogColor.w;
    r1.w = r2.w * r1.w;
    r2.xyz = r1.www * -r1.xyz + r1.xyz;
  }
  r1.xyz = relHDRExposure.yyy * r2.xyz;
  r2.xyz = cmp(r1.xyz >= float3(6.10351563e-05,6.10351563e-05,6.10351563e-05));
  r1.xyz = r2.xyz ? r1.xyz : 0;
  o1.xyz = min(float3(65024,65024,64512), r1.xyz);
  if (r0.w != 0) {
    r0.w = dot(v7.xyz, v7.xyz);
    r1.x = rsqrt(r0.w);
    r1.xyz = v7.xyz * r1.xxx;
    r0.w = sqrt(r0.w);
    r1.w = cmp(0 < fogConstants.blendAmount);
    if (r1.w != 0) {
      r2.xy = r0.ww * fogConstants.atmospherefogdistancedensityscale.xy + fogConstants.atmospherefogdistanceoffset.xy;
      r2.xy = fogConstants.atmospherefogdensityatcamera.xy * r2.xy;
      r1.w = cmp(0.00999999978 < abs(v7.z));
      r2.zw = fogConstants.atmospherefogheightdensityscale.xy * v7.zz;
      r3.xy = float2(-1.44269502,-1.44269502) * r2.zw;
      r3.xy = exp2(r3.xy);
      r3.xy = float2(1,1) + -r3.xy;
      r2.zw = r3.xy / r2.zw;
      r2.zw = r2.xy * r2.zw;
      r2.xy = r1.ww ? r2.zw : r2.xy;
      r2.xzw = fogConstants.atmosphereTotalDensity.xyz * r2.xxx;
      r2.xzw = exp2(r2.xzw);
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r2.yyy;
      r3.xyz = exp2(r3.xyz);
      r3.xyz = r3.xyz + -r2.xzw;
      r2.xyz = fogConstants.blendAmount * r3.xyz + r2.xzw;
    } else {
      r1.w = r0.w * fogConstants.atmospherefogdistancedensityscale.x + fogConstants.atmospherefogdistanceoffset.x;
      r1.w = fogConstants.atmospherefogdensityatcamera.x * r1.w;
      r2.w = cmp(0.00999999978 < abs(v7.z));
      r3.x = fogConstants.atmospherefogheightdensityscale.x * v7.z;
      r3.y = -1.44269502 * r3.x;
      r3.y = exp2(r3.y);
      r3.y = 1 + -r3.y;
      r3.x = r3.y / r3.x;
      r3.x = r3.x * r1.w;
      r1.w = r2.w ? r3.x : r1.w;
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.www;
      r2.xyz = exp2(r3.xyz);
    }
    r2.xyz = r2.xyz * fogConstants.atmosphereExtinctionIntensity + float3(1,1,1);
    r2.xyz = saturate(-fogConstants.atmosphereExtinctionIntensity + r2.xyz);
    r1.x = dot(fogConstants.wldSunFogDir.xyz, -r1.xyz);
    r1.y = -fogConstants.atmosphereMieSchlickK * fogConstants.atmosphereMieSchlickK + 1;
    r1.z = fogConstants.atmosphereMieSchlickK * -r1.x + 1;
    r1.z = r1.z * r1.z;
    r1.z = 12.566371 * r1.z;
    r1.y = r1.y / r1.z;
    r0.w = -fogConstants.atmospherehazebasedist + r0.w;
    r0.w = saturate(fogConstants.atmospherehazefadedist * r0.w);
    r0.w = r1.y * r0.w;
    r1.x = saturate(r1.x);
    r1.x = r1.x * r1.x + 1;
    r1.x = r1.x * 0.0596831031 + -1;
    r1.x = fogConstants.atmospheresunstrength * r1.x + 1;
    r1.yzw = fogConstants.atmosphereMieDensity.xyz * r0.www;
    r1.xyz = r1.xxx * fogConstants.atmosphereRayleighDensity.xyz + r1.yzw;
    r1.xyz = fogConstants.atmosphereInScatterIntensity * r1.xyz;
    r3.xyz = float3(1,1,1) + -r2.xyz;
    r1.xyz = r3.xyz * r1.xyz;
    r1.xyz = r0.xyz * r2.xyz + r1.xyz;
  } else {
    r0.w = fogConstants.heightFalloff * v7.z;
    r1.w = fogConstants.heightFalloff * v7.z + fogConstants.K0;
    r2.x = cmp(abs(r0.w) < 9.99999975e-05);
    r2.y = min(64, r1.w);
    r2.y = 1.44269502 * r2.y;
    r2.y = exp2(r2.y);
    r2.z = saturate(fogConstants.K0b);
    r2.w = cmp(r1.w < 0);
    r1.w = 1 + r1.w;
    r1.w = r2.w ? r2.y : r1.w;
    r1.w = -fogConstants.K0b + r1.w;
    r0.w = r2.x ? 1 : r0.w;
    r0.w = r1.w / r0.w;
    r0.w = r2.x ? r2.z : r0.w;
    r0.w = fogConstants.expMul * r0.w;
    r1.w = dot(v7.xyz, v7.xyz);
    r2.x = sqrt(r1.w);
    r0.w = r0.w * r2.x + fogConstants.expAdd;
    r0.w = exp2(r0.w);
    r0.w = min(1, r0.w);
    r0.w = 1 + -r0.w;
    r1.w = rsqrt(r1.w);
    r2.xyz = v7.xyz * r1.www;
    r1.w = dot(fogConstants.wldSunFogDir.xyz, r2.xyz);
    r1.w = saturate(fogConstants.sunFogAngles.y * r1.w + fogConstants.sunFogAngles.x);
    r2.xyzw = -fogConstants.sunFogColor.xyzw + fogConstants.fogColor.xyzw;
    r2.xyzw = r1.wwww * r2.xyzw + fogConstants.sunFogColor.xyzw;
    r0.w = r2.w * r0.w;
    r2.xyz = r2.xyz + -r0.xyz;
    r1.xyz = r0.www * r2.xyz + r0.xyz;
  }
  r0.xyz = relHDRExposure.yyy * r1.xyz;
  r1.xyz = cmp(r0.xyz >= float3(6.10351563e-05,6.10351563e-05,6.10351563e-05));
  r0.xyz = r1.xyz ? r0.xyz : 0;
  o0.xyz = min(float3(65024,65024,64512), r0.xyz);
  return;
}