...
cbuffer _Globals : register(b0)
{
  ...
  float3 colorTint : packoffset(c19);
  float3 colorDispersion : packoffset(c20);
}
...
Texture2D<float4> maskTexture : register(t6);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.x = maskTexture.Sample(bilinearSampler_s, v1.xy).x;
  r0.x = cmp(0 >= r0.x);
  if (r0.x != 0) discard;
  r0.xy = normalTexture.Sample(bilinearSampler_s, v1.xy).xy;
  r0.xy = r0.xy * float2(2,2) + float2(-1,-1);
  r1.x = dot(v2.yx, r0.xy);
  r0.zw = float2(-1,1) * v2.xy;
  r1.y = dot(r0.zw, r0.xy);
  r0.xy = renderTargetSize.zw * v2.ww;
  ...
  r1.xyz = colorDispersion.xyz * r0.zzz;
  r2.xyzw = r1.xxyy * float4(0.00100000005,0.00100000005,0.00100000005,0.00100000005) + r0.xyxy;
  r0.z = frameBuffer.Sample(bilinearSampler_s, r2.xy).x;
  r0.w = frameBuffer.Sample(bilinearSampler_s, r2.zw).y;
  r0.xy = r1.zz * float2(0.00100000005,0.00100000005) + r0.xy;
  r0.x = frameBuffer.Sample(bilinearSampler_s, r0.xy).z;
  o0.xyz = r0.zwx;
  o0.w = 1;
  return;
}