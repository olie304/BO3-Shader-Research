// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:22:50 2021
...
cbuffer _Globals : register(b0)
{
  ...
  float3 emissiveTint1 : packoffset(c18.y);
  bool relativeHDR : packoffset(c19);
  float distortionAmount : packoffset(c19.y);
  float2 distortionScrollUVs : packoffset(c19.z);
  float2 distortionScaleUVs : packoffset(c20);
  float2 emissiveScaleUVs : packoffset(c20.z);
  float2 emissiveScrollUVs : packoffset(c21);
  float2 scriptControl : packoffset(c21.z);
...
Texture2D<float4> emissiveMask : register(t24);
Texture2D<float4> distortionMap : register(t25);
...
// unknown dcl_: dcl_resource_structured t4, 24
  ...
  r2.xyzw = distortionScaleUVs.xyzw * w1.xyyz;
...
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r2.x, v2.w, l(4), t4.xxxx
r2.x = colorSampler[]..swiz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=384)(mixed,mixed,mixed,mixed) r2.xy, r2.x, l(36), t0.xyxx
r2.x = colorSampler[]..swiz;
r2.y = colorSampler[]..swiz;
  r2.x = saturate(r2.x);
  r0.w = r2.x * r0.w;
  r2.xyz = cmp(r2.yyy == float3(3,2,1));
