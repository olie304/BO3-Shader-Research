// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:36:37 2021

cbuffer _Globals : register(b0)
{
  float4 flagParams : packoffset(c0);
  float4 colorObjMin : packoffset(c1);
  float4 colorObjMax : packoffset(c2);
  float colorObjMinBaseBlend : packoffset(c3);
  float colorObjMaxBaseBlend : packoffset(c3.y);
  float2 uvScroll : packoffset(c3.z);
  float4 detailScale : packoffset(c4);
  float4 detailScale1 : packoffset(c5);
  float4 detailScale2 : packoffset(c6);
  float4 detailScale3 : packoffset(c7);
  float4 alphaRevealParms : packoffset(c8);
  float4 colorDetailScale : packoffset(c9);
  float3 colorTint : packoffset(c10);
  float baseNormalHeight : packoffset(c10.w);
  float2 glossRange : packoffset(c11);
  float3 backscatterTint : packoffset(c12);
  uint zFeatherComputeSprites : packoffset(c12.w);
}

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

cbuffer LightingGlobals : register(b2)
{
  uint numLights : packoffset(c0);
  uint numRefProbes : packoffset(c0.y);
  uint numRefProbeBlends : packoffset(c0.z);
  uint numDedicatedLights : packoffset(c0.w);
  float4 clearColor : packoffset(c1);
  float4 sssParams : packoffset(c2);

  struct
  {
    float4 fogColor;
    float4 sunFogColor;
    float K0;
    float skyK0;
    float expMul;
    float expAdd;
    float heightFalloff;
    float skyHeightFalloff;
    float K0b;
    float padding0;
    float skyK0b;
    float3 wldSunFogDir;
    float2 sunFogAngles;
    float atmospheresunstrength;
    float atmosphereMieSchlickK;
    float2 atmosphereskyfogdensityatcamera;
    float atmosphereExtinctionIntensity;
    float atmosphereInScatterIntensity;
    float3 atmosphereRayleighDensity;
    float atmospherehazebasedist;
    float3 atmosphereMieDensity;
    float atmospherehazefadedist;
    float3 atmosphereTotalDensity;
    float worldfogskysize;
    float3 atmosphereInScatterIntensityOverTotalDensity;
    float blendAmount;
    float2 atmosphereskyfogheightdensityscale;
    float2 atmospherefogdistanceoffset;
    float2 atmospherefogdistancedensityscale;
    float2 atmospherefogheightdensityscale;
    float2 atmospherefogdensityatcamera;
    float2 padding1;
  } fogConstants : packoffset(c3);


  struct
  {
    float3 wldDir;
    float splitDepthOffset;
    float3 color;
    float specScale;
    float globalProbeExposure;
    float3 avgGlobalProbeColor;
    float4 splitPinTransform[3];
    uint sunCookieIndex;
    float sunCookieIntensity;
    float sunVolumetricCookieIntensity;
    uint toolsGfxDisableSunShadow;
    float4 sunCookieTransform[2];
    float intensity;
    int splitArrayOffset;
    float2 pad0;

    struct
    {

      struct
      {
        float2 dimensionInTiles;
        float inchesPerTexel;
        float spanInInches;
      } constants;

      row_major float4x4 offToPinTransform;
      float coordScale;
      uint rootOffset;
      uint2 pad0;
    } sstLightingConstants;

  } sunConstants : packoffset(c17);


  struct
  {
    float2 sstToMinMaxScale;
    float2 pad0;
    float2 halfMinTexelSize;
    float rcpInchesDimLevel0;
    float shadowBiasInches;
  } minMaxConstants : packoffset(c33);


  struct
  {
    uint4 data[3];
  } globalProbeConstants : packoffset(c35);


  struct
  {
    float rain;
    float windSpeed;
    float windDirSin;
    float windDirCos;
    float weatherTile;
    float3 weatherVector;
    float3 weatherVector2;
    float padding0;
    float3 weatherTint;
    float padding1;
    float3 weatherTint2;
    float padding2;
  } weather : packoffset(c38);


  struct
  {

    struct
    {
      float2 dimensionInTiles;
      float inchesPerTexel;
      float spanInInches;
    } constants;

    row_major float4x4 offToPinTransform;
    float coordScale;
    uint rootOffset;
    uint2 pad0;
  } outdoorSSTConstants : packoffset(c43);

  float4 pinToWorldZ : packoffset(c49);

  struct
  {
    uint firstSpotLight;
    uint lastSpotLight;
    uint firstOmniLight;
    uint lastOmniLight;
    uint firstProbe;
    uint lastProbe;
    uint padding0;
    uint padding1;
  } volumetric : packoffset(c50);

  uint viewID : packoffset(c52);
  uint enableCountDebug : packoffset(c52.y);
  uint centerGroupIDx : packoffset(c52.z);
  uint centerGroupIDy : packoffset(c52.w);
  uint viewmodelSunShadowRaw : packoffset(c53);
  uint debugDrawOverlayType : packoffset(c53.y);
  float drawNumLighstScale : packoffset(c53.z);
  float drawNumLightsAlpha : packoffset(c53.w);
  uint probeDebug : packoffset(c54);
  float probeDebugRadius : packoffset(c54.y);
  uint showSunVis : packoffset(c54.z);
  uint enableAO : packoffset(c54.w);
  uint showAO : packoffset(c55);
  uint numGpuCullTilesWidth : packoffset(c55.y);
  uint numGpuCullTilesHeight : packoffset(c55.z);
  int permuteHighlight : packoffset(c55.w);
  uint overdrawOverlayMaxValue : packoffset(c56);
  float overdrawOverlayAlpha : packoffset(c56.y);
  uint overdrawDecals : packoffset(c56.z);
  uint overdrawDecalsLayers : packoffset(c56.w);
  uint permuteStride : packoffset(c57);
  uint numDecals : packoffset(c57.y);
  uint numStaticDecals : packoffset(c57.z);
  uint numDynamicDecals : packoffset(c57.w);
  uint numOverrideProbes : packoffset(c58);
  uint enableDitheredShadow : packoffset(c58.y);
  uint oitMaxEntries : packoffset(c58.z);
  uint numAttenuationVolumes : packoffset(c58.w);
}

SamplerState samp0_s : register(s0);
SamplerState colorSampler_s : register(s1);
SamplerState samplerLinear_s : register(s2);
SamplerState specColorSampler_s : register(s3);
SamplerState samplerLinearClamp_s : register(s4);
SamplerState normalSampler_s : register(s5);
SamplerComparisonState ShadowSamplerComparisonState_s : register(s13);
StructuredBuffer<shaderConstantSetBuffer> shaderConstantSetBuffer : register(t0);
StructuredBuffer<modelInstanceBuffer> modelInstanceBuffer : register(t4);
Texture2D<float2> gEnvBRDFGlass : register(t5);
Texture2DArray<uint> visibleLights : register(t6);
Texture2DArray<uint> visibleProbes : register(t7);
TextureCubeArray<float3> gReflectionProbeArray : register(t8);
Texture2DArray<float> gSpotShadowmapArray : register(t9);
TextureCubeArray<float> gOmniShadowmapArray : register(t10);
StructuredBuffer<gCullConstants> gCullConstants : register(t11);
StructuredBuffer<lightConstants> lightConstants : register(t12);
Texture2DArray<float> gSunShadowmapArray : register(t13);
StructuredBuffer<refProbeConstants> refProbeConstants : register(t15);
StructuredBuffer<refProbeAttenuationConstants> refProbeAttenuationConstants : register(t16);
Texture3D<float3> gProbeXArray : register(t17);
Texture3D<float3> gProbeYArray : register(t18);
Texture3D<float3> gProbeZArray : register(t19);
Texture2DArray<float4> gCookieArray : register(t20);
Texture2D<float4> colorMap : register(t21);
Texture2D<float4> tintMask : register(t22);
StructuredBuffer<gSunShadowTree> gSunShadowTree : register(t23);
Texture2D<float4> normalMap : register(t24);
Texture2D<float4> glossMap : register(t25);
Texture2DArray<float> gTransShadowmapArray : register(t31);
Texture2DArray<uint> visibleDecals : register(t35);
StructuredBuffer<forwardDecalConstants> forwardDecalConstants : register(t36);
Texture2DArray<float4> materialTexBC7 : register(t42);
Texture2DArray<float4> materialTexBC3 : register(t43);
Texture2DArray<float4> materialTexBC7linear : register(t44);
Texture2DArray<float4> materialTexMask : register(t45);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float4 v0 : SV_POSITION0,
  float v1 : COLOR1,
  float2 w1 : TEXCOORD0,
  nointerpolation uint4 v2 : TEXCOORD4,
  float4 v3 : TEXCOORD1,
  float4 v4 : TEXCOORD2,
  float3 v5 : TEXCOORD3,
  float3 v6 : OFFPOSITION0,
  uint v7 : SV_IsFrontFace0,
  out float4 o0 : SV_TARGET0)
{
  const float4 icb[] = { { -0.808114, 0.808114, 0, 0},
                              { 0, -1.000000, 0, 0},
                              { 0.606057, 0.606057, 0, 0},
                              { -0.714286, 0, 0, 0},
                              { 0.404114, -0.404114, 0, 0},
                              { 0, 0.428571, 0, 0},
                              { -0.202057, -0.202057, 0, 0},
                              { 0.142857, 0, 0, 0} };
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t0, 384
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t4, 24
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t11, 80
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t12, 240
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t15, 224
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t16, 96
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t23, 4
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t36, 128
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15,r16,r17,r18,r19,r20,r21,r22,r23,r24,r25,r26,r27,r28,r29,r30,r31,r32,r33,r34,r35,r36,r37,r38,r39,r40,r41,r42,r43,r44,r45,r46,r47,r48;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.xyzw = colorMap.Sample(colorSampler_s, w1.xy).xyzw;
  r1.x = tintMask.Sample(colorSampler_s, w1.xy).x;
  r1.yzw = float3(-1,-1,-1) + colorTint.xyz;
  r1.xyz = r1.xxx * r1.yzw + float3(1,1,1);
  r0.xyz = r1.xyz * r0.xyz;
  r0.w = v1.x * r0.w;
  r1.x = glossMap.Sample(specColorSampler_s, w1.xy).x;
  r1.y = glossRange.y + -glossRange.x;
  r1.x = r1.x * r1.y + glossRange.x;
  r1.x = saturate(0.0588235296 * r1.x);
  r1.yzw = normalMap.Sample(normalSampler_s, w1.xy).xyz;
  r1.yzw = float3(-0.5,-0.5,-0) + r1.yzw;
  r1.yzw = baseNormalHeight * r1.yzw + float3(0.5,0.5,0);
  r1.yz = r1.yz * float2(1.9921875,1.9921875) + float2(-1,-1);
  r2.x = dot(r1.yz, r1.yz);
  r2.x = 1 + -r2.x;
  r2.x = max(0, r2.x);
  r2.x = sqrt(r2.x);
  r1.w = r1.w * r1.w;
  r1.w = 0.333333343 * r1.w;
  r1.w = min(1, r1.w);
  r2.y = v7.x ? 1 : -1;
  r2.z = dot(v3.xyz, v3.xyz);
  r2.z = rsqrt(r2.z);
  r3.xyz = v3.xyz * r2.zzz;
  r3.xyz = r3.xyz * r2.yyy;
  r2.z = dot(v4.xyz, v4.xyz);
  r2.z = rsqrt(r2.z);
  r4.xyz = v4.xyz * r2.zzz;
  r4.xyz = r4.xyz * r2.yyy;
  r2.z = dot(v5.xyz, v5.xyz);
  r2.z = rsqrt(r2.z);
  r5.xyz = v5.xyz * r2.zzz;
  r5.xyz = r5.xyz * r2.yyy;
  r1.x = -17 * r1.x;
  r1.x = exp2(r1.x);
  r1.x = r1.w + r1.x;
  r1.x = log2(r1.x);
  r1.x = -0.0588235296 * r1.x;
  r1.x = max(0, r1.x);
  r5.xyz = r5.xyz * r1.zzz;
  r1.yzw = r4.xyz * r1.yyy + r5.xyz;
  r1.yzw = r3.xyz * r2.xxx + r1.yzw;
  r2.x = dot(r1.yzw, r1.yzw);
  r2.x = rsqrt(r2.x);
  r1.yzw = r2.xxx * r1.yzw;
  r2.x = cmp(v0.z >= 0.984375);
  r2.z = 1.01587307 * v0.z;
  r2.w = v0.z * 64 + -63;
  r2.x = r2.x ? r2.w : r2.z;
  r2.x = max(9.99999994e-09, r2.x);
  r2.x = rcp(r2.x);
  r2.zw = (uint2)v0.xy;
  r3.xyz = ddx_coarse(v6.xyz);
  r4.xyz = ddy_coarse(v6.xyz);
  r3.w = numRefProbes + numLights;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r5.xy, v2.x, l(4), t4.xyxx
r5.x = samp0[]..swiz;
r5.y = samp0[]..swiz;
  r5.yz = (int2)r5.yy & int2(268435456,536870912);
  if (r5.y != 0) {
    r6.xyz = eyeOffset.xyz + v6.xyz;
    r4.w = 0.0078125 * r2.x;
    r4.w = min(15, r4.w);
    r4.w = (uint)r4.w;
    r7.xy = (uint2)r2.zw >> int2(6,6);
    r7.z = (uint)r4.w << 4;
    r4.w = numStaticDecals & -32;
    r5.y = (int)-r4.w + numStaticDecals;
    r8.xy = float2(0,0);
    r9.w = 0;
    r6.w = 1;
    r10.xyzw = float4(0,0,0,0);
    r11.xyz = float3(0,0,0);
    r12.yz = float2(0,0);
    r13.xy = float2(0,0);
    r5.w = 0;
    while (true) {
      r7.w = cmp((uint)r5.w >= numStaticDecals);
      if (r7.w != 0) break;
      r8.z = (uint)r5.w >> 5;
      r9.xyz = (int3)r7.xyz + (int3)r8.xyz;
      r7.w = visibleDecals.Load(r9.xyzw).x;
      r8.z = cmp((int)r4.w == (int)r5.w);
      if (r5.y == 0) r8.w = 0; else if (r5.y+0 < 32) {       r8.w = (uint)r7.w << (32-(r5.y + 0)); r8.w = (uint)r8.w >> (32-r5.y);      } else r8.w = (uint)r7.w >> 0;
      r7.w = r8.z ? r8.w : r7.w;
      r8.z = (int)r3.w + (int)r5.w;
      r14.xyzw = r10.xzwy;
      r9.xyz = r11.xyz;
      r15.yz = r12.yz;
      r16.xy = r13.xy;
      r8.w = r7.w;
      while (true) {
        if (r8.w == 0) break;
        r11.w = firstbitlow((uint)r8.w);
        r12.w = 1 << (int)r11.w;
        r13.w = (int)r8.w & (int)r12.w;
        if (r13.w != 0) {
          r8.w = (int)r8.w ^ (int)r12.w;
          r12.w = (int)r8.z + (int)r11.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r17.xyzw, r12.w, l(0), t11.xyzw
        r17.x = samp0[]..swiz;
        r17.y = samp0[]..swiz;
        r17.z = samp0[]..swiz;
        r17.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r18.xy, r12.w, l(16), t11.xyxx
        r18.x = samp0[]..swiz;
        r18.y = samp0[]..swiz;
          r17.xyz = -v6.xyz + r17.xyz;
          r18.z = r17.w;
          r17.xyz = cmp(abs(r17.xyz) < r18.zxy);
          r12.w = r17.y ? r17.x : 0;
          r12.w = r17.z ? r12.w : 0;
          if (r12.w != 0) {
            r11.w = (int)r5.w + (int)r11.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r17.xyz, r11.w, l(0), t36.xyzx
          r17.x = samp0[]..swiz;
          r17.y = samp0[]..swiz;
          r17.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r18.xyz, r11.w, l(16), t36.xyzx
          r18.x = samp0[]..swiz;
          r18.y = samp0[]..swiz;
          r18.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r19.xyz, r11.w, l(32), t36.xyzx
          r19.x = samp0[]..swiz;
          r19.y = samp0[]..swiz;
          r19.z = samp0[]..swiz;
            r20.x = r17.x;
            r20.y = r18.x;
            r20.z = r19.x;
            r12.w = dot(r20.xyz, r20.xyz);
            r12.w = rsqrt(r12.w);
            r21.xyz = r20.xyz * r12.www;
            r12.w = dot(r21.xyz, v3.xyz);
            r12.w = cmp(r12.w >= 0.5);
            if (r12.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r22.xyw, r11.w, l(48), t36.xyxz
            r22.x = samp0[]..swiz;
            r22.y = samp0[]..swiz;
            r22.w = samp0[]..swiz;
              r20.w = r22.x;
              r20.x = dot(r6.xyzw, r20.xyzw);
              r23.x = r17.y;
              r23.y = r18.y;
              r23.z = r19.y;
              r23.w = r22.y;
              r20.y = dot(r6.xyzw, r23.xyzw);
              r22.x = r17.z;
              r22.y = r18.z;
              r22.z = r19.z;
              r20.z = dot(r6.xyzw, r22.xyzw);
              r17.xyz = cmp(abs(r20.xyz) < float3(1,1,1));
              r12.w = r17.y ? r17.x : 0;
              r12.w = r17.z ? r12.w : 0;
              if (r12.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r17.xyzw, r11.w, l(64), t36.xyzw
              r17.x = samp0[]..swiz;
              r17.y = samp0[]..swiz;
              r17.z = samp0[]..swiz;
              r17.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r18.xyzw, r11.w, l(80), t36.xyzw
              r18.x = samp0[]..swiz;
              r18.y = samp0[]..swiz;
              r18.z = samp0[]..swiz;
              r18.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r19.xyzw, r11.w, l(96), t36.xyzw
              r19.x = samp0[]..swiz;
              r19.y = samp0[]..swiz;
              r19.z = samp0[]..swiz;
              r19.w = samp0[]..swiz;
                r24.x = dot(r3.xyz, r23.xyz);
                r24.y = dot(r3.xyz, r22.xyz);
                r25.x = dot(r4.xyz, r23.xyz);
                r25.y = dot(r4.xyz, r22.xyz);
                r20.xy = r20.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r24.xyz = float3(0.5,-0.5,0.5) * r24.xyx;
                r25.xyz = float3(0.5,-0.5,0.5) * r25.xyx;
                r20.zw = (int2)r18.yz & int2(65535,65535);
                if (r20.z != 0) {
                  if (4 == 0) r12.w = 0; else if (4+24 < 32) {                   r12.w = (uint)r18.w << (32-(4 + 24)); r12.w = (uint)r12.w >> (32-4);                  } else r12.w = (uint)r18.w >> 24;
                  r12.w = 1 << (int)r12.w;
                  r13.w = (uint)r18.w >> 28;
                  r13.w = 1 << (int)r13.w;
                  r15.w = cmp((int)r20.z == 4);
                  if (r15.w != 0) {
                    r15.w = (int)r18.w & 4095;
                    if (12 == 0) r16.w = 0; else if (12+12 < 32) {                     r16.w = (uint)r18.w << (32-(12 + 12)); r16.w = (uint)r16.w >> (32-12);                    } else r16.w = (uint)r18.w >> 12;
                    r21.w = (int)r19.w & 1023;
                    r26.xz = (uint2)r12.ww;
                    r26.y = (uint)r13.w;
                    r27.xy = r20.xy * r26.zy + float2(0.5,0.5);
                    r27.zw = float2(-0.5,-0.5) + r26.zy;
                    r27.xy = min(r27.xy, r27.zw);
                    r28.x = (uint)r15.w;
                    r28.y = (uint)r16.w;
                    r27.xy = r28.xy + r27.xy;
                    r27.xy = invBcTexSizes.xy * r27.xy;
                    r27.z = (uint)r21.w;
                    r28.xyz = invBcTexSizes.xyx * r24.zyz;
                    r28.xyz = r28.xyz * r26.xyz;
                    r29.xyz = invBcTexSizes.xyx * r25.zyz;
                    r26.xyz = r29.xyz * r26.xyz;
                    r26.xyzw = materialTexBC7.SampleGrad(samplerLinearClamp_s, r27.xyz, r28.xyzx, r26.xyzx).xyzw;
                  } else {
                    r15.w = cmp((int)r20.z == 1);
                    if (r15.w != 0) {
                      r15.w = (int)r18.w & 4095;
                      if (12 == 0) r16.w = 0; else if (12+12 < 32) {                       r16.w = (uint)r18.w << (32-(12 + 12)); r16.w = (uint)r16.w >> (32-12);                      } else r16.w = (uint)r18.w >> 12;
                      r18.w = (int)r19.w & 1023;
                      r27.xz = (uint2)r12.ww;
                      r27.y = (uint)r13.w;
                      r28.xy = r20.xy * r27.zy + float2(0.5,0.5);
                      r28.zw = float2(-0.5,-0.5) + r27.zy;
                      r28.xy = min(r28.xy, r28.zw);
                      r29.x = (uint)r15.w;
                      r29.y = (uint)r16.w;
                      r28.xy = r29.xy + r28.xy;
                      r28.xy = invBcTexSizes.zw * r28.xy;
                      r28.z = (uint)r18.w;
                      r29.xyz = invBcTexSizes.zwz * r24.zyz;
                      r29.xyz = r29.xyz * r27.xyz;
                      r30.xyz = invBcTexSizes.zwz * r25.zyz;
                      r27.xyz = r30.xyz * r27.xyz;
                      r26.xyzw = materialTexBC3.SampleGrad(samplerLinearClamp_s, r28.xyz, r29.xyzx, r27.xyzx).xyzw;
                    } else {
                      r26.xyzw = float4(1,1,1,1);
                    }
                  }
                  r17.xyz = r26.xyz * r17.xyz;
                  r12.w = r26.w * r17.w;
                } else {
                  r17.xyz = float3(0,0,0);
                  r12.w = 0;
                }
                r13.w = cmp(r18.x != 1.000000);
                r15.w = r12.w + r17.w;
                r15.w = saturate(r15.w * r18.x + -r18.x);
                r12.w = r13.w ? r15.w : r12.w;
                if (r20.w != 0) {
                  if (4 == 0) r13.w = 0; else if (4+24 < 32) {                   r13.w = (uint)r19.y << (32-(4 + 24)); r13.w = (uint)r13.w >> (32-4);                  } else r13.w = (uint)r19.y >> 24;
                  r13.w = 1 << (int)r13.w;
                  r15.w = (uint)r19.y >> 28;
                  r15.w = 1 << (int)r15.w;
                  r16.w = cmp((int)r20.w == 2);
                  if (r16.w != 0) {
                    r16.w = (int)r19.y & 4095;
                    if (12 == 0) r18.x = 0; else if (12+12 < 32) {                     r18.x = (uint)r19.y << (32-(12 + 12)); r18.x = (uint)r18.x >> (32-12);                    } else r18.x = (uint)r19.y >> 12;
                    if (10 == 0) r18.w = 0; else if (10+20 < 32) {                     r18.w = (uint)r19.w << (32-(10 + 20)); r18.w = (uint)r18.w >> (32-10);                    } else r18.w = (uint)r19.w >> 20;
                    r26.xz = (uint2)r13.ww;
                    r26.y = (uint)r15.w;
                    r27.xy = r20.xy * r26.zy + float2(0.5,0.5);
                    r27.zw = float2(-0.5,-0.5) + r26.zy;
                    r27.xy = min(r27.xy, r27.zw);
                    r28.x = (uint)r16.w;
                    r28.y = (uint)r18.x;
                    r27.xy = r28.xy + r27.xy;
                    r27.xy = invMaskTexSizes.xy * r27.xy;
                    r27.z = (uint)r18.w;
                    r28.xyz = invMaskTexSizes.xyx * r24.zyz;
                    r28.xyz = r28.xyz * r26.xyz;
                    r29.xyz = invMaskTexSizes.xyx * r25.zyz;
                    r26.xyz = r29.xyz * r26.xyz;
                    r16.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r27.xyz, r28.x, r26.x).x;
                  } else {
                    r17.w = cmp((int)r20.w == 4);
                    if (r17.w != 0) {
                      r17.w = (int)r19.y & 4095;
                      if (12 == 0) r18.x = 0; else if (12+12 < 32) {                       r18.x = (uint)r19.y << (32-(12 + 12)); r18.x = (uint)r18.x >> (32-12);                      } else r18.x = (uint)r19.y >> 12;
                      if (10 == 0) r18.w = 0; else if (10+20 < 32) {                       r18.w = (uint)r19.w << (32-(10 + 20)); r18.w = (uint)r18.w >> (32-10);                      } else r18.w = (uint)r19.w >> 20;
                      r26.xz = (uint2)r13.ww;
                      r26.y = (uint)r15.w;
                      r20.zw = r20.xy * r26.zy + float2(0.5,0.5);
                      r27.xy = float2(-0.5,-0.5) + r26.zy;
                      r20.zw = min(r27.xy, r20.zw);
                      r27.x = (uint)r17.w;
                      r27.yz = (uint2)r18.xw;
                      r20.zw = r27.xy + r20.zw;
                      r27.xy = invBcTexSizes.xy * r20.zw;
                      r28.xyz = invBcTexSizes.xyx * r24.zyz;
                      r28.xyz = r28.xyz * r26.xyz;
                      r29.xyz = invBcTexSizes.xyx * r25.zyz;
                      r26.xyz = r29.xyz * r26.xyz;
                      r16.w = materialTexBC7.SampleGrad(samplerLinearClamp_s, r27.xyz, r28.x, r26.x).x;
                    } else {
                      r17.w = (int)r19.y & 4095;
                      if (12 == 0) r18.x = 0; else if (12+12 < 32) {                       r18.x = (uint)r19.y << (32-(12 + 12)); r18.x = (uint)r18.x >> (32-12);                      } else r18.x = (uint)r19.y >> 12;
                      if (10 == 0) r18.w = 0; else if (10+20 < 32) {                       r18.w = (uint)r19.w << (32-(10 + 20)); r18.w = (uint)r18.w >> (32-10);                      } else r18.w = (uint)r19.w >> 20;
                      r26.xz = (uint2)r13.ww;
                      r26.y = (uint)r15.w;
                      r20.zw = r20.xy * r26.zy + float2(0.5,0.5);
                      r27.xy = float2(-0.5,-0.5) + r26.zy;
                      r20.zw = min(r27.xy, r20.zw);
                      r27.x = (uint)r17.w;
                      r27.yz = (uint2)r18.xw;
                      r20.zw = r27.xy + r20.zw;
                      r27.xy = invBcTexSizes.zw * r20.zw;
                      r28.xyz = invBcTexSizes.zwz * r24.zyz;
                      r28.xyz = r28.xyz * r26.xyz;
                      r29.xyz = invBcTexSizes.zwz * r25.zyz;
                      r26.xyz = r29.xyz * r26.xyz;
                      r16.w = materialTexBC3.SampleGrad(samplerLinearClamp_s, r27.xyz, r28.x, r26.x).x;
                    }
                  }
                  r26.xyz = r16.www * float3(0.959999979,0.959999979,0.959999979) + float3(0.0399999991,0.0399999991,0.0399999991);
                  r13.w = saturate(1 + -r16.w);
                  r17.xyz = r17.xyz * r13.www;
                } else {
                  r26.xyz = float3(0.0399999991,0.0399999991,0.0399999991);
                }
                r18.xy = (uint2)r18.zy >> int2(16,16);
                if (r18.x != 0) {
                  r13.w = cmp((int)r18.x == 2);
                  if (r13.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r11.w, r11.w, l(112), t36.xxxx
                  r11.w = samp0[]..swiz;
                    if (4 == 0) r18.z = 0; else if (4+24 < 32) {                     r18.z = (uint)r19.z << (32-(4 + 24)); r18.z = (uint)r18.z >> (32-4);                    } else r18.z = (uint)r19.z >> 24;
                    if (12 == 0) r18.w = 0; else if (12+12 < 32) {                     r18.w = (uint)r19.z << (32-(12 + 12)); r18.w = (uint)r18.w >> (32-12);                    } else r18.w = (uint)r19.z >> 12;
                    r13.w = 1 << (int)r18.z;
                    r15.w = (uint)r19.z >> 28;
                    r15.w = 1 << (int)r15.w;
                    r16.w = (int)r19.z & 4095;
                    r11.w = (int)r11.w & 1023;
                    r27.xz = (uint2)r13.ww;
                    r27.y = (uint)r15.w;
                    r19.yz = r20.xy * r27.zy + float2(0.5,0.5);
                    r20.zw = float2(-0.5,-0.5) + r27.zy;
                    r19.yz = min(r20.zw, r19.yz);
                    r28.x = (uint)r16.w;
                    r28.y = (uint)r18.w;
                    r18.zw = r28.xy + r19.yz;
                    r28.xy = invMaskTexSizes.xy * r18.zw;
                    r28.z = (uint)r11.w;
                    r29.xyz = invMaskTexSizes.xyx * r24.zyz;
                    r29.xyz = r29.xyz * r27.xyz;
                    r30.xyz = invMaskTexSizes.xyx * r25.zyz;
                    r27.xyz = r30.xyz * r27.xyz;
                    r11.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r28.xyz, r29.x, r27.x).x;
                  } else {
                    r13.w = cmp((int)r18.x == 6);
                    r11.w = r13.w ? 0 : 1;
                  }
                } else {
                  r11.w = 1;
                }
                r11.w = saturate(0.764705896 * r11.w);
                if (r18.y != 0) {
                  r13.w = cmp((int)r18.y == 3);
                  if (r13.w != 0) {
                    if (4 == 0) r18.x = 0; else if (4+24 < 32) {                     r18.x = (uint)r19.x << (32-(4 + 24)); r18.x = (uint)r18.x >> (32-4);                    } else r18.x = (uint)r19.x >> 24;
                    if (12 == 0) r18.y = 0; else if (12+12 < 32) {                     r18.y = (uint)r19.x << (32-(12 + 12)); r18.y = (uint)r18.y >> (32-12);                    } else r18.y = (uint)r19.x >> 12;
                    if (10 == 0) r18.z = 0; else if (10+10 < 32) {                     r18.z = (uint)r19.w << (32-(10 + 10)); r18.z = (uint)r18.z >> (32-10);                    } else r18.z = (uint)r19.w >> 10;
                    r13.w = 1 << (int)r18.x;
                    r15.w = (uint)r19.x >> 28;
                    r15.w = 1 << (int)r15.w;
                    r16.w = (int)r19.x & 4095;
                    r19.xz = (uint2)r13.ww;
                    r19.y = (uint)r15.w;
                    r18.xw = r20.xy * r19.xy + float2(0.5,0.5);
                    r20.xy = float2(-0.5,-0.5) + r19.zy;
                    r18.xw = min(r20.xy, r18.xw);
                    r20.x = (uint)r16.w;
                    r20.yz = (uint2)r18.yz;
                    r18.xy = r20.xy + r18.xw;
                    r20.xy = invMaskTexSizes.zw * r18.xy;
                    r18.xyz = invMaskTexSizes.zwz * r24.xyz;
                    r18.xyz = r18.xyz * r19.xyz;
                    r24.xyz = invMaskTexSizes.zwz * r25.xyz;
                    r19.xyz = r24.xyz * r19.xyz;
                    r18.xyz = materialTexBC7linear.SampleGrad(samplerLinearClamp_s, r20.xyz, r18.xyz, r19.xyz).xyz;
                  } else {
                    r18.xyz = float3(0.5,0.5,1);
                  }
                  r18.xy = r18.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
                  r13.w = dot(r18.xy, r18.xy);
                  r13.w = 1 + -r13.w;
                  r13.w = max(0, r13.w);
                  r13.w = sqrt(r13.w);
                  r15.w = r18.z * r18.z;
                  r15.w = 0.333333343 * r15.w;
                  r15.w = min(1, r15.w);
                  r16.w = dot(r23.xyz, r23.xyz);
                  r16.w = rsqrt(r16.w);
                  r19.xyz = r23.xyz * r16.www;
                  r16.w = dot(-r22.xyz, -r22.xyz);
                  r16.w = rsqrt(r16.w);
                  r20.xyz = -r22.xyz * r16.www;
                  r21.xyz = r21.xyz * r2.yyy;
                  r19.xyz = r19.xyz * r2.yyy;
                  r20.xyz = r20.xyz * r2.yyy;
                  r16.w = -17 * r11.w;
                  r16.w = exp2(r16.w);
                  r15.w = r16.w + r15.w;
                  r15.w = log2(r15.w);
                  r15.w = -0.0588235296 * r15.w;
                  r11.w = max(0, r15.w);
                  r18.yzw = r20.xyz * r18.yyy;
                  r18.xyz = r19.xyz * r18.xxx + r18.yzw;
                  r18.xyz = r21.xyz * r13.www + r18.xyz;
                  r13.w = dot(r18.xyz, r18.xyz);
                  r13.w = rsqrt(r13.w);
                  r18.xyz = r18.xyz * r13.www;
                  r13.w = 1 + -r12.w;
                  r15.x = r14.x;
                  r19.xyz = r15.xyz * r13.www;
                  r15.xyz = r18.xyz * r12.www + r19.xyz;
                  r14.x = r15.x;
                }
                r13.w = 1 + -r12.w;
                r18.xyz = r13.www * r9.xyz;
                r9.xyz = r17.xyz * r12.www + r18.xyz;
                r16.z = r14.y;
                r17.xyz = r16.xyz * r13.www;
                r16.xyz = r26.xyz * r12.www + r17.xyz;
                r15.x = r14.w * r13.w;
                r14.z = r14.z * r13.w + r12.w;
                r14.w = r11.w * r12.w + r15.x;
                r14.y = r16.z;
              }
            }
          }
        }
      }
      r10.xyzw = r14.xwyz;
      r11.xyz = r9.xyz;
      r12.yz = r15.yz;
      r13.xy = r16.xy;
      r5.w = (int)r5.w + 32;
    }
    r12.x = r10.x;
    r13.z = r10.z;
    r10.xy = r10.wy;
  } else {
    r11.xyz = float3(0,0,0);
    r12.xyz = float3(0,0,0);
    r13.xyz = float3(0,0,0);
    r10.xy = float2(0,0);
  }
  if (r5.z != 0) {
    r6.xyz = eyeOffset.xyz + v6.xyz;
    r4.w = numDynamicDecals + numStaticDecals;
    r5.y = 0.0078125 * r2.x;
    r5.y = min(15, r5.y);
    r5.y = (uint)r5.y;
    r7.xy = (uint2)r2.zw >> int2(6,6);
    r7.z = (uint)r5.y << 4;
    r5.y = numStaticDecals & -32;
    r5.z = (int)-r5.y + numStaticDecals;
    r5.w = (int)r4.w & -32;
    r7.w = (int)r4.w + (int)-r5.w;
    r8.xy = float2(0,0);
    r9.w = 0;
    r6.w = 1;
    r14.xyz = r11.xyz;
    r15.xyz = r12.xyz;
    r16.xyz = r13.xyz;
    r17.xy = r10.xy;
    r8.w = r5.y;
    while (true) {
      r10.z = cmp((uint)r8.w >= (uint)r4.w);
      if (r10.z != 0) break;
      r8.z = (uint)r8.w >> 5;
      r9.xyz = (int3)r7.xyz + (int3)r8.xyz;
      r8.z = visibleDecals.Load(r9.xyzw).x;
      r9.x = cmp((int)r5.y == (int)r8.w);
      bitmask.y = ((~(-1 << r5.z)) << 0) & 0xffffffff;  r9.y = (((uint)0 << 0) & bitmask.y) | ((uint)r8.z & ~bitmask.y);
      r8.z = r9.x ? r9.y : r8.z;
      r9.x = cmp((int)r5.w == (int)r8.w);
      if (r7.w == 0) r9.y = 0; else if (r7.w+0 < 32) {       r9.y = (uint)r8.z << (32-(r7.w + 0)); r9.y = (uint)r9.y >> (32-r7.w);      } else r9.y = (uint)r8.z >> 0;
      r8.z = r9.x ? r9.y : r8.z;
      r9.x = (int)r3.w + (int)r8.w;
      r18.xyz = r14.xyz;
      r19.xyz = r15.xyz;
      r20.xyz = r16.xyz;
      r9.yz = r17.xy;
      r10.z = r8.z;
      while (true) {
        if (r10.z == 0) break;
        r10.w = firstbitlow((uint)r10.z);
        r11.w = 1 << (int)r10.w;
        r12.w = (int)r10.z & (int)r11.w;
        if (r12.w != 0) {
          r10.z = (int)r10.z ^ (int)r11.w;
          r11.w = (int)r9.x + (int)r10.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r21.xyzw, r11.w, l(0), t11.xyzw
        r21.x = samp0[]..swiz;
        r21.y = samp0[]..swiz;
        r21.z = samp0[]..swiz;
        r21.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r22.xy, r11.w, l(16), t11.xyxx
        r22.x = samp0[]..swiz;
        r22.y = samp0[]..swiz;
          r21.xyz = -v6.xyz + r21.xyz;
          r22.z = r21.w;
          r21.xyz = cmp(abs(r21.xyz) < r22.zxy);
          r11.w = r21.y ? r21.x : 0;
          r11.w = r21.z ? r11.w : 0;
          if (r11.w != 0) {
            r10.w = (int)r8.w + (int)r10.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r21.xyz, r10.w, l(0), t36.xyzx
          r21.x = samp0[]..swiz;
          r21.y = samp0[]..swiz;
          r21.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r22.xyz, r10.w, l(16), t36.xyzx
          r22.x = samp0[]..swiz;
          r22.y = samp0[]..swiz;
          r22.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r23.xyz, r10.w, l(32), t36.xyzx
          r23.x = samp0[]..swiz;
          r23.y = samp0[]..swiz;
          r23.z = samp0[]..swiz;
            r24.x = r21.x;
            r24.y = r22.x;
            r24.z = r23.x;
            r11.w = dot(r24.xyz, r24.xyz);
            r11.w = rsqrt(r11.w);
            r25.xyz = r24.xyz * r11.www;
            r11.w = dot(r25.xyz, v3.xyz);
            r11.w = cmp(r11.w >= 0.5);
            if (r11.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r26.xyw, r10.w, l(48), t36.xyxz
            r26.x = samp0[]..swiz;
            r26.y = samp0[]..swiz;
            r26.w = samp0[]..swiz;
              r24.w = r26.x;
              r24.x = dot(r6.xyzw, r24.xyzw);
              r27.x = r21.y;
              r27.y = r22.y;
              r27.z = r23.y;
              r27.w = r26.y;
              r24.y = dot(r6.xyzw, r27.xyzw);
              r26.x = r21.z;
              r26.y = r22.z;
              r26.z = r23.z;
              r24.z = dot(r6.xyzw, r26.xyzw);
              r21.xyz = cmp(abs(r24.xyz) < float3(1,1,1));
              r11.w = r21.y ? r21.x : 0;
              r11.w = r21.z ? r11.w : 0;
              if (r11.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r21.xyzw, r10.w, l(64), t36.xyzw
              r21.x = samp0[]..swiz;
              r21.y = samp0[]..swiz;
              r21.z = samp0[]..swiz;
              r21.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r22.xyzw, r10.w, l(80), t36.xyzw
              r22.x = samp0[]..swiz;
              r22.y = samp0[]..swiz;
              r22.z = samp0[]..swiz;
              r22.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r23.xyzw, r10.w, l(96), t36.xyzw
              r23.x = samp0[]..swiz;
              r23.y = samp0[]..swiz;
              r23.z = samp0[]..swiz;
              r23.w = samp0[]..swiz;
                r28.x = dot(r3.xyz, r27.xyz);
                r28.y = dot(r3.xyz, r26.xyz);
                r29.x = dot(r4.xyz, r27.xyz);
                r29.y = dot(r4.xyz, r26.xyz);
                r17.zw = r24.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r24.xyz = float3(0.5,-0.5,0.5) * r28.xyx;
                r28.xyz = float3(0.5,-0.5,0.5) * r29.xyx;
                r29.xy = (int2)r22.yz & int2(65535,65535);
                if (r29.x != 0) {
                  if (4 == 0) r11.w = 0; else if (4+24 < 32) {                   r11.w = (uint)r22.w << (32-(4 + 24)); r11.w = (uint)r11.w >> (32-4);                  } else r11.w = (uint)r22.w >> 24;
                  r11.w = 1 << (int)r11.w;
                  r12.w = (uint)r22.w >> 28;
                  r12.w = 1 << (int)r12.w;
                  r13.w = cmp((int)r29.x == 4);
                  if (r13.w != 0) {
                    r13.w = (int)r22.w & 4095;
                    if (12 == 0) r14.w = 0; else if (12+12 < 32) {                     r14.w = (uint)r22.w << (32-(12 + 12)); r14.w = (uint)r14.w >> (32-12);                    } else r14.w = (uint)r22.w >> 12;
                    r15.w = (int)r23.w & 1023;
                    r30.xz = (uint2)r11.ww;
                    r30.y = (uint)r12.w;
                    r29.zw = r17.zw * r30.zy + float2(0.5,0.5);
                    r31.xy = float2(-0.5,-0.5) + r30.zy;
                    r29.zw = min(r31.xy, r29.zw);
                    r31.x = (uint)r13.w;
                    r31.y = (uint)r14.w;
                    r29.zw = r31.xy + r29.zw;
                    r31.xy = invBcTexSizes.xy * r29.zw;
                    r31.z = (uint)r15.w;
                    r32.xyz = invBcTexSizes.xyx * r24.zyz;
                    r32.xyz = r32.xyz * r30.xyz;
                    r33.xyz = invBcTexSizes.xyx * r28.zyz;
                    r30.xyz = r33.xyz * r30.xyz;
                    r30.xyzw = materialTexBC7.SampleGrad(samplerLinearClamp_s, r31.xyz, r32.xyzx, r30.xyzx).xyzw;
                  } else {
                    r13.w = cmp((int)r29.x == 1);
                    if (r13.w != 0) {
                      r13.w = (int)r22.w & 4095;
                      if (12 == 0) r14.w = 0; else if (12+12 < 32) {                       r14.w = (uint)r22.w << (32-(12 + 12)); r14.w = (uint)r14.w >> (32-12);                      } else r14.w = (uint)r22.w >> 12;
                      r15.w = (int)r23.w & 1023;
                      r31.xz = (uint2)r11.ww;
                      r31.y = (uint)r12.w;
                      r29.xz = r17.zw * r31.zy + float2(0.5,0.5);
                      r32.xy = float2(-0.5,-0.5) + r31.zy;
                      r29.xz = min(r32.xy, r29.xz);
                      r32.x = (uint)r13.w;
                      r32.y = (uint)r14.w;
                      r29.xz = r32.xy + r29.xz;
                      r32.xy = invBcTexSizes.zw * r29.xz;
                      r32.z = (uint)r15.w;
                      r29.xzw = invBcTexSizes.zwz * r24.zyz;
                      r29.xzw = r29.xzw * r31.xyz;
                      r33.xyz = invBcTexSizes.zwz * r28.zyz;
                      r31.xyz = r33.xyz * r31.xyz;
                      r30.xyzw = materialTexBC3.SampleGrad(samplerLinearClamp_s, r32.xyz, r29.xzwx, r31.xyzx).xyzw;
                    } else {
                      r30.xyzw = float4(1,1,1,1);
                    }
                  }
                  r21.xyz = r30.xyz * r21.xyz;
                  r11.w = r30.w * r21.w;
                } else {
                  r21.xyz = float3(0,0,0);
                  r11.w = 0;
                }
                r12.w = cmp(r22.x != 1.000000);
                r13.w = r11.w + r21.w;
                r13.w = saturate(r13.w * r22.x + -r22.x);
                r11.w = r12.w ? r13.w : r11.w;
                if (r29.y != 0) {
                  if (4 == 0) r12.w = 0; else if (4+24 < 32) {                   r12.w = (uint)r23.y << (32-(4 + 24)); r12.w = (uint)r12.w >> (32-4);                  } else r12.w = (uint)r23.y >> 24;
                  r12.w = 1 << (int)r12.w;
                  r13.w = (uint)r23.y >> 28;
                  r13.w = 1 << (int)r13.w;
                  r14.w = cmp((int)r29.y == 2);
                  if (r14.w != 0) {
                    r14.w = (int)r23.y & 4095;
                    if (12 == 0) r22.x = 0; else if (12+12 < 32) {                     r22.x = (uint)r23.y << (32-(12 + 12)); r22.x = (uint)r22.x >> (32-12);                    } else r22.x = (uint)r23.y >> 12;
                    if (10 == 0) r22.w = 0; else if (10+20 < 32) {                     r22.w = (uint)r23.w << (32-(10 + 20)); r22.w = (uint)r22.w >> (32-10);                    } else r22.w = (uint)r23.w >> 20;
                    r30.xz = (uint2)r12.ww;
                    r30.y = (uint)r13.w;
                    r29.xz = r17.zw * r30.zy + float2(0.5,0.5);
                    r31.xy = float2(-0.5,-0.5) + r30.zy;
                    r29.xz = min(r31.xy, r29.xz);
                    r31.x = (uint)r14.w;
                    r31.yz = (uint2)r22.xw;
                    r29.xz = r31.xy + r29.xz;
                    r31.xy = invMaskTexSizes.xy * r29.xz;
                    r29.xzw = invMaskTexSizes.xyx * r24.zyz;
                    r29.xzw = r29.xzw * r30.xyz;
                    r32.xyz = invMaskTexSizes.xyx * r28.zyz;
                    r30.xyz = r32.xyz * r30.xyz;
                    r14.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r31.xyz, r29.x, r30.x).x;
                  } else {
                    r15.w = cmp((int)r29.y == 4);
                    if (r15.w != 0) {
                      r15.w = (int)r23.y & 4095;
                      if (12 == 0) r22.x = 0; else if (12+12 < 32) {                       r22.x = (uint)r23.y << (32-(12 + 12)); r22.x = (uint)r22.x >> (32-12);                      } else r22.x = (uint)r23.y >> 12;
                      if (10 == 0) r22.w = 0; else if (10+20 < 32) {                       r22.w = (uint)r23.w << (32-(10 + 20)); r22.w = (uint)r22.w >> (32-10);                      } else r22.w = (uint)r23.w >> 20;
                      r29.xz = (uint2)r12.ww;
                      r29.y = (uint)r13.w;
                      r30.xy = r17.zw * r29.zy + float2(0.5,0.5);
                      r30.zw = float2(-0.5,-0.5) + r29.zy;
                      r30.xy = min(r30.xy, r30.zw);
                      r31.x = (uint)r15.w;
                      r31.y = (uint)r22.x;
                      r30.xy = r31.xy + r30.xy;
                      r30.xy = invBcTexSizes.xy * r30.xy;
                      r30.z = (uint)r22.w;
                      r31.xyz = invBcTexSizes.xyx * r24.zyz;
                      r31.xyz = r31.xyz * r29.xyz;
                      r32.xyz = invBcTexSizes.xyx * r28.zyz;
                      r29.xyz = r32.xyz * r29.xyz;
                      r14.w = materialTexBC7.SampleGrad(samplerLinearClamp_s, r30.xyz, r31.x, r29.x).x;
                    } else {
                      r15.w = (int)r23.y & 4095;
                      if (12 == 0) r22.x = 0; else if (12+12 < 32) {                       r22.x = (uint)r23.y << (32-(12 + 12)); r22.x = (uint)r22.x >> (32-12);                      } else r22.x = (uint)r23.y >> 12;
                      if (10 == 0) r22.w = 0; else if (10+20 < 32) {                       r22.w = (uint)r23.w << (32-(10 + 20)); r22.w = (uint)r22.w >> (32-10);                      } else r22.w = (uint)r23.w >> 20;
                      r29.xz = (uint2)r12.ww;
                      r29.y = (uint)r13.w;
                      r30.xy = r17.zw * r29.zy + float2(0.5,0.5);
                      r30.zw = float2(-0.5,-0.5) + r29.zy;
                      r30.xy = min(r30.xy, r30.zw);
                      r31.x = (uint)r15.w;
                      r31.y = (uint)r22.x;
                      r30.xy = r31.xy + r30.xy;
                      r30.xy = invBcTexSizes.zw * r30.xy;
                      r30.z = (uint)r22.w;
                      r31.xyz = invBcTexSizes.zwz * r24.zyz;
                      r31.xyz = r31.xyz * r29.xyz;
                      r32.xyz = invBcTexSizes.zwz * r28.zyz;
                      r29.xyz = r32.xyz * r29.xyz;
                      r14.w = materialTexBC3.SampleGrad(samplerLinearClamp_s, r30.xyz, r31.x, r29.x).x;
                    }
                  }
                  r29.xyz = r14.www * float3(0.959999979,0.959999979,0.959999979) + float3(0.0399999991,0.0399999991,0.0399999991);
                  r12.w = saturate(1 + -r14.w);
                  r21.xyz = r21.xyz * r12.www;
                } else {
                  r29.xyz = float3(0.0399999991,0.0399999991,0.0399999991);
                }
                r22.xy = (uint2)r22.zy >> int2(16,16);
                if (r22.x != 0) {
                  r12.w = cmp((int)r22.x == 2);
                  if (r12.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r10.w, r10.w, l(112), t36.xxxx
                  r10.w = samp0[]..swiz;
                    if (4 == 0) r22.z = 0; else if (4+24 < 32) {                     r22.z = (uint)r23.z << (32-(4 + 24)); r22.z = (uint)r22.z >> (32-4);                    } else r22.z = (uint)r23.z >> 24;
                    if (12 == 0) r22.w = 0; else if (12+12 < 32) {                     r22.w = (uint)r23.z << (32-(12 + 12)); r22.w = (uint)r22.w >> (32-12);                    } else r22.w = (uint)r23.z >> 12;
                    r12.w = 1 << (int)r22.z;
                    r13.w = (uint)r23.z >> 28;
                    r13.w = 1 << (int)r13.w;
                    r14.w = (int)r23.z & 4095;
                    r10.w = (int)r10.w & 1023;
                    r30.xz = (uint2)r12.ww;
                    r30.y = (uint)r13.w;
                    r23.yz = r17.zw * r30.zy + float2(0.5,0.5);
                    r31.xy = float2(-0.5,-0.5) + r30.zy;
                    r23.yz = min(r31.xy, r23.yz);
                    r31.x = (uint)r14.w;
                    r31.y = (uint)r22.w;
                    r22.zw = r31.xy + r23.yz;
                    r31.xy = invMaskTexSizes.xy * r22.zw;
                    r31.z = (uint)r10.w;
                    r32.xyz = invMaskTexSizes.xyx * r24.zyz;
                    r32.xyz = r32.xyz * r30.xyz;
                    r33.xyz = invMaskTexSizes.xyx * r28.zyz;
                    r30.xyz = r33.xyz * r30.xyz;
                    r10.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r31.xyz, r32.x, r30.x).x;
                  } else {
                    r12.w = cmp((int)r22.x == 6);
                    r10.w = r12.w ? 0 : 1;
                  }
                } else {
                  r10.w = 1;
                }
                r10.w = saturate(0.764705896 * r10.w);
                if (r22.y != 0) {
                  r12.w = cmp((int)r22.y == 3);
                  if (r12.w != 0) {
                    if (4 == 0) r22.x = 0; else if (4+24 < 32) {                     r22.x = (uint)r23.x << (32-(4 + 24)); r22.x = (uint)r22.x >> (32-4);                    } else r22.x = (uint)r23.x >> 24;
                    if (12 == 0) r22.y = 0; else if (12+12 < 32) {                     r22.y = (uint)r23.x << (32-(12 + 12)); r22.y = (uint)r22.y >> (32-12);                    } else r22.y = (uint)r23.x >> 12;
                    if (10 == 0) r22.z = 0; else if (10+10 < 32) {                     r22.z = (uint)r23.w << (32-(10 + 10)); r22.z = (uint)r22.z >> (32-10);                    } else r22.z = (uint)r23.w >> 10;
                    r12.w = 1 << (int)r22.x;
                    r13.w = (uint)r23.x >> 28;
                    r13.w = 1 << (int)r13.w;
                    r14.w = (int)r23.x & 4095;
                    r23.xz = (uint2)r12.ww;
                    r23.y = (uint)r13.w;
                    r17.zw = r17.zw * r23.xy + float2(0.5,0.5);
                    r22.xw = float2(-0.5,-0.5) + r23.zy;
                    r17.zw = min(r22.xw, r17.zw);
                    r30.x = (uint)r14.w;
                    r30.yz = (uint2)r22.yz;
                    r17.zw = r30.xy + r17.zw;
                    r30.xy = invMaskTexSizes.zw * r17.zw;
                    r22.xyz = invMaskTexSizes.zwz * r24.xyz;
                    r22.xyz = r22.xyz * r23.xyz;
                    r24.xyz = invMaskTexSizes.zwz * r28.xyz;
                    r23.xyz = r24.xyz * r23.xyz;
                    r22.xyz = materialTexBC7linear.SampleGrad(samplerLinearClamp_s, r30.xyz, r22.xyz, r23.xyz).xyz;
                  } else {
                    r22.xyz = float3(0.5,0.5,1);
                  }
                  r17.zw = r22.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
                  r12.w = dot(r17.zw, r17.zw);
                  r12.w = 1 + -r12.w;
                  r12.w = max(0, r12.w);
                  r12.w = sqrt(r12.w);
                  r13.w = r22.z * r22.z;
                  r13.w = 0.333333343 * r13.w;
                  r13.w = min(1, r13.w);
                  r14.w = dot(r27.xyz, r27.xyz);
                  r14.w = rsqrt(r14.w);
                  r22.xyz = r27.xyz * r14.www;
                  r14.w = dot(-r26.xyz, -r26.xyz);
                  r14.w = rsqrt(r14.w);
                  r23.xyz = -r26.xyz * r14.www;
                  r24.xyz = r25.xyz * r2.yyy;
                  r22.xyz = r22.xyz * r2.yyy;
                  r23.xyz = r23.xyz * r2.yyy;
                  r14.w = -17 * r10.w;
                  r14.w = exp2(r14.w);
                  r13.w = r14.w + r13.w;
                  r13.w = log2(r13.w);
                  r13.w = -0.0588235296 * r13.w;
                  r10.w = max(0, r13.w);
                  r23.xyz = r23.xyz * r17.www;
                  r22.xyz = r22.xyz * r17.zzz + r23.xyz;
                  r22.xyz = r24.xyz * r12.www + r22.xyz;
                  r12.w = dot(r22.xyz, r22.xyz);
                  r12.w = rsqrt(r12.w);
                  r22.xyz = r22.xyz * r12.www;
                  r12.w = 1 + -r11.w;
                  r23.xyz = r19.xyz * r12.www;
                  r19.xyz = r22.xyz * r11.www + r23.xyz;
                }
                r12.w = 1 + -r11.w;
                r22.xyz = r18.xyz * r12.www;
                r18.xyz = r21.xyz * r11.www + r22.xyz;
                r21.xyz = r20.xyz * r12.www;
                r20.xyz = r29.xyz * r11.www + r21.xyz;
                r13.w = r12.w * r9.z;
                r9.y = r9.y * r12.w + r11.w;
                r9.z = r10.w * r11.w + r13.w;
              }
            }
          }
        }
      }
      r14.xyz = r18.xyz;
      r15.xyz = r19.xyz;
      r16.xyz = r20.xyz;
      r17.xy = r9.yz;
      r8.w = (int)r8.w + 32;
    }
    r11.xyz = r14.xyz;
    r12.xyz = r15.xyz;
    r13.xyz = r16.xyz;
    r10.xy = r17.xy;
  }
  r10.x = saturate(r10.x);
  r2.y = 1 + -r10.x;
  r0.xyz = r0.xyz * r2.yyy + r11.xyz;
  r0.w = r0.w * r2.y + r10.x;
  r1.yzw = r1.yzw * r2.yyy + r12.xyz;
  r3.x = dot(r1.yzw, r1.yzw);
  r3.x = rsqrt(r3.x);
  r3.xyz = r3.xxx * r1.yzw;
  r1.y = r1.x * r2.y + r10.y;
  r4.xyz = r2.yyy * float3(0.0799999982,0.0799999982,0.0799999982) + r13.xyz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=384)(mixed,mixed,mixed,mixed) r1.z, r5.x, l(0), t0.xxxx
r1.z = samp0[]..swiz;
  r0.w = saturate(r1.z + r0.w);
  r1.z = dot(-v6.xyz, -v6.xyz);
  r1.z = rsqrt(r1.z);
  r5.xyz = -v6.xyz * r1.zzz;
  r1.w = cmp(isDepthHack != 0);
  r6.xy = (uint2)r2.wz;
  r2.y = dot(r6.yx, float2(0.0671105608,0.00583714992));
  r2.y = frac(r2.y);
  r2.xy = float2(0.0078125,52.9829178) * r2.xy;
  r2.y = frac(r2.y);
  r2.y = r2.y * 6.28318548 + gameTick.w;
  sincos(r2.y, r7.x, r8.x);
  r2.y = dot(r6.xy, float2(0.0671105608,0.00583714992));
  r2.y = frac(r2.y);
  r2.y = 52.9829178 * r2.y;
  r2.y = frac(r2.y);
  r2.y = r2.y * 6.28318548 + gameTick.w;
  sincos(r2.y, r6.x, r9.x);
  r1.x = saturate(dot(r3.xyz, r5.xyz));
  r2.y = dot(-r5.xyz, r3.xyz);
  r2.y = r2.y + r2.y;
  r10.xyz = r3.xyz * -r2.yyy + -r5.xyz;
  r2.y = 17 * r1.y;
  r2.y = exp2(r2.y);
  r2.y = 2 + r2.y;
  r2.y = 2 / r2.y;
  r4.w = sqrt(r2.y);
  r6.yz = float2(1,1) + -r1.yx;
  r5.w = 5 * r6.y;
  r6.w = r6.y * 5 + -2.5;
  r6.w = saturate(0.400000006 * r6.w);
  r6.w = 100 * r6.w;
  r7.zw = -r6.yy * float2(10,5) + float2(6.85740995,7.08500004);
  r7.z = exp2(r7.z);
  r8.w = r5.w * r6.y;
  r7.w = -r8.w * 2.0159049 + r7.w;
  r7.w = exp2(r7.w);
  r7.zw = r7.zw * r1.xx;
  r7.zw = float2(9.1368103,9.70808983) * r7.zw;
  r7.z = max(r7.z, r7.w);
  r7.z = max(1.26815999, r7.z);
  r7.w = numRefProbes + -numOverrideProbes;
  r2.x = min(15, r2.x);
  r2.x = (uint)r2.x;
  r11.xy = (uint2)r2.zw >> int2(6,6);
  r11.z = (uint)r2.x << 4;
  r2.x = (int)r7.w & -32;
  r2.z = (int)-r2.x + (int)r7.w;
  r2.w = numRefProbes & -32;
  r8.w = (int)-r2.w + numRefProbes;
  r12.xy = float2(0,0);
  r13.w = 0;
  r14.yz = float2(0,1);
  r15.xy = float2(0,0);
  r16.x = 1;
  r17.xy = float2(0,0);
  r18.xy = float2(0,0);
  r19.xyzw = float4(0,0,0,0);
  r20.yzw = float3(0,0,0);
  r9.y = r2.x;
  while (true) {
    r9.z = cmp((uint)r9.y >= numRefProbes);
    if (r9.z != 0) break;
    r12.z = (uint)r9.y >> 5;
    r13.xyz = (int3)r11.xyz + (int3)r12.xyz;
    r9.z = visibleProbes.Load(r13.xyzw).x;
    r9.w = cmp((int)r2.x == (int)r9.y);
    bitmask.w = ((~(-1 << r2.z)) << 0) & 0xffffffff;  r11.w = (((uint)0 << 0) & bitmask.w) | ((uint)r9.z & ~bitmask.w);
    r9.z = r9.w ? r11.w : r9.z;
    r9.w = cmp((int)r2.w == (int)r9.y);
    if (r8.w == 0) r11.w = 0; else if (r8.w+0 < 32) {     r11.w = (uint)r9.z << (32-(r8.w + 0)); r11.w = (uint)r11.w >> (32-r8.w);    } else r11.w = (uint)r9.z >> 0;
    r9.z = r9.w ? r11.w : r9.z;
    r9.w = (int)r9.y + numLights;
    r21.xyzw = r19.xyzw;
    r22.xyz = r20.yzw;
    r11.w = r9.z;
    while (true) {
      if (r11.w == 0) break;
      r12.z = firstbitlow((uint)r11.w);
      r12.w = 1 << (int)r12.z;
      r13.x = (int)r11.w & (int)r12.w;
      if (r13.x != 0) {
        r11.w = (int)r11.w ^ (int)r12.w;
        r12.w = (int)r9.w + (int)r12.z;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r23.xyzw, r12.w, l(0), t11.xyzw
      r23.x = samp0[]..swiz;
      r23.y = samp0[]..swiz;
      r23.z = samp0[]..swiz;
      r23.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r13.xy, r12.w, l(16), t11.xyxx
      r13.x = samp0[]..swiz;
      r13.y = samp0[]..swiz;
        r23.xyz = -v6.xyz + r23.xyz;
        r13.z = r23.w;
        r13.xyz = cmp(abs(r23.xyz) < r13.zxy);
        r12.w = r13.y ? r13.x : 0;
        r12.w = r13.z ? r12.w : 0;
        if (r12.w != 0) {
          r12.z = (int)r9.y + (int)r12.z;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r23.xyzw, r12.z, l(0), t15.wxyz
        r23.x = samp0[]..swiz;
        r23.y = samp0[]..swiz;
        r23.z = samp0[]..swiz;
        r23.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r13.xy, r12.z, l(96), t15.xyxx
        r13.x = samp0[]..swiz;
        r13.y = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r12.z, l(116), t15.zwxy
        r24.x = samp0[]..swiz;
        r24.y = samp0[]..swiz;
        r24.z = samp0[]..swiz;
        r24.w = samp0[]..swiz;
          r25.xyz = v6.xyz + -r23.yzw;
          r12.w = (int)r24.w & 0x0000ffff;
          if (6 == 0) r13.z = 0; else if (6+25 < 32) {           r13.z = (uint)r24.w << (32-(6 + 25)); r13.z = (uint)r13.z >> (32-6);          } else r13.z = (uint)r24.w >> 25;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r26.xyzw, r12.w, l(0), t16.xyzw
        r26.x = samp0[]..swiz;
        r26.y = samp0[]..swiz;
        r26.z = samp0[]..swiz;
        r26.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r12.w, l(16), t16.xyzw
        r27.x = samp0[]..swiz;
        r27.y = samp0[]..swiz;
        r27.z = samp0[]..swiz;
        r27.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r12.w, l(32), t16.xyzw
        r28.x = samp0[]..swiz;
        r28.y = samp0[]..swiz;
        r28.z = samp0[]..swiz;
        r28.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r12.w, l(48), t16.xyzw
        r29.x = samp0[]..swiz;
        r29.y = samp0[]..swiz;
        r29.z = samp0[]..swiz;
        r29.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r12.w, l(64), t16.xyzw
        r30.x = samp0[]..swiz;
        r30.y = samp0[]..swiz;
        r30.z = samp0[]..swiz;
        r30.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r12.w, l(80), t16.xyzw
        r31.x = samp0[]..swiz;
        r31.y = samp0[]..swiz;
        r31.z = samp0[]..swiz;
        r31.w = samp0[]..swiz;
          r14.w = dot(r26.xyz, r25.xyz);
          r14.w = saturate(r14.w + r26.w);
          r15.w = dot(r27.xyz, r25.xyz);
          r15.w = saturate(r15.w + r27.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r28.xyz, r25.xyz);
          r15.w = saturate(r15.w + r28.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r29.xyz, r25.xyz);
          r15.w = saturate(r15.w + r29.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r30.xyz, r25.xyz);
          r15.w = saturate(r15.w + r30.w);
          r14.w = r15.w * r14.w;
          r15.w = dot(r31.xyz, r25.xyz);
          r15.w = saturate(r15.w + r31.w);
          r14.x = r15.w * r14.w;
          r14.w = r13.x ? 0.000000 : 0;
          r14.xw = r14.ww ? r14.xy : r14.zx;
          r15.w = r13.x;
          r26.xy = r14.xw;
          r16.z = 1;
          while (true) {
            r16.w = cmp((int)r16.z >= (int)r13.z);
            if (r16.w != 0) break;
            r16.w = (int)r12.w + (int)r16.z;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r16.w, l(0), t16.xyzw
          r27.x = samp0[]..swiz;
          r27.y = samp0[]..swiz;
          r27.z = samp0[]..swiz;
          r27.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r16.w, l(16), t16.xyzw
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r16.w, l(32), t16.xyzw
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r16.w, l(48), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r16.w, l(64), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r16.w, l(80), t16.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
            r16.w = dot(r27.xyz, r25.xyz);
            r16.w = saturate(r16.w + r27.w);
            r16.w = r26.x * r16.w;
            r17.w = dot(r28.xyz, r25.xyz);
            r17.w = saturate(r17.w + r28.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r29.xyz, r25.xyz);
            r17.w = saturate(r17.w + r29.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r30.xyz, r25.xyz);
            r17.w = saturate(r17.w + r30.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r31.xyz, r25.xyz);
            r17.w = saturate(r17.w + r31.w);
            r16.w = r17.w * r16.w;
            r17.w = dot(r32.xyz, r25.xyz);
            r17.w = saturate(r17.w + r32.w);
            r26.x = r17.w * r16.w;
            r18.w = (uint)r15.w >> 2;
            if (1 == 0) r23.w = 0; else if (1+2 < 32) {             r23.w = (uint)r15.w << (32-(1 + 2)); r23.w = (uint)r23.w >> (32-1);            } else r23.w = (uint)r15.w >> 2;
            r25.w = (int)r18.w & 2;
            r26.z = max(r26.y, r26.x);
            r16.w = -r16.w * r17.w + 1;
            r16.w = r26.y * r16.w;
            r16.y = r25.w ? r16.w : r26.z;
            r26.xy = r23.ww ? r26.xy : r16.xy;
            r16.z = (int)r16.z + 1;
            r15.w = r18.w;
          }
          r26.y = saturate(r26.y);
          r12.w = r26.y * r13.y;
          r13.x = cmp(0 < r12.w);
          if (r13.x != 0) {
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r27.xyzw, r12.z, l(16), t15.xyzw
          r27.x = samp0[]..swiz;
          r27.y = samp0[]..swiz;
          r27.z = samp0[]..swiz;
          r27.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r28.xyzw, r12.z, l(32), t15.yxwz
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r29.xyzw, r12.z, l(48), t15.xyzw
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r12.z, l(64), t15.zwxy
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r12.z, l(80), t15.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r12.z, l(132), t15.zwxy
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r12.z, l(148), t15.zwxy
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r12.z, l(164), t15.zwxy
          r34.x = samp0[]..swiz;
          r34.y = samp0[]..swiz;
          r34.z = samp0[]..swiz;
          r34.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r12.z, l(180), t15.zwxy
          r35.x = samp0[]..swiz;
          r35.y = samp0[]..swiz;
          r35.z = samp0[]..swiz;
          r35.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r12.z, l(196), t15.xyzw
          r36.x = samp0[]..swiz;
          r36.y = samp0[]..swiz;
          r36.z = samp0[]..swiz;
          r36.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xy, r12.z, l(212), t15.xyxx
          r37.x = samp0[]..swiz;
          r37.y = samp0[]..swiz;
            r22.z = r26.y * r13.y + r22.z;
            r12.z = r12.w * r24.z;
            r24.z = r32.z;
            r12.w = dot(r10.xyz, r24.xyz);
            r13.x = dot(r25.xyz, r24.xyz);
            r13.x = r13.x + -r32.w;
            r13.y = cmp(r13.x >= 0);
            r13.x = max(abs(r13.x), r6.w);
            r13.x = r13.y ? r13.x : -r13.x;
            r12.w = max(1.00000001e-07, -r12.w);
            r12.w = r13.x / r12.w;
            r12.w = min(131072, abs(r12.w));
            r32.z = r33.z;
            r13.x = dot(r10.xyz, r32.xyz);
            r13.y = dot(r25.xyz, r32.xyz);
            r13.y = r13.y + -r33.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r6.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r33.z = r34.z;
            r13.x = dot(r10.xyz, r33.xyz);
            r13.y = dot(r25.xyz, r33.xyz);
            r13.y = r13.y + -r34.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r6.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r34.z = r35.z;
            r13.x = dot(r10.xyz, r34.xyz);
            r13.y = dot(r25.xyz, r34.xyz);
            r13.y = r13.y + -r35.w;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r6.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r35.z = r36.x;
            r13.x = dot(r10.xyz, r35.xyz);
            r13.y = dot(r25.xyz, r35.xyz);
            r13.y = r13.y + -r36.y;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r6.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r37.zw = r36.zw;
            r13.x = dot(r10.zxy, r37.xzw);
            r13.y = dot(r25.zxy, r37.xzw);
            r13.y = r13.y + -r37.y;
            r13.z = cmp(r13.y >= 0);
            r13.y = max(abs(r13.y), r6.w);
            r13.y = r13.z ? r13.y : -r13.y;
            r13.x = max(1.00000001e-07, -r13.x);
            r13.x = r13.y / r13.x;
            r12.w = min(abs(r13.x), r12.w);
            r13.x = r29.w;
            r13.yz = r30.zw;
            r13.xyz = r13.xyz + r25.xyz;
            r13.xyz = r10.xyz * r12.www + r13.xyz;
            r14.x = dot(r13.xyz, r13.xyz);
            r14.x = sqrt(r14.x);
            r12.w = r12.w / r14.x;
            r12.w = r12.w + r12.w;
            r12.w = sqrt(r12.w);
            r12.w = r6.y * 5 + r12.w;
            r12.w = -0.844799995 + r12.w;
            r23.y = r27.z;
            r23.z = r28.x;
            r32.x = dot(r13.xyz, r23.xyz);
            r24.xy = r27.xw;
            r24.z = r28.w;
            r32.y = dot(r13.xyz, r24.xyz);
            r28.x = r27.y;
            r32.z = dot(r13.xyz, r28.xyz);
            if (9 == 0) r13.x = 0; else if (9+16 < 32) {             r13.x = (uint)r24.w << (32-(9 + 16)); r13.x = (uint)r13.x >> (32-9);            } else r13.x = (uint)r24.w >> 16;
            r32.w = (uint)r13.x;
            r13.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r32.xyzw, r12.w).xyz;
            r27.x = dot(r25.xyz, r23.xyz);
            r27.y = dot(r25.xyz, r24.xyz);
            r27.z = dot(r25.xyz, r28.xyz);
            r16.yzw = saturate(r27.xyz * r29.xyz + float3(0.5,0.5,0.5));
            r30.z = r31.x;
            r16.yzw = r16.yzw * r30.xyz + r31.yzw;
            r32.x = dot(r3.xyz, r23.xyz);
            r32.y = dot(r3.xyz, r24.xyz);
            r32.z = dot(r3.xyz, r28.xyz);
            r23.xyz = cmp(float3(0,0,0) < r32.xyz);
            r15.z = r23.x ? 0 : 0.5;
            r24.xyz = r16.yzw + r15.xyz;
            r24.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r24.xyz, 0).xyz;
            r25.xyz = r32.xyz * r32.xyz;
            r25.xyz = r25.xyz * r12.zzz;
            r17.z = r23.y ? 0 : 0.5;
            r23.xyw = r17.xyz + r16.yzw;
            r23.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r23.xyw, 0).xyz;
            r23.xyw = r23.xyw * r25.yyy;
            r23.xyw = r24.xyz * r25.xxx + r23.xyw;
            r18.z = r23.z ? 0 : 0.5;
            r16.yzw = r18.xyz + r16.yzw;
            r16.yzw = gProbeZArray.SampleLevel(samplerLinearClamp_s, r16.yzw, 0).xyz;
            r16.yzw = r16.yzw * r25.zzz + r23.xyw;
            r23.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r32.xyzw, 6).xyz;
            r24.xyz = r23.xyz * r16.yzw;
            r12.w = dot(r24.xyz, float3(0.212599993,0.715200007,0.0722000003));
            r21.xyz = r16.yzw * r23.xyz + r21.xyz;
            r14.x = r12.w * r7.z;
            r13.xyz = r13.xyz * r12.zzz;
            r12.z = dot(r13.xyz, float3(0.212599993,0.715200007,0.0722000003));
            r12.z = r7.z * r12.w + r12.z;
            r12.z = r14.x / r12.z;
            r23.x = r21.w;
            r23.yz = r22.xy;
            r22.xyw = r13.yzx * r12.zzz + r23.yzx;
            r21.w = r22.w;
          }
        }
      }
    }
    r19.xyzw = r21.xyzw;
    r20.yzw = r22.xyz;
    r9.y = (int)r9.y + 32;
  }
  r2.w = cmp(r20.w < 1);
  if (r2.w != 0) {
    r12.xy = float2(0,0);
    r13.w = 0;
    r14.yz = float2(0,1);
    r15.xy = float2(0,0);
    r16.x = 1;
    r17.xy = float2(0,0);
    r18.xy = float2(0,0);
    r21.x = r19.w;
    r21.yzw = r20.yzw;
    r9.yzw = r19.xyz;
    r2.w = r20.w;
    r8.w = 0;
    while (true) {
      r11.w = cmp((uint)r8.w >= (uint)r7.w);
      if (r11.w != 0) break;
      r12.z = (uint)r8.w >> 5;
      r13.xyz = (int3)r11.xyz + (int3)r12.xyz;
      r11.w = visibleProbes.Load(r13.xyzw).x;
      r12.z = cmp((int)r2.x == (int)r8.w);
      if (r2.z == 0) r12.w = 0; else if (r2.z+0 < 32) {       r12.w = (uint)r11.w << (32-(r2.z + 0)); r12.w = (uint)r12.w >> (32-r2.z);      } else r12.w = (uint)r11.w >> 0;
      r11.w = r12.z ? r12.w : r11.w;
      r12.z = (int)r8.w + numLights;
      r22.xyzw = r21.xyzw;
      r13.xyz = r9.yzw;
      r12.w = r2.w;
      r14.w = r11.w;
      while (true) {
        if (r14.w == 0) break;
        r15.w = firstbitlow((uint)r14.w);
        r16.z = 1 << (int)r15.w;
        r16.w = (int)r14.w & (int)r16.z;
        if (r16.w != 0) {
          r14.w = (int)r14.w ^ (int)r16.z;
          r16.z = (int)r12.z + (int)r15.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r23.xyzw, r16.z, l(0), t11.xyzw
        r23.x = samp0[]..swiz;
        r23.y = samp0[]..swiz;
        r23.z = samp0[]..swiz;
        r23.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r24.xy, r16.z, l(16), t11.xyxx
        r24.x = samp0[]..swiz;
        r24.y = samp0[]..swiz;
          r23.xyz = -v6.xyz + r23.xyz;
          r24.z = r23.w;
          r23.xyz = cmp(abs(r23.xyz) < r24.zxy);
          r16.z = r23.y ? r23.x : 0;
          r16.z = r23.z ? r16.z : 0;
          if (r16.z != 0) {
            r15.w = (int)r8.w + (int)r15.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r23.xyzw, r15.w, l(0), t15.wxyz
          r23.x = samp0[]..swiz;
          r23.y = samp0[]..swiz;
          r23.z = samp0[]..swiz;
          r23.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r16.zw, r15.w, l(96), t15.xxxy
          r16.z = samp0[]..swiz;
          r16.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r15.w, l(116), t15.zwxy
          r24.x = samp0[]..swiz;
          r24.y = samp0[]..swiz;
          r24.z = samp0[]..swiz;
          r24.w = samp0[]..swiz;
            r25.xyz = v6.xyz + -r23.yzw;
            r17.w = (int)r24.w & 0x0000ffff;
            if (6 == 0) r18.w = 0; else if (6+25 < 32) {             r18.w = (uint)r24.w << (32-(6 + 25)); r18.w = (uint)r18.w >> (32-6);            } else r18.w = (uint)r24.w >> 25;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r26.xyzw, r17.w, l(0), t16.xyzw
          r26.x = samp0[]..swiz;
          r26.y = samp0[]..swiz;
          r26.z = samp0[]..swiz;
          r26.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r17.w, l(16), t16.xyzw
          r27.x = samp0[]..swiz;
          r27.y = samp0[]..swiz;
          r27.z = samp0[]..swiz;
          r27.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r17.w, l(32), t16.xyzw
          r28.x = samp0[]..swiz;
          r28.y = samp0[]..swiz;
          r28.z = samp0[]..swiz;
          r28.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r17.w, l(48), t16.xyzw
          r29.x = samp0[]..swiz;
          r29.y = samp0[]..swiz;
          r29.z = samp0[]..swiz;
          r29.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r17.w, l(64), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r17.w, l(80), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
            r23.w = dot(r26.xyz, r25.xyz);
            r23.w = saturate(r23.w + r26.w);
            r25.w = dot(r27.xyz, r25.xyz);
            r25.w = saturate(r25.w + r27.w);
            r23.w = r25.w * r23.w;
            r25.w = dot(r28.xyz, r25.xyz);
            r25.w = saturate(r25.w + r28.w);
            r23.w = r25.w * r23.w;
            r25.w = dot(r29.xyz, r25.xyz);
            r25.w = saturate(r25.w + r29.w);
            r23.w = r25.w * r23.w;
            r25.w = dot(r30.xyz, r25.xyz);
            r25.w = saturate(r25.w + r30.w);
            r23.w = r25.w * r23.w;
            r25.w = dot(r31.xyz, r25.xyz);
            r25.w = saturate(r25.w + r31.w);
            r14.x = r25.w * r23.w;
            r23.w = (int)r16.z & 1;
            r26.xy = r23.ww ? r14.xy : r14.zx;
            r14.x = r16.z;
            r27.xy = r26.xy;
            r23.w = 1;
            while (true) {
              r25.w = cmp((int)r23.w >= (int)r18.w);
              if (r25.w != 0) break;
              r25.w = (int)r17.w + (int)r23.w;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r28.xyzw, r25.w, l(0), t16.xyzw
            r28.x = samp0[]..swiz;
            r28.y = samp0[]..swiz;
            r28.z = samp0[]..swiz;
            r28.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r29.xyzw, r25.w, l(16), t16.xyzw
            r29.x = samp0[]..swiz;
            r29.y = samp0[]..swiz;
            r29.z = samp0[]..swiz;
            r29.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r25.w, l(32), t16.xyzw
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r25.w, l(48), t16.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r25.w, l(64), t16.xyzw
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r25.w, l(80), t16.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
              r25.w = dot(r28.xyz, r25.xyz);
              r25.w = saturate(r25.w + r28.w);
              r25.w = r27.x * r25.w;
              r26.z = dot(r29.xyz, r25.xyz);
              r26.z = saturate(r26.z + r29.w);
              r25.w = r26.z * r25.w;
              r26.z = dot(r30.xyz, r25.xyz);
              r26.z = saturate(r26.z + r30.w);
              r25.w = r26.z * r25.w;
              r26.z = dot(r31.xyz, r25.xyz);
              r26.z = saturate(r26.z + r31.w);
              r25.w = r26.z * r25.w;
              r26.z = dot(r32.xyz, r25.xyz);
              r26.z = saturate(r26.z + r32.w);
              r25.w = r26.z * r25.w;
              r26.z = dot(r33.xyz, r25.xyz);
              r26.z = saturate(r26.z + r33.w);
              r27.x = r26.z * r25.w;
              r26.w = (uint)r14.x >> 2;
              if (1 == 0) r27.z = 0; else if (1+2 < 32) {               r27.z = (uint)r14.x << (32-(1 + 2)); r27.z = (uint)r27.z >> (32-1);              } else r27.z = (uint)r14.x >> 2;
              r27.w = (int)r26.w & 2;
              r28.x = max(r27.y, r27.x);
              r25.w = -r25.w * r26.z + 1;
              r25.w = r27.y * r25.w;
              r16.y = r27.w ? r25.w : r28.x;
              r27.xy = r27.zz ? r27.xy : r16.xy;
              r23.w = (int)r23.w + 1;
              r14.x = r26.w;
            }
            r14.x = saturate(r27.y + -r12.w);
            r16.y = r14.x * r16.w;
            r16.z = cmp(0 < r16.y);
            if (r16.z != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r26.xyzw, r15.w, l(16), t15.xyzw
            r26.x = samp0[]..swiz;
            r26.y = samp0[]..swiz;
            r26.z = samp0[]..swiz;
            r26.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r28.xyzw, r15.w, l(32), t15.yxwz
            r28.x = samp0[]..swiz;
            r28.y = samp0[]..swiz;
            r28.z = samp0[]..swiz;
            r28.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r29.xyzw, r15.w, l(48), t15.xyzw
            r29.x = samp0[]..swiz;
            r29.y = samp0[]..swiz;
            r29.z = samp0[]..swiz;
            r29.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r15.w, l(64), t15.zwxy
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r15.w, l(80), t15.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r15.w, l(132), t15.zwxy
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r15.w, l(148), t15.zwxy
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r15.w, l(164), t15.zwxy
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r15.w, l(180), t15.zwxy
            r35.x = samp0[]..swiz;
            r35.y = samp0[]..swiz;
            r35.z = samp0[]..swiz;
            r35.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r15.w, l(196), t15.xyzw
            r36.x = samp0[]..swiz;
            r36.y = samp0[]..swiz;
            r36.z = samp0[]..swiz;
            r36.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xy, r15.w, l(212), t15.xyxx
            r37.x = samp0[]..swiz;
            r37.y = samp0[]..swiz;
              r22.w = r14.x * r16.w + r22.w;
              r14.x = r16.y * r24.z;
              r24.z = r32.z;
              r15.w = dot(r10.xyz, r24.xyz);
              r16.y = dot(r25.xyz, r24.xyz);
              r16.y = r16.y + -r32.w;
              r16.z = cmp(r16.y >= 0);
              r16.y = max(abs(r16.y), r6.w);
              r16.y = r16.z ? r16.y : -r16.y;
              r15.w = max(1.00000001e-07, -r15.w);
              r15.w = r16.y / r15.w;
              r15.w = min(131072, abs(r15.w));
              r32.z = r33.z;
              r16.y = dot(r10.xyz, r32.xyz);
              r16.z = dot(r25.xyz, r32.xyz);
              r16.z = r16.z + -r33.w;
              r16.w = cmp(r16.z >= 0);
              r16.z = max(abs(r16.z), r6.w);
              r16.z = r16.w ? r16.z : -r16.z;
              r16.y = max(1.00000001e-07, -r16.y);
              r16.y = r16.z / r16.y;
              r15.w = min(abs(r16.y), r15.w);
              r33.z = r34.z;
              r16.y = dot(r10.xyz, r33.xyz);
              r16.z = dot(r25.xyz, r33.xyz);
              r16.z = r16.z + -r34.w;
              r16.w = cmp(r16.z >= 0);
              r16.z = max(abs(r16.z), r6.w);
              r16.z = r16.w ? r16.z : -r16.z;
              r16.y = max(1.00000001e-07, -r16.y);
              r16.y = r16.z / r16.y;
              r15.w = min(abs(r16.y), r15.w);
              r34.z = r35.z;
              r16.y = dot(r10.xyz, r34.xyz);
              r16.z = dot(r25.xyz, r34.xyz);
              r16.z = r16.z + -r35.w;
              r16.w = cmp(r16.z >= 0);
              r16.z = max(abs(r16.z), r6.w);
              r16.z = r16.w ? r16.z : -r16.z;
              r16.y = max(1.00000001e-07, -r16.y);
              r16.y = r16.z / r16.y;
              r15.w = min(abs(r16.y), r15.w);
              r35.z = r36.x;
              r16.y = dot(r10.xyz, r35.xyz);
              r16.z = dot(r25.xyz, r35.xyz);
              r16.z = r16.z + -r36.y;
              r16.w = cmp(r16.z >= 0);
              r16.z = max(abs(r16.z), r6.w);
              r16.z = r16.w ? r16.z : -r16.z;
              r16.y = max(1.00000001e-07, -r16.y);
              r16.y = r16.z / r16.y;
              r15.w = min(abs(r16.y), r15.w);
              r37.zw = r36.zw;
              r16.y = dot(r10.zxy, r37.xzw);
              r16.z = dot(r25.zxy, r37.xzw);
              r16.z = r16.z + -r37.y;
              r16.w = cmp(r16.z >= 0);
              r16.z = max(abs(r16.z), r6.w);
              r16.z = r16.w ? r16.z : -r16.z;
              r16.y = max(1.00000001e-07, -r16.y);
              r16.y = r16.z / r16.y;
              r15.w = min(abs(r16.y), r15.w);
              r24.x = r29.w;
              r24.yz = r30.zw;
              r16.yzw = r24.xyz + r25.xyz;
              r16.yzw = r10.xyz * r15.www + r16.yzw;
              r17.w = dot(r16.yzw, r16.yzw);
              r17.w = sqrt(r17.w);
              r15.w = r15.w / r17.w;
              r15.w = r15.w + r15.w;
              r15.w = sqrt(r15.w);
              r15.w = r6.y * 5 + r15.w;
              r15.w = -0.844799995 + r15.w;
              r23.y = r26.z;
              r23.z = r28.x;
              r32.x = dot(r16.yzw, r23.xyz);
              r24.xy = r26.xw;
              r24.z = r28.w;
              r32.y = dot(r16.yzw, r24.xyz);
              r28.x = r26.y;
              r32.z = dot(r16.yzw, r28.xyz);
              if (9 == 0) r16.y = 0; else if (9+16 < 32) {               r16.y = (uint)r24.w << (32-(9 + 16)); r16.y = (uint)r16.y >> (32-9);              } else r16.y = (uint)r24.w >> 16;
              r32.w = (uint)r16.y;
              r16.yzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r32.xyzw, r15.w).xyz;
              r26.x = dot(r25.xyz, r23.xyz);
              r26.y = dot(r25.xyz, r24.xyz);
              r26.z = dot(r25.xyz, r28.xyz);
              r25.xyz = saturate(r26.xyz * r29.xyz + float3(0.5,0.5,0.5));
              r30.z = r31.x;
              r25.xyz = r25.xyz * r30.xyz + r31.yzw;
              r32.x = dot(r3.xyz, r23.xyz);
              r32.y = dot(r3.xyz, r24.xyz);
              r32.z = dot(r3.xyz, r28.xyz);
              r23.xyz = cmp(float3(0,0,0) < r32.xyz);
              r15.z = r23.x ? 0 : 0.5;
              r24.xyz = r25.xyz + r15.xyz;
              r24.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r24.xyz, 0).xyz;
              r26.xyz = r32.xyz * r32.xyz;
              r26.xyz = r26.xyz * r14.xxx;
              r17.z = r23.y ? 0 : 0.5;
              r23.xyw = r25.xyz + r17.xyz;
              r23.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r23.xyw, 0).xyz;
              r23.xyw = r23.xyw * r26.yyy;
              r23.xyw = r24.xyz * r26.xxx + r23.xyw;
              r18.z = r23.z ? 0 : 0.5;
              r24.xyz = r25.xyz + r18.xyz;
              r24.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r24.xyz, 0).xyz;
              r23.xyz = r24.xyz * r26.zzz + r23.xyw;
              r24.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r32.xyzw, 6).xyz;
              r25.xyz = r24.xyz * r23.xyz;
              r15.z = dot(r25.xyz, float3(0.212599993,0.715200007,0.0722000003));
              r13.xyz = r23.xyz * r24.xyz + r13.xyz;
              r15.w = r15.z * r7.z;
              r16.yzw = r16.yzw * r14.xxx;
              r14.x = dot(r16.yzw, float3(0.212599993,0.715200007,0.0722000003));
              r14.x = r7.z * r15.z + r14.x;
              r14.x = r15.w / r14.x;
              r22.xyz = r16.yzw * r14.xxx + r22.xyz;
            }
          }
        }
      }
      r21.xyzw = r22.xyzw;
      r9.yzw = r13.xyz;
      r8.w = (int)r8.w + 32;
    }
    r20.xyzw = r21.zxyw;
    r19.xyz = r9.yzw;
    r20.xyzw = r20.yzxw;
  } else {
    r20.x = r19.w;
  }
  r2.x = max(1, r20.w);
  r2.x = rcp(r2.x);
  r20.w = saturate(r20.w);
  r12.xyz = r19.yzx * r2.xxx;
  r9.yzw = r20.xyz * r2.xxx;
  r2.z = cmp(r20.w < 0.99000001);
  if (r2.z != 0) {
    r2.z = 1 + -r20.w;
    r2.w = sunConstants.globalProbeExposure * r2.z;
    r13.xyz = -globalProbeConstants.data[0].xyz + v6.xyz;
    r14.x = globalProbeConstants.data[0].w * r13.x;
    r14.yz = globalProbeConstants.data[1].xy * r13.yz;
    r13.xyz = saturate(float3(0.5,0.5,0.5) + r14.xyz);
    r14.xy = globalProbeConstants.data[1].zw * r13.xy;
    r14.z = globalProbeConstants.data[2].x * r13.z;
    r13.xyz = globalProbeConstants.data[2].yzw + r14.xyz;
    r14.xyz = cmp(float3(0,0,0) < r3.xyz);
    r14.xyz = r14.xyz ? float3(0,0,0) : float3(0.5,0.5,0.5);
    r14.w = 0;
    r15.xyz = r14.wwx + r13.xyz;
    r15.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r15.xyz, 0).xyz;
    r16.xyz = r3.xyz * r3.xyz;
    r16.xyz = r16.xyz * r2.www;
    r17.xyz = r14.wwy + r13.xyz;
    r17.xyz = gProbeYArray.SampleLevel(samplerLinearClamp_s, r17.xyz, 0).xyz;
    r17.xyz = r17.xyz * r16.yyy;
    r15.xyz = r15.xyz * r16.xxx + r17.xyz;
    r13.xyz = r14.wwz + r13.xyz;
    r13.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r13.xyz, 0).xyz;
    r13.xyz = r13.xyz * r16.zzz + r15.xyz;
    r3.w = 0;
    r14.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r3.xyzw, 6).xyz;
    r13.xyz = r14.xyz * r13.xyz;
    r2.w = dot(r13.xyz, float3(0.212599993,0.715200007,0.0722000003));
    r12.xyz = r19.yzx * r2.xxx + r13.yzx;
    r10.w = 0;
    r13.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r10.xyzw, r5.w).xyz;
    r2.x = sunConstants.globalProbeExposure * r2.z + -r2.w;
    r2.x = r1.y * r2.x + r2.w;
    r9.yzw = r13.xyz * r2.xxx + r9.yzw;
  }
  r2.xz = r1.xy * float2(0.984375,0.984375) + float2(0.0078125,0.0078125);
  r2.xz = gEnvBRDFGlass.SampleLevel(samplerLinear_s, r2.xz, 0).xy;
  r13.xyz = r9.yzw * r2.xxx;
  r14.xyz = r9.yzw * r2.zzz;
  r1.y = sqrt(r4.w);
  r1.y = r1.y * 0.5 + 0.5;
  r1.y = r1.y * r1.y;
  r2.x = 0.5 * r1.y;
  r1.y = -r1.y * 0.5 + 1;
  r2.z = r1.x * r1.y + r2.x;
  r1.y = r2.z * r1.y;
  r2.x = r2.x * r2.z;
  r2.z = dot(r3.xyz, sunConstants.wldDir.xyz);
  r2.w = cmp(0 >= abs(r2.z));
  if (r2.w != 0) {
    r3.w = 0;
  }
  if (r2.w == 0) {
    r9.yzw = sunConstants.sstLightingConstants.offToPinTransform._m10_m11_m12 * v6.yyy;
    r9.yzw = v6.xxx * sunConstants.sstLightingConstants.offToPinTransform._m00_m01_m02 + r9.yzw;
    r9.yzw = v6.zzz * sunConstants.sstLightingConstants.offToPinTransform._m20_m21_m22 + r9.yzw;
    r9.yzw = sunConstants.sstLightingConstants.offToPinTransform._m30_m31_m32 + r9.yzw;
    r2.w = -sunConstants.splitDepthOffset + r9.w;
    r2.w = -r2.w * 6.10351563e-05 + 1;
    r5.w = saturate(r2.w);
    r5.w = cmp(r2.w == r5.w);
    if (r5.w != 0) {
      r5.w = 0;
      r6.y = 0;
      while (true) {
        r6.w = cmp(r5.w >= 3);
        if (r6.w != 0) break;
        r6.w = (uint)r5.w;
        r7.zw = -sunConstants.splitPinTransform[r6.w].xy + r9.yz;
        r7.z = max(abs(r7.z), abs(r7.w));
        r6.y = sunConstants.splitPinTransform[r6.w].z * r7.z;
        r6.w = cmp(r6.y < 1);
        if (r6.w != 0) {
          break;
        }
        r5.w = 1 + r5.w;
        r6.y = 0;
      }
    } else {
      r5.w = 3;
      r6.y = 0;
    }
    r6.w = cmp(r5.w >= 3);
    if (r6.w != 0) {
      r15.xyz = sunConstants.sstLightingConstants.constants.inchesPerTexel * float3(128,0.5,0.5);
      r16.xz = rcp(r15.xx);
      r16.y = -r16.z;
      r15.xyz = r9.yzy * r16.xyz + r15.yzy;
      r16.xyz = sunConstants.sstLightingConstants.constants.dimensionInTiles.xyx + float3(-0.0078125,-0.0078125,-0.0078125);
      r15.xyz = max(float3(0,0,0), r15.xyz);
      r15.xyz = min(r15.xyz, r16.xyz);
      r7.zw = sunConstants.sstLightingConstants.coordScale * r15.zy;
      r7.zw = floor(r7.zw);
      r7.w = sunConstants.sstLightingConstants.constants.dimensionInTiles.x * r7.w;
      r7.z = r7.w * sunConstants.sstLightingConstants.coordScale + r7.z;
      r7.z = (uint)r7.z;
      r7.z = (int)r7.z + (int10)sunConstants.sstLightingConstants.rootOffset;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r7.w, r7.z, l(0), t23.xxxx
    r7.w = samp0[]..swiz;
      r8.w = (int)r7.w & 0x40000000;
      r10.w = (uint)r7.w << 2;
      if (r8.w == 0) {
        r8.w = (int)r7.w & 0x01ffffff;
        r16.x = (int)r7.z + (int)r8.w;
        r7.z = (uint)r7.w >> 25;
        r7.z = (uint)r7.z;
        r15.xyz = r15.xyz * r7.zzz;
        r15.xyz = frac(r15.xyz);
        r15.xyz = float3(128,128,128) * r15.xyz;
        r15.xyz = (uint3)r15.xyz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r16.z, r16.x, l(0), t23.xxxx
      r16.z = samp0[]..swiz;
        r7.zw = (uint2)r15.zy >> int2(6,6);
        r8.w = (int)r16.z & 0xc0000000;
        r11.w = (int)r16.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r11.w, r11.w, l(0), t23.xxxx
      r11.w = samp0[]..swiz;
        r7.w = r7.w ? r16.z : r11.w;
        r11.w = (uint)r7.w >> 13;
        r7.z = r7.z ? r11.w : r7.w;
        r7.z = (int)r7.z & 8191;
        r17.x = (int)r7.z + (int)r16.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.z, r17.x, l(0), t23.xxxx
      r17.z = samp0[]..swiz;
        r16.y = 0;
        r17.y = 1;
        r17.xyz = r8.www ? r16.xyz : r17.xyz;
        r18.yz = r8.ww ? float2(8.40779079e-45,1.40129846e-45) : float2(7.00649232e-45,2.80259693e-45);
        r7.zw = (uint2)r15.zy >> (uint2)r18.yy;
        r7.zw = (int2)r7.zw & int2(1,1);
        r11.w = (int)r17.z & 0xc0000000;
        r13.w = (int)r17.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r13.w, r13.w, l(0), t23.xxxx
      r13.w = samp0[]..swiz;
        r7.w = r7.w ? r17.z : r13.w;
        r13.w = (uint)r7.w >> 13;
        r7.z = r7.z ? r13.w : r7.w;
        r7.z = (int)r7.z & 8191;
        r18.x = (int)r7.z + (int)r17.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.w, r18.x, l(0), t23.xxxx
      r18.w = samp0[]..swiz;
        r17.xyz = r11.www ? r17.xyz : r18.xzw;
        r16.xyz = r8.www ? r16.xyz : r17.xyz;
        r7.z = (int)r16.z & 0xc0000000;
        if (r7.z == 0) {
          r7.z = (int)-r16.y + 6;
          r7.zw = (uint2)r15.zy >> (uint2)r7.zz;
          r8.w = (int)r16.z | 0x40000000;
          bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r7.w = (((uint)r7.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
          bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r7.z = (((uint)r7.z << 0) & bitmask.z) | ((uint)r7.w & ~bitmask.z);
          r7.z = (int)r7.z * 10;
          r7.z = (uint)r8.w >> (uint)r7.z;
          r7.z = (int)r7.z & 1023;
          r17.x = (int)r7.z + (int)r16.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.w, r17.x, l(0), t23.xxxx
        r17.w = samp0[]..swiz;
          r17.yz = (int2)r16.yy + int2(1,2);
          r7.z = (int)-r17.y + 6;
          r7.zw = (uint2)r15.zy >> (uint2)r7.zz;
          r8.w = (int)r17.w & 0xc0000000;
          r11.w = (int)r17.w | 0x40000000;
          bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r7.w = (((uint)r7.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
          bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r7.z = (((uint)r7.z << 0) & bitmask.z) | ((uint)r7.w & ~bitmask.z);
          r7.z = (int)r7.z * 10;
          r7.z = (uint)r11.w >> (uint)r7.z;
          r7.z = (int)r7.z & 1023;
          r18.x = (int)r7.z + (int)r17.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.z, r18.x, l(0), t23.xxxx
        r18.z = samp0[]..swiz;
          r18.y = r17.z;
          r18.xyz = r8.www ? r17.xyw : r18.xyz;
          r7.z = (int)-r18.y + 6;
          r7.zw = (uint2)r15.zy >> (uint2)r7.zz;
          r11.w = (int)r18.z & 0xc0000000;
          r13.w = (int)r18.z | 0x40000000;
          bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r7.w = (((uint)r7.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
          bitmask.z = ((~(-1 << 1)) << 0) & 0xffffffff;  r7.z = (((uint)r7.z << 0) & bitmask.z) | ((uint)r7.w & ~bitmask.z);
          r7.z = (int)r7.z * 10;
          r7.z = (uint)r13.w >> (uint)r7.z;
          r7.z = (int)r7.z & 1023;
          r19.x = (int)r7.z + (int)r18.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r19.y, r19.x, l(0), t23.xxxx
        r19.y = samp0[]..swiz;
          r7.zw = r11.ww ? r18.xz : r19.xy;
          r16.xz = r8.ww ? r17.xw : r7.zw;
        }
        r7.z = (int)r16.z & 0xc0000000;
        if (r7.z == 0) {
          if (14 == 0) r7.w = 0; else if (14+15 < 32) {           r7.w = (uint)r16.z << (32-(14 + 15)); r7.w = (uint)r7.w >> (32-14);          } else r7.w = (uint)r16.z >> 15;
          r7.w = (uint)r7.w;
          r7.w = sunConstants.sstLightingConstants.constants.spanInInches * r7.w;
          r7.w = 6.10388815e-05 * r7.w;
          r16.yw = (int2)r16.zz & int2(32767,536870912);
          r8.w = (uint)r16.y;
          r8.w = sunConstants.sstLightingConstants.constants.spanInInches * r8.w;
          r8.w = 3.05185094e-05 * r8.w;
          r11.w = (int)r15.y & 3;
          r11.w = (int)r11.w + (int)r16.x;
          r11.w = (int)r11.w + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r11.w, r11.w, l(0), t23.xxxx
        r11.w = samp0[]..swiz;
          bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r15.x = (((uint)r15.x << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
          bitmask.y = ((~(-1 << 2)) << 1) & 0xffffffff;  r15.y = (((uint)r15.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.w = ((~(-1 << 1)) << 4) & 0xffffffff;  r15.w = (((uint)r15.z << 4) & bitmask.w) | ((uint)0 & ~bitmask.w);
          r11.w = (uint)r11.w >> (uint)r15.x;
          r11.w = (int)r11.w & 255;
          r11.w = (uint)r11.w;
          r11.w = r11.w * r8.w;
          r11.w = r11.w * 0.00392156886 + r7.w;
          r13.w = (int)r15.y + 1;
          if (1 == 0) r15.x = 0; else if (1+1 < 32) {           r15.x = (uint)r15.z << (32-(1 + 1)); r15.x = (uint)r15.x >> (32-1);          } else r15.x = (uint)r15.z >> 1;
          r13.w = (int)r13.w + (int)r15.x;
          r13.w = (int)r13.w + (int)r16.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r13.w, r13.w, l(0), t23.xxxx
        r13.w = samp0[]..swiz;
          r13.w = (uint)r13.w >> (uint)r15.w;
          r13.w = (int)r13.w & 0x0000ffff;
          r13.w = (uint)r13.w;
          r8.w = r13.w * r8.w;
          r7.w = r8.w * 1.52590219e-05 + r7.w;
          r10.w = r16.w ? r11.w : r7.w;
        } else {
          r7.w = (int)r16.z & 0x80000000;
          r8.w = (int)r16.x + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r8.w, r8.w, l(0), t23.xxxx
        r8.w = samp0[]..swiz;
          r7.w = r7.w ? r8.w : 0;
          r8.w = (uint)r16.z << 2;
          r11.w = (uint)r7.w >> 16;
          r11.w = f16tof32(r11.w);
          r7.w = (int)r7.w & 0x0000ffff;
          r7.w = f16tof32(r7.w);
          r8.w = r9.y * r11.w + r8.w;
          r7.w = r9.z * r7.w + r8.w;
          r10.w = r7.z ? r7.w : r10.w;
        }
      }
      r7.z = -sunConstants.sstLightingConstants.constants.inchesPerTexel * 2 + r9.w;
      r7.z = cmp(r10.w < r7.z);
      r3.w = r7.z ? 0 : 1;
    }
    if (r6.w == 0) {
      if (enableDitheredShadow == 0) {
        r6.w = (uint)r5.w;
        r7.zw = -sunConstants.splitPinTransform[r6.w].xy + r9.yz;
        r7.zw = sunConstants.splitPinTransform[r6.w].zz * r7.zw;
        r15.xy = r7.zw * float2(0.5,-0.5) + float2(0.5,0.5);
        r6.w = (int16)sunConstants.splitArrayOffset;
        r15.z = r6.w + r5.w;
        r6.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r2.w).x;
        r7.z = gTransShadowmapArray.SampleLevel(samp0_s, r15.xyz, 0).x;
        r6.w = r7.z + r6.w;
        r6.w = saturate(-1 + r6.w);
        r7.z = r6.w * r6.w;
        r3.w = r7.z * r6.w;
      }
      if (enableDitheredShadow != 0) {
        r15.x = -r7.x;
        r6.w = (uint)r5.w;
        r5.w = 1 + r5.w;
        r5.w = min(2, r5.w);
        r5.w = (uint)r5.w;
        r6.y = 1 + -r6.y;
        r6.y = 28 * r6.y;
        r6.y = (uint)r6.y;
        r7.zw = -sunConstants.splitPinTransform[r6.w].xy + r9.yz;
        r7.zw = sunConstants.splitPinTransform[r6.w].zz * r7.zw;
        r7.zw = r7.zw * float2(0.5,-0.5) + float2(0.5,0.5);
        r9.yz = -sunConstants.splitPinTransform[r5.w].xy + r9.yz;
        r9.yz = sunConstants.splitPinTransform[r5.w].zz * r9.yz;
        r9.yz = r9.yz * float2(0.5,-0.5) + float2(0.5,0.5);
        r15.y = r8.x;
        r15.z = r7.x;
        r8.w = 0;
        r9.w = 0;
        while (true) {
          r10.w = cmp((uint)r9.w >= 8);
          if (r10.w != 0) break;
          r10.w = cmp((uint)r6.y < (uint)r9.w);
          r16.xy = r10.ww ? r9.yz : r7.zw;
          r11.w = r10.w ? sunConstants.splitPinTransform[r5.w].w : sunConstants.splitPinTransform[r6.w].w;
          r10.w = r10.w ? r5.w : r6.w;
          r17.x = dot(icb[r9.w+0].yx, r15.xy);
          r17.y = dot(icb[r9.w+0].yx, r15.yz);
          r16.xy = r17.xy * r11.ww + r16.xy;
          r10.w = (int)r10.w + (int16)sunConstants.splitArrayOffset;
          r16.z = (uint)r10.w;
          r10.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r16.xyz, r2.w).x;
          r11.w = gTransShadowmapArray.SampleLevel(samp0_s, r16.xyz, 0).x;
          r10.w = r11.w + r10.w;
          r10.w = saturate(-1 + r10.w);
          r8.w = r10.w * 0.125 + r8.w;
          r9.w = (int)r9.w + 1;
        }
        r2.w = r8.w * r8.w;
        r3.w = r2.w * r8.w;
      }
    }
  }
  r2.w = cmp(0 < abs(r2.z));
  if (r2.w != 0) {
    r2.w = cmp(0 < r3.w);
    if (r2.w != 0) {
      r2.w = sunConstants.specScale * r2.y;
      r2.w = 0.25 * r2.w;
      if (sunConstants.sunCookieIndex != 0) {
        r15.xyz = eyeOffset.xyz + v6.xyz;
        r15.w = 1;
        r5.w = dot(sunConstants.sunCookieTransform[0].xyzw, r15.xyzw);
        r6.y = dot(sunConstants.sunCookieTransform[1].xyzw, r15.xyzw);
        r15.x = frac(r5.w);
        r15.y = frac(r6.y);
        r5.w = -1 + (int14)sunConstants.sunCookieIndex;
        r15.z = (uint)r5.w;
        r9.yzw = gCookieArray.SampleLevel(samplerLinear_s, r15.xyz, 0).xyz;
        r9.yzw = float3(-1,-1,-1) + r9.yzw;
        r9.yzw = sunConstants.sunCookieIntensity * r9.yzw + float3(1,1,1);
        r9.yzw = sunConstants.color.xyz * r9.yzw;
      } else {
        r9.yzw = sunConstants.color.xyz;
      }
      r5.w = viewmodelSunShadowRaw >> 16;
      r6.y = cmp(0 < (uint)r5.w);
      r6.y = r1.w ? r6.y : 0;
      if (r6.y != 0) {
        r5.w = (int)r5.w + numLights;
        r5.w = (int)r5.w + -1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r6.y, r5.w, l(52), t12.xxxx
      r6.y = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r5.w, l(68), t12.xyzw
      r15.x = samp0[]..swiz;
      r15.y = samp0[]..swiz;
      r15.z = samp0[]..swiz;
      r15.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r16.xyzw, r5.w, l(84), t12.xyzw
      r16.x = samp0[]..swiz;
      r16.y = samp0[]..swiz;
      r16.z = samp0[]..swiz;
      r16.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r17.xyzw, r5.w, l(132), t12.xyzw
      r17.x = samp0[]..swiz;
      r17.y = samp0[]..swiz;
      r17.z = samp0[]..swiz;
      r17.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r5.w, l(148), t12.xyzw
      r18.x = samp0[]..swiz;
      r18.y = samp0[]..swiz;
      r18.z = samp0[]..swiz;
      r18.w = samp0[]..swiz;
        r6.w = abs(r2.z) * -0.200000003 + 0.400000006;
        r7.z = cmp(r2.z < 0);
        r6.w = r7.z ? -r6.w : r6.w;
        r19.xyz = r3.xyz * r6.www + v6.xyz;
        r19.w = 1;
        r15.x = dot(r15.xyzw, r19.xyzw);
        r15.y = dot(r16.xyzw, r19.xyzw);
        r7.zw = r15.xy * float2(0.5,-0.5) + float2(0.5,0.5);
        r7.zw = r7.zw + r18.zw;
        r7.zw = r7.zw * r18.xy;
        r15.xy = r6.yy / r17.xz;
        r15.zw = float2(1,1) + -r15.xy;
        r15.zw = cmp(r7.zw >= r15.zw);
        r15.xy = cmp(r15.xy >= r7.zw);
        r15.xy = (int2)r15.xy | (int2)r15.zw;
        r6.w = (int)r15.y | (int)r15.x;
        if (r6.w == 0) {
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r6.w, r5.w, l(28), t12.xxxx
        r6.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r5.w, l(100), t12.xyzw
        r15.x = samp0[]..swiz;
        r15.y = samp0[]..swiz;
        r15.z = samp0[]..swiz;
        r15.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r16.xy, r5.w, l(164), t12.xyxx
        r16.x = samp0[]..swiz;
        r16.y = samp0[]..swiz;
          r5.w = dot(r15.xyzw, r19.xyzw);
          r7.zw = saturate(r7.zw);
          r15.xy = r7.zw * r17.xz + r17.yw;
          r5.w = r5.w + r16.x;
          r5.w = r5.w / r16.y;
          r5.w = max(6.10351563e-05, r5.w);
          r6.w = (int)r6.w & 0x0000ffff;
          if (enableDitheredShadow != 0) {
            r16.x = -r7.x;
            r17.z = (uint)r6.w;
            r16.y = r8.x;
            r16.z = r7.x;
            r7.zw = float2(0,0);
            while (true) {
              r8.w = cmp((int)r7.w >= 8);
              if (r8.w != 0) break;
              r18.x = dot(icb[r7.w+0].yx, r16.xy);
              r18.y = dot(icb[r7.w+0].yx, r16.yz);
              r17.xy = r18.xy * r6.yy + r15.xy;
              r8.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r17.xyz, r5.w).x;
              r7.z = r8.w * 0.125 + r7.z;
              r7.w = (int)r7.w + 1;
            }
          } else {
            r15.z = (uint)r6.w;
            r7.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r5.w).x;
          }
          r5.w = r7.z * r7.z;
          r5.w = r5.w * r7.z;
        } else {
          r5.w = 1;
        }
        r3.w = r5.w * r3.w;
      } else {
        r5.w = viewmodelSunShadowRaw & 0x0000ffff;
        r6.y = cmp(0 < (uint)r5.w);
        r6.w = ~(int)r1.w;
        r6.y = r6.y ? r6.w : 0;
        if (r6.y != 0) {
          r5.w = (int)r5.w + numLights;
          r5.w = (int)r5.w + -1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r6.y, r5.w, l(52), t12.xxxx
        r6.y = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r5.w, l(68), t12.xyzw
        r15.x = samp0[]..swiz;
        r15.y = samp0[]..swiz;
        r15.z = samp0[]..swiz;
        r15.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r16.xyzw, r5.w, l(84), t12.xyzw
        r16.x = samp0[]..swiz;
        r16.y = samp0[]..swiz;
        r16.z = samp0[]..swiz;
        r16.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r17.xyzw, r5.w, l(132), t12.xyzw
        r17.x = samp0[]..swiz;
        r17.y = samp0[]..swiz;
        r17.z = samp0[]..swiz;
        r17.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r18.xyzw, r5.w, l(148), t12.xyzw
        r18.x = samp0[]..swiz;
        r18.y = samp0[]..swiz;
        r18.z = samp0[]..swiz;
        r18.w = samp0[]..swiz;
          r6.w = abs(r2.z) * -0.200000003 + 0.400000006;
          r7.z = cmp(r2.z < 0);
          r6.w = r7.z ? -r6.w : r6.w;
          r19.xyz = r3.xyz * r6.www + v6.xyz;
          r19.w = 1;
          r15.x = dot(r15.xyzw, r19.xyzw);
          r15.y = dot(r16.xyzw, r19.xyzw);
          r7.zw = r15.xy * float2(0.5,-0.5) + float2(0.5,0.5);
          r7.zw = r7.zw + r18.zw;
          r7.zw = r7.zw * r18.xy;
          r15.xy = r6.yy / r17.xz;
          r15.zw = float2(1,1) + -r15.xy;
          r15.zw = cmp(r7.zw >= r15.zw);
          r15.xy = cmp(r15.xy >= r7.zw);
          r15.xy = (int2)r15.xy | (int2)r15.zw;
          r6.w = (int)r15.y | (int)r15.x;
          if (r6.w == 0) {
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r6.w, r5.w, l(28), t12.xxxx
          r6.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r5.w, l(100), t12.xyzw
          r15.x = samp0[]..swiz;
          r15.y = samp0[]..swiz;
          r15.z = samp0[]..swiz;
          r15.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r16.xy, r5.w, l(164), t12.xyxx
          r16.x = samp0[]..swiz;
          r16.y = samp0[]..swiz;
            r5.w = dot(r15.xyzw, r19.xyzw);
            r7.zw = saturate(r7.zw);
            r15.xy = r7.zw * r17.xz + r17.yw;
            r5.w = r5.w + r16.x;
            r5.w = r5.w / r16.y;
            r5.w = max(6.10351563e-05, r5.w);
            r6.w = (int)r6.w & 0x0000ffff;
            if (enableDitheredShadow != 0) {
              r16.x = -r7.x;
              r17.z = (uint)r6.w;
              r16.y = r8.x;
              r16.z = r7.x;
              r7.zw = float2(0,0);
              while (true) {
                r8.w = cmp((int)r7.w >= 8);
                if (r8.w != 0) break;
                r18.x = dot(icb[r7.w+0].yx, r16.xy);
                r18.y = dot(icb[r7.w+0].yx, r16.yz);
                r17.xy = r18.xy * r6.yy + r15.xy;
                r8.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r17.xyz, r5.w).x;
                r7.z = r8.w * 0.125 + r7.z;
                r7.w = (int)r7.w + 1;
              }
            } else {
              r15.z = (uint)r6.w;
              r7.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r5.w).x;
            }
            r5.w = r7.z * r7.z;
            r5.w = r5.w * r7.z;
          } else {
            r5.w = 1;
          }
          r3.w = r5.w * r3.w;
        }
      }
      r5.w = -r1.x * 0.5 + 1;
      r5.w = -abs(r2.z) * r5.w + 1;
      r5.w = r5.w * r5.w;
      r5.w = -r5.w * 0.620000005 + 0.620000005;
      r5.w = r5.w + -abs(r2.z);
      r5.w = r4.w * r5.w + abs(r2.z);
      r5.w = r5.w * r3.w;
      r6.y = cmp(0 < r2.z);
      r15.xyz = r5.www * r9.zwy + r12.xyz;
      r2.z = saturate(r2.z);
      r16.xyz = -v6.xyz * r1.zzz + sunConstants.wldDir.xyz;
      r1.z = dot(r16.xyz, r16.xyz);
      r1.z = rsqrt(r1.z);
      r16.xyz = r16.xyz * r1.zzz;
      r1.z = dot(r16.xyz, r5.xyz);
      r5.w = dot(r3.xyz, r16.xyz);
      r6.w = abs(r5.w) * r2.y + -abs(r5.w);
      r5.w = abs(r5.w) * r6.w + 1;
      r6.w = r2.z * r1.y + r2.x;
      r5.w = r5.w * r5.w;
      r5.w = r5.w * r6.w;
      r5.w = rcp(r5.w);
      r2.z = r2.z * r2.w;
      r2.z = r5.w * r2.z;
      r2.z = r2.z * r3.w;
      r16.yzw = r2.zzz * r9.yzw + r14.xyz;
      r1.z = saturate(1 + -r1.z);
      r1.z = log2(r1.z);
      r1.z = 3.4000001 * r1.z;
      r1.z = exp2(r1.z);
      r1.z = r2.z * r1.z;
      r9.yzw = r1.zzz * r9.yzw + r13.xyz;
      r15.w = r12.z;
      r12.w = r15.z;
      r17.xyzw = r6.yyyy ? r15.zxyw : r12.zxyw;
      r12.zw = r9.yz;
      r15.zw = r13.xy;
      r15.xyzw = r6.yyyy ? r12.xyzw : r15.xyzw;
      r16.x = r9.w;
      r14.w = r13.z;
      r14.xyzw = r6.yyyy ? r16.yzwx : r14.xyzw;
      r12.z = r17.w;
      r12.xy = r15.xy;
      r13.xy = r15.zw;
      r13.z = r14.w;
    } else {
      r17.xyz = r12.zxy;
    }
  } else {
    r17.xyz = r12.zxy;
  }
  r15.x = -r7.x;
  r1.z = ~(int)r1.w;
  r1.x = -r1.x * 0.5 + 1;
  r16.xy = float2(0,0);
  r18.w = 0;
  r19.xyz = v6.xyz;
  r19.w = 1;
  r20.w = 1;
  r15.z = r7.x;
  r21.w = 1;
  r22.z = 1;
  r23.w = 1;
  r15.yw = r8.xx;
  r7.y = r15.w;
  r24.w = 1;
  r8.y = r15.x;
  r8.z = r7.x;
  r25.w = 1;
  r26.x = r8.x;
  r26.y = r15.x;
  r26.z = r7.x;
  r27.x = r8.x;
  r27.y = r15.x;
  r27.z = r7.x;
  r28.x = r8.x;
  r28.y = r15.x;
  r28.z = r7.x;
  r9.yzw = r17.xyz;
  r29.xyz = r12.zxy;
  r30.xyz = r13.xyz;
  r31.xyz = r14.xyz;
  r2.z = enableDitheredShadow;
  r2.w = 0;
  while (true) {
    r3.w = cmp((uint)r2.w >= numLights);
    if (r3.w != 0) break;
    r16.z = (uint)r2.w >> 5;
    r18.xyz = (int3)r11.xyz + (int3)r16.xyz;
    r3.w = visibleLights.Load(r18.xyzw).x;
    r18.xyz = r9.yzw;
    r32.xyz = r29.xyz;
    r33.xyz = r30.xyz;
    r34.xyz = r31.xyz;
    r5.w = r2.z;
    r6.y = r3.w;
    while (true) {
      if (r6.y == 0) break;
      r6.w = firstbitlow((uint)r6.y);
      r7.z = 1 << (int)r6.w;
      r7.w = (int)r6.y & (int)r7.z;
      if (r7.w != 0) {
        r6.y = (int)r6.y ^ (int)r7.z;
        r6.w = (int)r2.w + (int)r6.w;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r35.xyzw, r6.w, l(0), t11.xyzw
      r35.x = samp0[]..swiz;
      r35.y = samp0[]..swiz;
      r35.z = samp0[]..swiz;
      r35.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r36.xy, r6.w, l(16), t11.xyxx
      r36.x = samp0[]..swiz;
      r36.y = samp0[]..swiz;
        r35.xyz = -v6.xyz + r35.xyz;
        r36.z = r35.w;
        r35.xyz = cmp(abs(r35.xyz) < r36.zxy);
        r7.z = r35.y ? r35.x : 0;
        r7.z = r35.z ? r7.z : 0;
        if (r7.z != 0) {
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r6.w, l(0), t12.wxyz
        r35.x = samp0[]..swiz;
        r35.y = samp0[]..swiz;
        r35.z = samp0[]..swiz;
        r35.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.xyzw, r6.w, l(16), t12.zxyw
        r36.x = samp0[]..swiz;
        r36.y = samp0[]..swiz;
        r36.z = samp0[]..swiz;
        r36.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyzw, r6.w, l(32), t12.xyzw
        r37.x = samp0[]..swiz;
        r37.y = samp0[]..swiz;
        r37.z = samp0[]..swiz;
        r37.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r38.xyzw, r6.w, l(48), t12.xyzw
        r38.x = samp0[]..swiz;
        r38.y = samp0[]..swiz;
        r38.z = samp0[]..swiz;
        r38.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r39.xyzw, r6.w, l(64), t12.yzwx
        r39.x = samp0[]..swiz;
        r39.y = samp0[]..swiz;
        r39.z = samp0[]..swiz;
        r39.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r6.w, l(80), t12.xyzw
        r40.x = samp0[]..swiz;
        r40.y = samp0[]..swiz;
        r40.z = samp0[]..swiz;
        r40.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r6.w, l(96), t12.yzwx
        r41.x = samp0[]..swiz;
        r41.y = samp0[]..swiz;
        r41.z = samp0[]..swiz;
        r41.w = samp0[]..swiz;
          if (3 == 0) r7.z = 0; else if (3+24 < 32) {           r7.z = (uint)r36.w << (32-(3 + 24)); r7.z = (uint)r7.z >> (32-3);          } else r7.z = (uint)r36.w >> 24;
          switch (r7.z) {
            case 4 :            r7.z = cmp(0 < r41.x);
            r42.xy = r40.zw;
            r42.z = r41.w;
            r43.xyz = -r42.xyz * float3(0.5,0.5,0.5) + r35.yzw;
            r44.xyz = -v6.xyz + r43.xyz;
            r7.w = dot(r42.xyz, r44.xyz);
            r8.w = saturate(-r7.w / r41.x);
            r45.xyz = r8.www * r42.xyz + r43.xyz;
            r45.xyz = r7.zzz ? r45.xyz : r35.yzw;
            r45.xyz = -v6.xyz + r45.xyz;
            r10.w = dot(r45.xyz, r45.xyz);
            r11.w = rsqrt(r10.w);
            r45.xyz = r45.xyz * r11.www;
            r11.w = dot(r3.xyz, r45.xyz);
            r12.w = cmp(0 < abs(r11.w));
            if (r12.w != 0) {
              r12.w = sqrt(r10.w);
              r13.w = r38.x * r38.x;
              r10.w = r13.w / r10.w;
              r10.w = min(1, r10.w);
              r16.zw = saturate(r12.ww * r37.xz + r37.yw);
              r45.xy = r16.zw * r16.zw;
              r16.zw = r16.zw * float2(-2,-2) + float2(3,3);
              r16.zw = r45.xy * r16.zw;
              r10.w = r16.z * r10.w;
              r10.w = r10.w * r16.w;
              r12.w = cmp(0 < r10.w);
              if (r12.w != 0) {
                if (3 == 0) r12.w = 0; else if (3+27 < 32) {                 r12.w = (uint)r36.w << (32-(3 + 27)); r12.w = (uint)r12.w >> (32-3);                } else r12.w = (uint)r36.w >> 27;
                r12.w = cmp((int)r12.w != 1);
                if (r12.w != 0) {
                  r12.w = abs(r11.w) * -0.200000003 + 0.400000006;
                  r13.w = cmp(r11.w < 0);
                  r12.w = r13.w ? -r12.w : r12.w;
                  r45.xyz = r3.xyz * r12.www + v6.xyz;
                  r45.xyz = r45.xyz + -r39.xyz;
                  r12.w = max(abs(r45.y), abs(r45.z));
                  r12.w = max(abs(r45.x), r12.w);
                  r12.w = r40.x / r12.w;
                  r12.w = r12.w + r40.y;
                  r13.w = dot(r45.xyz, r45.xyz);
                  r13.w = rsqrt(r13.w);
                  r12.w = max(6.10351563e-05, r12.w);
                  r14.w = (int)r36.w & 0x0000ffff;
                  r46.w = (uint)r14.w;
                  r14.w = 0;
                  r16.z = 0;
                  while (true) {
                    r16.w = cmp((int)r16.z >= 8);
                    if (r16.w != 0) break;
                    r47.y = dot(icb[r16.z+0].yx, r15.xy);
                    r47.z = dot(icb[r16.z+0].yx, r15.yz);
                    r47.yz = r47.yz * r38.yy;
                    r47.x = r47.y * r9.x;
                    r47.w = r47.y * r6.x;
                    r46.xyz = r45.xyz * r13.www + r47.xzw;
                    r16.w = gOmniShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r46.xyzw, r12.w).x;
                    r14.w = r16.w * 0.125 + r14.w;
                    r16.z = (int)r16.z + 1;
                  }
                } else {
                  r14.w = 1;
                }
                r10.w = r14.w * r10.w;
                r12.w = cmp(0 < r10.w);
                if (r12.w != 0) {
                  r12.w = r36.x * r2.y;
                  r12.w = 0.25 * r12.w;
                  r13.w = dot(r42.xyz, r10.xyz);
                  r16.z = dot(r10.xyz, r44.xyz);
                  r16.w = -r13.w * r13.w + r41.x;
                  r7.w = r13.w * r16.z + -r7.w;
                  r7.w = saturate(r7.w / r16.w);
                  r13.w = r16.w / r41.x;
                  r13.w = 10 * r13.w;
                  r13.w = min(1, r13.w);
                  r7.w = r7.w + -r8.w;
                  r7.w = r13.w * r7.w + r8.w;
                  r42.xyz = r7.www * r42.xyz + r43.xyz;
                  r42.xyz = r7.zzz ? r42.xyz : r35.yzw;
                  r42.xyz = -v6.xyz + r42.xyz;
                  r7.z = dot(r42.xyz, r42.xyz);
                  r7.z = rsqrt(r7.z);
                  r43.xyz = r42.xyz * r7.zzz;
                  if (4 == 0) r7.w = 0; else if (4+16 < 32) {                   r7.w = (uint)r36.w << (32-(4 + 16)); r7.w = (uint)r7.w >> (32-4);                  } else r7.w = (uint)r36.w >> 16;
                  r8.w = cmp(0 < (uint)r7.w);
                  r8.w = r1.w ? r8.w : 0;
                  if (r8.w != 0) {
                    r7.w = (int)r7.w + numLights;
                    r7.w = (int)r7.w + -1;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r8.w, r7.w, l(52), t12.xxxx
                  r8.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r7.w, l(100), t12.xyzw
                  r44.x = samp0[]..swiz;
                  r44.y = samp0[]..swiz;
                  r44.z = samp0[]..swiz;
                  r44.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r7.w, l(116), t12.xyzw
                  r45.x = samp0[]..swiz;
                  r45.y = samp0[]..swiz;
                  r45.z = samp0[]..swiz;
                  r45.w = samp0[]..swiz;
                    r13.w = abs(r11.w) * -0.200000003 + 0.400000006;
                    r16.z = cmp(r11.w < 0);
                    r13.w = r16.z ? -r13.w : r13.w;
                    r20.xyz = r3.xyz * r13.www + v6.xyz;
                    r13.w = dot(r44.xyzw, r20.xyzw);
                    r16.z = dot(r45.xyzw, r20.xyzw);
                    r16.w = cmp(r16.z < r13.w);
                    if (r16.w == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r7.w, l(68), t12.xyzw
                    r44.x = samp0[]..swiz;
                    r44.y = samp0[]..swiz;
                    r44.z = samp0[]..swiz;
                    r44.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r7.w, l(84), t12.xyzw
                    r45.x = samp0[]..swiz;
                    r45.y = samp0[]..swiz;
                    r45.z = samp0[]..swiz;
                    r45.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r46.xyzw, r7.w, l(132), t12.xyzw
                    r46.x = samp0[]..swiz;
                    r46.y = samp0[]..swiz;
                    r46.z = samp0[]..swiz;
                    r46.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r7.w, l(148), t12.xyzw
                    r47.x = samp0[]..swiz;
                    r47.y = samp0[]..swiz;
                    r47.z = samp0[]..swiz;
                    r47.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r48.xy, r7.w, l(164), t12.xyxx
                    r48.x = samp0[]..swiz;
                    r48.y = samp0[]..swiz;
                      r44.x = dot(r44.xyzw, r20.xyzw);
                      r44.y = dot(r45.xyzw, r20.xyzw);
                      r20.xy = r44.xy / r16.zz;
                      r20.xy = r20.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                      r20.xy = r20.xy + r47.zw;
                      r20.xy = r20.xy * r47.xy;
                      r44.xy = r8.ww / r46.xz;
                      r44.zw = float2(1,1) + -r44.xy;
                      r44.zw = cmp(r20.xy >= r44.zw);
                      r44.xy = cmp(r44.xy >= r20.xy);
                      r44.xy = (int2)r44.xy | (int2)r44.zw;
                      r17.w = (int)r44.y | (int)r44.x;
                      r20.xy = saturate(r20.xy);
                      r20.xy = r20.xy * r46.xz + r46.yw;
                      r22.w = r48.y * r16.z;
                      r13.w = r48.x * r16.z + r13.w;
                      r13.w = r13.w / r22.w;
                    } else {
                      r17.w = -1;
                    }
                    r16.z = (int)r16.w | (int)r17.w;
                    if (r16.z == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r7.w, r7.w, l(28), t12.xxxx
                    r7.w = samp0[]..swiz;
                      r13.w = max(6.10351563e-05, r13.w);
                      r7.w = (int)r7.w & 0x0000ffff;
                      if (r5.w != 0) {
                        r44.z = (uint)r7.w;
                        r16.zw = float2(0,0);
                        while (true) {
                          r17.w = cmp((int)r16.w >= 8);
                          if (r17.w != 0) break;
                          r45.x = dot(icb[r16.w+0].yx, r15.xw);
                          r45.y = dot(icb[r16.w+0].xy, r7.xy);
                          r44.xy = r45.xy * r8.ww + r20.xy;
                          r17.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r44.xyz, r13.w).x;
                          r16.z = r17.w * 0.125 + r16.z;
                          r16.w = (int)r16.w + 1;
                        }
                      } else {
                        r20.z = (uint)r7.w;
                        r16.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r13.w).x;
                      }
                      r7.w = r16.z * r16.z;
                      r7.w = r7.w * r16.z;
                    } else {
                      r7.w = 1;
                    }
                    r10.w = r10.w * r7.w;
                  } else {
                    if (4 == 0) r7.w = 0; else if (4+20 < 32) {                     r7.w = (uint)r36.w << (32-(4 + 20)); r7.w = (uint)r7.w >> (32-4);                    } else r7.w = (uint)r36.w >> 20;
                    r8.w = cmp(0 < (uint)r7.w);
                    r8.w = r8.w ? r1.z : 0;
                    if (r8.w != 0) {
                      r7.w = (int)r7.w + numLights;
                      r7.w = (int)r7.w + -1;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r8.w, r7.w, l(52), t12.xxxx
                    r8.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r7.w, l(100), t12.xyzw
                    r44.x = samp0[]..swiz;
                    r44.y = samp0[]..swiz;
                    r44.z = samp0[]..swiz;
                    r44.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r7.w, l(116), t12.xyzw
                    r45.x = samp0[]..swiz;
                    r45.y = samp0[]..swiz;
                    r45.z = samp0[]..swiz;
                    r45.w = samp0[]..swiz;
                      r13.w = abs(r11.w) * -0.200000003 + 0.400000006;
                      r16.w = cmp(r11.w < 0);
                      r13.w = r16.w ? -r13.w : r13.w;
                      r21.xyz = r3.xyz * r13.www + v6.xyz;
                      r13.w = dot(r44.xyzw, r21.xyzw);
                      r16.w = dot(r45.xyzw, r21.xyzw);
                      r17.w = cmp(r16.w < r13.w);
                      if (r17.w == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r7.w, l(68), t12.xyzw
                      r44.x = samp0[]..swiz;
                      r44.y = samp0[]..swiz;
                      r44.z = samp0[]..swiz;
                      r44.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r7.w, l(84), t12.xyzw
                      r45.x = samp0[]..swiz;
                      r45.y = samp0[]..swiz;
                      r45.z = samp0[]..swiz;
                      r45.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r46.xyzw, r7.w, l(132), t12.xyzw
                      r46.x = samp0[]..swiz;
                      r46.y = samp0[]..swiz;
                      r46.z = samp0[]..swiz;
                      r46.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r7.w, l(148), t12.xyzw
                      r47.x = samp0[]..swiz;
                      r47.y = samp0[]..swiz;
                      r47.z = samp0[]..swiz;
                      r47.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r20.xy, r7.w, l(164), t12.xyxx
                      r20.x = samp0[]..swiz;
                      r20.y = samp0[]..swiz;
                        r44.x = dot(r44.xyzw, r21.xyzw);
                        r44.y = dot(r45.xyzw, r21.xyzw);
                        r21.xy = r44.xy / r16.ww;
                        r21.xy = r21.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                        r21.xy = r21.xy + r47.zw;
                        r21.xy = r21.xy * r47.xy;
                        r44.xy = r8.ww / r46.xz;
                        r44.zw = float2(1,1) + -r44.xy;
                        r44.zw = cmp(r21.xy >= r44.zw);
                        r44.xy = cmp(r44.xy >= r21.xy);
                        r44.xy = (int2)r44.xy | (int2)r44.zw;
                        r20.z = (int)r44.y | (int)r44.x;
                        r21.xy = saturate(r21.xy);
                        r21.xy = r21.xy * r46.xz + r46.yw;
                        r20.y = r20.y * r16.w;
                        r13.w = r20.x * r16.w + r13.w;
                        r13.w = r13.w / r20.y;
                      } else {
                        r20.z = -1;
                      }
                      r16.w = (int)r17.w | (int)r20.z;
                      if (r16.w == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r7.w, r7.w, l(28), t12.xxxx
                      r7.w = samp0[]..swiz;
                        r13.w = max(6.10351563e-05, r13.w);
                        r7.w = (int)r7.w & 0x0000ffff;
                        if (r5.w != 0) {
                          r20.z = (uint)r7.w;
                          r16.w = 0;
                          r17.w = 0;
                          while (true) {
                            r22.w = cmp((int)r17.w >= 8);
                            if (r22.w != 0) break;
                            r44.x = dot(icb[r17.w+0].xy, r8.xy);
                            r44.y = dot(icb[r17.w+0].yx, r8.xz);
                            r20.xy = r44.xy * r8.ww + r21.xy;
                            r20.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r13.w).x;
                            r16.w = r20.x * 0.125 + r16.w;
                            r17.w = (int)r17.w + 1;
                          }
                        } else {
                          r21.z = (uint)r7.w;
                          r16.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r21.xyz, r13.w).x;
                        }
                        r7.w = r16.w * r16.w;
                        r7.w = r7.w * r16.w;
                      } else {
                        r7.w = 1;
                      }
                      r10.w = r10.w * r7.w;
                    }
                  }
                  r7.w = -abs(r11.w) * r1.x + 1;
                  r7.w = r7.w * r7.w;
                  r7.w = -r7.w * 0.620000005 + 0.620000005;
                  r7.w = r7.w + -abs(r11.w);
                  r7.w = r4.w * r7.w + abs(r11.w);
                  r7.w = r7.w * r10.w;
                  r36.x = r35.x;
                  r8.w = cmp(0 < r11.w);
                  r20.xyz = r7.www * r36.xyz + r18.xyz;
                  r11.w = saturate(dot(r3.xyz, r43.xyz));
                  r21.xyz = r42.xyz * r7.zzz + r5.xyz;
                  r7.z = dot(r21.xyz, r21.xyz);
                  r7.z = rsqrt(r7.z);
                  r21.xyz = r21.xyz * r7.zzz;
                  r7.z = dot(r21.xyz, r5.xyz);
                  r13.w = dot(r3.xyz, r21.xyz);
                  r17.w = abs(r13.w) * r2.y + -abs(r13.w);
                  r13.w = abs(r13.w) * r17.w + 1;
                  r17.w = r11.w * r1.y + r2.x;
                  r13.w = r13.w * r13.w;
                  r13.w = r13.w * r17.w;
                  r13.w = rcp(r13.w);
                  r11.w = r11.w * r12.w;
                  r11.w = r13.w * r11.w;
                  r10.w = r11.w * r10.w;
                  r21.xyz = r10.www * r36.xyz + r34.xyz;
                  r7.z = saturate(1 + -r7.z);
                  r7.z = log2(r7.z);
                  r7.z = 3.4000001 * r7.z;
                  r7.z = exp2(r7.z);
                  r7.z = r10.w * r7.z;
                  r42.xyz = r7.zzz * r36.xyz + r33.xyz;
                  r43.xyz = r7.www * r36.xyz + r32.xyz;
                  r18.xyz = r8.www ? r20.xyz : r18.xyz;
                  r32.xyz = r8.www ? r32.xyz : r43.xyz;
                  r33.xyz = r8.www ? r42.xyz : r33.xyz;
                  r34.xyz = r8.www ? r21.xyz : r34.xyz;
                }
              }
            }
            break;
            case 2 :            r20.xy = r38.zw;
            r20.z = r39.w;
            r20.xyz = -v6.xyz + r20.xyz;
            r7.z = dot(r20.xyz, r20.xyz);
            r7.z = rsqrt(r7.z);
            r21.xyz = r20.xyz * r7.zzz;
            r7.w = dot(r3.xyz, r21.xyz);
            r8.w = cmp(0 < abs(r7.w));
            if (r8.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r42.xyzw, r6.w, l(112), t12.yzwx
            r42.x = samp0[]..swiz;
            r42.y = samp0[]..swiz;
            r42.z = samp0[]..swiz;
            r42.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r43.xyzw, r6.w, l(128), t12.zxyw
            r43.x = samp0[]..swiz;
            r43.y = samp0[]..swiz;
            r43.z = samp0[]..swiz;
            r43.w = samp0[]..swiz;
              r44.xyz = r42.xyz;
              r44.w = r43.y;
              r8.w = dot(r44.xyzw, r19.xyzw);
              r10.w = cmp(r8.w < 1);
              if (r10.w != 0) {
                r21.xyz = float3(1,1,1);
                r10.w = 0;
              } else {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r6.w, l(172), t12.yzwx
              r45.x = samp0[]..swiz;
              r45.y = samp0[]..swiz;
              r45.z = samp0[]..swiz;
              r45.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r46.xyzw, r6.w, l(188), t12.wxyz
              r46.x = samp0[]..swiz;
              r46.y = samp0[]..swiz;
              r46.z = samp0[]..swiz;
              r46.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r47.xyzw, r6.w, l(204), t12.xyzw
              r47.x = samp0[]..swiz;
              r47.y = samp0[]..swiz;
              r47.z = samp0[]..swiz;
              r47.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r11.w, r6.w, l(236), t12.xxxx
              r11.w = samp0[]..swiz;
                r48.xyz = -v6.xyz + r35.yzw;
                r12.w = r38.x * r38.x;
                r13.w = dot(r48.xyz, r48.xyz);
                r12.w = r12.w / r13.w;
                r12.w = min(1, r12.w);
                r37.xy = saturate(r8.ww * r37.xz + r37.yw);
                r37.zw = r37.xy * r37.xy;
                r37.xy = r37.xy * float2(-2,-2) + float2(3,3);
                r37.xy = r37.zw * r37.xy;
                r12.w = r37.x * r12.w;
                r12.w = r12.w * r37.y;
                r39.w = r40.x;
                r37.x = dot(r39.xyzw, r19.xyzw);
                r48.xyz = r40.yzw;
                r48.w = r41.w;
                r37.y = dot(r48.xyzw, r19.xyzw);
                r22.xy = r37.xy / r8.ww;
                r8.w = cmp(r45.w < 0.00048828125);
                if (r8.w != 0) {
                  r46.y = r47.x;
                  r37.xy = saturate(abs(r22.xy) * r46.zw + r46.xy);
                  r37.zw = r37.xy * r37.xy;
                  r37.xy = r37.xy * float2(-2,-2) + float2(3,3);
                  r37.xy = r37.zw * r37.xy;
                  r8.w = r37.x * r37.y;
                } else {
                  r45.w = r46.y;
                  r37.xyzw = saturate(r45.xyzw * abs(r22.yyxx));
                  r37.xyzw = log2(r37.xyzw);
                  r37.xyzw = r46.xxxx * r37.xyzw;
                  r37.xyzw = exp2(r37.xyzw);
                  r37.xy = r37.xy + r37.zw;
                  r37.xy = log2(r37.xy);
                  r37.xy = r47.xx * r37.xy;
                  r37.xy = exp2(r37.xy);
                  r13.w = r46.z * r37.x;
                  r17.w = r46.w * r37.y + -1;
                  r13.w = r46.w * r37.y + -r13.w;
                  r13.w = saturate(r17.w / r13.w);
                  r17.w = r13.w * r13.w;
                  r13.w = r13.w * -2 + 3;
                  r8.w = r17.w * r13.w;
                }
                r10.w = r12.w * r8.w;
                r8.w = (int)r11.w & 255;
                if (r8.w != 0) {
                // Known bad code for instruction (needs manual fix):
                                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyz, r6.w, l(220), t12.xyzx
                r37.x = samp0[]..swiz;
                r37.y = samp0[]..swiz;
                r37.z = samp0[]..swiz;
                  r11.w = dot(r47.yzw, r22.xyz);
                  r12.w = dot(r37.xyz, r22.xyz);
                  r37.x = frac(r11.w);
                  r37.y = frac(r12.w);
                  r8.w = (int)r8.w + -1;
                  r37.z = (uint)r8.w;
                  r21.xyz = gCookieArray.SampleLevel(samplerLinear_s, r37.xyz, 0).xyz;
                } else {
                  r21.xyz = float3(1,1,1);
                }
              }
              r35.yz = r36.yz;
              r21.xyz = r35.xyz * r21.xyz;
              r8.w = cmp(0 < r10.w);
              if (r8.w != 0) {
                if (3 == 0) r8.w = 0; else if (3+27 < 32) {                 r8.w = (uint)r36.w << (32-(3 + 27)); r8.w = (uint)r8.w >> (32-3);                } else r8.w = (uint)r36.w >> 27;
                r8.w = cmp((int)r8.w != 1);
                if (r8.w != 0) {
                  r8.w = abs(r7.w) * -0.200000003 + 0.400000006;
                  r11.w = cmp(r7.w < 0);
                  r8.w = r11.w ? -r8.w : r8.w;
                  r23.xyz = r3.xyz * r8.www + v6.xyz;
                  r42.xyz = r41.xyz;
                  r8.w = dot(r42.xyzw, r23.xyzw);
                  r11.w = dot(r44.xyzw, r23.xyzw);
                  r12.w = cmp(r11.w >= r8.w);
                  if (r12.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r43.y, r6.w, l(144), t12.xxxx
                  r43.y = samp0[]..swiz;
                    r39.w = r40.x;
                    r22.x = dot(r39.xyzw, r23.xyzw);
                    r41.xyz = r40.yzw;
                    r22.y = dot(r41.xyzw, r23.xyzw);
                    r22.xy = r22.xy / r11.ww;
                    r22.xy = saturate(r22.xy * float2(0.5,-0.5) + float2(0.5,0.5));
                    r23.xy = r22.xy * r43.zw + r43.xy;
                    r6.w = r8.w / r11.w;
                    r6.w = max(6.10351563e-05, r6.w);
                    r8.w = (int)r36.w & 0x0000ffff;
                    if (r5.w != 0) {
                      r35.z = (uint)r8.w;
                      r11.w = 0;
                      r12.w = 0;
                      while (true) {
                        r13.w = cmp((int)r12.w >= 8);
                        if (r13.w != 0) break;
                        r22.x = dot(icb[r12.w+0].xy, r26.xy);
                        r22.y = dot(icb[r12.w+0].yx, r26.xz);
                        r35.xy = r22.xy * r38.yy + r23.xy;
                        r13.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r35.xyz, r6.w).x;
                        r11.w = r13.w * 0.125 + r11.w;
                        r12.w = (int)r12.w + 1;
                      }
                    } else {
                      r23.z = (uint)r8.w;
                      r11.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r23.xyz, r6.w).x;
                    }
                    r6.w = r11.w * r11.w;
                    r6.w = r6.w * r11.w;
                  } else {
                    r6.w = 1;
                  }
                } else {
                  r6.w = 1;
                }
                r6.w = r10.w * r6.w;
                r8.w = cmp(0 < r6.w);
                if (r8.w != 0) {
                  r8.w = r36.x * r2.y;
                  r8.w = 0.25 * r8.w;
                  if (4 == 0) r10.w = 0; else if (4+16 < 32) {                   r10.w = (uint)r36.w << (32-(4 + 16)); r10.w = (uint)r10.w >> (32-4);                  } else r10.w = (uint)r36.w >> 16;
                  r12.w = cmp(0 < (uint)r10.w);
                  r12.w = r1.w ? r12.w : 0;
                  if (r12.w != 0) {
                    r10.w = (int)r10.w + numLights;
                    r10.w = (int)r10.w + -1;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.w, r10.w, l(52), t12.xxxx
                  r12.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r10.w, l(68), t12.xyzw
                  r35.x = samp0[]..swiz;
                  r35.y = samp0[]..swiz;
                  r35.z = samp0[]..swiz;
                  r35.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyzw, r10.w, l(84), t12.xyzw
                  r37.x = samp0[]..swiz;
                  r37.y = samp0[]..swiz;
                  r37.z = samp0[]..swiz;
                  r37.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r38.xyzw, r10.w, l(100), t12.xyzw
                  r38.x = samp0[]..swiz;
                  r38.y = samp0[]..swiz;
                  r38.z = samp0[]..swiz;
                  r38.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r39.xyzw, r10.w, l(116), t12.xyzw
                  r39.x = samp0[]..swiz;
                  r39.y = samp0[]..swiz;
                  r39.z = samp0[]..swiz;
                  r39.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r10.w, l(132), t12.xyzw
                  r40.x = samp0[]..swiz;
                  r40.y = samp0[]..swiz;
                  r40.z = samp0[]..swiz;
                  r40.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r10.w, l(148), t12.xyzw
                  r41.x = samp0[]..swiz;
                  r41.y = samp0[]..swiz;
                  r41.z = samp0[]..swiz;
                  r41.w = samp0[]..swiz;
                    r13.w = abs(r7.w) * -0.200000003 + 0.400000006;
                    r17.w = cmp(r7.w < 0);
                    r13.w = r17.w ? -r13.w : r13.w;
                    r24.xyz = r3.xyz * r13.www + v6.xyz;
                    r22.x = dot(r35.xyzw, r24.xyzw);
                    r22.y = dot(r37.xyzw, r24.xyzw);
                    r13.w = dot(r38.xyzw, r24.xyzw);
                    r17.w = dot(r39.xyzw, r24.xyzw);
                    r22.w = cmp(r17.w < r13.w);
                    r22.xy = r22.xy / r17.ww;
                    r22.xy = r22.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                    r22.xy = r22.xy + r41.zw;
                    r22.xy = r22.xy * r41.xy;
                    r23.xy = r12.ww / r40.xz;
                    r24.xy = float2(1,1) + -r23.xy;
                    r24.xy = cmp(r22.xy >= r24.xy);
                    r23.xy = cmp(r23.xy >= r22.xy);
                    r23.xy = (int2)r23.xy | (int2)r24.xy;
                    r23.x = (int)r23.y | (int)r23.x;
                    r22.w = (int)r22.w | (int)r23.x;
                    if (r22.w == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r22.w, r10.w, l(28), t12.xxxx
                    r22.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r23.xy, r10.w, l(164), t12.xyxx
                    r23.x = samp0[]..swiz;
                    r23.y = samp0[]..swiz;
                      r22.xy = saturate(r22.xy);
                      r24.xy = r22.xy * r40.xz + r40.yw;
                      r10.w = r23.y * r17.w;
                      r13.w = r23.x * r17.w + r13.w;
                      r10.w = r13.w / r10.w;
                      r10.w = max(6.10351563e-05, r10.w);
                      r13.w = r22.w ? 0.000000 : 0;
                      if (r5.w != 0) {
                        r23.z = (uint)r13.w;
                        r17.w = 0;
                        r22.x = 0;
                        while (true) {
                          r22.y = cmp((int)r22.x >= 8);
                          if (r22.y != 0) break;
                          r35.x = dot(icb[r22.x+0].xy, r27.xy);
                          r35.y = dot(icb[r22.x+0].yx, r27.xz);
                          r23.xy = r35.xy * r12.ww + r24.xy;
                          r22.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r23.xyz, r10.w).x;
                          r17.w = r22.y * 0.125 + r17.w;
                          r22.x = (int)r22.x + 1;
                        }
                      } else {
                        r24.z = (uint)r13.w;
                        r17.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r24.xyz, r10.w).x;
                      }
                      r10.w = r17.w * r17.w;
                      r10.w = r10.w * r17.w;
                    } else {
                      r10.w = 1;
                    }
                    r6.w = r10.w * r6.w;
                  } else {
                    if (4 == 0) r10.w = 0; else if (4+20 < 32) {                     r10.w = (uint)r36.w << (32-(4 + 20)); r10.w = (uint)r10.w >> (32-4);                    } else r10.w = (uint)r36.w >> 20;
                    r12.w = cmp(0 < (uint)r10.w);
                    r12.w = r12.w ? r1.z : 0;
                    if (r12.w != 0) {
                      r10.w = (int)r10.w + numLights;
                      r10.w = (int)r10.w + -1;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.w, r10.w, l(52), t12.xxxx
                    r12.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r10.w, l(68), t12.xyzw
                    r35.x = samp0[]..swiz;
                    r35.y = samp0[]..swiz;
                    r35.z = samp0[]..swiz;
                    r35.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.xyzw, r10.w, l(84), t12.xyzw
                    r36.x = samp0[]..swiz;
                    r36.y = samp0[]..swiz;
                    r36.z = samp0[]..swiz;
                    r36.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyzw, r10.w, l(100), t12.xyzw
                    r37.x = samp0[]..swiz;
                    r37.y = samp0[]..swiz;
                    r37.z = samp0[]..swiz;
                    r37.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r38.xyzw, r10.w, l(116), t12.xyzw
                    r38.x = samp0[]..swiz;
                    r38.y = samp0[]..swiz;
                    r38.z = samp0[]..swiz;
                    r38.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r39.xyzw, r10.w, l(132), t12.xyzw
                    r39.x = samp0[]..swiz;
                    r39.y = samp0[]..swiz;
                    r39.z = samp0[]..swiz;
                    r39.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r10.w, l(148), t12.xyzw
                    r40.x = samp0[]..swiz;
                    r40.y = samp0[]..swiz;
                    r40.z = samp0[]..swiz;
                    r40.w = samp0[]..swiz;
                      r13.w = abs(r7.w) * -0.200000003 + 0.400000006;
                      r22.x = cmp(r7.w < 0);
                      r13.w = r22.x ? -r13.w : r13.w;
                      r25.xyz = r3.xyz * r13.www + v6.xyz;
                      r22.x = dot(r35.xyzw, r25.xyzw);
                      r22.y = dot(r36.xyzw, r25.xyzw);
                      r13.w = dot(r37.xyzw, r25.xyzw);
                      r22.w = dot(r38.xyzw, r25.xyzw);
                      r23.x = cmp(r22.w < r13.w);
                      r22.xy = r22.xy / r22.ww;
                      r22.xy = r22.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                      r22.xy = r22.xy + r40.zw;
                      r22.xy = r22.xy * r40.xy;
                      r23.yz = r12.ww / r39.xz;
                      r24.xy = float2(1,1) + -r23.yz;
                      r24.xy = cmp(r22.xy >= r24.xy);
                      r23.yz = cmp(r23.yz >= r22.xy);
                      r23.yz = (int2)r23.yz | (int2)r24.xy;
                      r23.y = (int)r23.z | (int)r23.y;
                      r23.x = (int)r23.x | (int)r23.y;
                      if (r23.x == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r23.x, r10.w, l(28), t12.xxxx
                      r23.x = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r23.yz, r10.w, l(164), t12.xxyx
                      r23.y = samp0[]..swiz;
                      r23.z = samp0[]..swiz;
                        r22.xy = saturate(r22.xy);
                        r24.xy = r22.xy * r39.xz + r39.yw;
                        r10.w = r23.z * r22.w;
                        r13.w = r23.y * r22.w + r13.w;
                        r10.w = r13.w / r10.w;
                        r10.w = max(6.10351563e-05, r10.w);
                        r13.w = r23.x ? 0.000000 : 0;
                        if (r5.w != 0) {
                          r23.z = (uint)r13.w;
                          r22.xy = float2(0,0);
                          while (true) {
                            r22.w = cmp((int)r22.y >= 8);
                            if (r22.w != 0) break;
                            r25.x = dot(icb[r22.y+0].xy, r28.xy);
                            r25.y = dot(icb[r22.y+0].yx, r28.xz);
                            r23.xy = r25.xy * r12.ww + r24.xy;
                            r22.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r23.xyz, r10.w).x;
                            r22.x = r22.w * 0.125 + r22.x;
                            r22.y = (int)r22.y + 1;
                          }
                        } else {
                          r24.z = (uint)r13.w;
                          r22.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r24.xyz, r10.w).x;
                        }
                        r10.w = r22.x * r22.x;
                        r10.w = r10.w * r22.x;
                      } else {
                        r10.w = 1;
                      }
                      r6.w = r10.w * r6.w;
                    }
                  }
                  r10.w = -abs(r7.w) * r1.x + 1;
                  r10.w = r10.w * r10.w;
                  r10.w = -r10.w * 0.620000005 + 0.620000005;
                  r10.w = r10.w + -abs(r7.w);
                  r10.w = r4.w * r10.w + abs(r7.w);
                  r10.w = r10.w * r6.w;
                  r12.w = cmp(0 < r7.w);
                  r23.xyz = r10.www * r21.xyz + r18.xyz;
                  r7.w = saturate(r7.w);
                  r20.xyz = r20.xyz * r7.zzz + r5.xyz;
                  r7.z = dot(r20.xyz, r20.xyz);
                  r7.z = rsqrt(r7.z);
                  r20.xyz = r20.xyz * r7.zzz;
                  r7.z = dot(r20.xyz, r5.xyz);
                  r13.w = dot(r3.xyz, r20.xyz);
                  r20.x = abs(r13.w) * r2.y + -abs(r13.w);
                  r13.w = abs(r13.w) * r20.x + 1;
                  r20.x = r7.w * r1.y + r2.x;
                  r13.w = r13.w * r13.w;
                  r13.w = r13.w * r20.x;
                  r13.w = rcp(r13.w);
                  r7.w = r7.w * r8.w;
                  r7.w = r13.w * r7.w;
                  r6.w = r7.w * r6.w;
                  r20.xyz = r6.www * r21.xyz + r34.xyz;
                  r7.z = saturate(1 + -r7.z);
                  r7.z = log2(r7.z);
                  r7.z = 3.4000001 * r7.z;
                  r7.z = exp2(r7.z);
                  r6.w = r7.z * r6.w;
                  r24.xyz = r6.www * r21.xyz + r33.xyz;
                  r21.xyz = r10.www * r21.xyz + r32.xyz;
                  r18.xyz = r12.www ? r23.xyz : r18.xyz;
                  r32.xyz = r12.www ? r32.xyz : r21.xyz;
                  r33.xyz = r12.www ? r24.xyz : r33.xyz;
                  r34.xyz = r12.www ? r20.xyz : r34.xyz;
                }
              }
            }
            break;
            default :
            break;
          }
        }
      }
    }
    r9.yzw = r18.xyz;
    r29.xyz = r32.xyz;
    r30.xyz = r33.xyz;
    r31.xyz = r34.xyz;
    r2.w = (int)r2.w + 32;
  }
  r1.x = log2(r6.z);
  r1.x = 3.4000001 * r1.x;
  r1.x = exp2(r1.x);
  r1.y = 1 + -r0.w;
  r0.w = r1.x * r1.y + r0.w;
  r0.xyz = r0.xyz * r0.www;
  r1.xyz = backscatterTint.xyz * r0.xyz;
  r1.xyz = r29.xyz * r1.xyz;
  r0.xyz = r9.yzw * r0.xyz + r1.xyz;
  r1.xyz = float3(1,1,1) + -r4.xyz;
  r1.xyz = r30.xyz * r1.xyz;
  r1.xyz = r31.xyz * r4.xyz + r1.xyz;
  r0.xyz = r1.xyz + r0.xyz;
  r1.x = cmp(0 < fogConstants.atmosphereExtinctionIntensity);
  if (r1.x != 0) {
    r1.x = dot(v6.xyz, v6.xyz);
    r1.y = rsqrt(r1.x);
    r1.yzw = v6.xyz * r1.yyy;
    r1.x = sqrt(r1.x);
    r2.x = cmp(0 < fogConstants.blendAmount);
    if (r2.x != 0) {
      r2.xy = r1.xx * fogConstants.atmospherefogdistancedensityscale.xy + fogConstants.atmospherefogdistanceoffset.xy;
      r2.xy = fogConstants.atmospherefogdensityatcamera.xy * r2.xy;
      r2.z = cmp(0.00999999978 < abs(v6.z));
      r3.xy = fogConstants.atmospherefogheightdensityscale.xy * v6.zz;
      r3.zw = float2(-1.44269502,-1.44269502) * r3.xy;
      r3.zw = exp2(r3.zw);
      r3.zw = float2(1,1) + -r3.zw;
      r3.xy = r3.zw / r3.xy;
      r3.xy = r3.xy * r2.xy;
      r2.xy = r2.zz ? r3.xy : r2.xy;
      r2.xzw = fogConstants.atmosphereTotalDensity.xyz * r2.xxx;
      r2.xzw = exp2(r2.xzw);
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r2.yyy;
      r3.xyz = exp2(r3.xyz);
      r3.xyz = r3.xyz + -r2.xzw;
      r2.xyz = fogConstants.blendAmount * r3.xyz + r2.xzw;
    } else {
      r2.w = r1.x * fogConstants.atmospherefogdistancedensityscale.x + fogConstants.atmospherefogdistanceoffset.x;
      r2.w = fogConstants.atmospherefogdensityatcamera.x * r2.w;
      r3.x = cmp(0.00999999978 < abs(v6.z));
      r3.y = fogConstants.atmospherefogheightdensityscale.x * v6.z;
      r3.z = -1.44269502 * r3.y;
      r3.z = exp2(r3.z);
      r3.z = 1 + -r3.z;
      r3.y = r3.z / r3.y;
      r3.y = r3.y * r2.w;
      r2.w = r3.x ? r3.y : r2.w;
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r2.www;
      r2.xyz = exp2(r3.xyz);
    }
    r2.xyz = r2.xyz * fogConstants.atmosphereExtinctionIntensity + float3(1,1,1);
    r2.xyz = saturate(-fogConstants.atmosphereExtinctionIntensity + r2.xyz);
    r1.y = dot(fogConstants.wldSunFogDir.xyz, -r1.yzw);
    r1.z = -fogConstants.atmosphereMieSchlickK * fogConstants.atmosphereMieSchlickK + 1;
    r1.w = fogConstants.atmosphereMieSchlickK * -r1.y + 1;
    r1.w = r1.w * r1.w;
    r1.w = 12.566371 * r1.w;
    r1.z = r1.z / r1.w;
    r1.x = -fogConstants.atmospherehazebasedist + r1.x;
    r1.x = saturate(fogConstants.atmospherehazefadedist * r1.x);
    r1.x = r1.z * r1.x;
    r1.y = saturate(r1.y);
    r1.y = r1.y * r1.y + 1;
    r1.y = r1.y * 0.0596831031 + -1;
    r1.y = fogConstants.atmospheresunstrength * r1.y + 1;
    r1.xzw = fogConstants.atmosphereMieDensity.xyz * r1.xxx;
    r1.xyz = r1.yyy * fogConstants.atmosphereRayleighDensity.xyz + r1.xzw;
    r1.xyz = fogConstants.atmosphereInScatterIntensity * r1.xyz;
    r3.xyz = float3(1,1,1) + -r2.xyz;
    r1.xyz = r3.xyz * r1.xyz;
    r1.xyz = r1.xyz * r0.www;
    r1.xyz = r0.xyz * r2.xyz + r1.xyz;
  } else {
    r1.w = fogConstants.heightFalloff * v6.z;
    r2.x = fogConstants.heightFalloff * v6.z + fogConstants.K0;
    r2.y = cmp(abs(r1.w) < 9.99999975e-05);
    r2.z = min(64, r2.x);
    r2.z = 1.44269502 * r2.z;
    r2.z = exp2(r2.z);
    r2.w = saturate(fogConstants.K0b);
    r3.x = cmp(r2.x < 0);
    r2.x = 1 + r2.x;
    r2.x = r3.x ? r2.z : r2.x;
    r2.x = -fogConstants.K0b + r2.x;
    r1.w = r2.y ? 1 : r1.w;
    r1.w = r2.x / r1.w;
    r1.w = r2.y ? r2.w : r1.w;
    r1.w = fogConstants.expMul * r1.w;
    r2.x = dot(v6.xyz, v6.xyz);
    r2.y = sqrt(r2.x);
    r1.w = r1.w * r2.y + fogConstants.expAdd;
    r1.w = exp2(r1.w);
    r1.w = min(1, r1.w);
    r1.w = 1 + -r1.w;
    r2.x = rsqrt(r2.x);
    r2.xyz = v6.xyz * r2.xxx;
    r2.x = dot(fogConstants.wldSunFogDir.xyz, r2.xyz);
    r2.x = saturate(fogConstants.sunFogAngles.y * r2.x + fogConstants.sunFogAngles.x);
    r3.xyzw = -fogConstants.sunFogColor.xyzw + fogConstants.fogColor.xyzw;
    r2.xyzw = r2.xxxx * r3.xyzw + fogConstants.sunFogColor.xyzw;
    r1.w = r2.w * r1.w;
    r2.xyz = r2.xyz * r0.www + -r0.xyz;
    r1.xyz = r1.www * r2.xyz + r0.xyz;
  }
  r0.xyz = relHDRExposure.yyy * r1.xyz;
  r1.xyz = cmp(r0.xyz >= float3(6.10351563e-05,6.10351563e-05,6.10351563e-05));
  r0.xyz = r1.xyz ? r0.xyz : 0;
  o0.xyz = min(float3(65024,65024,64512), r0.xyz);
  o0.w = r0.w;
  return;
}