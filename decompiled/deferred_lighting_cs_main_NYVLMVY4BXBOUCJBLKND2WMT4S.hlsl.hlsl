// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:23:44 2021

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

cbuffer LightingGlobals : register(b2)
{
  uint numLights : packoffset(c0);
  uint numRefProbes : packoffset(c0.y);
  uint numRefProbeBlends : packoffset(c0.z);
  uint numDedicatedLights : packoffset(c0.w);
  float4 clearColor : packoffset(c1);
  float4 sssParams : packoffset(c2);

  struct
  {
    float4 fogColor;
    float4 sunFogColor;
    float K0;
    float skyK0;
    float expMul;
    float expAdd;
    float heightFalloff;
    float skyHeightFalloff;
    float K0b;
    float padding0;
    float skyK0b;
    float3 wldSunFogDir;
    float2 sunFogAngles;
    float atmospheresunstrength;
    float atmosphereMieSchlickK;
    float2 atmosphereskyfogdensityatcamera;
    float atmosphereExtinctionIntensity;
    float atmosphereInScatterIntensity;
    float3 atmosphereRayleighDensity;
    float atmospherehazebasedist;
    float3 atmosphereMieDensity;
    float atmospherehazefadedist;
    float3 atmosphereTotalDensity;
    float worldfogskysize;
    float3 atmosphereInScatterIntensityOverTotalDensity;
    float blendAmount;
    float2 atmosphereskyfogheightdensityscale;
    float2 atmospherefogdistanceoffset;
    float2 atmospherefogdistancedensityscale;
    float2 atmospherefogheightdensityscale;
    float2 atmospherefogdensityatcamera;
    float2 padding1;
  } fogConstants : packoffset(c3);


  struct
  {
    float3 wldDir;
    float splitDepthOffset;
    float3 color;
    float specScale;
    float globalProbeExposure;
    float3 avgGlobalProbeColor;
    float4 splitPinTransform[3];
    uint sunCookieIndex;
    float sunCookieIntensity;
    float sunVolumetricCookieIntensity;
    uint toolsGfxDisableSunShadow;
    float4 sunCookieTransform[2];
    float intensity;
    int splitArrayOffset;
    float2 pad0;

    struct
    {

      struct
      {
        float2 dimensionInTiles;
        float inchesPerTexel;
        float spanInInches;
      } constants;

      row_major float4x4 offToPinTransform;
      float coordScale;
      uint rootOffset;
      uint2 pad0;
    } sstLightingConstants;

  } sunConstants : packoffset(c17);


  struct
  {
    float2 sstToMinMaxScale;
    float2 pad0;
    float2 halfMinTexelSize;
    float rcpInchesDimLevel0;
    float shadowBiasInches;
  } minMaxConstants : packoffset(c33);


  struct
  {
    uint4 data[3];
  } globalProbeConstants : packoffset(c35);


  struct
  {
    float rain;
    float windSpeed;
    float windDirSin;
    float windDirCos;
    float weatherTile;
    float3 weatherVector;
    float3 weatherVector2;
    float padding0;
    float3 weatherTint;
    float padding1;
    float3 weatherTint2;
    float padding2;
  } weather : packoffset(c38);


  struct
  {

    struct
    {
      float2 dimensionInTiles;
      float inchesPerTexel;
      float spanInInches;
    } constants;

    row_major float4x4 offToPinTransform;
    float coordScale;
    uint rootOffset;
    uint2 pad0;
  } outdoorSSTConstants : packoffset(c43);

  float4 pinToWorldZ : packoffset(c49);

  struct
  {
    uint firstSpotLight;
    uint lastSpotLight;
    uint firstOmniLight;
    uint lastOmniLight;
    uint firstProbe;
    uint lastProbe;
    uint padding0;
    uint padding1;
  } volumetric : packoffset(c50);

  uint viewID : packoffset(c52);
  uint enableCountDebug : packoffset(c52.y);
  uint centerGroupIDx : packoffset(c52.z);
  uint centerGroupIDy : packoffset(c52.w);
  uint viewmodelSunShadowRaw : packoffset(c53);
  uint debugDrawOverlayType : packoffset(c53.y);
  float drawNumLighstScale : packoffset(c53.z);
  float drawNumLightsAlpha : packoffset(c53.w);
  uint probeDebug : packoffset(c54);
  float probeDebugRadius : packoffset(c54.y);
  uint showSunVis : packoffset(c54.z);
  uint enableAO : packoffset(c54.w);
  uint showAO : packoffset(c55);
  uint numGpuCullTilesWidth : packoffset(c55.y);
  uint numGpuCullTilesHeight : packoffset(c55.z);
  int permuteHighlight : packoffset(c55.w);
  uint overdrawOverlayMaxValue : packoffset(c56);
  float overdrawOverlayAlpha : packoffset(c56.y);
  uint overdrawDecals : packoffset(c56.z);
  uint overdrawDecalsLayers : packoffset(c56.w);
  uint permuteStride : packoffset(c57);
  uint numDecals : packoffset(c57.y);
  uint numStaticDecals : packoffset(c57.z);
  uint numDynamicDecals : packoffset(c57.w);
  uint numOverrideProbes : packoffset(c58);
  uint enableDitheredShadow : packoffset(c58.y);
  uint oitMaxEntries : packoffset(c58.z);
  uint numAttenuationVolumes : packoffset(c58.w);
}

cbuffer LightingPackCBuffer : register(b9)
{

  struct
  {
    uint4 data[15];
  } lightConstantsCB[273] : packoffset(c0);

}

cbuffer RefProbePackCBuffer : register(b10)
{

  struct
  {
    uint4 data[14];
  } refProbeConstantsCB[273] : packoffset(c0);

}

cbuffer RefProbeBlendPackCBuffer : register(b11)
{

  struct
  {
    uint4 data[6];
  } refProbeAttenuationConstantsCB[682] : packoffset(c0);

}

SamplerState samp0_s : register(s0);
SamplerState samplerLinear_s : register(s2);
SamplerState samplerLinearClamp_s : register(s4);
SamplerComparisonState ShadowSamplerComparisonState_s : register(s13);
Texture2D<float4> GBufferDiffuseColor : register(t0);
Texture2D<float4> GBufferNormalAndGloss : register(t1);
Texture2D<float4> GBufferSpecularColorAndOcclusion : register(t2);
Texture2D<float> GBufferDepth : register(t4);
Texture2D<float2> gEnvBRDFGeneric : register(t5);
TextureCubeArray<float3> gReflectionProbeArray : register(t8);
Texture2DArray<float> gSpotShadowmapArray : register(t9);
TextureCubeArray<float> gOmniShadowmapArray : register(t10);
Texture2DArray<float> gSunShadowmapArray : register(t13);
Texture3D<float3> gProbeXArray : register(t17);
Texture3D<float3> gProbeYArray : register(t18);
Texture3D<float3> gProbeZArray : register(t19);
Texture2DArray<float4> gCookieArray : register(t20);
StructuredBuffer<gSunShadowTree> gSunShadowTree : register(t23);
StructuredBuffer<gPermutedTilingBuffer> gPermutedTilingBuffer : register(t25);
StructuredBuffer<gDeferredCullBits> gDeferredCullBits : register(t26);
Texture2D<float> gSSAOTexture : register(t27);
Texture2DArray<float> gTransShadowmapArray : register(t31);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main()
{
  const float4 icb[] = { { -0.808114, 0.808114, 0, 0},
                              { 0, -1.000000, 0, 0},
                              { 0.606057, 0.606057, 0, 0},
                              { -0.714286, 0, 0, 0},
                              { 0.404114, -0.404114, 0, 0},
                              { 0, 0.428571, 0, 0},
                              { -0.202057, -0.202057, 0, 0},
                              { 0.142857, 0, 0, 0} };
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t23, 4
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t25, 8
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t26, 4
// Needs manual fix for instruction: 
// unknown dcl_: dcl_uav_typed_texture2d (float,float,float,float) u0
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15,r16,r17,r18,r19,r20,r21,r22,r23,r24,r25,r26,r27,r28,r29,r30,r31,r32,r33;
  uint4 bitmask, uiDest;
  float4 fDest;

// Needs manual fix for instruction: 
// unknown dcl_: dcl_thread_group 8, 8, 1
  r0.x = permuteStride << 1;
  r0.x = (int)r0.x + (int)vThreadGroupID.x;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=8)(mixed,mixed,mixed,mixed) r0.xy, r0.x, l(0), t25.xyxx
r0.x = samp0[]..swiz;
r0.y = samp0[]..swiz;
  r0.yz = (int2)r0.xy & int2(4095,4095);
  r1.xy = mad((int2)r0.yz, int2(8,8), (int2)vThreadIDInGroup.xy);
  r1.zw = float2(0,0);
  r2.xyzw = GBufferNormalAndGloss.Load(r1.xyw).xyzw;
  r3.xy = (uint2)renderTargetSize.xy;
  r3.xy = cmp((uint2)r1.xy < (uint2)r3.xy);
  r0.w = r3.y ? r3.x : 0;
  r3.x = cmp(0 < r2.z);
  r0.w = r3.x ? r0.w : 0;
  if (r0.w == 0) {
    return;
  }
  r3.x = cmp(r2.z >= 0.5);
  r3.y = GBufferDepth.Load(r1.xyw).x;
  r4.xyz = GBufferDiffuseColor.Load(r1.xyw).xyz;
  r5.xyzw = GBufferSpecularColorAndOcclusion.Load(r1.xyw).xyzw;
  r6.xyz = GBufferSpecularColorAndOcclusion.Load(r1.xyw, int3(0, 0, 0)).xyw;
  r7.xyz = GBufferSpecularColorAndOcclusion.Load(r1.xyw, int3(0, 0, 0)).xyw;
  r8.xyz = GBufferSpecularColorAndOcclusion.Load(r1.xyw, int3(0, 0, 0)).xyw;
  r9.xyz = GBufferSpecularColorAndOcclusion.Load(r1.xyz, int3(0, 0, 0)).xyw;
  r1.zw = (uint2)r1.xy;
  r3.zw = float2(0.5,0.5) + r1.zw;
  r10.x = cmp(r3.y >= 0.984375);
  r4.w = 1.01587307 * r3.y;
  r3.y = r3.y * 64 + -63;
  r3.y = r10.x ? r3.y : r4.w;
  r3.y = max(9.99999994e-09, r3.y);
  r10.y = rcp(r3.y);
  r10.xy = r10.xy ? r0.ww : 0;
  r10.zw = -subpixelOffset.xy + r3.zw;
  r10.zw = renderTargetSize.zw * r10.zw;
  r10.zw = r10.zw * float2(2,-2) + float2(-1,1);
  r11.x = inverseProjectionMatrix._m00 * r10.z;
  r11.y = inverseProjectionMatrix._m11 * r10.w;
  r11.z = 1;
  r10.yzw = r11.xyz * r10.yyy;
  r11.xyz = inverseViewMatrix._m10_m11_m12 * r10.zzz;
  r11.xyz = r10.yyy * inverseViewMatrix._m00_m01_m02 + r11.xyz;
  r11.xyz = r10.www * inverseViewMatrix._m20_m21_m22 + r11.xyz;
  r0.w = r2.w * 3 + 0.5;
  r0.w = (uint)r0.w;
  r2.xy = r2.xy * float2(2,2) + float2(-1,-1);
  r2.xy = float2(0.850000024,0.850000024) * r2.xy;
  r2.w = dot(r2.xy, r2.xy);
  r12.zw = float2(2,1) + -r2.ww;
  r2.w = sqrt(r12.z);
  r12.xy = r2.xy * r2.ww;
  r13.xyzw = (int4)r0.wwww & int4(2,1,2,1);
  r13.xyzw = r13.zwxy ? float4(-1,-1,1.40129846e-45,1.40129846e-45) : float4(1,1,0,0);
  r0.w = (int)r13.w ^ (int)r13.z;
  r13.z = r0.w ? -1 : 1;
  r2.xyw = float3(-1,0,1) * r13.xyz;
  r10.yzw = float3(1,-2,1) * r13.xyz;
  r12.xyz = float3(0.408248305,0.707106769,0.577350259) * r12.xyw;
  r2.xyw = r12.yyy * r2.xyw;
  r2.xyw = r10.yzw * r12.xxx + r2.xyw;
  r12.xyz = r13.xyz * r12.zzz + r2.xyw;
  r0.w = r3.x ? -0.5 : -0.00146627566;
  r0.w = r2.z + r0.w;
  r2.y = saturate(2.00982332 * r0.w);
  r0.w = dot(-r11.xyz, -r11.xyz);
  r0.w = rsqrt(r0.w);
  r10.yzw = -r11.xyz * r0.www;
  r1.z = dot(r1.zw, float2(0.0671105608,0.00583714992));
  r1.z = frac(r1.z);
  r1.z = 52.9829178 * r1.z;
  r1.z = frac(r1.z);
  r1.z = r1.z * 6.28318548 + gameTick.w;
  sincos(r1.z, r13.x, r14.x);
  r2.x = saturate(dot(r12.xyz, r10.yzw));
  r1.z = dot(-r10.yzw, r12.xyz);
  r1.z = r1.z + r1.z;
  r15.xyz = r12.xyz * -r1.zzz + -r10.yzw;
  r1.z = 17 * r2.y;
  r1.z = exp2(r1.z);
  r1.z = 2 + r1.z;
  r1.z = 2 / r1.z;
  r1.w = sqrt(r1.z);
  r2.zw = renderTargetSize.zw * r3.zw;
  r2.z = gSSAOTexture.SampleLevel(samplerLinearClamp_s, r2.zw, 0).x;
  r2.w = r5.z * r2.z;
  r3.y = dot(r12.xyz, sunConstants.wldDir.xyz);
  r3.z = saturate(r3.y);
  r3.w = r3.x ? abs(r3.y) : r3.z;
  r4.w = cmp(0 >= r3.w);
  if (r4.w != 0) {
    r6.w = 0;
  }
  if (r4.w == 0) {
    r13.yzw = sunConstants.sstLightingConstants.offToPinTransform._m10_m11_m12 * r11.yyy;
    r13.yzw = r11.xxx * sunConstants.sstLightingConstants.offToPinTransform._m00_m01_m02 + r13.yzw;
    r13.yzw = r11.zzz * sunConstants.sstLightingConstants.offToPinTransform._m20_m21_m22 + r13.yzw;
    r13.yzw = sunConstants.sstLightingConstants.offToPinTransform._m30_m31_m32 + r13.yzw;
    r4.w = -sunConstants.splitDepthOffset + r13.w;
    r4.w = -r4.w * 6.10351563e-05 + 1;
    r7.w = saturate(r4.w);
    r7.w = cmp(r4.w == r7.w);
    if (r7.w != 0) {
      r7.w = 0;
      r8.w = 0;
      while (true) {
        r9.w = cmp(r7.w >= 3);
        if (r9.w != 0) break;
        r9.w = (uint)r7.w;
        r14.yz = -sunConstants.splitPinTransform[r9.w].xy + r13.yz;
        r14.y = max(abs(r14.y), abs(r14.z));
        r8.w = sunConstants.splitPinTransform[r9.w].z * r14.y;
        r9.w = cmp(r8.w < 1);
        if (r9.w != 0) {
          break;
        }
        r7.w = 1 + r7.w;
        r8.w = 0;
      }
    } else {
      r7.w = 3;
      r8.w = 0;
    }
    r9.w = cmp(r7.w >= 3);
    if (r9.w != 0) {
      r14.yzw = sunConstants.sstLightingConstants.constants.inchesPerTexel * float3(128,0.5,0.5);
      r16.xz = rcp(r14.yy);
      r16.y = -r16.z;
      r14.yzw = r13.yzy * r16.xyz + r14.zwz;
      r16.xyz = sunConstants.sstLightingConstants.constants.dimensionInTiles.xyx + float3(-0.0078125,-0.0078125,-0.0078125);
      r14.yzw = max(float3(0,0,0), r14.yzw);
      r14.yzw = min(r14.yzw, r16.xyz);
      r16.xy = sunConstants.sstLightingConstants.coordScale * r14.wz;
      r16.xy = floor(r16.xy);
      r16.y = sunConstants.sstLightingConstants.constants.dimensionInTiles.x * r16.y;
      r16.x = r16.y * sunConstants.sstLightingConstants.coordScale + r16.x;
      r16.x = (uint)r16.x;
      r16.x = (int)r16.x + (int10)sunConstants.sstLightingConstants.rootOffset;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r16.y, r16.x, l(0), t23.xxxx
    r16.y = samp0[]..swiz;
      r16.z = (int)r16.y & 0x40000000;
      r16.w = (uint)r16.y << 2;
      if (r16.z == 0) {
        r16.z = (int)r16.y & 0x01ffffff;
        r17.x = (int)r16.z + (int)r16.x;
        r16.x = (uint)r16.y >> 25;
        r16.x = (uint)r16.x;
        r14.yzw = r16.xxx * r14.yzw;
        r14.yzw = frac(r14.yzw);
        r14.yzw = float3(128,128,128) * r14.yzw;
        r14.yzw = (uint3)r14.yzw;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.z, r17.x, l(0), t23.xxxx
      r17.z = samp0[]..swiz;
        r16.xy = (uint2)r14.wz >> int2(6,6);
        r16.z = (int)r17.z & 0xc0000000;
        r17.w = (int)r17.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.w, r17.w, l(0), t23.xxxx
      r17.w = samp0[]..swiz;
        r16.y = r16.y ? r17.z : r17.w;
        r17.w = (uint)r16.y >> 13;
        r16.x = r16.x ? r17.w : r16.y;
        r16.x = (int)r16.x & 8191;
        r18.x = (int)r16.x + (int)r17.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.z, r18.x, l(0), t23.xxxx
      r18.z = samp0[]..swiz;
        r17.y = 0;
        r18.y = 1;
        r18.xyz = r16.zzz ? r17.xyz : r18.xyz;
        r19.yz = r16.zz ? float2(8.40779079e-45,1.40129846e-45) : float2(7.00649232e-45,2.80259693e-45);
        r16.xy = (uint2)r14.wz >> (uint2)r19.yy;
        r16.xy = (int2)r16.xy & int2(1,1);
        r17.w = (int)r18.z & 0xc0000000;
        r18.w = (int)r18.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.w, r18.w, l(0), t23.xxxx
      r18.w = samp0[]..swiz;
        r16.y = r16.y ? r18.z : r18.w;
        r18.w = (uint)r16.y >> 13;
        r16.x = r16.x ? r18.w : r16.y;
        r16.x = (int)r16.x & 8191;
        r19.x = (int)r16.x + (int)r18.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r19.w, r19.x, l(0), t23.xxxx
      r19.w = samp0[]..swiz;
        r18.xyz = r17.www ? r18.xyz : r19.xzw;
        r16.xyz = r16.zzz ? r17.xyz : r18.xyz;
        r17.x = (int)r16.z & 0xc0000000;
        if (r17.x == 0) {
          r17.x = (int)-r16.y + 6;
          r17.xy = (uint2)r14.wz >> (uint2)r17.xx;
          r17.z = (int)r16.z | 0x40000000;
          bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r17.y = (((uint)r17.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r17.x = (((uint)r17.x << 0) & bitmask.x) | ((uint)r17.y & ~bitmask.x);
          r17.x = (int)r17.x * 10;
          r17.x = (uint)r17.z >> (uint)r17.x;
          r17.x = (int)r17.x & 1023;
          r17.x = (int)r16.x + (int)r17.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.w, r17.x, l(0), t23.xxxx
        r17.w = samp0[]..swiz;
          r17.yz = (int2)r16.yy + int2(1,2);
          r16.y = (int)-r17.y + 6;
          r18.xy = (uint2)r14.wz >> (uint2)r16.yy;
          r16.y = (int)r17.w & 0xc0000000;
          r18.z = (int)r17.w | 0x40000000;
          bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r18.y = (((uint)r18.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r18.x = (((uint)r18.x << 0) & bitmask.x) | ((uint)r18.y & ~bitmask.x);
          r18.x = (int)r18.x * 10;
          r18.x = (uint)r18.z >> (uint)r18.x;
          r18.x = (int)r18.x & 1023;
          r18.x = (int)r17.x + (int)r18.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r18.z, r18.x, l(0), t23.xxxx
        r18.z = samp0[]..swiz;
          r18.y = r17.z;
          r18.xyz = r16.yyy ? r17.xyw : r18.xyz;
          r17.y = (int)-r18.y + 6;
          r17.yz = (uint2)r14.wz >> (uint2)r17.yy;
          r18.y = (int)r18.z & 0xc0000000;
          r18.w = (int)r18.z | 0x40000000;
          bitmask.z = ((~(-1 << 1)) << 1) & 0xffffffff;  r17.z = (((uint)r17.z << 1) & bitmask.z) | ((uint)0 & ~bitmask.z);
          bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r17.y = (((uint)r17.y << 0) & bitmask.y) | ((uint)r17.z & ~bitmask.y);
          r17.y = (int)r17.y * 10;
          r17.y = (uint)r18.w >> (uint)r17.y;
          r17.y = (int)r17.y & 1023;
          r19.x = (int)r17.y + (int)r18.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r19.y, r19.x, l(0), t23.xxxx
        r19.y = samp0[]..swiz;
          r17.yz = r18.yy ? r18.xz : r19.xy;
          r16.xz = r16.yy ? r17.xw : r17.yz;
        }
        r16.y = (int)r16.z & 0xc0000000;
        if (r16.y == 0) {
          if (14 == 0) r17.x = 0; else if (14+15 < 32) {           r17.x = (uint)r16.z << (32-(14 + 15)); r17.x = (uint)r17.x >> (32-14);          } else r17.x = (uint)r16.z >> 15;
          r17.x = (uint)r17.x;
          r17.x = sunConstants.sstLightingConstants.constants.spanInInches * r17.x;
          r17.yz = (int2)r16.zz & int2(32767,536870912);
          r17.y = (uint)r17.y;
          r17.y = sunConstants.sstLightingConstants.constants.spanInInches * r17.y;
          r17.xy = float2(6.10388815e-05,3.05185094e-05) * r17.xy;
          r17.w = (int)r14.z & 3;
          r17.w = (int)r16.x + (int)r17.w;
          r17.w = (int)r17.w + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.w, r17.w, l(0), t23.xxxx
        r17.w = samp0[]..swiz;
          bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r18.x = (((uint)r14.y << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
          bitmask.y = ((~(-1 << 2)) << 1) & 0xffffffff;  r18.y = (((uint)r14.z << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.z = ((~(-1 << 1)) << 4) & 0xffffffff;  r18.z = (((uint)r14.w << 4) & bitmask.z) | ((uint)0 & ~bitmask.z);
          r14.y = (uint)r17.w >> (uint)r18.x;
          r14.y = (int)r14.y & 255;
          r14.y = (uint)r14.y;
          r14.y = r14.y * r17.y;
          r14.y = r14.y * 0.00392156886 + r17.x;
          r14.z = (int)r18.y + 1;
          if (1 == 0) r14.w = 0; else if (1+1 < 32) {           r14.w = (uint)r14.w << (32-(1 + 1)); r14.w = (uint)r14.w >> (32-1);          } else r14.w = (uint)r14.w >> 1;
          r14.z = (int)r14.w + (int)r14.z;
          r14.z = (int)r14.z + (int)r16.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.z, r14.z, l(0), t23.xxxx
        r14.z = samp0[]..swiz;
          r14.z = (uint)r14.z >> (uint)r18.z;
          r14.z = (int)r14.z & 0x0000ffff;
          r14.z = (uint)r14.z;
          r14.z = r14.z * r17.y;
          r14.z = r14.z * 1.52590219e-05 + r17.x;
          r16.w = r17.z ? r14.y : r14.z;
        } else {
          r14.y = (int)r16.z & 0x80000000;
          r14.z = (int)r16.x + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.z, r14.z, l(0), t23.xxxx
        r14.z = samp0[]..swiz;
          r14.y = r14.y ? r14.z : 0;
          r14.z = (uint)r16.z << 2;
          r14.w = (uint)r14.y >> 16;
          r14.y = (int)r14.y & 0x0000ffff;
          r14.yw = f16tof32(r14.yw);
          r14.z = r13.y * r14.w + r14.z;
          r14.y = r13.z * r14.y + r14.z;
          r16.w = r16.y ? r14.y : r16.w;
        }
      }
      r13.w = -sunConstants.sstLightingConstants.constants.inchesPerTexel * 2 + r13.w;
      r13.w = cmp(r16.w < r13.w);
      r6.w = r13.w ? 0 : 1;
    }
    if (r9.w == 0) {
      if (enableDitheredShadow == 0) {
        r9.w = (uint)r7.w;
        r14.yz = -sunConstants.splitPinTransform[r9.w].xy + r13.yz;
        r14.yz = sunConstants.splitPinTransform[r9.w].zz * r14.yz;
        r16.xy = r14.yz * float2(0.5,-0.5) + float2(0.5,0.5);
        r9.w = (int16)sunConstants.splitArrayOffset;
        r16.z = r9.w + r7.w;
        r9.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r16.xyz, r4.w).x;
        r13.w = gTransShadowmapArray.SampleLevel(samp0_s, r16.xyz, 0).x;
        r9.w = r13.w + r9.w;
        r9.w = saturate(-1 + r9.w);
        r13.w = r9.w * r9.w;
        r6.w = r13.w * r9.w;
      }
      if (enableDitheredShadow != 0) {
        r16.x = -r13.x;
        r9.w = (uint)r7.w;
        r7.w = 1 + r7.w;
        r7.w = min(2, r7.w);
        r7.w = (uint)r7.w;
        r8.w = 1 + -r8.w;
        r8.w = 28 * r8.w;
        r8.w = (uint)r8.w;
        r14.yz = -sunConstants.splitPinTransform[r9.w].xy + r13.yz;
        r14.yz = sunConstants.splitPinTransform[r9.w].zz * r14.yz;
        r14.yz = r14.yz * float2(0.5,-0.5) + float2(0.5,0.5);
        r13.yz = -sunConstants.splitPinTransform[r7.w].xy + r13.yz;
        r13.yz = sunConstants.splitPinTransform[r7.w].zz * r13.yz;
        r13.yz = r13.yz * float2(0.5,-0.5) + float2(0.5,0.5);
        r16.y = r14.x;
        r16.z = r13.x;
        r13.w = 0;
        r14.w = 0;
        while (true) {
          r16.w = cmp((uint)r14.w >= 8);
          if (r16.w != 0) break;
          r16.w = cmp((uint)r8.w < (uint)r14.w);
          r17.xy = r16.ww ? r13.yz : r14.yz;
          r17.z = r16.w ? sunConstants.splitPinTransform[r7.w].w : sunConstants.splitPinTransform[r9.w].w;
          r16.w = r16.w ? r7.w : r9.w;
          r18.x = dot(icb[r14.w+0].yx, r16.xy);
          r18.y = dot(icb[r14.w+0].yx, r16.yz);
          r17.xy = r18.xy * r17.zz + r17.xy;
          r16.w = (int)r16.w + (int16)sunConstants.splitArrayOffset;
          r17.z = (uint)r16.w;
          r16.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r17.xyz, r4.w).x;
          r17.x = gTransShadowmapArray.SampleLevel(samp0_s, r17.xyz, 0).x;
          r16.w = r17.x + r16.w;
          r16.w = saturate(-1 + r16.w);
          r13.w = r16.w * 0.125 + r13.w;
          r14.w = (int)r14.w + 1;
        }
        r4.w = r13.w * r13.w;
        r6.w = r4.w * r13.w;
      }
    }
  }
  r4.w = 1 + -r2.y;
  r7.w = 5 * r4.w;
  r8.w = r4.w * 5 + -2.5;
  r8.w = 0.400000006 * r8.w;
  r8.w = max(0, r8.w);
  r8.w = 100 * r8.w;
  r13.yz = -r4.ww * float2(10,5) + float2(6.85740995,7.08500004);
  r9.w = exp2(r13.y);
  r9.w = r9.w * r2.x;
  r9.w = 9.1368103 * r9.w;
  r13.y = r7.w * r4.w;
  r13.y = -r13.y * 2.0159049 + r13.z;
  r13.y = exp2(r13.y);
  r13.y = r13.y * r2.x;
  r13.y = 9.70808983 * r13.y;
  r9.w = max(r13.y, r9.w);
  r9.w = max(1.26815999, r9.w);
  r13.y = numRefProbes + -numOverrideProbes;
  r13.z = (uint)renderTargetSize.x;
  r13.z = (int)r13.z + 7;
  r13.z = (uint)r13.z >> 3;
  r0.y = mad((int)r0.z, (int)r13.z, (int)r0.y);
  bitmask.y = ((~(-1 << 27)) << 5) & 0xffffffff;  r0.y = (((uint)r0.y << 5) & bitmask.y) | ((uint)16 & ~bitmask.y);
  r0.z = (int)r13.y & -32;
  r13.z = (int)-r0.z + (int)r13.y;
  r13.w = numRefProbes & -32;
  r14.y = (int)-r13.w + numRefProbes;
  r16.yz = float2(0,1);
  r17.xy = float2(0,0);
  r18.x = 1;
  r19.xy = float2(0,0);
  r20.xy = float2(0,0);
  r21.xyzw = float4(0,0,0,0);
  r22.yzw = float3(0,0,0);
  r14.z = r0.z;
  while (true) {
    r14.w = cmp((uint)r14.z >= numRefProbes);
    if (r14.w != 0) break;
    r14.w = (uint)r14.z >> 5;
    r14.w = (int)r0.y + (int)r14.w;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.w, r14.w, l(0), t26.xxxx
  r14.w = samp0[]..swiz;
    r16.w = cmp((int)r0.z == (int)r14.z);
    bitmask.w = ((~(-1 << r13.z)) << 0) & 0xffffffff;  r17.w = (((uint)0 << 0) & bitmask.w) | ((uint)r14.w & ~bitmask.w);
    r14.w = r16.w ? r17.w : r14.w;
    r16.w = cmp((int)r13.w == (int)r14.z);
    if (r14.y == 0) r17.w = 0; else if (r14.y+0 < 32) {     r17.w = (uint)r14.w << (32-(r14.y + 0)); r17.w = (uint)r17.w >> (32-r14.y);    } else r17.w = (uint)r14.w >> 0;
    r14.w = r16.w ? r17.w : r14.w;
    r23.xyzw = r21.xyzw;
    r24.xyz = r22.yzw;
    r16.w = r14.w;
    while (true) {
      if (r16.w == 0) break;
      r17.w = firstbitlow((uint)r16.w);
      r18.z = 1 << (int)r17.w;
      r18.z = ~(int)r18.z;
      r16.w = (int)r16.w & (int)r18.z;
      r17.w = (int)r14.z + (int)r17.w;
      r17.w = (int)r17.w * 14;
      r25.xyz = -refProbeConstantsCB[r17.w].data[0].xyz + r11.xyz;
      r18.z = 0x0000ffff & (int)refProbeConstantsCB[r17.w].data[7].z;
      if (6 == 0) r18.w = 0; else if (6+25 < 32) {       r18.w = (uint)refProbeConstantsCB[r17.w].data[7].z << (32-(6 + 25)); r18.w = (uint)r18.w >> (32-6);      } else r18.w = (uint)refProbeConstantsCB[r17.w].data[7].z >> 25;
      r19.w = (int)r18.z * 6;
      r20.w = dot(refProbeAttenuationConstantsCB[r19.w].data[0].xyz, r25.xyz);
      r20.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[0].w + r20.w);
      r25.w = dot(refProbeAttenuationConstantsCB[r19.w].data[1].xyz, r25.xyz);
      r25.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[1].w + r25.w);
      r20.w = r25.w * r20.w;
      r25.w = dot(refProbeAttenuationConstantsCB[r19.w].data[2].xyz, r25.xyz);
      r25.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[2].w + r25.w);
      r20.w = r25.w * r20.w;
      r25.w = dot(refProbeAttenuationConstantsCB[r19.w].data[3].xyz, r25.xyz);
      r25.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[3].w + r25.w);
      r20.w = r25.w * r20.w;
      r25.w = dot(refProbeAttenuationConstantsCB[r19.w].data[4].xyz, r25.xyz);
      r25.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[4].w + r25.w);
      r20.w = r25.w * r20.w;
      r25.w = dot(refProbeAttenuationConstantsCB[r19.w].data[5].xyz, r25.xyz);
      r19.w = saturate(refProbeAttenuationConstantsCB[r19.w].data[5].w + r25.w);
      r16.x = r20.w * r19.w;
      r19.w = 1 & (int)refProbeConstantsCB[r17.w].data[6].x;
      r26.xy = r19.ww ? r16.xy : r16.zx;
      r16.x = refProbeConstantsCB[r17.w].data[6].x;
      r27.xy = r26.xy;
      r19.w = 1;
      while (true) {
        r20.w = cmp((int)r19.w >= (int)r18.w);
        if (r20.w != 0) break;
        r20.w = (int)r18.z + (int)r19.w;
        r20.w = (int)r20.w * 6;
        r25.w = dot(refProbeAttenuationConstantsCB[r20.w].data[0].xyz, r25.xyz);
        r25.w = saturate(refProbeAttenuationConstantsCB[r20.w].data[0].w + r25.w);
        r25.w = r27.x * r25.w;
        r26.z = dot(refProbeAttenuationConstantsCB[r20.w].data[1].xyz, r25.xyz);
        r26.z = saturate(refProbeAttenuationConstantsCB[r20.w].data[1].w + r26.z);
        r25.w = r26.z * r25.w;
        r26.z = dot(refProbeAttenuationConstantsCB[r20.w].data[2].xyz, r25.xyz);
        r26.z = saturate(refProbeAttenuationConstantsCB[r20.w].data[2].w + r26.z);
        r25.w = r26.z * r25.w;
        r26.z = dot(refProbeAttenuationConstantsCB[r20.w].data[3].xyz, r25.xyz);
        r26.z = saturate(refProbeAttenuationConstantsCB[r20.w].data[3].w + r26.z);
        r25.w = r26.z * r25.w;
        r26.z = dot(refProbeAttenuationConstantsCB[r20.w].data[4].xyz, r25.xyz);
        r26.z = saturate(refProbeAttenuationConstantsCB[r20.w].data[4].w + r26.z);
        r25.w = r26.z * r25.w;
        r26.z = dot(refProbeAttenuationConstantsCB[r20.w].data[5].xyz, r25.xyz);
        r20.w = saturate(refProbeAttenuationConstantsCB[r20.w].data[5].w + r26.z);
        r27.x = r25.w * r20.w;
        r26.z = (uint)r16.x >> 2;
        if (1 == 0) r26.w = 0; else if (1+2 < 32) {         r26.w = (uint)r16.x << (32-(1 + 2)); r26.w = (uint)r26.w >> (32-1);        } else r26.w = (uint)r16.x >> 2;
        r27.z = (int)r26.z & 2;
        r27.w = max(r27.y, r27.x);
        r20.w = -r25.w * r20.w + 1;
        r20.w = r27.y * r20.w;
        r18.y = r27.z ? r20.w : r27.w;
        r27.xy = r26.ww ? r27.xy : r18.xy;
        r19.w = (int)r19.w + 1;
        r16.x = r26.z;
      }
      r27.y = saturate(r27.y);
      r16.x = refProbeConstantsCB[r17.w].data[6].y * r27.y;
      r18.y = cmp(0 < r16.x);
      if (r18.y != 0) {
        r24.z = r27.y * refProbeConstantsCB[r17.w].data[6].y + r24.z;
        r16.x = refProbeConstantsCB[r17.w].data[7].y * r16.x;
        r26.x = refProbeConstantsCB[r17.w].data[7].w;
        r26.yz = refProbeConstantsCB[r17.w].data[8].xy;
        r18.y = dot(r15.xyz, r26.xyz);
        r18.z = dot(r25.xyz, r26.xyz);
        r18.z = -refProbeConstantsCB[r17.w].data[8].z + r18.z;
        r18.w = cmp(r18.z >= 0);
        r18.z = max(abs(r18.z), r8.w);
        r18.z = r18.w ? r18.z : -r18.z;
        r18.y = max(1.00000001e-07, -r18.y);
        r18.y = r18.z / r18.y;
        r18.y = min(131072, abs(r18.y));
        r26.x = refProbeConstantsCB[r17.w].data[8].w;
        r26.yz = refProbeConstantsCB[r17.w].data[9].xy;
        r18.z = dot(r15.xyz, r26.xyz);
        r18.w = dot(r25.xyz, r26.xyz);
        r18.w = -refProbeConstantsCB[r17.w].data[9].z + r18.w;
        r19.w = cmp(r18.w >= 0);
        r18.w = max(abs(r18.w), r8.w);
        r18.w = r19.w ? r18.w : -r18.w;
        r18.z = max(1.00000001e-07, -r18.z);
        r18.z = r18.w / r18.z;
        r18.y = min(r18.y, abs(r18.z));
        r26.x = refProbeConstantsCB[r17.w].data[9].w;
        r26.yz = refProbeConstantsCB[r17.w].data[10].xy;
        r18.z = dot(r15.xyz, r26.xyz);
        r18.w = dot(r25.xyz, r26.xyz);
        r18.w = -refProbeConstantsCB[r17.w].data[10].z + r18.w;
        r19.w = cmp(r18.w >= 0);
        r18.w = max(abs(r18.w), r8.w);
        r18.w = r19.w ? r18.w : -r18.w;
        r18.z = max(1.00000001e-07, -r18.z);
        r18.z = r18.w / r18.z;
        r18.y = min(r18.y, abs(r18.z));
        r26.x = refProbeConstantsCB[r17.w].data[10].w;
        r26.yz = refProbeConstantsCB[r17.w].data[11].xy;
        r18.z = dot(r15.xyz, r26.xyz);
        r18.w = dot(r25.xyz, r26.xyz);
        r18.w = -refProbeConstantsCB[r17.w].data[11].z + r18.w;
        r19.w = cmp(r18.w >= 0);
        r18.w = max(abs(r18.w), r8.w);
        r18.w = r19.w ? r18.w : -r18.w;
        r18.z = max(1.00000001e-07, -r18.z);
        r18.z = r18.w / r18.z;
        r18.y = min(r18.y, abs(r18.z));
        r26.x = refProbeConstantsCB[r17.w].data[11].w;
        r26.yz = refProbeConstantsCB[r17.w].data[12].xy;
        r18.z = dot(r15.xyz, r26.xyz);
        r18.w = dot(r25.xyz, r26.xyz);
        r18.w = -refProbeConstantsCB[r17.w].data[12].z + r18.w;
        r19.w = cmp(r18.w >= 0);
        r18.w = max(abs(r18.w), r8.w);
        r18.w = r19.w ? r18.w : -r18.w;
        r18.z = max(1.00000001e-07, -r18.z);
        r18.z = r18.w / r18.z;
        r18.y = min(r18.y, abs(r18.z));
        r26.x = refProbeConstantsCB[r17.w].data[12].w;
        r26.yz = refProbeConstantsCB[r17.w].data[13].xy;
        r18.z = dot(r15.xyz, r26.xyz);
        r18.w = dot(r25.xyz, r26.xyz);
        r18.w = -refProbeConstantsCB[r17.w].data[13].z + r18.w;
        r19.w = cmp(r18.w >= 0);
        r18.w = max(abs(r18.w), r8.w);
        r18.w = r19.w ? r18.w : -r18.w;
        r18.z = max(1.00000001e-07, -r18.z);
        r18.z = r18.w / r18.z;
        r18.y = min(r18.y, abs(r18.z));
        r26.x = refProbeConstantsCB[r17.w].data[3].w + r25.x;
        r26.yz = refProbeConstantsCB[r17.w].data[4].xy + r25.yz;
        r26.xyz = r15.xyz * r18.yyy + r26.xyz;
        r18.z = dot(r26.xyz, r26.xyz);
        r18.z = sqrt(r18.z);
        r18.y = r18.y / r18.z;
        r18.y = r18.y + r18.y;
        r18.y = sqrt(r18.y);
        r18.y = r4.w * 5 + r18.y;
        r18.y = -0.844799995 + r18.y;
        r28.x = refProbeConstantsCB[r17.w].data[0].w;
        r28.y = refProbeConstantsCB[r17.w].data[1].z;
        r28.z = refProbeConstantsCB[r17.w].data[2].y;
        r29.x = dot(r26.xyz, r28.xyz);
        r30.xy = refProbeConstantsCB[r17.w].data[1].xw;
        r30.z = refProbeConstantsCB[r17.w].data[2].z;
        r29.y = dot(r26.xyz, r30.xyz);
        r31.x = refProbeConstantsCB[r17.w].data[1].y;
        r31.yz = refProbeConstantsCB[r17.w].data[2].xw;
        r29.z = dot(r26.xyz, r31.xyz);
        if (9 == 0) r18.z = 0; else if (9+16 < 32) {         r18.z = (uint)refProbeConstantsCB[r17.w].data[7].z << (32-(9 + 16)); r18.z = (uint)r18.z >> (32-9);        } else r18.z = (uint)refProbeConstantsCB[r17.w].data[7].z >> 16;
        r29.w = (uint)r18.z;
        r18.yzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r29.xyzw, r18.y).xyz;
        r26.x = dot(r25.xyz, r28.xyz);
        r26.y = dot(r25.xyz, r30.xyz);
        r26.z = dot(r25.xyz, r31.xyz);
        r25.xyz = saturate(r26.xyz * refProbeConstantsCB[r17.w].data[3].xyz + float3(0.5,0.5,0.5));
        r26.xy = refProbeConstantsCB[r17.w].data[4].zw * r25.xy;
        r26.z = refProbeConstantsCB[r17.w].data[5].x * r25.z;
        r25.xyz = refProbeConstantsCB[r17.w].data[5].yzw + r26.xyz;
        r29.x = dot(r12.xyz, r28.xyz);
        r29.y = dot(r12.xyz, r30.xyz);
        r29.z = dot(r12.xyz, r31.xyz);
        r26.xyz = cmp(float3(0,0,0) < r29.xyz);
        r17.z = r26.x ? 0 : 0.5;
        r27.xzw = r25.xyz + r17.xyz;
        r27.xzw = gProbeXArray.SampleLevel(samplerLinearClamp_s, r27.xzw, 0).xyz;
        r28.xyz = r29.xyz * r29.xyz;
        r28.xyz = r28.xyz * r16.xxx;
        r19.z = r26.y ? 0 : 0.5;
        r26.xyw = r25.xyz + r19.xyz;
        r26.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r26.xyw, 0).xyz;
        r26.xyw = r26.xyw * r28.yyy;
        r26.xyw = r27.xzw * r28.xxx + r26.xyw;
        r20.z = r26.z ? 0 : 0.5;
        r25.xyz = r25.xyz + r20.xyz;
        r25.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r25.xyz, 0).xyz;
        r25.xyz = r25.xyz * r28.zzz + r26.xyw;
        r26.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r29.xyzw, 6).xyz;
        r27.xzw = r26.xyz * r25.xyz;
        r17.z = dot(r27.xzw, float3(0.212599993,0.715200007,0.0722000003));
        r23.xyz = r25.xyz * r26.xyz + r23.xyz;
        r17.w = r17.z * r9.w;
        r18.yzw = r18.yzw * r16.xxx;
        r16.x = dot(r18.yzw, float3(0.212599993,0.715200007,0.0722000003));
        r16.x = r9.w * r17.z + r16.x;
        r16.x = r17.w / r16.x;
        r25.x = r23.w;
        r25.yz = r24.xy;
        r24.xyw = r18.zwy * r16.xxx + r25.yzx;
        r23.w = r24.w;
      }
    }
    r21.xyzw = r23.xyzw;
    r22.yzw = r24.xyz;
    r14.z = (int)r14.z + 32;
  }
  r13.w = cmp(r22.w < 1);
  if (r13.w != 0) {
    r16.yz = float2(0,1);
    r17.xy = float2(0,0);
    r18.x = 1;
    r19.xy = float2(0,0);
    r20.xy = float2(0,0);
    r23.x = r21.w;
    r23.yzw = r22.yzw;
    r14.yzw = r21.xyz;
    r13.w = r22.w;
    r16.w = 0;
    while (true) {
      r17.w = cmp((uint)r16.w >= (uint)r13.y);
      if (r17.w != 0) break;
      r17.w = (uint)r16.w >> 5;
      r17.w = (int)r0.y + (int)r17.w;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r17.w, r17.w, l(0), t26.xxxx
    r17.w = samp0[]..swiz;
      r18.z = cmp((int)r0.z == (int)r16.w);
      if (r13.z == 0) r18.w = 0; else if (r13.z+0 < 32) {       r18.w = (uint)r17.w << (32-(r13.z + 0)); r18.w = (uint)r18.w >> (32-r13.z);      } else r18.w = (uint)r17.w >> 0;
      r17.w = r18.z ? r18.w : r17.w;
      r24.xyzw = r23.xyzw;
      r25.xyz = r14.yzw;
      r18.z = r13.w;
      r18.w = r17.w;
      while (true) {
        if (r18.w == 0) break;
        r19.w = firstbitlow((uint)r18.w);
        r20.w = 1 << (int)r19.w;
        r20.w = ~(int)r20.w;
        r18.w = (int)r18.w & (int)r20.w;
        r19.w = (int)r16.w + (int)r19.w;
        r19.w = (int)r19.w * 14;
        r26.xyz = -refProbeConstantsCB[r19.w].data[0].xyz + r11.xyz;
        r20.w = 0x0000ffff & (int)refProbeConstantsCB[r19.w].data[7].z;
        if (6 == 0) r25.w = 0; else if (6+25 < 32) {         r25.w = (uint)refProbeConstantsCB[r19.w].data[7].z << (32-(6 + 25)); r25.w = (uint)r25.w >> (32-6);        } else r25.w = (uint)refProbeConstantsCB[r19.w].data[7].z >> 25;
        r26.w = (int)r20.w * 6;
        r27.x = dot(refProbeAttenuationConstantsCB[r26.w].data[0].xyz, r26.xyz);
        r27.x = saturate(refProbeAttenuationConstantsCB[r26.w].data[0].w + r27.x);
        r27.y = dot(refProbeAttenuationConstantsCB[r26.w].data[1].xyz, r26.xyz);
        r27.y = saturate(refProbeAttenuationConstantsCB[r26.w].data[1].w + r27.y);
        r27.x = r27.x * r27.y;
        r27.y = dot(refProbeAttenuationConstantsCB[r26.w].data[2].xyz, r26.xyz);
        r27.y = saturate(refProbeAttenuationConstantsCB[r26.w].data[2].w + r27.y);
        r27.x = r27.x * r27.y;
        r27.y = dot(refProbeAttenuationConstantsCB[r26.w].data[3].xyz, r26.xyz);
        r27.y = saturate(refProbeAttenuationConstantsCB[r26.w].data[3].w + r27.y);
        r27.x = r27.x * r27.y;
        r27.y = dot(refProbeAttenuationConstantsCB[r26.w].data[4].xyz, r26.xyz);
        r27.y = saturate(refProbeAttenuationConstantsCB[r26.w].data[4].w + r27.y);
        r27.x = r27.x * r27.y;
        r27.y = dot(refProbeAttenuationConstantsCB[r26.w].data[5].xyz, r26.xyz);
        r26.w = saturate(refProbeAttenuationConstantsCB[r26.w].data[5].w + r27.y);
        r16.x = r27.x * r26.w;
        r26.w = 1 & (int)refProbeConstantsCB[r19.w].data[6].x;
        r27.xy = r26.ww ? r16.xy : r16.zx;
        r16.x = refProbeConstantsCB[r19.w].data[6].x;
        r28.xy = r27.xy;
        r26.w = 1;
        while (true) {
          r27.z = cmp((int)r26.w >= (int)r25.w);
          if (r27.z != 0) break;
          r27.z = (int)r20.w + (int)r26.w;
          r27.z = (int)r27.z * 6;
          r27.w = dot(refProbeAttenuationConstantsCB[r27.z].data[0].xyz, r26.xyz);
          r27.w = saturate(refProbeAttenuationConstantsCB[r27.z].data[0].w + r27.w);
          r27.w = r28.x * r27.w;
          r28.z = dot(refProbeAttenuationConstantsCB[r27.z].data[1].xyz, r26.xyz);
          r28.z = saturate(refProbeAttenuationConstantsCB[r27.z].data[1].w + r28.z);
          r27.w = r28.z * r27.w;
          r28.z = dot(refProbeAttenuationConstantsCB[r27.z].data[2].xyz, r26.xyz);
          r28.z = saturate(refProbeAttenuationConstantsCB[r27.z].data[2].w + r28.z);
          r27.w = r28.z * r27.w;
          r28.z = dot(refProbeAttenuationConstantsCB[r27.z].data[3].xyz, r26.xyz);
          r28.z = saturate(refProbeAttenuationConstantsCB[r27.z].data[3].w + r28.z);
          r27.w = r28.z * r27.w;
          r28.z = dot(refProbeAttenuationConstantsCB[r27.z].data[4].xyz, r26.xyz);
          r28.z = saturate(refProbeAttenuationConstantsCB[r27.z].data[4].w + r28.z);
          r27.w = r28.z * r27.w;
          r28.z = dot(refProbeAttenuationConstantsCB[r27.z].data[5].xyz, r26.xyz);
          r27.z = saturate(refProbeAttenuationConstantsCB[r27.z].data[5].w + r28.z);
          r28.x = r27.w * r27.z;
          r28.z = (uint)r16.x >> 2;
          if (1 == 0) r28.w = 0; else if (1+2 < 32) {           r28.w = (uint)r16.x << (32-(1 + 2)); r28.w = (uint)r28.w >> (32-1);          } else r28.w = (uint)r16.x >> 2;
          r29.x = (int)r28.z & 2;
          r29.y = max(r28.y, r28.x);
          r27.z = -r27.w * r27.z + 1;
          r27.z = r28.y * r27.z;
          r18.y = r29.x ? r27.z : r29.y;
          r28.xy = r28.ww ? r28.xy : r18.xy;
          r26.w = (int)r26.w + 1;
          r16.x = r28.z;
        }
        r16.x = saturate(r28.y + -r18.z);
        r18.y = refProbeConstantsCB[r19.w].data[6].y * r16.x;
        r20.w = cmp(0 < r18.y);
        if (r20.w != 0) {
          r24.w = r16.x * refProbeConstantsCB[r19.w].data[6].y + r24.w;
          r16.x = refProbeConstantsCB[r19.w].data[7].y * r18.y;
          r27.x = refProbeConstantsCB[r19.w].data[7].w;
          r27.yz = refProbeConstantsCB[r19.w].data[8].xy;
          r18.y = dot(r15.xyz, r27.xyz);
          r20.w = dot(r26.xyz, r27.xyz);
          r20.w = -refProbeConstantsCB[r19.w].data[8].z + r20.w;
          r25.w = cmp(r20.w >= 0);
          r20.w = max(abs(r20.w), r8.w);
          r20.w = r25.w ? r20.w : -r20.w;
          r18.y = max(1.00000001e-07, -r18.y);
          r18.y = r20.w / r18.y;
          r18.y = min(131072, abs(r18.y));
          r27.x = refProbeConstantsCB[r19.w].data[8].w;
          r27.yz = refProbeConstantsCB[r19.w].data[9].xy;
          r20.w = dot(r15.xyz, r27.xyz);
          r25.w = dot(r26.xyz, r27.xyz);
          r25.w = -refProbeConstantsCB[r19.w].data[9].z + r25.w;
          r26.w = cmp(r25.w >= 0);
          r25.w = max(abs(r25.w), r8.w);
          r25.w = r26.w ? r25.w : -r25.w;
          r20.w = max(1.00000001e-07, -r20.w);
          r20.w = r25.w / r20.w;
          r18.y = min(abs(r20.w), r18.y);
          r27.x = refProbeConstantsCB[r19.w].data[9].w;
          r27.yz = refProbeConstantsCB[r19.w].data[10].xy;
          r20.w = dot(r15.xyz, r27.xyz);
          r25.w = dot(r26.xyz, r27.xyz);
          r25.w = -refProbeConstantsCB[r19.w].data[10].z + r25.w;
          r26.w = cmp(r25.w >= 0);
          r25.w = max(abs(r25.w), r8.w);
          r25.w = r26.w ? r25.w : -r25.w;
          r20.w = max(1.00000001e-07, -r20.w);
          r20.w = r25.w / r20.w;
          r18.y = min(abs(r20.w), r18.y);
          r27.x = refProbeConstantsCB[r19.w].data[10].w;
          r27.yz = refProbeConstantsCB[r19.w].data[11].xy;
          r20.w = dot(r15.xyz, r27.xyz);
          r25.w = dot(r26.xyz, r27.xyz);
          r25.w = -refProbeConstantsCB[r19.w].data[11].z + r25.w;
          r26.w = cmp(r25.w >= 0);
          r25.w = max(abs(r25.w), r8.w);
          r25.w = r26.w ? r25.w : -r25.w;
          r20.w = max(1.00000001e-07, -r20.w);
          r20.w = r25.w / r20.w;
          r18.y = min(abs(r20.w), r18.y);
          r27.x = refProbeConstantsCB[r19.w].data[11].w;
          r27.yz = refProbeConstantsCB[r19.w].data[12].xy;
          r20.w = dot(r15.xyz, r27.xyz);
          r25.w = dot(r26.xyz, r27.xyz);
          r25.w = -refProbeConstantsCB[r19.w].data[12].z + r25.w;
          r26.w = cmp(r25.w >= 0);
          r25.w = max(abs(r25.w), r8.w);
          r25.w = r26.w ? r25.w : -r25.w;
          r20.w = max(1.00000001e-07, -r20.w);
          r20.w = r25.w / r20.w;
          r18.y = min(abs(r20.w), r18.y);
          r27.x = refProbeConstantsCB[r19.w].data[12].w;
          r27.yz = refProbeConstantsCB[r19.w].data[13].xy;
          r20.w = dot(r15.xyz, r27.xyz);
          r25.w = dot(r26.xyz, r27.xyz);
          r25.w = -refProbeConstantsCB[r19.w].data[13].z + r25.w;
          r26.w = cmp(r25.w >= 0);
          r25.w = max(abs(r25.w), r8.w);
          r25.w = r26.w ? r25.w : -r25.w;
          r20.w = max(1.00000001e-07, -r20.w);
          r20.w = r25.w / r20.w;
          r18.y = min(abs(r20.w), r18.y);
          r27.x = refProbeConstantsCB[r19.w].data[3].w + r26.x;
          r27.yz = refProbeConstantsCB[r19.w].data[4].xy + r26.yz;
          r27.xyz = r15.xyz * r18.yyy + r27.xyz;
          r20.w = dot(r27.xyz, r27.xyz);
          r20.w = sqrt(r20.w);
          r18.y = r18.y / r20.w;
          r18.y = r18.y + r18.y;
          r18.y = sqrt(r18.y);
          r18.y = r4.w * 5 + r18.y;
          r18.y = -0.844799995 + r18.y;
          r29.x = refProbeConstantsCB[r19.w].data[0].w;
          r29.y = refProbeConstantsCB[r19.w].data[1].z;
          r29.z = refProbeConstantsCB[r19.w].data[2].y;
          r30.x = dot(r27.xyz, r29.xyz);
          r31.xy = refProbeConstantsCB[r19.w].data[1].xw;
          r31.z = refProbeConstantsCB[r19.w].data[2].z;
          r30.y = dot(r27.xyz, r31.xyz);
          r32.x = refProbeConstantsCB[r19.w].data[1].y;
          r32.yz = refProbeConstantsCB[r19.w].data[2].xw;
          r30.z = dot(r27.xyz, r32.xyz);
          if (9 == 0) r20.w = 0; else if (9+16 < 32) {           r20.w = (uint)refProbeConstantsCB[r19.w].data[7].z << (32-(9 + 16)); r20.w = (uint)r20.w >> (32-9);          } else r20.w = (uint)refProbeConstantsCB[r19.w].data[7].z >> 16;
          r30.w = (uint)r20.w;
          r27.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r30.xyzw, r18.y).xyz;
          r33.x = dot(r26.xyz, r29.xyz);
          r33.y = dot(r26.xyz, r31.xyz);
          r33.z = dot(r26.xyz, r32.xyz);
          r26.xyz = saturate(r33.xyz * refProbeConstantsCB[r19.w].data[3].xyz + float3(0.5,0.5,0.5));
          r33.xy = refProbeConstantsCB[r19.w].data[4].zw * r26.xy;
          r33.z = refProbeConstantsCB[r19.w].data[5].x * r26.z;
          r26.xyz = refProbeConstantsCB[r19.w].data[5].yzw + r33.xyz;
          r30.x = dot(r12.xyz, r29.xyz);
          r30.y = dot(r12.xyz, r31.xyz);
          r30.z = dot(r12.xyz, r32.xyz);
          r28.xzw = cmp(float3(0,0,0) < r30.xyz);
          r17.z = r28.x ? 0 : 0.5;
          r29.xyz = r26.xyz + r17.xyz;
          r29.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r29.xyz, 0).xyz;
          r31.xyz = r30.xyz * r30.xyz;
          r31.xyz = r31.xyz * r16.xxx;
          r19.z = r28.z ? 0 : 0.5;
          r32.xyz = r26.xyz + r19.xyz;
          r32.xyz = gProbeYArray.SampleLevel(samplerLinearClamp_s, r32.xyz, 0).xyz;
          r32.xyz = r32.xyz * r31.yyy;
          r29.xyz = r29.xyz * r31.xxx + r32.xyz;
          r20.z = r28.w ? 0 : 0.5;
          r26.xyz = r26.xyz + r20.xyz;
          r26.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r26.xyz, 0).xyz;
          r26.xyz = r26.xyz * r31.zzz + r29.xyz;
          r28.xzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r30.xyzw, 6).xyz;
          r29.xyz = r28.xzw * r26.xyz;
          r17.z = dot(r29.xyz, float3(0.212599993,0.715200007,0.0722000003));
          r25.xyz = r26.xyz * r28.xzw + r25.xyz;
          r18.y = r17.z * r9.w;
          r26.xyz = r27.xyz * r16.xxx;
          r16.x = dot(r26.xyz, float3(0.212599993,0.715200007,0.0722000003));
          r16.x = r9.w * r17.z + r16.x;
          r16.x = r18.y / r16.x;
          r24.xyz = r26.xyz * r16.xxx + r24.xyz;
        }
      }
      r23.xyzw = r24.xyzw;
      r14.yzw = r25.xyz;
      r16.w = (int)r16.w + 32;
    }
    r22.xyzw = r23.zxyw;
    r21.xyz = r14.yzw;
    r22.xyzw = r22.yzxw;
  } else {
    r22.x = r21.w;
  }
  r0.y = max(1, r22.w);
  r0.y = rcp(r0.y);
  r22.w = saturate(r22.w);
  r13.yzw = r21.xyz * r0.yyy;
  r14.yzw = r22.xyz * r0.yyy;
  r0.z = cmp(r22.w < 0.99000001);
  if (r0.z != 0) {
    r0.z = 1 + -r22.w;
    r4.w = sunConstants.globalProbeExposure * r0.z;
    r16.xyz = -globalProbeConstants.data[0].xyz + r11.xyz;
    r17.x = globalProbeConstants.data[0].w * r16.x;
    r17.yz = globalProbeConstants.data[1].xy * r16.yz;
    r16.xyz = saturate(float3(0.5,0.5,0.5) + r17.xyz);
    r17.xy = globalProbeConstants.data[1].zw * r16.xy;
    r17.z = globalProbeConstants.data[2].x * r16.z;
    r16.xyz = globalProbeConstants.data[2].yzw + r17.xyz;
    r17.xyz = cmp(float3(0,0,0) < r12.xyz);
    r17.xyz = r17.xyz ? float3(0,0,0) : float3(0.5,0.5,0.5);
    r17.w = 0;
    r18.xyz = r17.wwx + r16.xyz;
    r18.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r18.xyz, 0).xyz;
    r19.xyz = r12.xyz * r12.xyz;
    r19.xyz = r19.xyz * r4.www;
    r20.xyz = r17.wwy + r16.xyz;
    r20.xyz = gProbeYArray.SampleLevel(samplerLinearClamp_s, r20.xyz, 0).xyz;
    r20.xyz = r20.xyz * r19.yyy;
    r18.xyz = r18.xyz * r19.xxx + r20.xyz;
    r16.xyz = r17.wwz + r16.xyz;
    r16.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r16.xyz, 0).xyz;
    r16.xyz = r16.xyz * r19.zzz + r18.xyz;
    r12.w = 0;
    r17.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r12.xyzw, 6).xyz;
    r16.xyz = r17.xyz * r16.xyz;
    r4.w = dot(r16.xyz, float3(0.212599993,0.715200007,0.0722000003));
    r13.yzw = r21.xyz * r0.yyy + r16.xyz;
    r15.w = 0;
    r16.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r15.xyzw, r7.w).xyz;
    r0.y = sunConstants.globalProbeExposure * r0.z + -r4.w;
    r0.y = r2.y * r0.y + r4.w;
    r14.yzw = r16.xyz * r0.yyy + r14.yzw;
  }
  r0.y = r5.z * r2.z + r2.x;
  r0.y = log2(abs(r0.y));
  r0.y = r1.w * r0.y;
  r0.y = exp2(r0.y);
  r0.y = r5.z * r2.z + r0.y;
  r0.y = saturate(-1 + r0.y);
  r16.xyz = r13.yzw * r2.www;
  r17.xyz = r3.xxx ? r16.yzx : 0;
  r2.yz = r2.xy * float2(0.984375,0.984375) + float2(0.0078125,0.0078125);
  r2.yz = gEnvBRDFGeneric.SampleLevel(samplerLinear_s, r2.yz, 0).xy;
  r14.yzw = r14.yzw * r0.yyy;
  r18.xyz = r14.yzw * r2.yyy;
  r19.xyz = r14.yzw * r2.zzz;
  r0.y = sqrt(r1.w);
  r0.y = r0.y * 0.5 + 0.5;
  r0.y = r0.y * r0.y;
  r0.z = 0.5 * r0.y;
  r0.y = -r0.y * 0.5 + 1;
  r2.y = r2.x * r0.y + r0.z;
  r0.yz = r2.yy * r0.yz;
  r2.y = cmp(0 < r3.w);
  if (r2.y != 0) {
    r2.y = cmp(0 < r6.w);
    if (r2.y != 0) {
      r2.y = sunConstants.specScale * r1.z;
      r2.y = r2.y * r3.z;
      if (sunConstants.sunCookieIndex != 0) {
        r20.xyz = eyeOffset.xyz + r11.xyz;
        r20.w = 1;
        r2.z = dot(sunConstants.sunCookieTransform[0].xyzw, r20.xyzw);
        r4.w = dot(sunConstants.sunCookieTransform[1].xyzw, r20.xyzw);
        r20.x = frac(r2.z);
        r20.y = frac(r4.w);
        r2.z = -1 + (int14)sunConstants.sunCookieIndex;
        r20.z = (uint)r2.z;
        r14.yzw = gCookieArray.SampleLevel(samplerLinear_s, r20.xyz, 0).xyz;
        r14.yzw = float3(-1,-1,-1) + r14.yzw;
        r14.yzw = sunConstants.sunCookieIntensity * r14.yzw + float3(1,1,1);
        r14.yzw = sunConstants.color.xyz * r14.yzw;
      } else {
        r14.yzw = sunConstants.color.xyz;
      }
      r2.z = viewmodelSunShadowRaw >> 16;
      r4.w = cmp(0 < (uint)r2.z);
      r4.w = r10.x ? r4.w : 0;
      if (r4.w != 0) {
        r2.z = (int)r2.z + numLights;
        r2.z = mad((int)r2.z, 15, -15);
        r4.w = abs(r3.y) * -0.200000003 + 0.400000006;
        r5.z = cmp(r3.y < 0);
        r5.z = r3.x ? r5.z : 0;
        r4.w = r5.z ? -r4.w : r4.w;
        r20.xyz = r12.xyz * r4.www + r11.xyz;
        r21.xyz = lightConstantsCB[r2.z].data[4].yzw;
        r21.w = lightConstantsCB[r2.z].data[5].x;
        r20.w = 1;
        r21.x = dot(r21.xyzw, r20.xyzw);
        r22.xyz = lightConstantsCB[r2.z].data[5].yzw;
        r22.w = lightConstantsCB[r2.z].data[6].x;
        r21.y = dot(r22.xyzw, r20.xyzw);
        r21.xy = r21.xy * float2(0.5,-0.5) + float2(0.5,0.5);
        r22.x = lightConstantsCB[r2.z].data[9].w + r21.x;
        r22.y = lightConstantsCB[r2.z].data[10].x + r21.y;
        r21.xy = lightConstantsCB[r2.z].data[9].yz * r22.xy;
        r21.zw = lightConstantsCB[r2.z].data[3].yy / lightConstantsCB[r2.z].data[8].yw;
        r22.xy = float2(1,1) + -r21.zw;
        r22.xy = cmp(r21.xy >= r22.xy);
        r21.zw = cmp(r21.zw >= r21.xy);
        r21.zw = (int2)r21.zw | (int2)r22.xy;
        r4.w = (int)r21.w | (int)r21.z;
        if (r4.w == 0) {
          r22.xyz = lightConstantsCB[r2.z].data[6].yzw;
          r22.w = lightConstantsCB[r2.z].data[7].x;
          r4.w = dot(r22.xyzw, r20.xyzw);
          r21.xy = saturate(r21.xy);
          r20.x = r21.x * lightConstantsCB[r2.z].data[8].y + lightConstantsCB[r2.z].data[8].z;
          r20.y = r21.y * lightConstantsCB[r2.z].data[8].w + lightConstantsCB[r2.z].data[9].x;
          r4.w = lightConstantsCB[r2.z].data[10].y + r4.w;
          r4.w = r4.w / lightConstantsCB[r2.z].data[10].z;
          r4.w = max(6.10351563e-05, r4.w);
          r5.z = 0x0000ffff & (int)lightConstantsCB[r2.z].data[1].w;
          if (enableDitheredShadow != 0) {
            r21.x = -r13.x;
            r22.z = (uint)r5.z;
            r21.y = r14.x;
            r21.z = r13.x;
            r7.w = 0;
            r8.w = 0;
            while (true) {
              r9.w = cmp((int)r8.w >= 8);
              if (r9.w != 0) break;
              r23.x = dot(icb[r8.w+0].yx, r21.xy);
              r23.y = dot(icb[r8.w+0].yx, r21.yz);
              r22.xy = r23.xy * lightConstantsCB[r2.z].data[3].yy + r20.xy;
              r9.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r4.w).x;
              r7.w = r9.w * 0.125 + r7.w;
              r8.w = (int)r8.w + 1;
            }
          } else {
            r20.z = (uint)r5.z;
            r7.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r4.w).x;
          }
          r2.z = r7.w * r7.w;
          r2.z = r2.z * r7.w;
        } else {
          r2.z = 1;
        }
        r6.w = r6.w * r2.z;
      } else {
        r2.z = viewmodelSunShadowRaw & 0x0000ffff;
        r4.w = cmp(0 < (uint)r2.z);
        r5.z = ~(int)r10.x;
        r4.w = r4.w ? r5.z : 0;
        if (r4.w != 0) {
          r2.z = (int)r2.z + numLights;
          r2.z = mad((int)r2.z, 15, -15);
          r4.w = abs(r3.y) * -0.200000003 + 0.400000006;
          r5.z = cmp(r3.y < 0);
          r5.z = r3.x ? r5.z : 0;
          r4.w = r5.z ? -r4.w : r4.w;
          r20.xyz = r12.xyz * r4.www + r11.xyz;
          r21.xyz = lightConstantsCB[r2.z].data[4].yzw;
          r21.w = lightConstantsCB[r2.z].data[5].x;
          r20.w = 1;
          r21.x = dot(r21.xyzw, r20.xyzw);
          r22.xyz = lightConstantsCB[r2.z].data[5].yzw;
          r22.w = lightConstantsCB[r2.z].data[6].x;
          r21.y = dot(r22.xyzw, r20.xyzw);
          r21.xy = r21.xy * float2(0.5,-0.5) + float2(0.5,0.5);
          r22.x = lightConstantsCB[r2.z].data[9].w + r21.x;
          r22.y = lightConstantsCB[r2.z].data[10].x + r21.y;
          r21.xy = lightConstantsCB[r2.z].data[9].yz * r22.xy;
          r21.zw = lightConstantsCB[r2.z].data[3].yy / lightConstantsCB[r2.z].data[8].yw;
          r22.xy = float2(1,1) + -r21.zw;
          r22.xy = cmp(r21.xy >= r22.xy);
          r21.zw = cmp(r21.zw >= r21.xy);
          r21.zw = (int2)r21.zw | (int2)r22.xy;
          r4.w = (int)r21.w | (int)r21.z;
          if (r4.w == 0) {
            r22.xyz = lightConstantsCB[r2.z].data[6].yzw;
            r22.w = lightConstantsCB[r2.z].data[7].x;
            r4.w = dot(r22.xyzw, r20.xyzw);
            r21.xy = saturate(r21.xy);
            r20.x = r21.x * lightConstantsCB[r2.z].data[8].y + lightConstantsCB[r2.z].data[8].z;
            r20.y = r21.y * lightConstantsCB[r2.z].data[8].w + lightConstantsCB[r2.z].data[9].x;
            r4.w = lightConstantsCB[r2.z].data[10].y + r4.w;
            r4.w = r4.w / lightConstantsCB[r2.z].data[10].z;
            r4.w = max(6.10351563e-05, r4.w);
            r5.z = 0x0000ffff & (int)lightConstantsCB[r2.z].data[1].w;
            if (enableDitheredShadow != 0) {
              r21.x = -r13.x;
              r22.z = (uint)r5.z;
              r21.y = r14.x;
              r21.z = r13.x;
              r7.w = 0;
              r8.w = 0;
              while (true) {
                r9.w = cmp((int)r8.w >= 8);
                if (r9.w != 0) break;
                r23.x = dot(icb[r8.w+0].yx, r21.xy);
                r23.y = dot(icb[r8.w+0].yx, r21.yz);
                r22.xy = r23.xy * lightConstantsCB[r2.z].data[3].yy + r20.xy;
                r9.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r4.w).x;
                r7.w = r9.w * 0.125 + r7.w;
                r8.w = (int)r8.w + 1;
              }
            } else {
              r20.z = (uint)r5.z;
              r7.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r4.w).x;
            }
            r2.z = r7.w * r7.w;
            r2.z = r2.z * r7.w;
          } else {
            r2.z = 1;
          }
          r6.w = r6.w * r2.z;
        }
      }
      r2.z = -r2.x * 0.5 + 1;
      r2.z = -r3.w * r2.z + 1;
      r2.z = r2.z * r2.z;
      r2.z = -r2.z * 0.620000005 + 0.620000005;
      r2.z = r2.z + -r3.w;
      r2.z = r1.w * r2.z + r3.w;
      r2.z = r2.z * r6.w;
      r20.xyz = r2.zzz * r14.yzw;
      r3.y = cmp(0 < r3.y);
      r20.xyz = r13.yzw * r2.www + r20.xyz;
      r13.yzw = -r11.xyz * r0.www + sunConstants.wldDir.xyz;
      r0.w = dot(r13.yzw, r13.yzw);
      r0.w = rsqrt(r0.w);
      r13.yzw = r13.yzw * r0.www;
      r0.w = dot(r13.yzw, r10.yzw);
      r2.w = dot(r12.xyz, r13.yzw);
      r3.w = abs(r2.w) * r1.z + -abs(r2.w);
      r2.w = abs(r2.w) * r3.w + 1;
      r3.z = r3.z * r0.y + r0.z;
      r2.w = r2.w * r2.w;
      r2.w = r2.w * r3.z;
      r2.w = rcp(r2.w);
      r2.y = r2.w * r2.y;
      r2.y = r6.w * r2.y;
      r2.y = 0.25 * r2.y;
      r21.yzw = r2.yyy * r14.yzw + r19.xyz;
      r0.w = saturate(1 + -r0.w);
      r2.w = r0.w * r0.w;
      r2.w = r2.w * r2.w;
      r0.w = r2.w * r0.w;
      r0.w = r2.y * r0.w;
      r13.yzw = r0.www * r14.yzw + r18.xyz;
      r2.yzw = r2.zzz * r14.yzw + r17.zxy;
      r22.xyz = r3.xxx ? r2.zwy : 0;
      r20.w = r17.z;
      r16.w = r22.z;
      r16.xyzw = r3.yyyy ? r20.xyzw : r16.xyzw;
      r17.zw = r13.yz;
      r22.zw = r18.xy;
      r20.xyzw = r3.yyyy ? r17.xyzw : r22.xyzw;
      r21.x = r13.w;
      r19.w = r18.z;
      r19.xyzw = r3.yyyy ? r21.yzwx : r19.xyzw;
      r17.z = r16.w;
      r17.xy = r20.xy;
      r18.xy = r20.zw;
      r18.z = r19.w;
    }
  }
  r0.x = (uint)r0.x >> 24;
  r0.x = (int)r0.x * 15;
  if (3 == 0) r0.w = 0; else if (3+24 < 32) {   r0.w = (uint)lightConstantsCB[r0.x].data[1].w << (32-(3 + 24)); r0.w = (uint)r0.w >> (32-3);  } else r0.w = (uint)lightConstantsCB[r0.x].data[1].w >> 24;
  switch (r0.w) {
    case 4 :    r0.w = cmp(0 < lightConstantsCB[r0.x].data[6].y);
    r20.xy = lightConstantsCB[r0.x].data[5].zw;
    r20.z = lightConstantsCB[r0.x].data[6].x;
    r2.yzw = -r20.xyz * float3(0.5,0.5,0.5) + lightConstantsCB[r0.x].data[0].xyz;
    r3.yzw = r2.yzw + -r11.xyz;
    r4.w = dot(r20.xyz, r3.yzw);
    r5.z = saturate(-r4.w / lightConstantsCB[r0.x].data[6].y);
    r13.yzw = r5.zzz * r20.xyz + r2.yzw;
    r13.yzw = r0.www ? r13.yzw : lightConstantsCB[r0.x].data[0].xyz;
    r13.yzw = r13.yzw + -r11.xyz;
    r6.w = dot(r13.yzw, r13.yzw);
    r7.w = rsqrt(r6.w);
    r13.yzw = r13.yzw * r7.www;
    r7.w = dot(r12.xyz, r13.yzw);
    r8.w = saturate(r7.w);
    r8.w = r3.x ? abs(r7.w) : r8.w;
    r9.w = cmp(0 < r8.w);
    if (r9.w != 0) {
      r9.w = sqrt(r6.w);
      r12.w = lightConstantsCB[r0.x].data[3].x * lightConstantsCB[r0.x].data[3].x;
      r6.w = r12.w / r6.w;
      r6.w = min(1, r6.w);
      r13.yz = saturate(r9.ww * lightConstantsCB[r0.x].data[2].xz + lightConstantsCB[r0.x].data[2].yw);
      r14.yz = r13.yz * r13.yz;
      r13.yz = r13.yz * float2(-2,-2) + float2(3,3);
      r13.yz = r14.yz * r13.yz;
      r6.w = r13.y * r6.w;
      r6.w = r6.w * r13.z;
      r9.w = cmp(0 < r6.w);
      if (r9.w != 0) {
        if (3 == 0) r9.w = 0; else if (3+27 < 32) {         r9.w = (uint)lightConstantsCB[r0.x].data[1].w << (32-(3 + 27)); r9.w = (uint)r9.w >> (32-3);        } else r9.w = (uint)lightConstantsCB[r0.x].data[1].w >> 27;
        r9.w = cmp((int)r9.w != 1);
        if (r9.w != 0) {
          r13.yz = (uint2)r1.yx;
          r9.w = dot(r13.yz, float2(0.0671105608,0.00583714992));
          r9.w = frac(r9.w);
          r9.w = 52.9829178 * r9.w;
          r9.w = frac(r9.w);
          r9.w = r9.w * 6.28318548 + gameTick.w;
          sincos(r9.w, r21.x, r22.x);
          r9.w = abs(r7.w) * -0.200000003 + 0.400000006;
          r12.w = cmp(r7.w < 0);
          r12.w = r3.x ? r12.w : 0;
          r9.w = r12.w ? -r9.w : r9.w;
          r13.yzw = r12.xyz * r9.www + r11.xyz;
          r13.yzw = -lightConstantsCB[r0.x].data[4].yzw + r13.yzw;
          r9.w = max(abs(r13.z), abs(r13.w));
          r9.w = max(abs(r13.y), r9.w);
          r9.w = lightConstantsCB[r0.x].data[5].x / r9.w;
          r9.w = lightConstantsCB[r0.x].data[5].y + r9.w;
          r12.w = dot(r13.yzw, r13.yzw);
          r12.w = rsqrt(r12.w);
          r9.w = max(6.10351563e-05, r9.w);
          r14.y = 0x0000ffff & (int)lightConstantsCB[r0.x].data[1].w;
          r23.x = -r13.x;
          r24.w = (uint)r14.y;
          r23.y = r14.x;
          r23.z = r13.x;
          r14.yz = float2(0,0);
          while (true) {
            r14.w = cmp((int)r14.z >= 8);
            if (r14.w != 0) break;
            r21.y = dot(icb[r14.z+0].yx, r23.xy);
            r21.z = dot(icb[r14.z+0].yx, r23.yz);
            r25.yz = lightConstantsCB[r0.x].data[3].yy * r21.yz;
            r25.x = r25.y * r22.x;
            r25.w = r25.y * r21.x;
            r24.xyz = r13.yzw * r12.www + r25.xzw;
            r14.w = gOmniShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r24.xyzw, r9.w).x;
            r14.y = r14.w * 0.125 + r14.y;
            r14.z = (int)r14.z + 1;
          }
        } else {
          r14.y = 1;
        }
        r6.w = r14.y * r6.w;
        r9.w = cmp(0 < r6.w);
        if (r9.w != 0) {
          r9.w = lightConstantsCB[r0.x].data[1].z * r1.z;
          r9.w = 0.25 * r9.w;
          r12.w = dot(r20.xyz, r15.xyz);
          r3.y = dot(r15.xyz, r3.yzw);
          r3.z = -r12.w * r12.w + lightConstantsCB[r0.x].data[6].y;
          r3.y = r12.w * r3.y + -r4.w;
          r3.y = saturate(r3.y / r3.z);
          r3.z = r3.z / lightConstantsCB[r0.x].data[6].y;
          r3.z = 10 * r3.z;
          r3.z = min(1, r3.z);
          r3.y = r3.y + -r5.z;
          r3.y = r3.z * r3.y + r5.z;
          r2.yzw = r3.yyy * r20.xyz + r2.yzw;
          r2.yzw = r0.www ? r2.yzw : lightConstantsCB[r0.x].data[0].xyz;
          r2.yzw = r2.yzw + -r11.xyz;
          r0.w = dot(r2.yzw, r2.yzw);
          r0.w = rsqrt(r0.w);
          r3.yzw = r2.yzw * r0.www;
          if (4 == 0) r4.w = 0; else if (4+16 < 32) {           r4.w = (uint)lightConstantsCB[r0.x].data[1].w << (32-(4 + 16)); r4.w = (uint)r4.w >> (32-4);          } else r4.w = (uint)lightConstantsCB[r0.x].data[1].w >> 16;
          r5.z = cmp(0 < (uint)r4.w);
          r5.z = r10.x ? r5.z : 0;
          if (r5.z != 0) {
            r4.w = (int)r4.w + numLights;
            r4.w = mad((int)r4.w, 15, -15);
            r5.z = abs(r7.w) * -0.200000003 + 0.400000006;
            r12.w = cmp(r7.w < 0);
            r12.w = r3.x ? r12.w : 0;
            r5.z = r12.w ? -r5.z : r5.z;
            r15.xyz = r12.xyz * r5.zzz + r11.xyz;
            r20.xyz = lightConstantsCB[r4.w].data[6].yzw;
            r20.w = lightConstantsCB[r4.w].data[7].x;
            r15.w = 1;
            r5.z = dot(r20.xyzw, r15.xyzw);
            r20.xyz = lightConstantsCB[r4.w].data[7].yzw;
            r20.w = lightConstantsCB[r4.w].data[8].x;
            r12.w = dot(r20.xyzw, r15.xyzw);
            r13.y = cmp(r12.w < r5.z);
            if (r13.y == 0) {
              r20.xyz = lightConstantsCB[r4.w].data[4].yzw;
              r20.w = lightConstantsCB[r4.w].data[5].x;
              r20.x = dot(r20.xyzw, r15.xyzw);
              r21.xyz = lightConstantsCB[r4.w].data[5].yzw;
              r21.w = lightConstantsCB[r4.w].data[6].x;
              r20.y = dot(r21.xyzw, r15.xyzw);
              r13.zw = r20.xy / r12.ww;
              r13.zw = r13.zw * float2(0.5,-0.5) + float2(0.5,0.5);
              r15.x = lightConstantsCB[r4.w].data[9].w + r13.z;
              r15.y = lightConstantsCB[r4.w].data[10].x + r13.w;
              r13.zw = lightConstantsCB[r4.w].data[9].yz * r15.xy;
              r14.yz = lightConstantsCB[r4.w].data[3].yy / lightConstantsCB[r4.w].data[8].yw;
              r15.xy = float2(1,1) + -r14.yz;
              r15.xy = cmp(r13.zw >= r15.xy);
              r14.yz = cmp(r14.yz >= r13.zw);
              r14.yz = (int2)r14.yz | (int2)r15.xy;
              r14.y = (int)r14.z | (int)r14.y;
              if (r14.y == 0) {
                r13.zw = saturate(r13.zw);
                r15.x = r13.z * lightConstantsCB[r4.w].data[8].y + lightConstantsCB[r4.w].data[8].z;
                r15.y = r13.w * lightConstantsCB[r4.w].data[8].w + lightConstantsCB[r4.w].data[9].x;
                r13.z = lightConstantsCB[r4.w].data[10].z * r12.w;
                r5.z = lightConstantsCB[r4.w].data[10].y * r12.w + r5.z;
                r5.z = r5.z / r13.z;
              }
            } else {
              r14.y = -1;
            }
            r12.w = (int)r13.y | (int)r14.y;
            if (r12.w == 0) {
              r5.z = max(6.10351563e-05, r5.z);
              r12.w = 0x0000ffff & (int)lightConstantsCB[r4.w].data[1].w;
              if (enableDitheredShadow != 0) {
                r20.x = -r13.x;
                r21.z = (uint)r12.w;
                r20.y = r14.x;
                r20.z = r13.x;
                r13.yz = float2(0,0);
                while (true) {
                  r13.w = cmp((int)r13.z >= 8);
                  if (r13.w != 0) break;
                  r22.x = dot(icb[r13.z+0].yx, r20.xy);
                  r22.y = dot(icb[r13.z+0].yx, r20.yz);
                  r21.xy = r22.xy * lightConstantsCB[r4.w].data[3].yy + r15.xy;
                  r13.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r21.xyz, r5.z).x;
                  r13.y = r13.w * 0.125 + r13.y;
                  r13.z = (int)r13.z + 1;
                }
              } else {
                r15.z = (uint)r12.w;
                r13.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r5.z).x;
              }
              r4.w = r13.y * r13.y;
              r4.w = r4.w * r13.y;
            } else {
              r4.w = 1;
            }
            r6.w = r6.w * r4.w;
          } else {
            if (4 == 0) r4.w = 0; else if (4+20 < 32) {             r4.w = (uint)lightConstantsCB[r0.x].data[1].w << (32-(4 + 20)); r4.w = (uint)r4.w >> (32-4);            } else r4.w = (uint)lightConstantsCB[r0.x].data[1].w >> 20;
            r5.z = cmp(0 < (uint)r4.w);
            r12.w = ~(int)r10.x;
            r5.z = r5.z ? r12.w : 0;
            if (r5.z != 0) {
              r4.w = (int)r4.w + numLights;
              r4.w = mad((int)r4.w, 15, -15);
              r5.z = abs(r7.w) * -0.200000003 + 0.400000006;
              r12.w = cmp(r7.w < 0);
              r12.w = r3.x ? r12.w : 0;
              r5.z = r12.w ? -r5.z : r5.z;
              r15.xyz = r12.xyz * r5.zzz + r11.xyz;
              r20.xyz = lightConstantsCB[r4.w].data[6].yzw;
              r20.w = lightConstantsCB[r4.w].data[7].x;
              r15.w = 1;
              r5.z = dot(r20.xyzw, r15.xyzw);
              r20.xyz = lightConstantsCB[r4.w].data[7].yzw;
              r20.w = lightConstantsCB[r4.w].data[8].x;
              r12.w = dot(r20.xyzw, r15.xyzw);
              r13.y = cmp(r12.w < r5.z);
              if (r13.y == 0) {
                r20.xyz = lightConstantsCB[r4.w].data[4].yzw;
                r20.w = lightConstantsCB[r4.w].data[5].x;
                r20.x = dot(r20.xyzw, r15.xyzw);
                r21.xyz = lightConstantsCB[r4.w].data[5].yzw;
                r21.w = lightConstantsCB[r4.w].data[6].x;
                r20.y = dot(r21.xyzw, r15.xyzw);
                r13.zw = r20.xy / r12.ww;
                r13.zw = r13.zw * float2(0.5,-0.5) + float2(0.5,0.5);
                r15.x = lightConstantsCB[r4.w].data[9].w + r13.z;
                r15.y = lightConstantsCB[r4.w].data[10].x + r13.w;
                r13.zw = lightConstantsCB[r4.w].data[9].yz * r15.xy;
                r14.yz = lightConstantsCB[r4.w].data[3].yy / lightConstantsCB[r4.w].data[8].yw;
                r15.xy = float2(1,1) + -r14.yz;
                r15.xy = cmp(r13.zw >= r15.xy);
                r14.yz = cmp(r14.yz >= r13.zw);
                r14.yz = (int2)r14.yz | (int2)r15.xy;
                r14.y = (int)r14.z | (int)r14.y;
                if (r14.y == 0) {
                  r13.zw = saturate(r13.zw);
                  r15.x = r13.z * lightConstantsCB[r4.w].data[8].y + lightConstantsCB[r4.w].data[8].z;
                  r15.y = r13.w * lightConstantsCB[r4.w].data[8].w + lightConstantsCB[r4.w].data[9].x;
                  r13.z = lightConstantsCB[r4.w].data[10].z * r12.w;
                  r5.z = lightConstantsCB[r4.w].data[10].y * r12.w + r5.z;
                  r5.z = r5.z / r13.z;
                }
              } else {
                r14.y = -1;
              }
              r12.w = (int)r13.y | (int)r14.y;
              if (r12.w == 0) {
                r5.z = max(6.10351563e-05, r5.z);
                r12.w = 0x0000ffff & (int)lightConstantsCB[r4.w].data[1].w;
                if (enableDitheredShadow != 0) {
                  r20.x = -r13.x;
                  r21.z = (uint)r12.w;
                  r20.y = r14.x;
                  r20.z = r13.x;
                  r13.yz = float2(0,0);
                  while (true) {
                    r13.w = cmp((int)r13.z >= 8);
                    if (r13.w != 0) break;
                    r22.x = dot(icb[r13.z+0].yx, r20.xy);
                    r22.y = dot(icb[r13.z+0].yx, r20.yz);
                    r21.xy = r22.xy * lightConstantsCB[r4.w].data[3].yy + r15.xy;
                    r13.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r21.xyz, r5.z).x;
                    r13.y = r13.w * 0.125 + r13.y;
                    r13.z = (int)r13.z + 1;
                  }
                } else {
                  r15.z = (uint)r12.w;
                  r13.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r5.z).x;
                }
                r4.w = r13.y * r13.y;
                r4.w = r4.w * r13.y;
              } else {
                r4.w = 1;
              }
              r6.w = r6.w * r4.w;
            }
          }
          r4.w = -r2.x * 0.5 + 1;
          r4.w = -r8.w * r4.w + 1;
          r4.w = r4.w * r4.w;
          r4.w = -r4.w * 0.620000005 + 0.620000005;
          r4.w = r4.w + -r8.w;
          r4.w = r1.w * r4.w + r8.w;
          r4.w = r4.w * r6.w;
          r15.x = lightConstantsCB[r0.x].data[0].w;
          r15.yz = lightConstantsCB[r0.x].data[1].xy;
          r5.z = cmp(0 < r7.w);
          r13.yzw = r4.www * r15.xyz + r16.xyz;
          r3.y = saturate(dot(r12.xyz, r3.yzw));
          r2.yzw = r2.yzw * r0.www + r10.yzw;
          r0.w = dot(r2.yzw, r2.yzw);
          r0.w = rsqrt(r0.w);
          r2.yzw = r2.yzw * r0.www;
          r0.w = dot(r2.yzw, r10.yzw);
          r2.y = dot(r12.xyz, r2.yzw);
          r2.z = abs(r2.y) * r1.z + -abs(r2.y);
          r2.y = abs(r2.y) * r2.z + 1;
          r2.z = r3.y * r0.y + r0.z;
          r2.y = r2.y * r2.y;
          r2.y = r2.y * r2.z;
          r2.y = rcp(r2.y);
          r2.z = r3.y * r9.w;
          r2.y = r2.y * r2.z;
          r2.y = r2.y * r6.w;
          r3.yzw = r2.yyy * r15.xyz + r19.xyz;
          r0.w = saturate(1 + -r0.w);
          r2.z = r0.w * r0.w;
          r2.z = r2.z * r2.z;
          r0.w = r2.z * r0.w;
          r0.w = r2.y * r0.w;
          r2.yzw = r0.www * r15.xyz + r18.xyz;
          r14.yzw = r4.www * r15.xyz + r17.zxy;
          r14.yzw = r3.xxx ? r14.yzw : r17.zxy;
          r16.xyz = r5.zzz ? r13.yzw : r16.xyz;
          r17.xyz = r5.zzz ? r17.xyz : r14.zwy;
          r18.xyz = r5.zzz ? r2.yzw : r18.xyz;
          r19.xyz = r5.zzz ? r3.yzw : r19.xyz;
        }
      }
    }
    break;
    case 2 :    r15.xy = lightConstantsCB[r0.x].data[3].zw + -r11.xy;
    r15.z = lightConstantsCB[r0.x].data[4].x + -r11.z;
    r0.w = dot(r15.xyz, r15.xyz);
    r0.w = rsqrt(r0.w);
    r2.yzw = r15.xyz * r0.www;
    r2.y = dot(r12.xyz, r2.yzw);
    r2.z = saturate(r2.y);
    r2.w = r3.x ? abs(r2.y) : r2.z;
    r3.y = cmp(0 < r2.w);
    if (r3.y != 0) {
      r20.xyz = lightConstantsCB[r0.x].data[7].yzw;
      r20.w = lightConstantsCB[r0.x].data[8].x;
      r11.w = 1;
      r3.y = dot(r20.xyzw, r11.xyzw);
      r3.z = cmp(r3.y < 1);
      if (r3.z != 0) {
        r13.yzw = float3(1,1,1);
        r3.z = 0;
      } else {
        r14.yzw = lightConstantsCB[r0.x].data[0].xyz + -r11.xyz;
        r3.w = lightConstantsCB[r0.x].data[3].x * lightConstantsCB[r0.x].data[3].x;
        r4.w = dot(r14.yzw, r14.yzw);
        r3.w = r3.w / r4.w;
        r3.w = min(1, r3.w);
        r14.yz = saturate(r3.yy * lightConstantsCB[r0.x].data[2].xz + lightConstantsCB[r0.x].data[2].yw);
        r21.xy = r14.yz * r14.yz;
        r14.yz = r14.yz * float2(-2,-2) + float2(3,3);
        r14.yz = r21.xy * r14.yz;
        r3.w = r14.y * r3.w;
        r3.w = r3.w * r14.z;
        r21.xyz = lightConstantsCB[r0.x].data[4].yzw;
        r21.w = lightConstantsCB[r0.x].data[5].x;
        r21.x = dot(r21.xyzw, r11.xyzw);
        r22.xyz = lightConstantsCB[r0.x].data[5].yzw;
        r22.w = lightConstantsCB[r0.x].data[6].x;
        r21.y = dot(r22.xyzw, r11.xyzw);
        r21.xy = r21.xy / r3.yy;
        r3.y = cmp(lightConstantsCB[r0.x].data[10].w < 0.00048828125);
        if (r3.y != 0) {
          r14.yz = saturate(abs(r21.xy) * lightConstantsCB[r0.x].data[12].xy + lightConstantsCB[r0.x].data[12].zw);
          r22.xy = r14.yz * r14.yz;
          r14.yz = r14.yz * float2(-2,-2) + float2(3,3);
          r14.yz = r22.xy * r14.yz;
          r3.y = r14.y * r14.z;
        } else {
          r22.xyzw = saturate(lightConstantsCB[r0.x].data[11].xyzw * abs(r21.yyxx));
          r22.xyzw = log2(r22.xyzw);
          r22.xyzw = lightConstantsCB[r0.x].data[12].zzzz * r22.xyzw;
          r22.xyzw = exp2(r22.xyzw);
          r14.yz = r22.xy + r22.zw;
          r14.yz = log2(r14.yz);
          r14.yz = lightConstantsCB[r0.x].data[12].ww * r14.yz;
          r14.yz = exp2(r14.yz);
          r4.w = lightConstantsCB[r0.x].data[12].x * r14.y;
          r5.z = lightConstantsCB[r0.x].data[12].y * r14.z + -1;
          r4.w = lightConstantsCB[r0.x].data[12].y * r14.z + -r4.w;
          r4.w = saturate(r5.z / r4.w);
          r5.z = r4.w * r4.w;
          r4.w = r4.w * -2 + 3;
          r3.y = r5.z * r4.w;
        }
        r3.z = r3.w * r3.y;
        r3.y = 255 & (int)lightConstantsCB[r0.x].data[14].w;
        if (r3.y != 0) {
          r21.z = 1;
          r3.w = dot(lightConstantsCB[r0.x].data[13].xyz, r21.xyz);
          r22.x = lightConstantsCB[r0.x].data[13].w;
          r22.yz = lightConstantsCB[r0.x].data[14].xy;
          r4.w = dot(r22.xyz, r21.xyz);
          r21.x = frac(r3.w);
          r21.y = frac(r4.w);
          r3.y = (int)r3.y + -1;
          r21.z = (uint)r3.y;
          r13.yzw = gCookieArray.SampleLevel(samplerLinear_s, r21.xyz, 0).xyz;
        } else {
          r13.yzw = float3(1,1,1);
        }
      }
      r21.x = lightConstantsCB[r0.x].data[0].w;
      r21.yz = lightConstantsCB[r0.x].data[1].xy;
      r13.yzw = r21.xyz * r13.yzw;
      r3.y = cmp(0 < r3.z);
      if (r3.y != 0) {
        if (3 == 0) r3.y = 0; else if (3+27 < 32) {         r3.y = (uint)lightConstantsCB[r0.x].data[1].w << (32-(3 + 27)); r3.y = (uint)r3.y >> (32-3);        } else r3.y = (uint)lightConstantsCB[r0.x].data[1].w >> 27;
        r3.y = cmp((int)r3.y != 1);
        if (r3.y != 0) {
          r3.y = abs(r2.y) * -0.200000003 + 0.400000006;
          r3.w = cmp(r2.y < 0);
          r3.w = r3.w ? r3.x : 0;
          r3.y = r3.w ? -r3.y : r3.y;
          r21.xyz = r12.xyz * r3.yyy + r11.xyz;
          r22.xyz = lightConstantsCB[r0.x].data[6].yzw;
          r22.w = lightConstantsCB[r0.x].data[7].x;
          r21.w = 1;
          r3.y = dot(r22.xyzw, r21.xyzw);
          r3.w = dot(r20.xyzw, r21.xyzw);
          r4.w = cmp(r3.w >= r3.y);
          if (r4.w != 0) {
            r20.xyz = lightConstantsCB[r0.x].data[4].yzw;
            r20.w = lightConstantsCB[r0.x].data[5].x;
            r20.x = dot(r20.xyzw, r21.xyzw);
            r22.xyz = lightConstantsCB[r0.x].data[5].yzw;
            r22.w = lightConstantsCB[r0.x].data[6].x;
            r20.y = dot(r22.xyzw, r21.xyzw);
            r14.yz = r20.xy / r3.ww;
            r14.yz = saturate(r14.yz * float2(0.5,-0.5) + float2(0.5,0.5));
            r20.x = r14.y * lightConstantsCB[r0.x].data[8].y + lightConstantsCB[r0.x].data[8].z;
            r20.y = r14.z * lightConstantsCB[r0.x].data[8].w + lightConstantsCB[r0.x].data[9].x;
            r3.y = r3.y / r3.w;
            r3.y = max(6.10351563e-05, r3.y);
            r3.w = 0x0000ffff & (int)lightConstantsCB[r0.x].data[1].w;
            if (enableDitheredShadow != 0) {
              r21.x = -r13.x;
              r22.z = (uint)r3.w;
              r21.y = r14.x;
              r21.z = r13.x;
              r4.w = 0;
              r5.z = 0;
              while (true) {
                r6.w = cmp((int)r5.z >= 8);
                if (r6.w != 0) break;
                r23.x = dot(icb[r5.z+0].yx, r21.xy);
                r23.y = dot(icb[r5.z+0].yx, r21.yz);
                r22.xy = r23.xy * lightConstantsCB[r0.x].data[3].yy + r20.xy;
                r6.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r3.y).x;
                r4.w = r6.w * 0.125 + r4.w;
                r5.z = (int)r5.z + 1;
              }
            } else {
              r20.z = (uint)r3.w;
              r4.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r3.y).x;
            }
            r3.y = r4.w * r4.w;
            r3.y = r3.y * r4.w;
          } else {
            r3.y = 1;
          }
        } else {
          r3.y = 1;
        }
        r3.y = r3.z * r3.y;
        r3.z = cmp(0 < r3.y);
        if (r3.z != 0) {
          r3.z = lightConstantsCB[r0.x].data[1].z * r1.z;
          r3.z = r3.z * r2.z;
          if (4 == 0) r3.w = 0; else if (4+16 < 32) {           r3.w = (uint)lightConstantsCB[r0.x].data[1].w << (32-(4 + 16)); r3.w = (uint)r3.w >> (32-4);          } else r3.w = (uint)lightConstantsCB[r0.x].data[1].w >> 16;
          r4.w = cmp(0 < (uint)r3.w);
          r4.w = r10.x ? r4.w : 0;
          if (r4.w != 0) {
            r3.w = (int)r3.w + numLights;
            r3.w = mad((int)r3.w, 15, -15);
            r4.w = abs(r2.y) * -0.200000003 + 0.400000006;
            r5.z = cmp(r2.y < 0);
            r5.z = r3.x ? r5.z : 0;
            r4.w = r5.z ? -r4.w : r4.w;
            r20.xyz = r12.xyz * r4.www + r11.xyz;
            r21.xyz = lightConstantsCB[r3.w].data[6].yzw;
            r21.w = lightConstantsCB[r3.w].data[7].x;
            r20.w = 1;
            r4.w = dot(r21.xyzw, r20.xyzw);
            r21.xyz = lightConstantsCB[r3.w].data[7].yzw;
            r21.w = lightConstantsCB[r3.w].data[8].x;
            r5.z = dot(r21.xyzw, r20.xyzw);
            r6.w = cmp(r5.z < r4.w);
            if (r6.w == 0) {
              r21.xyz = lightConstantsCB[r3.w].data[4].yzw;
              r21.w = lightConstantsCB[r3.w].data[5].x;
              r21.x = dot(r21.xyzw, r20.xyzw);
              r22.xyz = lightConstantsCB[r3.w].data[5].yzw;
              r22.w = lightConstantsCB[r3.w].data[6].x;
              r21.y = dot(r22.xyzw, r20.xyzw);
              r14.yz = r21.xy / r5.zz;
              r14.yz = r14.yz * float2(0.5,-0.5) + float2(0.5,0.5);
              r20.x = lightConstantsCB[r3.w].data[9].w + r14.y;
              r20.y = lightConstantsCB[r3.w].data[10].x + r14.z;
              r14.yz = lightConstantsCB[r3.w].data[9].yz * r20.xy;
              r20.xy = lightConstantsCB[r3.w].data[3].yy / lightConstantsCB[r3.w].data[8].yw;
              r20.zw = float2(1,1) + -r20.xy;
              r20.zw = cmp(r14.yz >= r20.zw);
              r20.xy = cmp(r20.xy >= r14.yz);
              r20.xy = (int2)r20.xy | (int2)r20.zw;
              r7.w = (int)r20.y | (int)r20.x;
              if (r7.w == 0) {
                r14.yz = saturate(r14.yz);
                r20.x = r14.y * lightConstantsCB[r3.w].data[8].y + lightConstantsCB[r3.w].data[8].z;
                r20.y = r14.z * lightConstantsCB[r3.w].data[8].w + lightConstantsCB[r3.w].data[9].x;
                r8.w = lightConstantsCB[r3.w].data[10].z * r5.z;
                r4.w = lightConstantsCB[r3.w].data[10].y * r5.z + r4.w;
                r4.w = r4.w / r8.w;
              }
            } else {
              r7.w = -1;
            }
            r5.z = (int)r6.w | (int)r7.w;
            if (r5.z == 0) {
              r4.w = max(6.10351563e-05, r4.w);
              r5.z = 0x0000ffff & (int)lightConstantsCB[r3.w].data[1].w;
              if (enableDitheredShadow != 0) {
                r21.x = -r13.x;
                r22.z = (uint)r5.z;
                r21.y = r14.x;
                r21.z = r13.x;
                r6.w = 0;
                r7.w = 0;
                while (true) {
                  r8.w = cmp((int)r7.w >= 8);
                  if (r8.w != 0) break;
                  r23.x = dot(icb[r7.w+0].yx, r21.xy);
                  r23.y = dot(icb[r7.w+0].yx, r21.yz);
                  r22.xy = r23.xy * lightConstantsCB[r3.w].data[3].yy + r20.xy;
                  r8.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r4.w).x;
                  r6.w = r8.w * 0.125 + r6.w;
                  r7.w = (int)r7.w + 1;
                }
              } else {
                r20.z = (uint)r5.z;
                r6.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r4.w).x;
              }
              r3.w = r6.w * r6.w;
              r3.w = r3.w * r6.w;
            } else {
              r3.w = 1;
            }
            r3.y = r3.y * r3.w;
          } else {
            if (4 == 0) r0.x = 0; else if (4+20 < 32) {             r0.x = (uint)lightConstantsCB[r0.x].data[1].w << (32-(4 + 20)); r0.x = (uint)r0.x >> (32-4);            } else r0.x = (uint)lightConstantsCB[r0.x].data[1].w >> 20;
            r3.w = cmp(0 < (uint)r0.x);
            r4.w = ~(int)r10.x;
            r3.w = r3.w ? r4.w : 0;
            if (r3.w != 0) {
              r0.x = (int)r0.x + numLights;
              r0.x = mad((int)r0.x, 15, -15);
              r3.w = abs(r2.y) * -0.200000003 + 0.400000006;
              r4.w = cmp(r2.y < 0);
              r4.w = r3.x ? r4.w : 0;
              r3.w = r4.w ? -r3.w : r3.w;
              r20.xyz = r12.xyz * r3.www + r11.xyz;
              r21.xyz = lightConstantsCB[r0.x].data[6].yzw;
              r21.w = lightConstantsCB[r0.x].data[7].x;
              r20.w = 1;
              r3.w = dot(r21.xyzw, r20.xyzw);
              r21.xyz = lightConstantsCB[r0.x].data[7].yzw;
              r21.w = lightConstantsCB[r0.x].data[8].x;
              r4.w = dot(r21.xyzw, r20.xyzw);
              r5.z = cmp(r4.w < r3.w);
              if (r5.z == 0) {
                r21.xyz = lightConstantsCB[r0.x].data[4].yzw;
                r21.w = lightConstantsCB[r0.x].data[5].x;
                r21.x = dot(r21.xyzw, r20.xyzw);
                r22.xyz = lightConstantsCB[r0.x].data[5].yzw;
                r22.w = lightConstantsCB[r0.x].data[6].x;
                r21.y = dot(r22.xyzw, r20.xyzw);
                r14.yz = r21.xy / r4.ww;
                r14.yz = r14.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r20.x = lightConstantsCB[r0.x].data[9].w + r14.y;
                r20.y = lightConstantsCB[r0.x].data[10].x + r14.z;
                r14.yz = lightConstantsCB[r0.x].data[9].yz * r20.xy;
                r20.xy = lightConstantsCB[r0.x].data[3].yy / lightConstantsCB[r0.x].data[8].yw;
                r20.zw = float2(1,1) + -r20.xy;
                r20.zw = cmp(r14.yz >= r20.zw);
                r20.xy = cmp(r20.xy >= r14.yz);
                r20.xy = (int2)r20.xy | (int2)r20.zw;
                r6.w = (int)r20.y | (int)r20.x;
                if (r6.w == 0) {
                  r14.yz = saturate(r14.yz);
                  r20.x = r14.y * lightConstantsCB[r0.x].data[8].y + lightConstantsCB[r0.x].data[8].z;
                  r20.y = r14.z * lightConstantsCB[r0.x].data[8].w + lightConstantsCB[r0.x].data[9].x;
                  r7.w = lightConstantsCB[r0.x].data[10].z * r4.w;
                  r3.w = lightConstantsCB[r0.x].data[10].y * r4.w + r3.w;
                  r3.w = r3.w / r7.w;
                }
              } else {
                r6.w = -1;
              }
              r4.w = (int)r5.z | (int)r6.w;
              if (r4.w == 0) {
                r3.w = max(6.10351563e-05, r3.w);
                r4.w = 0x0000ffff & (int)lightConstantsCB[r0.x].data[1].w;
                if (enableDitheredShadow != 0) {
                  r21.x = -r13.x;
                  r22.z = (uint)r4.w;
                  r21.y = r14.x;
                  r21.z = r13.x;
                  r5.z = 0;
                  r6.w = 0;
                  while (true) {
                    r7.w = cmp((int)r6.w >= 8);
                    if (r7.w != 0) break;
                    r14.x = dot(icb[r6.w+0].yx, r21.xy);
                    r14.y = dot(icb[r6.w+0].yx, r21.yz);
                    r22.xy = r14.xy * lightConstantsCB[r0.x].data[3].yy + r20.xy;
                    r7.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r3.w).x;
                    r5.z = r7.w * 0.125 + r5.z;
                    r6.w = (int)r6.w + 1;
                  }
                } else {
                  r20.z = (uint)r4.w;
                  r5.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r20.xyz, r3.w).x;
                }
                r0.x = r5.z * r5.z;
                r0.x = r0.x * r5.z;
              } else {
                r0.x = 1;
              }
              r3.y = r3.y * r0.x;
            }
          }
          r0.x = -r2.x * 0.5 + 1;
          r0.x = -r2.w * r0.x + 1;
          r0.x = r0.x * r0.x;
          r0.x = -r0.x * 0.620000005 + 0.620000005;
          r0.x = r0.x + -r2.w;
          r0.x = r1.w * r0.x + r2.w;
          r0.x = r0.x * r3.y;
          r1.w = cmp(0 < r2.y);
          r2.xyw = r0.xxx * r13.yzw + r16.xyz;
          r14.xyz = r15.xyz * r0.www + r10.yzw;
          r0.w = dot(r14.xyz, r14.xyz);
          r0.w = rsqrt(r0.w);
          r14.xyz = r14.xyz * r0.www;
          r0.w = dot(r14.xyz, r10.yzw);
          r3.w = dot(r12.xyz, r14.xyz);
          r1.z = abs(r3.w) * r1.z + -abs(r3.w);
          r1.z = abs(r3.w) * r1.z + 1;
          r0.y = r2.z * r0.y + r0.z;
          r0.z = r1.z * r1.z;
          r0.y = r0.z * r0.y;
          r0.y = rcp(r0.y);
          r0.y = r0.y * r3.z;
          r0.y = r3.y * r0.y;
          r0.y = 0.25 * r0.y;
          r3.yzw = r0.yyy * r13.yzw + r19.xyz;
          r0.z = saturate(1 + -r0.w);
          r0.w = r0.z * r0.z;
          r0.w = r0.w * r0.w;
          r0.z = r0.z * r0.w;
          r0.y = r0.y * r0.z;
          r0.yzw = r0.yyy * r13.yzw + r18.xyz;
          r10.xyz = r0.xxx * r13.yzw + r17.zxy;
          r10.xyz = r3.xxx ? r10.xyz : r17.zxy;
          r16.xyz = r1.www ? r2.xyw : r16.xyz;
          r17.xyz = r1.www ? r17.xyz : r10.yzx;
          r18.xyz = r1.www ? r0.yzw : r18.xyz;
          r19.xyz = r1.www ? r3.yzw : r19.xyz;
        }
      }
    }
    break;
    default :
    break;
  }
  r0.xyz = r5.xwy * float3(1,3,2) + float3(0,0.5,-1);
  r1.z = (uint)r0.y;
  r2.xyz = r6.zxy * float3(3,1,2) + float3(0.5,0,-1);
  r1.w = (uint)r2.x;
  r1.w = cmp((int)r1.z == (int)r1.w);
  r2.xy = r1.ww ? r2.yz : float2(-1,0);
  r3.yzw = r7.zxy * float3(3,1,2) + float3(0.5,0,-1);
  r1.w = (uint)r3.y;
  r1.w = cmp((int)r1.z == (int)r1.w);
  r2.zw = r1.ww ? r3.zw : float2(-1,0);
  r3.yzw = r8.zxy * float3(3,1,2) + float3(0.5,0,-1);
  r1.w = (uint)r3.y;
  r1.w = cmp((int)r1.z == (int)r1.w);
  r3.yz = r1.ww ? r3.zw : float2(-1,0);
  r5.xyz = r9.zxy * float3(3,1,2) + float3(0.5,0,-1);
  r1.w = (uint)r5.x;
  r1.z = cmp((int)r1.z == (int)r1.w);
  r1.zw = r1.zz ? r5.yz : float2(-1,0);
  r5.xy = r2.xz;
  r5.z = r3.y;
  r5.w = r1.z;
  r5.xyzw = r5.xyzw + -r0.xxxx;
  r5.xyzw = cmp(abs(r5.xyzw) >= float4(0.00902149174,0.00902149174,0.00902149174,0.00902149174));
  r5.xyzw = r5.xyzw ? float4(0,0,0,0) : float4(1,1,1,1);
  r1.z = r5.x + r5.y;
  r1.z = r1.z + r5.z;
  r1.z = r1.z + r5.w;
  r2.x = r5.y * r2.w;
  r2.x = r5.x * r2.y + r2.x;
  r2.x = r5.z * r3.z + r2.x;
  r1.w = r5.w * r1.w + r2.x;
  r0.w = r1.w / r1.z;
  r1.z = cmp(0 >= r1.z);
  r0.yz = r1.zz ? float2(0,0) : r0.zw;
  r1.zw = (int2)r1.xy & int2(1,1);
  r0.w = cmp((int)r1.w == (int)r1.z);
  r0.xyz = r0.www ? r0.xyz : r0.xzy;
  r0.x = -r0.z * 0.5 + r0.x;
  r2.y = r0.z + r0.x;
  r2.z = -r0.y * 0.5 + r0.x;
  r2.x = r2.z + r0.y;
  r0.xyz = r16.xyz * r4.xyz;
  r0.xyz = r17.zxy * r2.xyz + r0.xyz;
  r3.yzw = float3(0.959999979,0.959999979,0.959999979) * r18.xyz;
  r3.yzw = r19.xyz * float3(0.0399999991,0.0399999991,0.0399999991) + r3.yzw;
  r0.xyz = r3.yzw + r0.xyz;
  r3.yzw = float3(1,1,1) + -r2.xyz;
  r3.yzw = r18.xyz * r3.yzw;
  r2.xyz = r19.xyz * r2.xyz + r3.yzw;
  r2.xyz = r16.xyz * r4.xyz + r2.xyz;
  r0.xyz = r3.xxx ? r0.xyz : r2.xyz;
  r0.w = cmp(0 < fogConstants.atmosphereExtinctionIntensity);
  if (r0.w != 0) {
    r0.w = dot(r11.xyz, r11.xyz);
    r1.z = rsqrt(r0.w);
    r2.xyz = r11.xyz * r1.zzz;
    r0.w = sqrt(r0.w);
    r1.z = cmp(0 < fogConstants.blendAmount);
    if (r1.z != 0) {
      r1.zw = r0.ww * fogConstants.atmospherefogdistancedensityscale.xy + fogConstants.atmospherefogdistanceoffset.xy;
      r1.zw = fogConstants.atmospherefogdensityatcamera.xy * r1.zw;
      r2.w = cmp(0.00999999978 < abs(r11.z));
      r3.xy = fogConstants.atmospherefogheightdensityscale.xy * r11.zz;
      r3.zw = float2(-1.44269502,-1.44269502) * r3.xy;
      r3.zw = exp2(r3.zw);
      r3.zw = float2(1,1) + -r3.zw;
      r3.xy = r3.zw / r3.xy;
      r3.xy = r3.xy * r1.zw;
      r1.zw = r2.ww ? r3.xy : r1.zw;
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.zzz;
      r3.xyz = exp2(r3.xyz);
      r4.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.www;
      r4.xyz = exp2(r4.xyz);
      r4.xyz = r4.xyz + -r3.xyz;
      r3.xyz = fogConstants.blendAmount * r4.xyz + r3.xyz;
    } else {
      r1.z = r0.w * fogConstants.atmospherefogdistancedensityscale.x + fogConstants.atmospherefogdistanceoffset.x;
      r1.z = fogConstants.atmospherefogdensityatcamera.x * r1.z;
      r1.w = cmp(0.00999999978 < abs(r11.z));
      r2.w = fogConstants.atmospherefogheightdensityscale.x * r11.z;
      r3.w = -1.44269502 * r2.w;
      r3.w = exp2(r3.w);
      r3.w = 1 + -r3.w;
      r2.w = r3.w / r2.w;
      r2.w = r2.w * r1.z;
      r1.z = r1.w ? r2.w : r1.z;
      r4.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.zzz;
      r3.xyz = exp2(r4.xyz);
    }
    r3.xyz = r3.xyz * fogConstants.atmosphereExtinctionIntensity + float3(1,1,1);
    r3.xyz = saturate(-fogConstants.atmosphereExtinctionIntensity + r3.xyz);
    r1.z = dot(fogConstants.wldSunFogDir.xyz, -r2.xyz);
    r1.w = -fogConstants.atmosphereMieSchlickK * fogConstants.atmosphereMieSchlickK + 1;
    r2.x = fogConstants.atmosphereMieSchlickK * -r1.z + 1;
    r2.x = r2.x * r2.x;
    r2.x = 12.566371 * r2.x;
    r1.w = r1.w / r2.x;
    r0.w = -fogConstants.atmospherehazebasedist + r0.w;
    r0.w = saturate(fogConstants.atmospherehazefadedist * r0.w);
    r0.w = r1.w * r0.w;
    r1.z = saturate(r1.z);
    r1.z = r1.z * r1.z + 1;
    r1.z = r1.z * 0.0596831031 + -1;
    r1.z = fogConstants.atmospheresunstrength * r1.z + 1;
    r2.xyz = fogConstants.atmosphereMieDensity.xyz * r0.www;
    r2.xyz = r1.zzz * fogConstants.atmosphereRayleighDensity.xyz + r2.xyz;
    r2.xyz = fogConstants.atmosphereInScatterIntensity * r2.xyz;
    r4.xyz = float3(1,1,1) + -r3.xyz;
    r2.xyz = r4.xyz * r2.xyz;
    r2.xyz = r0.xyz * r3.xyz + r2.xyz;
  } else {
    r0.w = fogConstants.heightFalloff * r11.z;
    r1.z = fogConstants.heightFalloff * r11.z + fogConstants.K0;
    r1.w = cmp(abs(r0.w) < 9.99999975e-05);
    r2.w = min(64, r1.z);
    r2.w = 1.44269502 * r2.w;
    r2.w = exp2(r2.w);
    r3.x = saturate(fogConstants.K0b);
    r3.y = cmp(r1.z < 0);
    r1.z = 1 + r1.z;
    r1.z = r3.y ? r2.w : r1.z;
    r1.z = -fogConstants.K0b + r1.z;
    r0.w = r1.w ? 1 : r0.w;
    r0.w = r1.z / r0.w;
    r0.w = r1.w ? r3.x : r0.w;
    r0.w = fogConstants.expMul * r0.w;
    r1.z = dot(r11.xyz, r11.xyz);
    r1.w = sqrt(r1.z);
    r0.w = r0.w * r1.w + fogConstants.expAdd;
    r0.w = exp2(r0.w);
    r0.w = min(1, r0.w);
    r0.w = 1 + -r0.w;
    r1.z = rsqrt(r1.z);
    r3.xyz = r11.xyz * r1.zzz;
    r1.z = dot(fogConstants.wldSunFogDir.xyz, r3.xyz);
    r1.z = saturate(fogConstants.sunFogAngles.y * r1.z + fogConstants.sunFogAngles.x);
    r3.xyzw = -fogConstants.sunFogColor.xyzw + fogConstants.fogColor.xyzw;
    r3.xyzw = r1.zzzz * r3.xyzw + fogConstants.sunFogColor.xyzw;
    r0.w = r3.w * r0.w;
    r3.xyz = r3.xyz + -r0.xyz;
    r2.xyz = r0.www * r3.xyz + r0.xyz;
  }
  r0.xyzw = relHDRExposure.yyyy * r2.xyzx;
  r2.xyzw = cmp(r0.wyzw >= float4(6.10351563e-05,6.10351563e-05,6.10351563e-05,6.10351563e-05));
  r0.xyzw = r2.xyzw ? r0.xyzw : 0;
  r0.xyzw = min(float4(65024,65024,64512,65024), r0.xyzw);
// No code for instruction (needs manual fix):
store_uav_typed u0.xyzw, r1.xyyy, r0.xyzw
  return;
}