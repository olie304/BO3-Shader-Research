// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:25:22 2021
...
// unknown dcl_: dcl_input_sgv v7.x, vertex_id
  ...
  o2.xyzw = v2.xxxx ? r5.xyzw : r0.zwzw;
  o4.x = v2.x ? r1.w : 0;
  r0.x = (uint)r0.y;
  r0.y = (uint)r2.y;
  ...
  o3.xyz = -eyeOffset.xyz + r1.xyz;
  r0.x = cmp(0 < useOldHDRScale);
  r0.x = r0.x ? useOldHDRScale : r3.z;
  r0.y = max(0.100000001, r0.x);
  r0.y = 1 / r0.y;
  r1.w = cmp(1 >= r0.y);
  ...
  r3.xzw = float3(-0.0078125,-0.0078125,-0.0078125) + outdoorSSTConstants.constants.dimensionInTiles.xyx;
  r1.xyw = max(float3(0,0,0), r1.xyw);
  r1.xyw = min(r1.xyw, r3.xzw);
  r3.xz = floor(r1.wy);
  r3.x = r3.z * outdoorSSTConstants.constants.dimensionInTiles.x + r3.x;
  r3.x = (uint)r3.x;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r3.z, r3.x, l(0), t0.xxxx
r3.z = outdoorOcclusionTree[]..swiz;
  r3.w = (int)r3.z & 0x40000000;
  r4.z = (uint)r3.z << 2;
  if (r3.w == 0) {
    r3.w = (int)r3.z & 0x01ffffff;
    r5.x = (int)r3.w + (int)r3.x;
    r3.x = (uint)r3.z >> 25;
    r3.x = (uint)r3.x;
  ...
  r6.z = outdoorOcclusionTree[]..swiz;
    ...
    r3.xz = (uint2)r1.wy >> (uint2)r7.yy;
    r3.xz = (int2)r3.xz & int2(1,1);
    r4.w = (int)r6.z & 0xc0000000;
    r5.w = (int)r6.x + 1;
  // Known bad code for instruction (needs manual fix):
  ...
  r7.w = outdoorOcclusionTree[]..swiz;
    r6.xyz = r4.www ? r6.xyz : r7.xzw;
    r3.xzw = r3.www ? r5.xyz : r6.xyz;
    r4.w = (int)r3.w & 0xc0000000;
    if (r4.w == 0) {
      r4.w = (int)-r3.z + 6;
      r5.xy = (uint2)r1.wy >> (uint2)r4.ww;
      r4.w = (int)r3.w | 0x40000000;
      bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r5.y = (((uint)r5.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
      bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r5.x = (((uint)r5.x << 0) & bitmask.x) | ((uint)r5.y & ~bitmask.x);
      r5.x = (int)r5.x * 10;
      r4.w = (uint)r4.w >> (uint)r5.x;
      r4.w = (int)r4.w & 1023;
      r5.x = (int)r3.x + (int)r4.w;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r5.w, r5.x, l(0), t0.xxxx
    r5.w = outdoorOcclusionTree[]..swiz;
      r5.yz = (int2)r3.zz + int2(1,2);
      r3.z = (int)-r5.y + 6;
      r6.xy = (uint2)r1.wy >> (uint2)r3.zz;
      r3.z = (int)r5.w & 0xc0000000;
      r4.w = (int)r5.w | 0x40000000;
      bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r6.y = (((uint)r6.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
      bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r6.x = (((uint)r6.x << 0) & bitmask.x) | ((uint)r6.y & ~bitmask.x);
      r6.x = (int)r6.x * 10;
      r4.w = (uint)r4.w >> (uint)r6.x;
      r4.w = (int)r4.w & 1023;
      r6.x = (int)r4.w + (int)r5.x;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r6.z, r6.x, l(0), t0.xxxx
    r6.z = outdoorOcclusionTree[]..swiz;
      r6.y = r5.z;
      r6.xyz = r3.zzz ? r5.xyw : r6.xyz;
      r4.w = (int)-r6.y + 6;
      r5.yz = (uint2)r1.wy >> (uint2)r4.ww;
      r4.w = (int)r6.z & 0xc0000000;
      r6.y = (int)r6.z | 0x40000000;
      bitmask.z = ((~(-1 << 1)) << 1) & 0xffffffff;  r5.z = (((uint)r5.z << 1) & bitmask.z) | ((uint)0 & ~bitmask.z);
      bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r5.y = (((uint)r5.y << 0) & bitmask.y) | ((uint)r5.z & ~bitmask.y);
      r5.y = (int)r5.y * 10;
      r5.y = (uint)r6.y >> (uint)r5.y;
      r5.y = (int)r5.y & 1023;
      r7.x = (int)r5.y + (int)r6.x;
    // Known bad code for instruction (needs manual fix):
    ...
    r7.y = outdoorOcclusionTree[]..swiz;
      r5.yz = r4.ww ? r6.xz : r7.xy;
      r3.xw = r3.zz ? r5.xw : r5.yz;
    }
    r3.z = (int)r3.w & 0xc0000000;
    if (r3.z == 0) {
      if (14 == 0) r4.w = 0; else if (14+15 < 32) {       r4.w = (uint)r3.w << (32-(14 + 15)); r4.w = (uint)r4.w >> (32-14);      } else r4.w = (uint)r3.w >> 15;
      r4.w = (uint)r4.w;
      r4.w = outdoorSSTConstants.constants.spanInInches * r4.w;
      r4.w = 6.10388815e-05 * r4.w;
      r5.xy = (int2)r3.ww & int2(32767,536870912);
      r5.x = (uint)r5.x;
      r5.x = outdoorSSTConstants.constants.spanInInches * r5.x;
      r5.x = 3.05185094e-05 * r5.x;
      r5.z = (int)r1.y & 3;
      r5.z = (int)r3.x + (int)r5.z;
      r5.z = (int)r5.z + 1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r5.z, r5.z, l(0), t0.xxxx
    r5.z = outdoorOcclusionTree[]..swiz;
      bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r6.x = (((uint)r1.x << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
      bitmask.y = ((~(-1 << 2)) << 1) & 0xffffffff;  r6.y = (((uint)r1.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
      bitmask.z = ((~(-1 << 1)) << 4) & 0xffffffff;  r6.z = (((uint)r1.w << 4) & bitmask.z) | ((uint)0 & ~bitmask.z);
      r1.x = (uint)r5.z >> (uint)r6.x;
      r1.x = (int)r1.x & 255;
      r1.x = (uint)r1.x;
      r1.x = r1.x * r5.x;
      r1.x = r1.x * 0.00392156886 + r4.w;
      r1.y = (int)r6.y + 1;
    ...
    r1.y = outdoorOcclusionTree[]..swiz;
      r1.y = (uint)r1.y >> (uint)r6.z;
      r1.y = (int)r1.y & 0x0000ffff;
      r1.y = (uint)r1.y;
      r1.y = r1.y * r5.x;
      r1.y = r1.y * 1.52590219e-05 + r4.w;
      r4.z = r5.y ? r1.x : r1.y;
    } else {
      r1.x = (int)r3.w & 0x80000000;
      r1.y = (int)r3.x + 1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r1.y, r1.y, l(0), t0.xxxx
    r1.y = outdoorOcclusionTree[]..swiz;
