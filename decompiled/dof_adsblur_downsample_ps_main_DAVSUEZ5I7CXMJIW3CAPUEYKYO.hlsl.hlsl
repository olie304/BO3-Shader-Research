...
Texture2D<float4> sceneDepth : register(t6);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.xyz = sceneColor.Sample(bilinearClamp_s, v0.xy, int2(0, 0)).xyz;
  r1.x = dot(r0.xyz, float3(0.212599993,0.715200007,0.0722000003));
  r1.x = 1 + r1.x;
  r1.x = (int)-r1.x + 0x7ef311c2;
  r0.w = 1;
  r0.xyzw = r1.xxxx * r0.xyzw;
  r1.xyz = sceneColor.Sample(bilinearClamp_s, v0.xy, int2(0, 0)).xyz;
  ...
  r0.xy = float2(4,4) * v1.xy;
  r0.xy = (int2)r0.xy;
  r0.zw = float2(0,0);
  r0.x = sceneStencil.Load(r0.xyz).y;
  r0.x = cmp((int)r0.x != 128);
  r0.x = r0.x ? postFxControl0.z : 0;
  r1.xyzw = sceneDepth.Gather(bilinearClamp_s, v0.xy, int2(-1, -1)).xyzw;
  r2.xyzw = sceneDepth.Gather(bilinearClamp_s, v0.xy, int2(-1, 1)).xyzw;
  ...
  r0.y = saturate(r0.y * postFxControl0.x + postFxControl0.y);
  o0.w = r0.z ? r0.y : r0.x;
  return;
}