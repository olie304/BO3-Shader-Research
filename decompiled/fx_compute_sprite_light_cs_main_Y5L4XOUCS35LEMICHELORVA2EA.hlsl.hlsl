...
// Needs manual fix for instruction: 
// unknown dcl_: dcl_tgsm_raw g0, 8
// Needs manual fix for instruction: 
...
r0.w = samp0[]..swiz;
  ...
  if (r0.y != 0) {
    ...
    r1.w = samp0[]..swiz;
      ...
      r0.y = -sunConstants.splitDepthOffset + r3.z;
      r0.y = -r0.y * 6.10351563e-05 + 1;
      r1.w = (int16)sunConstants.splitArrayOffset + 2;
      r6.z = (uint)r1.w;
      r1.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r6.xyz, r0.y, int2(1, 1)).x;
      r3.x = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r6.xyz, r0.y, int2(1, -1)).x;
      r1.w = r3.x + r1.w;
      r3.x = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r6.xyz, r0.y, int2(-1, 1)).x;
      r1.w = r3.x + r1.w;
      r0.y = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r6.xyz, r0.y, int2(-1, -1)).x;
      r0.y = r1.w + r0.y;
      r1.w = gTransShadowmapArray.SampleLevel(samp0_s, r6.xyz, 0).x;
      r0.y = r0.y * 0.25 + r1.w;
      r5.x = saturate(-1 + r0.y);
    }
    // Needs manual fix for instruction:
    imm_atomic_alloc r3.x, u0
    InterlockedExchange ?(dest, value, orig_value);
    r5.yz = float2(1.40129846e-45,4.34402524e-44);
    [r3.x].76 = u0.xyz;
    [r3.x].0 = u0.x;
    // Needs manual fix for instruction:
    imm_atomic_alloc r3.x, u0
    ...
    InterlockedExchange ?(dest, value, orig_value);
    r3.yz = float2(2.80259693e-45,4.34402524e-44);
    [r5.x].76 = u0.xyz;
    [r5.x].0 = u0.x;
    // Needs manual fix for instruction:
    imm_atomic_alloc r5.x, u0
    ...
    imm_atomic_alloc r5.x, u0
    InterlockedExchange ?(dest, value, orig_value);
    r3.yz = float2(5.60519386e-45,4.34402524e-44);
    [r5.x].76 = u0.xyz;
    [r5.x].0 = u0.x;
    // Needs manual fix for instruction:
    imm_atomic_alloc r5.x, u0
    ...
    imm_atomic_alloc r5.x, u0
    InterlockedExchange ?(dest, value, orig_value);
    r3.yz = float2(1.12103877e-44,4.34402524e-44);
    [r5.x].76 = u0.xyz;
    [r5.x].0 = u0.x;
    // Needs manual fix for instruction:
    imm_atomic_alloc r5.x, u0
    ...
    imm_atomic_alloc r5.x, u0
    InterlockedExchange ?(dest, value, orig_value);
    r3.yz = float2(2.24207754e-44,4.34402524e-44);
    [r5.x].76 = u0.xyz;
    [r5.x].0 = u0.x;
    // Needs manual fix for instruction:
    imm_atomic_alloc r5.x, u0
    InterlockedExchange ?(dest, value, orig_value);
    r0.y = r5.x + r3.x;
    r1.w = cmp((int)r0.x == 32);
    if (r0.x == 0) {
    // No code for instruction (needs manual fix):
        store_raw g0.x, l(0), r0.y
    }
    if (r1.w != 0) {
    // No code for instruction (needs manual fix):
        store_raw g0.x, l(4), r0.y
    }
    GroupMemoryBarrierWithGroupSync();
  // No code for instruction (needs manual fix):
    ld_raw r3.xy, l(0), g0.xyxx
    r0.y = r3.x + r3.y;
    r0.y = 0.015625 * r0.y;
