// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:41:28 2021
...
// unknown dcl_: dcl_uav_typed_texture2darray (uint,uint,uint,uint) u7
  ...
  if (r0.x != 0) {
    ...
    r2.y = dot(r0.xyz, v5.xyz);
    r0.x = dot(r0.xyz, v3.xyz);
    r0.y = cmp(0 < r0.x);
    r0.xz = -r2.xy / r0.xx;
    r0.xy = r0.yy ? r0.xz : 0;
    r0.zw = float2(-0.5,-0.5) + v7.xy;
...
r0.z = colorSampler[]..swiz;
  ...
  r0.xyz = scriptControl.zzz * r0.xyz + r2.xyz;
  ...
  r1.xy = (uint2)v0.xy;
  bitmask.x = ((~(-1 << 1)) << 16) & 0xffffffff;  r2.x = (((uint)zFeatherComputeSprites << 16) & bitmask.x) | ((uint)0 & ~bitmask.x);
  r2.x = (int)r2.x + 1;
  // Needs manual fix for instruction:
imm_atomic_iadd r2.x, u6, r1.xyxx, r2.x
  InterlockedAdd(dest, imm_value, orig_value);
  r1.zw = (int2)r2.xx & int2(65535,65535);
  ...
  if (r2.x != 0) {
    ...
    r2.y = (uint)v0.z << 2;
    bitmask.y = ((~(-1 << 11)) << 0) & 0xffffffff;  r2.y = (((uint)2046 << 0) & bitmask.y) | ((uint)r2.y & ~bitmask.y);
    bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r3.y = (((uint)zFeatherComputeSprites << 0) & bitmask.y) | ((uint)r2.y & ~bitmask.y);
