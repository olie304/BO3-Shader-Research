// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:31:25 2021

cbuffer _Globals : register(b0)
{
  float4 flagParams : packoffset(c0);
  float4 colorObjMin : packoffset(c1);
  float4 colorObjMax : packoffset(c2);
  float colorObjMinBaseBlend : packoffset(c3);
  float colorObjMaxBaseBlend : packoffset(c3.y);
  float2 uvScroll : packoffset(c3.z);
  float4 detailScale : packoffset(c4);
  float4 detailScale1 : packoffset(c5);
  float4 detailScale2 : packoffset(c6);
  float4 detailScale3 : packoffset(c7);
  float4 alphaRevealParms : packoffset(c8);
  float4 colorDetailScale : packoffset(c9);
  float3 specColorTint : packoffset(c10);
  float baseNormalHeight : packoffset(c10.w);
  float2 glossRange : packoffset(c11);
  float2 mixScale : packoffset(c11.z);
  float detailScaleHeight1 : packoffset(c12);
  float detailScaleHeight2 : packoffset(c12.y);
  float4 colorTint : packoffset(c13);
  float4 colorTint1 : packoffset(c14);
  float2 glossRange2 : packoffset(c15);
  float rotateUVs1 : packoffset(c15.z);
  float rotateUVs2 : packoffset(c15.w);
  bool useUVOffsets1 : packoffset(c16);
  bool useUVOffsets2 : packoffset(c16.y);
  float detailScaleHeight3 : packoffset(c16.z);
  float4 colorTint2 : packoffset(c17);
  float2 glossRange3 : packoffset(c18);
  float rotateUVs3 : packoffset(c18.z);
  bool useUVOffsets3 : packoffset(c18.w);
  float4 specColorTint1 : packoffset(c19);
  float4 specColorTint2 : packoffset(c20);
  float2 mixScale1 : packoffset(c21);
  float alphaRevealSoftEdge1 : packoffset(c21.z);
  float alphaRevealRamp1 : packoffset(c21.w);
  float alphaRevealSoftEdge2 : packoffset(c22);
  float alphaRevealRamp2 : packoffset(c22.y);
}

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

SamplerState colorSampler_s : register(s1);
SamplerState specColorSampler_s : register(s2);
SamplerState normalSampler_s : register(s3);
SamplerState aoSampler_s : register(s4);
SamplerState mixSampler_s : register(s5);
Texture2D<float4> colorMap : register(t0);
Texture2D<float4> specColorMap : register(t6);
Texture2D<float4> normalMap : register(t7);
Texture2D<float4> glossMap : register(t9);
Texture2D<float4> aoMap : register(t10);
Texture2D<float4> detailMap1 : register(t11);
Texture2D<float4> detailMap2 : register(t12);
Texture2D<float4> glossMapDetail2 : register(t14);
Texture2D<float4> colorMapDetail2 : register(t15);
Texture2D<float4> detailMap3 : register(t16);
Texture2D<float4> colorMapDetail3 : register(t17);
Texture2D<float4> mixMap : register(t18);
Texture2D<float4> specularMapDetail2 : register(t19);
Texture2D<float4> specularMapDetail3 : register(t20);
Texture2D<float4> mixMap1 : register(t21);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float4 v0 : SV_POSITION0,
  float3 v1 : COLOR0,
  float w1 : COLOR1,
  float2 v2 : TEXCOORD0,
  nointerpolation float3 v3 : UVOFFSETS0,
  float4 v4 : TEXCOORD1,
  float4 v5 : TEXCOORD2,
  float4 v6 : TEXCOORD3,
  float4 v7 : TEXCOORD7,
  uint v8 : SV_IsFrontFace0,
  out float4 o0 : SV_TARGET0,
  out float4 o1 : SV_TARGET1,
  out float4 o2 : SV_TARGET2,
  out float2 o3 : SV_TARGET3)
{
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.xy = mixScale.xy * v2.xy;
  r0.x = mixMap.Sample(mixSampler_s, r0.xy).x;
  r0.yz = saturate(v1.xy * float2(0.998000026,0.998000026) + float2(0.00100000005,0.00100000005));
  r1.xy = float2(1,1) + -r0.yz;
  r0.w = saturate(alphaRevealRamp1);
  r0.yz = log2(r0.yz);
  r0.y = r0.w * r0.y;
  r0.y = exp2(r0.y);
  r2.x = -r0.y;
  r1.zw = log2(r1.xy);
  r0.y = r1.z * r0.w;
  r2.y = exp2(r0.y);
  r0.yw = saturate(alphaRevealSoftEdge1 * r2.xy + r1.xx);
  r0.x = r0.x + -r0.y;
  r0.y = r0.w + -r0.y;
  r0.x = saturate(r0.x / r0.y);
  r0.yw = mixScale1.xy * v2.xy;
  r0.y = mixMap1.Sample(mixSampler_s, r0.yw).x;
  r0.w = saturate(alphaRevealRamp2);
  r0.z = r0.w * r0.z;
  r0.z = exp2(r0.z);
  r2.x = -r0.z;
  r0.z = r0.w * r1.w;
  r2.y = exp2(r0.z);
  r0.zw = saturate(alphaRevealSoftEdge2 * r2.xy + r1.yy);
  r0.y = r0.y + -r0.z;
  r0.z = r0.w + -r0.z;
  r0.y = saturate(r0.y / r0.z);
  r1.xyz = normalMap.Sample(normalSampler_s, v2.xy).xyz;
  r1.xyz = float3(-0.5,-0.5,-0) + r1.xyz;
  r1.xyz = baseNormalHeight * r1.xyz + float3(0.5,0.5,0);
  r0.zw = r1.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r1.x = dot(r0.zw, r0.zw);
  r1.x = 1 + -r1.x;
  r1.x = max(0, r1.x);
  r2.z = sqrt(r1.x);
  r1.x = r1.z * r1.z;
  r1.x = 0.333333343 * r1.x;
  r1.x = min(1, r1.x);
  sincos(rotateUVs1, r3.x, r4.x);
  r5.x = -r3.x;
  r1.yz = float2(-0.5,-0.5) + v2.xy;
  r5.y = r4.x;
  r5.z = r3.x;
  r3.x = dot(r1.yz, r5.yz);
  r3.y = dot(r1.yz, r5.xy);
  r3.zw = v3.yx + r3.xy;
  r1.w = 0.5 + v3.z;
  r3.zw = r3.zw + r1.ww;
  r3.xy = float2(0.5,0.5) + r3.xy;
  r3.xyzw = detailScale1.xyxy * r3.xyzw;
  r3.xy = useUVOffsets1 ? r3.zw : r3.xy;
  r4.xyzw = colorMap.Sample(colorSampler_s, r3.xy).xyzw;
  r5.xyz = float3(-1,-1,-1) + colorTint.xyz;
  r5.xyz = r4.www * r5.xyz + float3(1,1,1);
  r4.xyz = r5.xyz * r4.xyz;
  r2.w = specColorMap.Sample(specColorSampler_s, r3.xy).x;
  r5.xyz = float3(-0.0399999991,-0.0399999991,-0.0399999991) + specColorTint.xyz;
  r5.xyz = r2.www * r5.xyz + float3(0.0399999991,0.0399999991,0.0399999991);
  r3.z = glossMap.Sample(specColorSampler_s, r3.xy).x;
  r3.xyw = detailMap1.Sample(normalSampler_s, r3.xy).xyz;
  r3.xy = r3.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r3.w = r3.w * r3.w;
  r3.w = 0.333333343 * r3.w;
  r3.w = min(1, r3.w);
  sincos(rotateUVs2, r6.x, r7.x);
  r8.x = -r6.x;
  r8.y = r7.x;
  r8.z = r6.x;
  r6.x = dot(r1.yz, r8.yz);
  r6.y = dot(r1.yz, r8.xy);
  r6.zw = v3.yx + r6.xy;
  r6.zw = r6.zw + r1.ww;
  r6.xy = float2(0.5,0.5) + r6.xy;
  r6.xyzw = detailScale2.xyxy * r6.xyzw;
  r6.xy = useUVOffsets1 ? r6.zw : r6.xy;
  r7.xyzw = colorMapDetail2.Sample(colorSampler_s, r6.xy).xyzw;
  r8.xyz = float3(-1,-1,-1) + colorTint1.xyz;
  r8.xyz = r7.www * r8.xyz + float3(1,1,1);
  r4.w = specularMapDetail2.Sample(specColorSampler_s, r6.xy).x;
  r9.xyz = float3(-0.0399999991,-0.0399999991,-0.0399999991) + specColorTint1.xyz;
  r9.xyz = r4.www * r9.xyz + float3(0.0399999991,0.0399999991,0.0399999991);
  r10.xyz = detailMap2.Sample(normalSampler_s, r6.xy).xyz;
  sincos(rotateUVs3, r11.x, r12.x);
  r13.x = -r11.x;
  r13.y = r12.x;
  r13.z = r11.x;
  r11.x = dot(r1.yz, r13.yz);
  r11.y = dot(r1.yz, r13.xy);
  r1.yz = v3.yx + r11.xy;
  r1.yz = r1.yz + r1.ww;
  r1.yz = detailScale3.xy * r1.yz;
  r6.zw = float2(0.5,0.5) + r11.xy;
  r6.zw = detailScale3.xy * r6.zw;
  r1.yz = useUVOffsets3 ? r1.yz : r6.zw;
  r11.xyzw = colorMapDetail3.Sample(colorSampler_s, r1.yz).xyzw;
  r12.xyz = float3(-1,-1,-1) + colorTint2.xyz;
  r12.xyz = r11.www * r12.xyz + float3(1,1,1);
  r13.xyz = detailMap3.Sample(normalSampler_s, r1.yz).xyz;
  r1.y = specularMapDetail3.Sample(specColorSampler_s, r1.yz).x;
  r14.xyz = float3(-0.0399999991,-0.0399999991,-0.0399999991) + specColorTint2.xyz;
  r14.xyz = r1.yyy * r14.xyz + float3(0.0399999991,0.0399999991,0.0399999991);
  r1.z = glossMapDetail2.Sample(specColorSampler_s, r6.xy).x;
  r6.xyz = r7.xyz * r8.xyz + -r4.xyz;
  r4.xyz = r0.xxx * r6.xyz + r4.xyz;
  r6.xyz = r11.xyz * r12.xyz + -r4.xyz;
  r4.xyz = r0.yyy * r6.xyz + r4.xyz;
  r1.w = r4.w + -r2.w;
  r1.w = r0.x * r1.w + r2.w;
  r6.xyz = r9.xyz + -r5.xyz;
  r5.xyz = r0.xxx * r6.xyz + r5.xyz;
  r6.xyz = r14.xyz + -r5.xyz;
  r5.xyz = r0.yyy * r6.xyz + r5.xyz;
  r1.y = r1.y + -r1.w;
  r1.y = r0.y * r1.y + r1.w;
  r1.y = 1 + -r1.y;
  r4.xyz = r4.xyz * r1.yyy;
  r1.y = glossRange.y + -glossRange.x;
  r1.w = r3.z * r1.y + glossRange.x;
  r2.w = glossRange2.y + -glossRange2.x;
  r1.z = r1.z * r2.w + glossRange2.x;
  r1.zw = saturate(float2(0.0588235296,0.0588235296) * r1.zw);
  r2.w = saturate(0.0588235296 * glossRange3.y);
  r1.z = r1.z + -r1.w;
  r1.z = r0.x * r1.z + r1.w;
  r1.w = r2.w + -r1.z;
  r1.z = r0.y * r1.w + r1.z;
  r1.w = aoMap.Sample(aoSampler_s, v2.xy).x;
  r6.xy = r10.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r2.w = r10.z * r10.z;
  r2.w = 0.333333343 * r2.w;
  r2.w = min(1, r2.w);
  r2.xy = r3.xy * detailScaleHeight1 + r0.zw;
  r3.x = dot(r2.xyz, r2.xyz);
  r3.x = rsqrt(r3.x);
  r7.xyz = r3.xxx * r2.xyz;
  r7.w = r3.w * detailScaleHeight1 + r1.x;
  r2.xy = r6.xy * detailScaleHeight2 + r0.zw;
  r3.x = dot(r2.xyz, r2.xyz);
  r3.x = rsqrt(r3.x);
  r3.xyz = r3.xxx * r2.xyz;
  r3.w = r2.w * detailScaleHeight2 + r1.x;
  r6.xy = r13.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r2.w = r13.z * r13.z;
  r2.w = 0.333333343 * r2.w;
  r2.w = min(1, r2.w);
  r2.xy = r6.xy * useUVOffsets1 + r0.zw;
  r0.z = dot(r2.xyz, r2.xyz);
  r0.z = rsqrt(r0.z);
  r6.xyz = r2.xyz * r0.zzz;
  r6.w = r2.w * useUVOffsets1 + r1.x;
  r2.xyzw = r3.xyzw + -r7.xyzw;
  r2.xyzw = r0.xxxx * r2.xyzw + r7.xyzw;
  r3.xyzw = r6.xyzw + -r2.xyzw;
  r0.xyzw = r0.yyyy * r3.xyzw + r2.xyzw;
  r1.x = v8.x ? 1 : -1;
  r2.x = dot(v4.xyz, v4.xyz);
  r2.x = rsqrt(r2.x);
  r2.yzw = v4.xyz * r2.xxx;
  r3.xyz = r2.yzw * r1.xxx;
  r3.w = dot(v5.xyz, v5.xyz);
  r3.w = rsqrt(r3.w);
  r6.xyz = v5.xyz * r3.www;
  r6.xyz = r6.xyz * r1.xxx;
  r3.w = dot(v6.xyz, v6.xyz);
  r3.w = rsqrt(r3.w);
  r7.xyz = v6.xyz * r3.www;
  r7.xyz = r7.xyz * r1.xxx;
  r1.z = -17 * r1.z;
  r1.z = exp2(r1.z);
  r0.w = r1.z + r0.w;
  r0.w = log2(r0.w);
  r0.w = saturate(-0.0588235296 * r0.w);
  r7.xyz = r7.xyz * r0.yyy;
  r6.xyz = r6.xyz * r0.xxx + r7.xyz;
  r0.xyz = r3.xyz * r0.zzz + r6.xyz;
  r1.z = dot(r0.xyz, r0.xyz);
  r1.z = rsqrt(r1.z);
  r0.xyz = r1.zzz * r0.xyz;
  r1.y = debugGlossOverride.x * r1.y + glossRange.x;
  r1.y = saturate(0.0588235296 * r1.y);
  r1.z = cmp(0 < debugStreamerControl.w);
  if (r1.z != 0) {
    r1.z = (int)debugStreamerControl.w;
    r3.z = colorMap.CalculateLevelOfDetailUnclamped(colorSampler_s, v2.xy);
    r3.w = colorMap.CalculateLevelOfDetail(colorSampler_s, v2.xy);
    r3.z = cmp(r3.z < r3.w);
    r3.w = cmp(0 < r3.w);
    r6.xyz = (int3)r1.zzz & int3(1,2,4);
    r7.xyz = r6.xxx ? float3(1,0,1) : r4.xyz;
    r6.xyw = r6.yyy ? float3(0,1,1) : r4.xyz;
    r6.xyw = r3.www ? r7.xyz : r6.xyw;
    r7.xyz = r6.zzz ? float3(0,1,0) : r4.xyz;
    r4.xyz = r3.zzz ? r6.xyw : r7.xyz;
  }
  r6.xyz = debugColorOverride.xyz + -r4.xyz;
  o0.xyz = debugColorOverride.www * r6.xyz + r4.xyz;
  r4.xyz = debugSpecularOverride.xyz + -r5.xyz;
  r4.xyz = debugSpecularOverride.www * r4.xyz + r5.xyz;
  r1.y = r1.y * 1 + -r0.w;
  r0.w = debugGlossOverride.w * r1.y + r0.w;
  r1.y = debugOcclusionOverride.x + -r1.w;
  o2.z = debugOcclusionOverride.w * r1.y + r1.w;
  r1.yzw = v4.xyz * r2.xxx + -r0.xyz;
  r5.yzw = debugNormalOverride.www * r1.yzw + r0.xyz;
  r0.xy = v7.xy / v7.ww;
  r1.yz = float2(0.5,0.5) * renderTargetSize.xy;
  r6.x = r1.y * r0.x;
  r6.y = -r1.z * r0.y;
  r0.xy = renderTargetSize.xy * float2(0.5,0.5) + r6.xy;
  r0.xy = v0.xy + -r0.xy;
  r0.z = r3.x + r3.y;
  r0.z = r2.w * r1.x + r0.z;
  r0.z = 0.5 * r0.z;
  r1.xyz = r2.yzw * r1.xxx + -r0.zzz;
  r1.w = max(r1.y, r1.z);
  r1.w = max(r1.x, r1.w);
  r0.z = max(r1.w, r0.z);
  r1.x = cmp(r0.z == r1.x);
  r2.yzw = float3(1,-1,-1) * r5.yzw;
  r1.w = 2 + r0.z;
  r2.x = 1;
  r5.x = 0;
  r2.xyzw = r1.xxxx ? r2.xyzw : r5.xyzw;
  r0.z = r1.x ? r1.w : r0.z;
  r1.x = cmp(r0.z == r1.y);
  r3.yzw = float3(-1,1,-1) * r2.yzw;
  r1.y = 2 + r0.z;
  r3.x = 2;
  r2.xyzw = r1.xxxx ? r3.xyzw : r2.xyzw;
  r0.z = r1.x ? r1.y : r0.z;
  r0.z = cmp(r0.z == r1.z);
  r1.yzw = float3(-1,-1,1) * r2.yzw;
  r1.x = 3;
  r1.xyzw = r0.zzzz ? r1.xyzw : r2.xyzw;
  r0.z = r1.y + r1.z;
  r2.z = r0.z + r1.w;
  r2.y = r1.w + -r1.y;
  r2.x = -r1.z * 3 + r2.z;
  r1.yzw = float3(0.408248305,0.707106769,0.577350259) * r2.xyz;
  r0.z = 1 + abs(r1.w);
  r0.z = rsqrt(r0.z);
  r1.yz = r1.yz * r0.zz;
  o1.xy = r1.yz * float2(0.588235319,0.588235319) + float2(0.5,0.5);
  o1.z = r0.w * 0.49755621 + 0.00146627566;
  r0.z = (uint)r1.x;
  o1.w = 0.333333343 * r0.z;
  r0.zw = (uint2)v0.xy;
  r1.y = r4.x + -r4.z;
  r1.w = r1.y * 0.5 + r4.z;
  r1.z = r4.y + -r1.w;
  r1.x = r1.z * 0.5 + r1.w;
  r0.zw = (int2)r0.zw & int2(1,1);
  r0.z = cmp((int)r0.w == (int)r0.z);
  r0.zw = r0.zz ? r1.xy : r1.xz;
  o2.xy = r0.zw * float2(1,0.5) + float2(0,0.5);
  r0.zw = float2(-10,-10) + abs(r0.xy);
  r0.zw = saturate(float2(0.0333333351,0.0333333351) * r0.zw);
  r1.xy = float2(0.100000001,0.100000001) * abs(r0.xy);
  r1.xy = min(float2(1,1), r1.xy);
  r1.xy = r1.xy + -r0.zw;
  r0.zw = r1.xy * float2(0.5,0.5) + r0.zw;
  r0.xy = cmp(r0.xy >= float2(0,0));
  o3.xy = r0.xy ? r0.zw : -r0.zw;
  o0.w = 1;
  o2.w = 0.333333343;
  return;
}