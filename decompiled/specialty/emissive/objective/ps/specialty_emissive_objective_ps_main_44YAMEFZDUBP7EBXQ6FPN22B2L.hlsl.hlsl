...
cbuffer _Globals : register(b0)
{
  ...
  uint zFeatherComputeSprites : packoffset(c16.w);
  float hdrScale : packoffset(c17);
  float emissiveFalloffPower : packoffset(c17.y);
  float frameDarken : packoffset(c17.z);
  float warpAmount : packoffset(c17.w);
  float3 emissiveTint1 : packoffset(c18);
  float3 scriptControl : packoffset(c19);
  float3 tint1 : packoffset(c20);
  float3 tint2 : packoffset(c21);
  float3 tint3 : packoffset(c22);
  bool relativeHDR : packoffset(c22.w);
  bool invertFalloff : packoffset(c23);
  float uvMotionToggle1 : packoffset(c23.y);
  float4 scanlineConsts : packoffset(c24);
  float minimumOpacity : packoffset(c25);
}
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.yzw = emissiveMap.Sample(colorSampler_s, r1.xy).xyz;
  r0.yzw = emissiveTint1.xyz * r0.yzw;
  r1.x = log2(r0.x);
  ...
  o0.w = saturate(minimumOpacity + r0.w);
