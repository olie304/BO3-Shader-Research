...
ld_raw r2.z, l(0), g1.xxxx
  ...
  GroupMemoryBarrierWithGroupSync();
  if (r3.y == 0) {
    r0.zw = (uint2)r0.xy >> int2(1,1);
    r1.z = max(r1.x, r1.y);
    r1.z = min(65504, r1.z);
    bitmask.w = ((~(-1 << 2)) << 2) & 0xffffffff;  r1.w = (((uint)r0.w << 2) & bitmask.w) | ((uint)0 & ~bitmask.w);
    bitmask.z = ((~(-1 << 2)) << 0) & 0xffffffff;  r4.z = (((uint)r0.z << 0) & bitmask.z) | ((uint)r1.w & ~bitmask.z);
    bitmask.w = ((~(-1 << 2)) << 0) & 0xffffffff;  r4.w = (((uint)r0.z << 0) & bitmask.w) | ((uint)r1.w & ~bitmask.w);
  // No code for instruction (needs manual fix):
    store_uav_typed u3.xyzw, r0.zwww, r1.zzzz
    r4.xy = (uint2)r0.zw >> int2(2,2);
  // No code for instruction (needs manual fix):
    store_uav_typed u4.xyzw, r4.xyzw, r1.zzzz
  }
  if (r3.z == 0) {
  ...
  GroupMemoryBarrierWithGroupSync();
  if (r3.w == 0) {
    r0.xz = (uint2)r0.xy >> int2(2,2);
    r0.w = max(r1.x, r1.y);
    r0.w = min(65504, r0.w);
    r0.y = (int)r0.y & 12;
    bitmask.z = ((~(-1 << 2)) << 0) & 0xffffffff;  r3.z = (((uint)r0.x << 0) & bitmask.z) | ((uint)r0.y & ~bitmask.z);
    bitmask.w = ((~(-1 << 2)) << 0) & 0xffffffff;  r3.w = (((uint)r0.x << 0) & bitmask.w) | ((uint)r0.y & ~bitmask.w);
  // No code for instruction (needs manual fix):
    store_uav_typed u5.xyzw, r0.xzzz, r0.wwww
    r3.xy = (uint2)r0.xz >> int2(2,2);
  // No code for instruction (needs manual fix):
    store_uav_typed u6.xyzw, r3.xyzw, r0.wwww
  }
  ...
  r0.y = gGBufferNormalAndGloss[]..swiz;
    ...
    r1.x = cmp((int)r2.z == 64);
    r1.y = 0.00100000005 + r0.x;
    r0.y = max(r1.y, r0.y);
    r2.x = r0.w ? 3.40282347e+38 : r0.x;
    r2.y = r1.x ? -r0.y : r0.y;
