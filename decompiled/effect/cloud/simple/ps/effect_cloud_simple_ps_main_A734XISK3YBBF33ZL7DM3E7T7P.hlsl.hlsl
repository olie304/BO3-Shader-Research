// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:24:30 2021
...
// unknown dcl_: dcl_uav_typed_texture2darray (uint,uint,uint,uint) u7
  ...
  r1.xy = rcp(r1.xy);
  r1.z = cmp(featherParms.x == 0.000000);
  r1.w = 1 / featherParms.x;
  r1.z = r1.z ? 60000 : r1.w;
  r1.w = cmp(alphaDissolve != 1.000000);
  r2.x = max(9.99999975e-05, r0.w);
  r2.xyz = r0.xyz / r2.xxx;
  r2.w = v1.w + r0.w;
  r3.y = saturate(r2.w * alphaDissolve + -alphaDissolve);
  r3.xzw = r3.yyy * r2.xyz;
  r0.xyzw = r1.wwww ? r3.xzwy : r0.xyzw;
  r3.x = 1;
  r2.xy = r1.ww ? r3.xy : v1.ww;
  r1.w = cmp(r1.y < r1.x);
  r1.x = r1.x + -r1.y;
  ...
  r1.x = emissiveMap.Sample(colorSampler_s, v2.xy).x;
  r1.x = r1.x * r1.y;
  r1.y = dot(v1.xyz, float3(0.298999995,0.587000012,0.114));
  r1.y = max(9.99999975e-05, r1.y);
  r1.y = max(relHDRExposure.x, r1.y);
  r1.y = hdrScale * r1.y;
  r1.y = v3.w * r1.y;
  r1.y = useOldHDRScale ? hdrScale : r1.y;
  r1.x = r1.y * r1.x;
  r1.xyz = v3.xyz * r1.xxx + v1.xyz;
  ...
  if (r2.x == 0) {
    ...
    if (r3.x != 0) {
      ...
      r0.y = (uint)v0.z << 2;
      r0.x = (int)r0.x & 2046;
      bitmask.y = ((~(-1 << 11)) << 0) & 0xffffffff;  r3.y = (((uint)r0.x << 0) & bitmask.y) | ((uint)r0.y & ~bitmask.y);
