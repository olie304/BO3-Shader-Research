...
// unknown dcl_: dcl_uav_typed_texture2darray (uint,uint,uint,uint) u7
  ...
  if (r1.x == 0) {
    ...
    r1.zw = (int2)r3.xx & int2(65535,65535);
    r3.x = cmp((uint)r1.w < oitMaxEntries);
    if (r3.x != 0) {
      r3.x = max(r2.y, r2.z);
      r3.x = max(r3.x, r2.x);
      ...
      r2.y = (uint)r3.x << 1;
      r3.xzw = (int3)r2.yyy + (int3)r2.xxx;
      r2.x = saturate(r2.w);
      r2.x = 2046 * r2.x;
      r2.x = (uint)r2.x;
      r2.y = (uint)v0.z << 2;
      r2.x = (int)r2.x & 2046;
      bitmask.x = ((~(-1 << 11)) << 0) & 0xffffffff;  r2.x = (((uint)r2.x << 0) & bitmask.x) | ((uint)r2.y & ~bitmask.x);
      bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r3.y = (((uint)zFeatherComputeSprites << 0) & bitmask.y) | ((uint)r2.x & ~bitmask.y);
