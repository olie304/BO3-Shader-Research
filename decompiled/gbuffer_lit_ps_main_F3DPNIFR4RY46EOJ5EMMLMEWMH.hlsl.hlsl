  ...
  out float2 o3 : SV_TARGET3)
{
  ...
  r5.yzw = debugNormalOverride.www * r5.xyz + r2.xyz;
  r0.x = aoMap.Sample(aoSampler_s, r0.xy).x;
  r0.yw = v7.xy / v7.ww;
  r2.xy = float2(0.5,0.5) * renderTargetSize.xy;
  r6.x = r2.x * r0.y;
  r6.y = -r2.y * r0.w;
  r0.yw = renderTargetSize.xy * float2(0.5,0.5) + r6.xy;
  r0.yw = v0.xy + -r0.yw;
  r1.w = r4.x + r4.y;
  r1.w = r3.z * r2.w + r1.w;
  r1.w = 0.5 * r1.w;
  r2.xyz = r3.xyz * r2.www + -r1.www;
  r2.w = max(r2.y, r2.z);
  r2.w = max(r2.x, r2.w);
  r1.w = max(r2.w, r1.w);
  r2.x = cmp(r1.w == r2.x);
  ...
  r0.z = cmp((int)r1.y == (int)r1.x);
  r1.xy = r0.zz ? r3.xy : r3.xz;
  o2.xy = r1.xy * float2(1,0.5) + float2(0,0.5);
  r1.xy = float2(-10,-10) + abs(r0.yw);
  r1.xy = saturate(float2(0.0333333351,0.0333333351) * r1.xy);
  r1.zw = float2(0.100000001,0.100000001) * abs(r0.yw);
  r1.zw = min(float2(1,1), r1.zw);
  r1.zw = r1.zw + -r1.xy;
  r1.xy = r1.zw * float2(0.5,0.5) + r1.xy;
  r0.yz = cmp(r0.yw >= float2(0,0));
  o3.xy = r0.yz ? r1.xy : -r1.xy;
  o0.w = 1;
  o2.z = r0.x;
  o2.w = 0.333333343;
  return;
