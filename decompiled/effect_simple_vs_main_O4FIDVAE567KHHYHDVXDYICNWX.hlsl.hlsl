// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:25:38 2021
...
// unknown dcl_: dcl_resource_structured t0, 4
  ...
  r1.xy = outdoorSSTConstants.offToPinTransform._m10_m11 * v0.yy;
  r1.xy = v0.xx * outdoorSSTConstants.offToPinTransform._m00_m01 + r1.xy;
  r1.xy = v0.zz * outdoorSSTConstants.offToPinTransform._m20_m21 + r1.xy;
  r1.xy = outdoorSSTConstants.offToPinTransform._m30_m31 + r1.xy;
  r2.xyz = float3(128,0.5,0.5) * outdoorSSTConstants.constants.inchesPerTexel;
  r3.xz = rcp(r2.xx);
  r3.y = -r3.z;
  r2.xyz = r1.xyx * r3.xyz + r2.yzy;
  r3.xyz = float3(-0.0078125,-0.0078125,-0.0078125) + outdoorSSTConstants.constants.dimensionInTiles.xyx;
  r2.xyz = max(float3(0,0,0), r2.xyz);
  r2.xyz = min(r2.xyz, r3.xyz);
  r3.xy = floor(r2.zy);
  r0.w = r3.y * outdoorSSTConstants.constants.dimensionInTiles.x + r3.x;
  r0.w = (uint)r0.w;
...
r1.w = outdoorOcclusionTree[]..swiz;
  ...
  r3.z = outdoorOcclusionTree[]..swiz;
    r4.xy = (uint2)r2.zy >> int2(6,6);
    r0.w = (int)r3.z & 0xc0000000;
    r1.w = (int)r3.x + 1;
  // Known bad code for instruction (needs manual fix):
  ...
  r4.z = outdoorOcclusionTree[]..swiz;
    ...
    r5.yz = r0.ww ? float2(8.40779079e-45,1.40129846e-45) : float2(7.00649232e-45,2.80259693e-45);
    r6.xy = (uint2)r2.zy >> (uint2)r5.yy;
    r6.xy = (int2)r6.xy & int2(1,1);
    r1.w = (int)r4.z & 0xc0000000;
    r2.w = (int)r4.x + 1;
  // Known bad code for instruction (needs manual fix):
  ...
  r5.w = outdoorOcclusionTree[]..swiz;
    ...
    if (r0.w == 0) {
      r0.w = (int)-r3.y + 6;
      r4.xy = (uint2)r2.zy >> (uint2)r0.ww;
      r0.w = (int)r3.z | 0x40000000;
      bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r1.w = (((uint)r4.y << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
      bitmask.w = ((~(-1 << 1)) << 0) & 0xffffffff;  r1.w = (((uint)r4.x << 0) & bitmask.w) | ((uint)r1.w & ~bitmask.w);
      r1.w = (int)r1.w * 10;
      r0.w = (uint)r0.w >> (uint)r1.w;
      r0.w = (int)r0.w & 1023;
      r4.x = (int)r0.w + (int)r3.x;
    // Known bad code for instruction (needs manual fix):
    ...
    r4.w = outdoorOcclusionTree[]..swiz;
      r4.yz = (int2)r3.yy + int2(1,2);
      r0.w = (int)-r4.y + 6;
      r3.yw = (uint2)r2.zy >> (uint2)r0.ww;
      r0.w = (int)r4.w & 0xc0000000;
      r1.w = (int)r4.w | 0x40000000;
      bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r2.w = (((uint)r3.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
      bitmask.w = ((~(-1 << 1)) << 0) & 0xffffffff;  r2.w = (((uint)r3.y << 0) & bitmask.w) | ((uint)r2.w & ~bitmask.w);
      r2.w = (int)r2.w * 10;
    ...
    r5.z = outdoorOcclusionTree[]..swiz;
      r5.y = r4.z;
      r5.xyz = r0.www ? r4.xyw : r5.xyz;
      r1.w = (int)-r5.y + 6;
      r3.yw = (uint2)r2.zy >> (uint2)r1.ww;
      r1.w = (int)r5.z & 0xc0000000;
      r2.w = (int)r5.z | 0x40000000;
      bitmask.w = ((~(-1 << 1)) << 1) & 0xffffffff;  r3.w = (((uint)r3.w << 1) & bitmask.w) | ((uint)0 & ~bitmask.w);
      bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r3.y = (((uint)r3.y << 0) & bitmask.y) | ((uint)r3.w & ~bitmask.y);
      r3.y = (int)r3.y * 10;
      r2.w = (uint)r2.w >> (uint)r3.y;
      r2.w = (int)r2.w & 1023;
      r6.x = (int)r2.w + (int)r5.x;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r6.y, r6.x, l(0), t0.xxxx
    r6.y = outdoorOcclusionTree[]..swiz;
      r3.yw = r1.ww ? r5.xz : r6.xy;
      r3.xz = r0.ww ? r4.xw : r3.yw;
    }
    r0.w = (int)r3.z & 0xc0000000;
    if (r0.w == 0) {
      if (14 == 0) r1.w = 0; else if (14+15 < 32) {       r1.w = (uint)r3.z << (32-(14 + 15)); r1.w = (uint)r1.w >> (32-14);      } else r1.w = (uint)r3.z >> 15;
      r1.w = (uint)r1.w;
      r1.w = outdoorSSTConstants.constants.spanInInches * r1.w;
      r1.w = 6.10388815e-05 * r1.w;
      r3.yw = (int2)r3.zz & int2(32767,536870912);
      r2.w = (uint)r3.y;
      r2.w = outdoorSSTConstants.constants.spanInInches * r2.w;
      r2.w = 3.05185094e-05 * r2.w;
      r3.y = (int)r2.y & 3;
      r3.y = (int)r3.y + (int)r3.x;
      r3.y = (int)r3.y + 1;
    // Known bad code for instruction (needs manual fix):
    ...
    r3.y = outdoorOcclusionTree[]..swiz;
      bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r4.x = (((uint)r2.x << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
      bitmask.y = ((~(-1 << 2)) << 1) & 0xffffffff;  r4.y = (((uint)r2.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
      bitmask.z = ((~(-1 << 1)) << 4) & 0xffffffff;  r4.z = (((uint)r2.z << 4) & bitmask.z) | ((uint)0 & ~bitmask.z);
      r2.x = (uint)r3.y >> (uint)r4.x;
      r2.x = (int)r2.x & 255;
      r2.x = (uint)r2.x;
      r2.x = r2.x * r2.w;
      r2.x = r2.x * 0.00392156886 + r1.w;
      r2.y = (int)r4.y + 1;
      if (1 == 0) r2.z = 0; else if (1+1 < 32) {       r2.z = (uint)r2.z << (32-(1 + 1)); r2.z = (uint)r2.z >> (32-1);      } else r2.z = (uint)r2.z >> 1;
    ...
    r2.y = outdoorOcclusionTree[]..swiz;
      r2.y = (uint)r2.y >> (uint)r4.z;
      r2.y = (int)r2.y & 0x0000ffff;
      r2.y = (uint)r2.y;
      r2.y = r2.y * r2.w;
      r1.w = r2.y * 1.52590219e-05 + r1.w;
      r1.z = r3.w ? r2.x : r1.w;
    } else {
      r1.w = (int)r3.z & 0x80000000;
      r2.x = (int)r3.x + 1;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r2.x, r2.x, l(0), t0.xxxx
    r2.x = outdoorOcclusionTree[]..swiz;
      r1.w = r1.w ? r2.x : 0;
      r2.x = (uint)r3.z << 2;
      r2.y = (uint)r1.w >> 16;
      r2.y = f16tof32(r2.y);
  ...
  r0.w = dot(r1.xyz, pinToWorldZ.xyz);
