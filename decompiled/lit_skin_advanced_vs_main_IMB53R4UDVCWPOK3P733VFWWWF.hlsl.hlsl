// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:38:43 2021

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

Texture2D<float4> gpuSkinBase : register(t1);
Texture2D<float4> gpuSkinQuat : register(t2);
Texture2D<float4> gpuSkinPos : register(t3);
StructuredBuffer<modelInstanceBuffer> modelInstanceBuffer : register(t4);
StructuredBuffer<boneMatrixBuffer> boneMatrixBuffer : register(t5);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float3 v0 : POSITION0,
  float4 v1 : COLOR0,
  float2 v2 : TEXCOORD0,
  float3 v3 : NORMAL0,
  float4 v4 : TANGENT0,
  float4 v5 : BLENDWEIGHT0,
  uint4 v6 : BLENDINDICES0,
  uint v7 : TEXCOORD15,
  out float4 o0 : SV_POSITION0,
  out float o1 : COLOR1,
  out float2 p1 : TEXCOORD0,
  out float3 o2 : BENTNORMAL0,
  out uint4 o3 : TEXCOORD4,
  out float4 o4 : TEXCOORD1,
  out float4 o5 : TEXCOORD2,
  out float3 o6 : TEXCOORD3,
  out float3 o7 : OFFPOSITION0,
  out float3 o8 : TENSION0)
{
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t4, 24
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t5, 64
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15,r16;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.xyz = v3.xyz * float3(2.00195694,2.00195694,2.00195694) + float3(-1.00195694,-1.00195694,-1.00195694);
  r1.xyz = v4.xyz * float3(2.00195694,2.00195694,2.00195694) + float3(-1.00195694,-1.00195694,-1.00195694);
  r0.w = v4.w * 2 + -1;
  r2.xyz = v1.yxz * float3(-2,2,2) + float3(1,-1,-1);
  r1.w = dot(r2.xyz, r0.xyz);
  r1.w = saturate(r1.w + r1.w);
  r2.xyz = r2.xyz + -r0.xyz;
  r2.xyz = r1.www * r2.xyz + r0.xyz;
  r1.w = dot(r2.xyz, r2.xyz);
  r1.w = rsqrt(r1.w);
  r2.xyz = r2.xyz * r1.www;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r3.xy, v7.x, l(12), t4.xyxx
r3.x = gpuSkinBase[]..swiz;
r3.y = gpuSkinBase[]..swiz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r3.y, l(48), t5.xyzw
r4.x = gpuSkinBase[]..swiz;
r4.y = gpuSkinBase[]..swiz;
r4.z = gpuSkinBase[]..swiz;
r4.w = gpuSkinBase[]..swiz;
  r4.xyzw = r3.yyyy ? r4.xyzw : float4(0,0,0,0);
  r1.w = cmp(0 != r4.w);
  if (r1.w != 0) {
    r1.w = (uint)r4.w;
    if (r1.w != 0) {
      r2.w = (int)r1.w + (int)v6.x;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r5.xyzw, r2.w, l(0), t5.xyzw
    r5.x = gpuSkinBase[]..swiz;
    r5.y = gpuSkinBase[]..swiz;
    r5.z = gpuSkinBase[]..swiz;
    r5.w = gpuSkinBase[]..swiz;
      r3.y = dot(r5.xyz, v0.xyz);
      r3.y = r3.y + r5.w;
      r6.x = v5.x * r3.y;
      r3.y = dot(r5.xyz, r0.xyz);
      r7.x = v5.x * r3.y;
      r3.y = dot(r5.xyz, r1.xyz);
      r5.x = v5.x * r3.y;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r8.xyzw, r2.w, l(16), t5.xyzw
    r8.x = gpuSkinBase[]..swiz;
    r8.y = gpuSkinBase[]..swiz;
    r8.z = gpuSkinBase[]..swiz;
    r8.w = gpuSkinBase[]..swiz;
      r3.y = dot(r8.xyz, v0.xyz);
      r3.y = r3.y + r8.w;
      r6.y = v5.x * r3.y;
      r3.y = dot(r8.xyz, r0.xyz);
      r7.y = v5.x * r3.y;
      r3.y = dot(r8.xyz, r1.xyz);
      r5.y = v5.x * r3.y;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r8.xyzw, r2.w, l(32), t5.xyzw
    r8.x = gpuSkinBase[]..swiz;
    r8.y = gpuSkinBase[]..swiz;
    r8.z = gpuSkinBase[]..swiz;
    r8.w = gpuSkinBase[]..swiz;
      r3.y = dot(r8.xyz, v0.xyz);
      r3.y = r3.y + r8.w;
      r6.z = v5.x * r3.y;
      r3.y = dot(r8.xyz, r0.xyz);
      r7.z = v5.x * r3.y;
      r3.y = dot(r8.xyz, r1.xyz);
      r5.z = v5.x * r3.y;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.w, r2.w, l(48), t5.xxxx
    r2.w = gpuSkinBase[]..swiz;
      r8.z = v5.x * r2.w;
      r2.w = cmp(v5.y != 0.000000);
      if (r2.w != 0) {
        r2.w = (int)r1.w + (int)v6.y;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(0), t5.xyzw
      r9.x = gpuSkinBase[]..swiz;
      r9.y = gpuSkinBase[]..swiz;
      r9.z = gpuSkinBase[]..swiz;
      r9.w = gpuSkinBase[]..swiz;
        r3.y = dot(r9.xyz, v0.xyz);
        r3.y = r3.y + r9.w;
        r6.x = r3.y * v5.y + r6.x;
        r3.y = dot(r9.xyz, r0.xyz);
        r7.x = r3.y * v5.y + r7.x;
        r3.y = dot(r9.xyz, r1.xyz);
        r5.x = r3.y * v5.y + r5.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(16), t5.xyzw
      r9.x = gpuSkinBase[]..swiz;
      r9.y = gpuSkinBase[]..swiz;
      r9.z = gpuSkinBase[]..swiz;
      r9.w = gpuSkinBase[]..swiz;
        r3.y = dot(r9.xyz, v0.xyz);
        r3.y = r3.y + r9.w;
        r6.y = r3.y * v5.y + r6.y;
        r3.y = dot(r9.xyz, r0.xyz);
        r7.y = r3.y * v5.y + r7.y;
        r3.y = dot(r9.xyz, r1.xyz);
        r5.y = r3.y * v5.y + r5.y;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(32), t5.xyzw
      r9.x = gpuSkinBase[]..swiz;
      r9.y = gpuSkinBase[]..swiz;
      r9.z = gpuSkinBase[]..swiz;
      r9.w = gpuSkinBase[]..swiz;
        r3.y = dot(r9.xyz, v0.xyz);
        r3.y = r3.y + r9.w;
        r6.z = r3.y * v5.y + r6.z;
        r3.y = dot(r9.xyz, r0.xyz);
        r7.z = r3.y * v5.y + r7.z;
        r3.y = dot(r9.xyz, r1.xyz);
        r5.z = r3.y * v5.y + r5.z;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.w, r2.w, l(48), t5.xxxx
      r2.w = gpuSkinBase[]..swiz;
        r8.z = r2.w * v5.y + r8.z;
        r2.w = cmp(v5.z != 0.000000);
        if (r2.w != 0) {
          r2.w = (int)r1.w + (int)v6.z;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(0), t5.xyzw
        r9.x = gpuSkinBase[]..swiz;
        r9.y = gpuSkinBase[]..swiz;
        r9.z = gpuSkinBase[]..swiz;
        r9.w = gpuSkinBase[]..swiz;
          r3.y = dot(r9.xyz, v0.xyz);
          r3.y = r3.y + r9.w;
          r6.x = r3.y * v5.z + r6.x;
          r3.y = dot(r9.xyz, r0.xyz);
          r7.x = r3.y * v5.z + r7.x;
          r3.y = dot(r9.xyz, r1.xyz);
          r5.x = r3.y * v5.z + r5.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(16), t5.xyzw
        r9.x = gpuSkinBase[]..swiz;
        r9.y = gpuSkinBase[]..swiz;
        r9.z = gpuSkinBase[]..swiz;
        r9.w = gpuSkinBase[]..swiz;
          r3.y = dot(r9.xyz, v0.xyz);
          r3.y = r3.y + r9.w;
          r6.y = r3.y * v5.z + r6.y;
          r3.y = dot(r9.xyz, r0.xyz);
          r7.y = r3.y * v5.z + r7.y;
          r3.y = dot(r9.xyz, r1.xyz);
          r5.y = r3.y * v5.z + r5.y;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r2.w, l(32), t5.xyzw
        r9.x = gpuSkinBase[]..swiz;
        r9.y = gpuSkinBase[]..swiz;
        r9.z = gpuSkinBase[]..swiz;
        r9.w = gpuSkinBase[]..swiz;
          r3.y = dot(r9.xyz, v0.xyz);
          r3.y = r3.y + r9.w;
          r6.z = r3.y * v5.z + r6.z;
          r3.y = dot(r9.xyz, r0.xyz);
          r7.z = r3.y * v5.z + r7.z;
          r3.y = dot(r9.xyz, r1.xyz);
          r5.z = r3.y * v5.z + r5.z;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.w, r2.w, l(48), t5.xxxx
        r2.w = gpuSkinBase[]..swiz;
          r8.z = r2.w * v5.z + r8.z;
          r2.w = cmp(v5.w != 0.000000);
          if (r2.w != 0) {
            r1.w = (int)r1.w + (int)v6.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r1.w, l(0), t5.xyzw
          r9.x = gpuSkinBase[]..swiz;
          r9.y = gpuSkinBase[]..swiz;
          r9.z = gpuSkinBase[]..swiz;
          r9.w = gpuSkinBase[]..swiz;
            r2.w = dot(r9.xyz, v0.xyz);
            r2.w = r2.w + r9.w;
            r6.x = r2.w * v5.w + r6.x;
            r2.w = dot(r9.xyz, r0.xyz);
            r7.x = r2.w * v5.w + r7.x;
            r2.w = dot(r9.xyz, r1.xyz);
            r5.x = r2.w * v5.w + r5.x;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r1.w, l(16), t5.xyzw
          r9.x = gpuSkinBase[]..swiz;
          r9.y = gpuSkinBase[]..swiz;
          r9.z = gpuSkinBase[]..swiz;
          r9.w = gpuSkinBase[]..swiz;
            r2.w = dot(r9.xyz, v0.xyz);
            r2.w = r2.w + r9.w;
            r6.y = r2.w * v5.w + r6.y;
            r2.w = dot(r9.xyz, r0.xyz);
            r7.y = r2.w * v5.w + r7.y;
            r2.w = dot(r9.xyz, r1.xyz);
            r5.y = r2.w * v5.w + r5.y;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r9.xyzw, r1.w, l(32), t5.xyzw
          r9.x = gpuSkinBase[]..swiz;
          r9.y = gpuSkinBase[]..swiz;
          r9.z = gpuSkinBase[]..swiz;
          r9.w = gpuSkinBase[]..swiz;
            r2.w = dot(r9.xyz, v0.xyz);
            r2.w = r2.w + r9.w;
            r6.z = r2.w * v5.w + r6.z;
            r2.w = dot(r9.xyz, r0.xyz);
            r7.z = r2.w * v5.w + r7.z;
            r2.w = dot(r9.xyz, r1.xyz);
            r5.z = r2.w * v5.w + r5.z;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.w, r1.w, l(48), t5.xxxx
          r1.w = gpuSkinBase[]..swiz;
            r8.z = r1.w * v5.w + r8.z;
          }
        }
      }
      r1.w = dot(r7.xyz, r7.xyz);
      r1.w = rsqrt(r1.w);
      r0.xyz = r7.xyz * r1.www;
      r1.w = dot(r5.xyz, r5.xyz);
      r1.w = rsqrt(r1.w);
      r1.xyz = r5.xyz * r1.www;
    } else {
      r6.xyz = v0.xyz;
      r8.z = 0;
    }
  } else {
    r5.yw = (uint2)r4.xy;
    r5.x = v6.x;
    r5.z = 0;
    r7.xyzw = gpuSkinQuat.Load(r5.xyz).xyzw;
    r9.xyzw = gpuSkinQuat.Load(r5.xwz).xyzw;
    r10.xyzw = gpuSkinPos.Load(r5.xyz).xyzw;
    r11.xyzw = gpuSkinPos.Load(r5.xwz).xyzw;
    r3.yzw = gpuSkinBase.Load(r5.xzz).xyz;
    r9.xyzw = r9.xyzw + -r7.xyzw;
    r7.xyzw = r4.zzzz * r9.xyzw + r7.xyzw;
    r9.xyzw = r11.xyzw + -r10.xyzw;
    r9.xyzw = r4.zzzz * r9.xyzw + r10.xyzw;
    r4.xyw = v0.xyz + -r3.yzw;
    r1.w = dot(r7.xyzw, r7.xyzw);
    r1.w = rsqrt(r1.w);
    r7.xyzw = r7.xyzw * r1.wwww;
    r10.xyz = r7.xyz + r7.xyz;
    r11.xyz = r10.xxx * r7.xzw;
    r1.w = r10.y * r7.w;
    r10.zw = r10.zz * r7.wz;
    r2.w = r7.y * r10.y + r10.w;
    r12.x = 1 + -r2.w;
    r13.x = r7.z * r10.x + -r1.w;
    r12.y = r7.y * r10.x + -r10.z;
    r14.xy = r7.yx * r10.xx + r10.zw;
    r14.z = r7.z * r10.y + -r11.z;
    r7.xyz = r7.zwy * r10.yyy + r11.zyx;
    r12.z = r7.y;
    r10.x = dot(r4.xyw, r12.xyz);
    r14.w = 1 + -r14.y;
    r10.y = dot(r4.xwy, r14.xzw);
    r13.yz = r7.xz * float2(1,-1) + float2(0,1);
    r10.z = dot(r4.xyw, r13.xyz);
    r7.x = dot(r0.xyz, r12.xyz);
    r7.y = dot(r0.xzy, r14.xzw);
    r7.z = dot(r0.xyz, r13.xyz);
    r11.x = dot(r1.xyz, r12.xyz);
    r11.y = dot(r1.xzy, r14.xzw);
    r11.z = dot(r1.xyz, r13.xyz);
    r3.yzw = r10.xyz + r3.yzw;
    r3.yzw = r3.yzw + r9.xyz;
    r6.xyz = v5.xxx * r3.yzw;
    r3.yzw = v5.xxx * r7.xyz;
    r4.xyw = v5.xxx * r11.xyz;
    r8.z = v5.x * r9.w;
    r1.w = cmp(v5.y != 0.000000);
    if (r1.w != 0) {
      r5.x = v6.y;
      r7.xyzw = gpuSkinQuat.Load(r5.xyz).xyzw;
      r9.xyzw = gpuSkinQuat.Load(r5.xwz).xyzw;
      r10.xyzw = gpuSkinPos.Load(r5.xyz).xyzw;
      r11.xyzw = gpuSkinPos.Load(r5.xwz).xyzw;
      r12.xyz = gpuSkinBase.Load(r5.xzz).xyz;
      r9.xyzw = r9.xyzw + -r7.xyzw;
      r7.xyzw = r4.zzzz * r9.xyzw + r7.xyzw;
      r9.xyzw = r11.xyzw + -r10.xyzw;
      r9.xyzw = r4.zzzz * r9.xyzw + r10.xyzw;
      r10.xyz = v0.xyz + -r12.xyz;
      r1.w = dot(r7.xyzw, r7.xyzw);
      r1.w = rsqrt(r1.w);
      r7.xyzw = r7.xyzw * r1.wwww;
      r11.xyz = r7.xyz + r7.xyz;
      r13.xyz = r11.xxx * r7.xzw;
      r1.w = r11.y * r7.w;
      r11.zw = r11.zz * r7.wz;
      r2.w = r7.y * r11.y + r11.w;
      r14.x = 1 + -r2.w;
      r15.x = r7.z * r11.x + -r1.w;
      r14.y = r7.y * r11.x + -r11.z;
      r16.xy = r7.yx * r11.xx + r11.zw;
      r16.z = r7.z * r11.y + -r13.z;
      r7.xyz = r7.zwy * r11.yyy + r13.zyx;
      r14.z = r7.y;
      r11.x = dot(r10.xyz, r14.xyz);
      r16.w = 1 + -r16.y;
      r11.y = dot(r10.xzy, r16.xzw);
      r15.yz = r7.xz * float2(1,-1) + float2(0,1);
      r11.z = dot(r10.xyz, r15.xyz);
      r7.x = dot(r0.xyz, r14.xyz);
      r7.y = dot(r0.xzy, r16.xzw);
      r7.z = dot(r0.xyz, r15.xyz);
      r10.x = dot(r1.xyz, r14.xyz);
      r10.y = dot(r1.xzy, r16.xzw);
      r10.z = dot(r1.xyz, r15.xyz);
      r11.xyz = r11.xyz + r12.xyz;
      r9.xyz = r11.xyz + r9.xyz;
      r6.xyz = r9.xyz * v5.yyy + r6.xyz;
      r3.yzw = r7.xyz * v5.yyy + r3.yzw;
      r4.xyw = r10.xyz * v5.yyy + r4.xyw;
      r8.z = r9.w * v5.y + r8.z;
      r1.w = cmp(v5.z != 0.000000);
      if (r1.w != 0) {
        r5.x = v6.z;
        r7.xyzw = gpuSkinQuat.Load(r5.xyz).xyzw;
        r9.xyzw = gpuSkinQuat.Load(r5.xwz).xyzw;
        r10.xyzw = gpuSkinPos.Load(r5.xyz).xyzw;
        r11.xyzw = gpuSkinPos.Load(r5.xwz).xyzw;
        r12.xyz = gpuSkinBase.Load(r5.xzz).xyz;
        r9.xyzw = r9.xyzw + -r7.xyzw;
        r7.xyzw = r4.zzzz * r9.xyzw + r7.xyzw;
        r9.xyzw = r11.xyzw + -r10.xyzw;
        r9.xyzw = r4.zzzz * r9.xyzw + r10.xyzw;
        r10.xyz = v0.xyz + -r12.xyz;
        r1.w = dot(r7.xyzw, r7.xyzw);
        r1.w = rsqrt(r1.w);
        r7.xyzw = r7.xyzw * r1.wwww;
        r11.xyz = r7.xyz + r7.xyz;
        r13.xyz = r11.xxx * r7.xzw;
        r1.w = r11.y * r7.w;
        r11.zw = r11.zz * r7.wz;
        r2.w = r7.y * r11.y + r11.w;
        r14.x = 1 + -r2.w;
        r15.x = r7.z * r11.x + -r1.w;
        r14.y = r7.y * r11.x + -r11.z;
        r16.xy = r7.yx * r11.xx + r11.zw;
        r16.z = r7.z * r11.y + -r13.z;
        r7.xyz = r7.zwy * r11.yyy + r13.zyx;
        r14.z = r7.y;
        r11.x = dot(r10.xyz, r14.xyz);
        r16.w = 1 + -r16.y;
        r11.y = dot(r10.xzy, r16.xzw);
        r15.yz = r7.xz * float2(1,-1) + float2(0,1);
        r11.z = dot(r10.xyz, r15.xyz);
        r7.x = dot(r0.xyz, r14.xyz);
        r7.y = dot(r0.xzy, r16.xzw);
        r7.z = dot(r0.xyz, r15.xyz);
        r10.x = dot(r1.xyz, r14.xyz);
        r10.y = dot(r1.xzy, r16.xzw);
        r10.z = dot(r1.xyz, r15.xyz);
        r11.xyz = r11.xyz + r12.xyz;
        r9.xyz = r11.xyz + r9.xyz;
        r6.xyz = r9.xyz * v5.zzz + r6.xyz;
        r3.yzw = r7.xyz * v5.zzz + r3.yzw;
        r4.xyw = r10.xyz * v5.zzz + r4.xyw;
        r8.z = r9.w * v5.z + r8.z;
        r1.w = cmp(v5.w != 0.000000);
        if (r1.w != 0) {
          r5.x = v6.w;
          r7.xyzw = gpuSkinQuat.Load(r5.xyz).xyzw;
          r9.xyzw = gpuSkinQuat.Load(r5.xwz).xyzw;
          r10.xyzw = gpuSkinPos.Load(r5.xyz).xyzw;
          r11.xyzw = gpuSkinPos.Load(r5.xwz).xyzw;
          r5.xyz = gpuSkinBase.Load(r5.xzz).xyz;
          r9.xyzw = r9.xyzw + -r7.xyzw;
          r7.xyzw = r4.zzzz * r9.xyzw + r7.xyzw;
          r9.xyzw = r11.xyzw + -r10.xyzw;
          r9.xyzw = r4.zzzz * r9.xyzw + r10.xyzw;
          r10.xyz = v0.xyz + -r5.xyz;
          r1.w = dot(r7.xyzw, r7.xyzw);
          r1.w = rsqrt(r1.w);
          r7.xyzw = r7.xyzw * r1.wwww;
          r11.xyz = r7.xyz + r7.xyz;
          r12.xyz = r11.xxx * r7.xzw;
          r1.w = r11.y * r7.w;
          r11.zw = r11.zz * r7.wz;
          r2.w = r7.y * r11.y + r11.w;
          r13.x = 1 + -r2.w;
          r14.x = r7.z * r11.x + -r1.w;
          r13.y = r7.y * r11.x + -r11.z;
          r15.xy = r7.yx * r11.xx + r11.zw;
          r15.z = r7.z * r11.y + -r12.z;
          r7.xyz = r7.zwy * r11.yyy + r12.zyx;
          r13.z = r7.y;
          r11.x = dot(r10.xyz, r13.xyz);
          r15.w = 1 + -r15.y;
          r11.y = dot(r10.xzy, r15.xzw);
          r14.yz = r7.xz * float2(1,-1) + float2(0,1);
          r11.z = dot(r10.xyz, r14.xyz);
          r7.x = dot(r0.xyz, r13.xyz);
          r7.y = dot(r0.xzy, r15.xzw);
          r7.z = dot(r0.xyz, r14.xyz);
          r10.x = dot(r1.xyz, r13.xyz);
          r10.y = dot(r1.xzy, r15.xzw);
          r10.z = dot(r1.xyz, r14.xyz);
          r5.xyz = r11.xyz + r5.xyz;
          r5.xyz = r5.xyz + r9.xyz;
          r6.xyz = r5.xyz * v5.www + r6.xyz;
          r3.yzw = r7.xyz * v5.www + r3.yzw;
          r4.xyw = r10.xyz * v5.www + r4.xyw;
          r8.z = r9.w * v5.w + r8.z;
        }
      }
    }
    r1.w = dot(r3.yzw, r3.yzw);
    r1.w = rsqrt(r1.w);
    r0.xyz = r3.yzw * r1.www;
    r1.w = dot(r4.xyw, r4.xyw);
    r1.w = rsqrt(r1.w);
    r1.xyz = r4.xyw * r1.www;
  }
  r3.yzw = r1.yzx * r0.zxy;
  r3.yzw = r0.yzx * r1.zxy + -r3.yzw;
  r3.yzw = r3.yzw * r0.www;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r0.w, v7.x, l(0), t4.xxxx
r0.w = gpuSkinBase[]..swiz;
  r0.w = (int)r0.w + (int)r3.x;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.w, l(0), t5.xyzw
r4.x = gpuSkinBase[]..swiz;
r4.y = gpuSkinBase[]..swiz;
r4.z = gpuSkinBase[]..swiz;
r4.w = gpuSkinBase[]..swiz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r5.xyzw, r0.w, l(16), t5.xyzw
r5.x = gpuSkinBase[]..swiz;
r5.y = gpuSkinBase[]..swiz;
r5.z = gpuSkinBase[]..swiz;
r5.w = gpuSkinBase[]..swiz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r0.w, l(32), t5.xyzw
r7.x = gpuSkinBase[]..swiz;
r7.y = gpuSkinBase[]..swiz;
r7.z = gpuSkinBase[]..swiz;
r7.w = gpuSkinBase[]..swiz;
  r9.x = r4.w;
  r9.y = r5.w;
  r9.z = r7.w;
  r9.xyw = -eyeOffset.xyz + r9.xyz;
  r6.w = 1;
  r4.w = r9.x;
  r10.x = dot(r6.xyzw, r4.xyzw);
  r5.w = r9.y;
  r10.y = dot(r6.xyzw, r5.xyzw);
  r9.xyz = r7.xyz;
  r10.z = dot(r6.xyzw, r9.xyzw);
  r6.x = dot(r0.xyz, r4.xyz);
  r6.y = dot(r0.xyz, r5.xyz);
  r6.z = dot(r0.xyz, r9.xyz);
  r0.x = dot(r6.xyz, r6.xyz);
  r0.x = rsqrt(r0.x);
  o4.xyz = r6.xyz * r0.xxx;
  r0.x = dot(r1.xyz, r4.xyz);
  r0.y = dot(r1.xyz, r5.xyz);
  r0.z = dot(r1.xyz, r9.xyz);
  r0.w = dot(r0.xyz, r0.xyz);
  r0.w = rsqrt(r0.w);
  o5.xyz = r0.xyz * r0.www;
  r0.x = dot(r3.yzw, r4.xyz);
  r0.y = dot(r3.yzw, r5.xyz);
  r0.z = dot(r3.yzw, r9.xyz);
  r0.w = dot(r0.xyz, r0.xyz);
  r0.w = rsqrt(r0.w);
  o6.xyz = r0.xyz * r0.www;
  r0.x = dot(r2.xyz, r4.xyz);
  r0.y = dot(r2.xyz, r5.xyz);
  r0.z = dot(r2.xyz, r9.xyz);
  r0.w = dot(r0.xyz, r0.xyz);
  r0.w = rsqrt(r0.w);
  o2.xyz = r0.xyz * r0.www;
  r0.xyzw = viewProjectionMatrix._m10_m11_m12_m13 * r10.yyyy;
  r0.xyzw = r10.xxxx * viewProjectionMatrix._m00_m01_m02_m03 + r0.xyzw;
  r0.xyzw = r10.zzzz * viewProjectionMatrix._m20_m21_m22_m23 + r0.xyzw;
  o0.xyzw = viewProjectionMatrix._m30_m31_m32_m33 + r0.xyzw;
  r8.x = saturate(r8.z);
  r8.y = saturate(-r8.z);
  o1.x = v1.w;
  p1.xy = v2.xy;
  o7.xyz = r10.xyz;
  o8.xyz = r8.xyz;
  o3.x = v7.x;
  return;
}