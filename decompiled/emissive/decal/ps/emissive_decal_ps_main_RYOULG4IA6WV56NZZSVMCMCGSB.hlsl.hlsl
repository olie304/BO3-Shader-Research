...
SamplerState cbcrSampler_s : register(s3);
Texture2D<float4> yTexture : register(t0);
Texture2D<float4> cbcrTexture : register(t21);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.x = yTexture.Sample(ySampler_s, v2.xy).x;
  r0.x = -0.0625 + r0.x;
  r1.xy = cbcrTexture.Sample(cbcrSampler_s, v2.xy).xy;
