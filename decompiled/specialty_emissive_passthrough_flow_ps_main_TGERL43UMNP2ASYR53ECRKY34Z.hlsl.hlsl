// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:40:58 2021
...
// unknown dcl_: dcl_uav_typed_texture2darray (uint,uint,uint,uint) u7
  ...
  r0.xyzw = emissiveMap.Sample(colorSampler_s, v2.xy).xyzw;
  r1.xy = gameTime.ww * noiseParams.xy;
  r1.xy = v2.xy * noiseParams.zz + r1.xy;
  r1.xy = noiseMap.Sample(noiseSampler_s, r1.xy).xy;
  r1.xy = noiseParams.ww * r1.xy;
  r1.xy = v2.xy * flowParams.xx + r1.xy;
  r1.zw = frac(r1.xy);
  ...
  r3.z = 0.5 * r3.z;
  if (flowAlpha != 0) {
    r4.xyz = emissiveFlowMap.Sample(colorSampler_s, v2.xy).xyz;
    r3.w = gameTime.w * flowParams.y;
    r5.xy = r2.xy * float2(-1,1) + float2(1,-1);
    r5.xy = r5.xy * r3.ww + v2.xy;
  ...
  if (r1.w != 0) {
    ...
    if (r2.w != 0) {
      ...
      r4.w = fogConstants.atmospherefogheightdensityscale.x * v6.z;
  ...
  if (r1.w == 0) {
    ...
    r1.w = (int)r1.w + 1;
    // Needs manual fix for instruction:
    imm_atomic_iadd r4.x, u6, r2.xyxx, r1.w
    InterlockedAdd(dest, imm_value, orig_value);
    r2.zw = (int2)r4.xx & int2(65535,65535);
    r1.w = cmp((uint)r2.w < oitMaxEntries);
    if (r1.w != 0) {
      r1.w = max(r0.y, r0.z);
      r1.w = max(r1.w, r0.x);
      r1.w = abs(r1.w);
      ...
      r4.x = (uint)v0.z << 2;
      r3.w = (int)r3.w & 2046;
      bitmask.w = ((~(-1 << 11)) << 0) & 0xffffffff;  r3.w = (((uint)r3.w << 0) & bitmask.w) | ((uint)r4.x & ~bitmask.w);
      bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r1.y = (((uint)zFeatherComputeSprites << 0) & bitmask.y) | ((uint)r3.w & ~bitmask.y);
