// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:41:33 2021

cbuffer _Globals : register(b0)
{
  float4 flagParams : packoffset(c0);
  float4 colorObjMin : packoffset(c1);
  float4 colorObjMax : packoffset(c2);
  float colorObjMinBaseBlend : packoffset(c3);
  float colorObjMaxBaseBlend : packoffset(c3.y);
  float2 uvScroll : packoffset(c3.z);
  float4 detailScale : packoffset(c4);
  float4 detailScale1 : packoffset(c5);
  float4 detailScale2 : packoffset(c6);
  float4 detailScale3 : packoffset(c7);
  float4 alphaRevealParms : packoffset(c8);
  float4 colorDetailScale : packoffset(c9);
  float3 colorTint : packoffset(c10);
  float3 specColorTint : packoffset(c11);
  float baseNormalHeight : packoffset(c11.w);
  float2 glossRange : packoffset(c12);
  float2 rotateUVs : packoffset(c12.z);
  float2 scrollUVs : packoffset(c13);
  float2 scaleUVs : packoffset(c13.z);
  float2 offsetUVs : packoffset(c14);
  float2 zoomUVs : packoffset(c14.z);
  float zoomRate : packoffset(c15);
  float layerDepth : packoffset(c15.y);
  bool clampU : packoffset(c15.z);
  bool clampV : packoffset(c15.w);
  float hdrScale : packoffset(c16);
  float emissiveFalloffPower : packoffset(c16.y);
  float3 emissiveTint1 : packoffset(c17);
  float revealDriver : packoffset(c17.w);
  bool invertFalloff : packoffset(c18);
}

cbuffer PerSceneConsts : register(b1)
{
  row_major float4x4 projectionMatrix : packoffset(c0);
  row_major float4x4 viewMatrix : packoffset(c4);
  row_major float4x4 viewProjectionMatrix : packoffset(c8);
  row_major float4x4 inverseProjectionMatrix : packoffset(c12);
  row_major float4x4 inverseViewMatrix : packoffset(c16);
  row_major float4x4 inverseViewProjectionMatrix : packoffset(c20);
  float4 eyeOffset : packoffset(c24);
  float4 adsZScale : packoffset(c25);
  float4 hdrControl0 : packoffset(c26);
  float4 hdrControl1 : packoffset(c27);
  float4 fogColor : packoffset(c28);
  float4 fogConsts : packoffset(c29);
  float4 fogConsts2 : packoffset(c30);
  float4 fogConsts3 : packoffset(c31);
  float4 fogConsts4 : packoffset(c32);
  float4 fogConsts5 : packoffset(c33);
  float4 fogConsts6 : packoffset(c34);
  float4 fogConsts7 : packoffset(c35);
  float4 fogConsts8 : packoffset(c36);
  float4 fogConsts9 : packoffset(c37);
  float3 sunFogDir : packoffset(c38);
  float4 sunFogColor : packoffset(c39);
  float2 sunFog : packoffset(c40);
  float4 zNear : packoffset(c41);
  float3 clothPrimaryTint : packoffset(c42);
  float3 clothSecondaryTint : packoffset(c43);
  float4 renderTargetSize : packoffset(c44);
  float4 upscaledTargetSize : packoffset(c45);
  float4 materialColor : packoffset(c46);
  float4 cameraUp : packoffset(c47);
  float4 cameraLook : packoffset(c48);
  float4 cameraSide : packoffset(c49);
  float4 cameraVelocity : packoffset(c50);
  float4 skyMxR : packoffset(c51);
  float4 skyMxG : packoffset(c52);
  float4 skyMxB : packoffset(c53);
  float4 sunMxR : packoffset(c54);
  float4 sunMxG : packoffset(c55);
  float4 sunMxB : packoffset(c56);
  float4 skyRotationTransition : packoffset(c57);
  float4 debugColorOverride : packoffset(c58);
  float4 debugAlphaOverride : packoffset(c59);
  float4 debugNormalOverride : packoffset(c60);
  float4 debugSpecularOverride : packoffset(c61);
  float4 debugGlossOverride : packoffset(c62);
  float4 debugOcclusionOverride : packoffset(c63);
  float4 debugStreamerControl : packoffset(c64);
  float4 emblemLUTSelector : packoffset(c65);
  float4 colorMatrixR : packoffset(c66);
  float4 colorMatrixG : packoffset(c67);
  float4 colorMatrixB : packoffset(c68);
  float4 gameTime : packoffset(c69);
  float4 gameTick : packoffset(c70);
  float4 subpixelOffset : packoffset(c71);
  float4 viewportDimensions : packoffset(c72);
  float4 viewSpaceScaleBias : packoffset(c73);
  float4 ui3dUVSetup0 : packoffset(c74);
  float4 ui3dUVSetup1 : packoffset(c75);
  float4 ui3dUVSetup2 : packoffset(c76);
  float4 ui3dUVSetup3 : packoffset(c77);
  float4 ui3dUVSetup4 : packoffset(c78);
  float4 ui3dUVSetup5 : packoffset(c79);
  float4 clipSpaceLookupScale : packoffset(c80);
  float4 clipSpaceLookupOffset : packoffset(c81);
  uint4 computeSpriteControl : packoffset(c82);
  float4 invBcTexSizes : packoffset(c83);
  float4 invMaskTexSizes : packoffset(c84);
  float4 relHDRExposure : packoffset(c85);
  uint4 triDensityFlags : packoffset(c86);
  float4 triDensityParams : packoffset(c87);
  float4 voldecalRevealTextureInfo : packoffset(c88);
  float4 extraClipPlane0 : packoffset(c89);
  float4 extraClipPlane1 : packoffset(c90);
  float4 shaderDebug : packoffset(c91);
  uint isDepthHack : packoffset(c92);
}

cbuffer LightingGlobals : register(b2)
{
  uint numLights : packoffset(c0);
  uint numRefProbes : packoffset(c0.y);
  uint numRefProbeBlends : packoffset(c0.z);
  uint numDedicatedLights : packoffset(c0.w);
  float4 clearColor : packoffset(c1);
  float4 sssParams : packoffset(c2);

  struct
  {
    float4 fogColor;
    float4 sunFogColor;
    float K0;
    float skyK0;
    float expMul;
    float expAdd;
    float heightFalloff;
    float skyHeightFalloff;
    float K0b;
    float padding0;
    float skyK0b;
    float3 wldSunFogDir;
    float2 sunFogAngles;
    float atmospheresunstrength;
    float atmosphereMieSchlickK;
    float2 atmosphereskyfogdensityatcamera;
    float atmosphereExtinctionIntensity;
    float atmosphereInScatterIntensity;
    float3 atmosphereRayleighDensity;
    float atmospherehazebasedist;
    float3 atmosphereMieDensity;
    float atmospherehazefadedist;
    float3 atmosphereTotalDensity;
    float worldfogskysize;
    float3 atmosphereInScatterIntensityOverTotalDensity;
    float blendAmount;
    float2 atmosphereskyfogheightdensityscale;
    float2 atmospherefogdistanceoffset;
    float2 atmospherefogdistancedensityscale;
    float2 atmospherefogheightdensityscale;
    float2 atmospherefogdensityatcamera;
    float2 padding1;
  } fogConstants : packoffset(c3);


  struct
  {
    float3 wldDir;
    float splitDepthOffset;
    float3 color;
    float specScale;
    float globalProbeExposure;
    float3 avgGlobalProbeColor;
    float4 splitPinTransform[3];
    uint sunCookieIndex;
    float sunCookieIntensity;
    float sunVolumetricCookieIntensity;
    uint toolsGfxDisableSunShadow;
    float4 sunCookieTransform[2];
    float intensity;
    int splitArrayOffset;
    float2 pad0;

    struct
    {

      struct
      {
        float2 dimensionInTiles;
        float inchesPerTexel;
        float spanInInches;
      } constants;

      row_major float4x4 offToPinTransform;
      float coordScale;
      uint rootOffset;
      uint2 pad0;
    } sstLightingConstants;

  } sunConstants : packoffset(c17);


  struct
  {
    float2 sstToMinMaxScale;
    float2 pad0;
    float2 halfMinTexelSize;
    float rcpInchesDimLevel0;
    float shadowBiasInches;
  } minMaxConstants : packoffset(c33);


  struct
  {
    uint4 data[3];
  } globalProbeConstants : packoffset(c35);


  struct
  {
    float rain;
    float windSpeed;
    float windDirSin;
    float windDirCos;
    float weatherTile;
    float3 weatherVector;
    float3 weatherVector2;
    float padding0;
    float3 weatherTint;
    float padding1;
    float3 weatherTint2;
    float padding2;
  } weather : packoffset(c38);


  struct
  {

    struct
    {
      float2 dimensionInTiles;
      float inchesPerTexel;
      float spanInInches;
    } constants;

    row_major float4x4 offToPinTransform;
    float coordScale;
    uint rootOffset;
    uint2 pad0;
  } outdoorSSTConstants : packoffset(c43);

  float4 pinToWorldZ : packoffset(c49);

  struct
  {
    uint firstSpotLight;
    uint lastSpotLight;
    uint firstOmniLight;
    uint lastOmniLight;
    uint firstProbe;
    uint lastProbe;
    uint padding0;
    uint padding1;
  } volumetric : packoffset(c50);

  uint viewID : packoffset(c52);
  uint enableCountDebug : packoffset(c52.y);
  uint centerGroupIDx : packoffset(c52.z);
  uint centerGroupIDy : packoffset(c52.w);
  uint viewmodelSunShadowRaw : packoffset(c53);
  uint debugDrawOverlayType : packoffset(c53.y);
  float drawNumLighstScale : packoffset(c53.z);
  float drawNumLightsAlpha : packoffset(c53.w);
  uint probeDebug : packoffset(c54);
  float probeDebugRadius : packoffset(c54.y);
  uint showSunVis : packoffset(c54.z);
  uint enableAO : packoffset(c54.w);
  uint showAO : packoffset(c55);
  uint numGpuCullTilesWidth : packoffset(c55.y);
  uint numGpuCullTilesHeight : packoffset(c55.z);
  int permuteHighlight : packoffset(c55.w);
  uint overdrawOverlayMaxValue : packoffset(c56);
  float overdrawOverlayAlpha : packoffset(c56.y);
  uint overdrawDecals : packoffset(c56.z);
  uint overdrawDecalsLayers : packoffset(c56.w);
  uint permuteStride : packoffset(c57);
  uint numDecals : packoffset(c57.y);
  uint numStaticDecals : packoffset(c57.z);
  uint numDynamicDecals : packoffset(c57.w);
  uint numOverrideProbes : packoffset(c58);
  uint enableDitheredShadow : packoffset(c58.y);
  uint oitMaxEntries : packoffset(c58.z);
  uint numAttenuationVolumes : packoffset(c58.w);
}

SamplerState samp0_s : register(s0);
SamplerState colorSampler_s : register(s1);
SamplerState samplerLinear_s : register(s2);
SamplerState specColorSampler_s : register(s3);
SamplerState samplerLinearClamp_s : register(s4);
SamplerState normalSampler_s : register(s5);
SamplerState aoSampler_s : register(s6);
SamplerState revealSampler_s : register(s7);
SamplerComparisonState ShadowSamplerComparisonState_s : register(s13);
StructuredBuffer<shaderConstantSetBuffer> shaderConstantSetBuffer : register(t0);
StructuredBuffer<modelInstanceBuffer> modelInstanceBuffer : register(t4);
Texture2D<float2> gEnvBRDFGeneric : register(t5);
Texture2DArray<uint> visibleLights : register(t6);
Texture2DArray<uint> visibleProbes : register(t7);
TextureCubeArray<float3> gReflectionProbeArray : register(t8);
Texture2DArray<float> gSpotShadowmapArray : register(t9);
TextureCubeArray<float> gOmniShadowmapArray : register(t10);
StructuredBuffer<gCullConstants> gCullConstants : register(t11);
StructuredBuffer<lightConstants> lightConstants : register(t12);
Texture2DArray<float> gSunShadowmapArray : register(t13);
StructuredBuffer<refProbeConstants> refProbeConstants : register(t15);
StructuredBuffer<refProbeAttenuationConstants> refProbeAttenuationConstants : register(t16);
Texture3D<float3> gProbeXArray : register(t17);
Texture3D<float3> gProbeYArray : register(t18);
Texture3D<float3> gProbeZArray : register(t19);
Texture2DArray<float4> gCookieArray : register(t20);
Texture2D<float4> colorMap : register(t21);
Texture2D<float4> specColorMap : register(t22);
StructuredBuffer<gSunShadowTree> gSunShadowTree : register(t23);
Texture2D<float4> normalMap : register(t24);
Texture2D<float4> glossMap : register(t25);
Texture2D<float4> aoMap : register(t26);
Texture2D<float4> emissiveMap : register(t29);
Texture2D<float4> revealMap : register(t30);
Texture2DArray<float> gTransShadowmapArray : register(t31);
Texture2DArray<uint> visibleDecals : register(t35);
StructuredBuffer<forwardDecalConstants> forwardDecalConstants : register(t36);
Texture2DArray<float4> materialTexBC7 : register(t42);
Texture2DArray<float4> materialTexBC3 : register(t43);
Texture2DArray<float4> materialTexBC7linear : register(t44);
Texture2DArray<float4> materialTexMask : register(t45);


// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float4 v0 : SV_POSITION0,
  float v1 : COLOR1,
  float2 w1 : TEXCOORD0,
  nointerpolation uint4 v2 : TEXCOORD4,
  float4 v3 : TEXCOORD1,
  float4 v4 : TEXCOORD2,
  float3 v5 : TEXCOORD3,
  float3 v6 : OFFPOSITION0,
  uint v7 : SV_IsFrontFace0,
  out float4 o0 : SV_TARGET0)
{
  const float4 icb[] = { { -0.808114, 0.808114, 0, 0},
                              { 0, -1.000000, 0, 0},
                              { 0.606057, 0.606057, 0, 0},
                              { -0.714286, 0, 0, 0},
                              { 0.404114, -0.404114, 0, 0},
                              { 0, 0.428571, 0, 0},
                              { -0.202057, -0.202057, 0, 0},
                              { 0.142857, 0, 0, 0} };
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t0, 384
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t4, 24
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t11, 80
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t12, 240
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t15, 224
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t16, 96
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t23, 4
// Needs manual fix for instruction: 
// unknown dcl_: dcl_resource_structured t36, 128
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14,r15,r16,r17,r18,r19,r20,r21,r22,r23,r24,r25,r26,r27,r28,r29,r30,r31,r32,r33,r34,r35,r36,r37,r38,r39,r40,r41,r42,r43,r44,r45,r46;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.xyzw = colorMap.Sample(colorSampler_s, w1.xy).xyzw;
  r1.xyz = float3(-1,-1,-1) + colorTint.xyz;
  r1.xyz = r0.www * r1.xyz + float3(1,1,1);
  r0.xyz = r1.xyz * r0.xyz;
  r1.x = v1.x * r0.w;
  r1.yzw = specColorMap.Sample(specColorSampler_s, w1.xy).xyz;
  r2.x = aoMap.Sample(aoSampler_s, w1.xy).x;
  r2.y = glossMap.Sample(specColorSampler_s, w1.xy).x;
  r2.z = glossRange.y + -glossRange.x;
  r2.y = r2.y * r2.z + glossRange.x;
  r2.y = saturate(0.0588235296 * r2.y);
  r3.xyz = normalMap.Sample(normalSampler_s, w1.xy).xyz;
  r3.xyz = float3(-0.5,-0.5,-0) + r3.xyz;
  r3.xyz = baseNormalHeight * r3.xyz + float3(0.5,0.5,0);
  r3.xy = r3.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
  r2.w = dot(r3.xy, r3.xy);
  r2.w = 1 + -r2.w;
  r2.w = max(0, r2.w);
  r2.w = sqrt(r2.w);
  r3.z = r3.z * r3.z;
  r3.z = 0.333333343 * r3.z;
  r3.z = min(1, r3.z);
  r3.w = v7.x ? 1 : -1;
  r4.x = dot(v3.xyz, v3.xyz);
  r4.x = rsqrt(r4.x);
  r4.xyz = v3.xyz * r4.xxx;
  r4.xyz = r4.xyz * r3.www;
  r4.w = dot(v4.xyz, v4.xyz);
  r4.w = rsqrt(r4.w);
  r5.xyz = v4.xyz * r4.www;
  r5.xyz = r5.xyz * r3.www;
  r4.w = dot(v5.xyz, v5.xyz);
  r4.w = rsqrt(r4.w);
  r6.xyz = v5.xyz * r4.www;
  r6.xyz = r6.xyz * r3.www;
  r2.y = -17 * r2.y;
  r2.y = exp2(r2.y);
  r2.y = r3.z + r2.y;
  r2.y = log2(r2.y);
  r2.y = -0.0588235296 * r2.y;
  r6.xyz = r6.xyz * r3.yyy;
  r5.xyz = r5.xyz * r3.xxx + r6.xyz;
  r5.xyz = r4.xyz * r2.www + r5.xyz;
  r2.w = dot(r5.xyz, r5.xyz);
  r2.w = rsqrt(r2.w);
  r5.xyz = r5.xyz * r2.www;
  r2.z = debugGlossOverride.x * r2.z + glossRange.x;
  r2.z = saturate(0.0588235296 * r2.z);
  r2.z = -17 * r2.z;
  r2.z = exp2(r2.z);
  r2.z = r3.z + r2.z;
  r2.z = log2(r2.z);
  r2.z = -0.0588235296 * r2.z;
  r2.yz = max(float2(0,0), r2.yz);
  r2.w = cmp(0 < debugStreamerControl.w);
  if (r2.w != 0) {
    r2.w = (int)debugStreamerControl.w;
    r3.x = colorMap.CalculateLevelOfDetailUnclamped(colorSampler_s, w1.xy);
    r3.y = colorMap.CalculateLevelOfDetail(colorSampler_s, w1.xy);
    r3.x = cmp(r3.x < r3.y);
    r3.y = cmp(0 < r3.y);
    r6.xyz = (int3)r2.www & int3(1,2,4);
    r7.xyz = r6.xxx ? float3(1,0,1) : r0.xyz;
    r6.xyw = r6.yyy ? float3(0,1,1) : r0.xyz;
    r6.xyw = r3.yyy ? r7.xyz : r6.xyw;
    r7.xyz = r6.zzz ? float3(0,1,0) : r0.xyz;
    r0.xyz = r3.xxx ? r6.xyw : r7.xyz;
  }
  r3.xyz = debugColorOverride.xyz + -r0.xyz;
  r0.xyz = debugColorOverride.www * r3.xyz + r0.xyz;
  r0.w = -r0.w * v1.x + debugAlphaOverride.x;
  r0.w = debugAlphaOverride.w * r0.w + r1.x;
  r3.xyz = debugSpecularOverride.xyz + -r1.yzw;
  r1.xyz = debugSpecularOverride.www * r3.xyz + r1.yzw;
  r1.w = r2.z + -r2.y;
  r1.w = debugGlossOverride.w * r1.w + r2.y;
  r2.y = debugOcclusionOverride.x + -r2.x;
  r2.x = debugOcclusionOverride.w * r2.y + r2.x;
  r2.yzw = v7.xxx ? v3.xyz : -v3.xyz;
  r3.x = dot(r2.yzw, r2.yzw);
  r3.x = rsqrt(r3.x);
  r2.yzw = r2.yzw * r3.xxx + -r5.xyz;
  r2.yzw = debugNormalOverride.www * r2.yzw + r5.xyz;
  r3.x = cmp(v0.z >= 0.984375);
  r3.y = 1.01587307 * v0.z;
  r3.z = v0.z * 64 + -63;
  r3.x = r3.x ? r3.z : r3.y;
  r3.x = max(9.99999994e-09, r3.x);
  r3.x = rcp(r3.x);
  r3.yz = (uint2)v0.xy;
  r5.xyz = ddx_coarse(v6.xyz);
  r6.xyz = ddy_coarse(v6.xyz);
  r4.w = numRefProbes + numLights;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r7.xy, v2.x, l(4), t4.xyxx
r7.x = samp0[]..swiz;
r7.y = samp0[]..swiz;
  r7.yz = (int2)r7.yy & int2(268435456,536870912);
  if (r7.y != 0) {
    r8.xyz = eyeOffset.xyz + v6.xyz;
    r5.w = 0.0078125 * r3.x;
    r5.w = min(15, r5.w);
    r5.w = (uint)r5.w;
    r9.xy = (uint2)r3.yz >> int2(6,6);
    r9.z = (uint)r5.w << 4;
    r5.w = numStaticDecals & -32;
    r6.w = (int)-r5.w + numStaticDecals;
    r10.xy = float2(0,0);
    r11.w = 0;
    r8.w = 1;
    r12.xyzw = float4(0,0,0,0);
    r13.xyz = float3(0,0,0);
    r14.yz = float2(0,0);
    r15.xy = float2(0,0);
    r7.y = 0;
    while (true) {
      r7.w = cmp((uint)r7.y >= numStaticDecals);
      if (r7.w != 0) break;
      r10.z = (uint)r7.y >> 5;
      r11.xyz = (int3)r9.xyz + (int3)r10.xyz;
      r7.w = visibleDecals.Load(r11.xyzw).x;
      r9.w = cmp((int)r5.w == (int)r7.y);
      if (r6.w == 0) r10.z = 0; else if (r6.w+0 < 32) {       r10.z = (uint)r7.w << (32-(r6.w + 0)); r10.z = (uint)r10.z >> (32-r6.w);      } else r10.z = (uint)r7.w >> 0;
      r7.w = r9.w ? r10.z : r7.w;
      r9.w = (int)r4.w + (int)r7.y;
      r16.xyzw = r12.xzwy;
      r11.xyz = r13.xyz;
      r17.yz = r14.yz;
      r18.xy = r15.xy;
      r10.z = r7.w;
      while (true) {
        if (r10.z == 0) break;
        r10.w = firstbitlow((uint)r10.z);
        r13.w = 1 << (int)r10.w;
        r14.w = (int)r10.z & (int)r13.w;
        if (r14.w != 0) {
          r10.z = (int)r10.z ^ (int)r13.w;
          r13.w = (int)r9.w + (int)r10.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r19.xyzw, r13.w, l(0), t11.xyzw
        r19.x = samp0[]..swiz;
        r19.y = samp0[]..swiz;
        r19.z = samp0[]..swiz;
        r19.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r20.xy, r13.w, l(16), t11.xyxx
        r20.x = samp0[]..swiz;
        r20.y = samp0[]..swiz;
          r19.xyz = -v6.xyz + r19.xyz;
          r20.z = r19.w;
          r19.xyz = cmp(abs(r19.xyz) < r20.zxy);
          r13.w = r19.y ? r19.x : 0;
          r13.w = r19.z ? r13.w : 0;
          if (r13.w != 0) {
            r10.w = (int)r7.y + (int)r10.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r19.xyz, r10.w, l(0), t36.xyzx
          r19.x = samp0[]..swiz;
          r19.y = samp0[]..swiz;
          r19.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r20.xyz, r10.w, l(16), t36.xyzx
          r20.x = samp0[]..swiz;
          r20.y = samp0[]..swiz;
          r20.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r21.xyz, r10.w, l(32), t36.xyzx
          r21.x = samp0[]..swiz;
          r21.y = samp0[]..swiz;
          r21.z = samp0[]..swiz;
            r22.x = r19.x;
            r22.y = r20.x;
            r22.z = r21.x;
            r13.w = dot(r22.xyz, r22.xyz);
            r13.w = rsqrt(r13.w);
            r23.xyz = r22.xyz * r13.www;
            r13.w = dot(r23.xyz, v3.xyz);
            r13.w = cmp(r13.w >= 0.5);
            if (r13.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r24.xyw, r10.w, l(48), t36.xyxz
            r24.x = samp0[]..swiz;
            r24.y = samp0[]..swiz;
            r24.w = samp0[]..swiz;
              r22.w = r24.x;
              r22.x = dot(r8.xyzw, r22.xyzw);
              r25.x = r19.y;
              r25.y = r20.y;
              r25.z = r21.y;
              r25.w = r24.y;
              r22.y = dot(r8.xyzw, r25.xyzw);
              r24.x = r19.z;
              r24.y = r20.z;
              r24.z = r21.z;
              r22.z = dot(r8.xyzw, r24.xyzw);
              r19.xyz = cmp(abs(r22.xyz) < float3(1,1,1));
              r13.w = r19.y ? r19.x : 0;
              r13.w = r19.z ? r13.w : 0;
              if (r13.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r19.xyzw, r10.w, l(64), t36.xyzw
              r19.x = samp0[]..swiz;
              r19.y = samp0[]..swiz;
              r19.z = samp0[]..swiz;
              r19.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r20.xyzw, r10.w, l(80), t36.xyzw
              r20.x = samp0[]..swiz;
              r20.y = samp0[]..swiz;
              r20.z = samp0[]..swiz;
              r20.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r21.xyzw, r10.w, l(96), t36.xyzw
              r21.x = samp0[]..swiz;
              r21.y = samp0[]..swiz;
              r21.z = samp0[]..swiz;
              r21.w = samp0[]..swiz;
                r26.x = dot(r5.xyz, r25.xyz);
                r26.y = dot(r5.xyz, r24.xyz);
                r27.x = dot(r6.xyz, r25.xyz);
                r27.y = dot(r6.xyz, r24.xyz);
                r22.xy = r22.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r26.xyz = float3(0.5,-0.5,0.5) * r26.xyx;
                r27.xyz = float3(0.5,-0.5,0.5) * r27.xyx;
                r22.zw = (int2)r20.yz & int2(65535,65535);
                if (r22.z != 0) {
                  if (4 == 0) r13.w = 0; else if (4+24 < 32) {                   r13.w = (uint)r20.w << (32-(4 + 24)); r13.w = (uint)r13.w >> (32-4);                  } else r13.w = (uint)r20.w >> 24;
                  r13.w = 1 << (int)r13.w;
                  r14.w = (uint)r20.w >> 28;
                  r14.w = 1 << (int)r14.w;
                  r15.w = cmp((int)r22.z == 4);
                  if (r15.w != 0) {
                    r15.w = (int)r20.w & 4095;
                    if (12 == 0) r17.w = 0; else if (12+12 < 32) {                     r17.w = (uint)r20.w << (32-(12 + 12)); r17.w = (uint)r17.w >> (32-12);                    } else r17.w = (uint)r20.w >> 12;
                    r18.w = (int)r21.w & 1023;
                    r28.xz = (uint2)r13.ww;
                    r28.y = (uint)r14.w;
                    r29.xy = r22.xy * r28.zy + float2(0.5,0.5);
                    r29.zw = float2(-0.5,-0.5) + r28.zy;
                    r29.xy = min(r29.xy, r29.zw);
                    r30.x = (uint)r15.w;
                    r30.y = (uint)r17.w;
                    r29.xy = r30.xy + r29.xy;
                    r29.xy = invBcTexSizes.xy * r29.xy;
                    r29.z = (uint)r18.w;
                    r30.xyz = invBcTexSizes.xyx * r26.zyz;
                    r30.xyz = r30.xyz * r28.xyz;
                    r31.xyz = invBcTexSizes.xyx * r27.zyz;
                    r28.xyz = r31.xyz * r28.xyz;
                    r28.xyzw = materialTexBC7.SampleGrad(samplerLinearClamp_s, r29.xyz, r30.xyzx, r28.xyzx).xyzw;
                  } else {
                    r15.w = cmp((int)r22.z == 1);
                    if (r15.w != 0) {
                      r15.w = (int)r20.w & 4095;
                      if (12 == 0) r17.w = 0; else if (12+12 < 32) {                       r17.w = (uint)r20.w << (32-(12 + 12)); r17.w = (uint)r17.w >> (32-12);                      } else r17.w = (uint)r20.w >> 12;
                      r18.w = (int)r21.w & 1023;
                      r29.xz = (uint2)r13.ww;
                      r29.y = (uint)r14.w;
                      r30.xy = r22.xy * r29.zy + float2(0.5,0.5);
                      r30.zw = float2(-0.5,-0.5) + r29.zy;
                      r30.xy = min(r30.xy, r30.zw);
                      r31.x = (uint)r15.w;
                      r31.y = (uint)r17.w;
                      r30.xy = r31.xy + r30.xy;
                      r30.xy = invBcTexSizes.zw * r30.xy;
                      r30.z = (uint)r18.w;
                      r31.xyz = invBcTexSizes.zwz * r26.zyz;
                      r31.xyz = r31.xyz * r29.xyz;
                      r32.xyz = invBcTexSizes.zwz * r27.zyz;
                      r29.xyz = r32.xyz * r29.xyz;
                      r28.xyzw = materialTexBC3.SampleGrad(samplerLinearClamp_s, r30.xyz, r31.xyzx, r29.xyzx).xyzw;
                    } else {
                      r28.xyzw = float4(1,1,1,1);
                    }
                  }
                  r19.xyz = r28.xyz * r19.xyz;
                  r13.w = r28.w * r19.w;
                } else {
                  r19.xyz = float3(0,0,0);
                  r13.w = 0;
                }
                r14.w = cmp(r20.x != 1.000000);
                r15.w = r13.w + r19.w;
                r15.w = saturate(r15.w * r20.x + -r20.x);
                r13.w = r14.w ? r15.w : r13.w;
                if (r22.w != 0) {
                  if (4 == 0) r14.w = 0; else if (4+24 < 32) {                   r14.w = (uint)r21.y << (32-(4 + 24)); r14.w = (uint)r14.w >> (32-4);                  } else r14.w = (uint)r21.y >> 24;
                  r14.w = 1 << (int)r14.w;
                  r15.w = (uint)r21.y >> 28;
                  r15.w = 1 << (int)r15.w;
                  r17.w = cmp((int)r22.w == 2);
                  if (r17.w != 0) {
                    r17.w = (int)r21.y & 4095;
                    if (12 == 0) r20.x = 0; else if (12+12 < 32) {                     r20.x = (uint)r21.y << (32-(12 + 12)); r20.x = (uint)r20.x >> (32-12);                    } else r20.x = (uint)r21.y >> 12;
                    if (10 == 0) r20.w = 0; else if (10+20 < 32) {                     r20.w = (uint)r21.w << (32-(10 + 20)); r20.w = (uint)r20.w >> (32-10);                    } else r20.w = (uint)r21.w >> 20;
                    r28.xz = (uint2)r14.ww;
                    r28.y = (uint)r15.w;
                    r29.xy = r22.xy * r28.zy + float2(0.5,0.5);
                    r29.zw = float2(-0.5,-0.5) + r28.zy;
                    r29.xy = min(r29.xy, r29.zw);
                    r30.x = (uint)r17.w;
                    r30.y = (uint)r20.x;
                    r29.xy = r30.xy + r29.xy;
                    r29.xy = invMaskTexSizes.xy * r29.xy;
                    r29.z = (uint)r20.w;
                    r30.xyz = invMaskTexSizes.xyx * r26.zyz;
                    r30.xyz = r30.xyz * r28.xyz;
                    r31.xyz = invMaskTexSizes.xyx * r27.zyz;
                    r28.xyz = r31.xyz * r28.xyz;
                    r17.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r29.xyz, r30.x, r28.x).x;
                  } else {
                    r18.w = cmp((int)r22.w == 4);
                    if (r18.w != 0) {
                      r18.w = (int)r21.y & 4095;
                      if (12 == 0) r20.x = 0; else if (12+12 < 32) {                       r20.x = (uint)r21.y << (32-(12 + 12)); r20.x = (uint)r20.x >> (32-12);                      } else r20.x = (uint)r21.y >> 12;
                      if (10 == 0) r20.w = 0; else if (10+20 < 32) {                       r20.w = (uint)r21.w << (32-(10 + 20)); r20.w = (uint)r20.w >> (32-10);                      } else r20.w = (uint)r21.w >> 20;
                      r28.xz = (uint2)r14.ww;
                      r28.y = (uint)r15.w;
                      r22.zw = r22.xy * r28.zy + float2(0.5,0.5);
                      r29.xy = float2(-0.5,-0.5) + r28.zy;
                      r22.zw = min(r29.xy, r22.zw);
                      r29.x = (uint)r18.w;
                      r29.yz = (uint2)r20.xw;
                      r22.zw = r29.xy + r22.zw;
                      r29.xy = invBcTexSizes.xy * r22.zw;
                      r30.xyz = invBcTexSizes.xyx * r26.zyz;
                      r30.xyz = r30.xyz * r28.xyz;
                      r31.xyz = invBcTexSizes.xyx * r27.zyz;
                      r28.xyz = r31.xyz * r28.xyz;
                      r17.w = materialTexBC7.SampleGrad(samplerLinearClamp_s, r29.xyz, r30.x, r28.x).x;
                    } else {
                      r18.w = (int)r21.y & 4095;
                      if (12 == 0) r20.x = 0; else if (12+12 < 32) {                       r20.x = (uint)r21.y << (32-(12 + 12)); r20.x = (uint)r20.x >> (32-12);                      } else r20.x = (uint)r21.y >> 12;
                      if (10 == 0) r20.w = 0; else if (10+20 < 32) {                       r20.w = (uint)r21.w << (32-(10 + 20)); r20.w = (uint)r20.w >> (32-10);                      } else r20.w = (uint)r21.w >> 20;
                      r28.xz = (uint2)r14.ww;
                      r28.y = (uint)r15.w;
                      r22.zw = r22.xy * r28.zy + float2(0.5,0.5);
                      r29.xy = float2(-0.5,-0.5) + r28.zy;
                      r22.zw = min(r29.xy, r22.zw);
                      r29.x = (uint)r18.w;
                      r29.yz = (uint2)r20.xw;
                      r22.zw = r29.xy + r22.zw;
                      r29.xy = invBcTexSizes.zw * r22.zw;
                      r30.xyz = invBcTexSizes.zwz * r26.zyz;
                      r30.xyz = r30.xyz * r28.xyz;
                      r31.xyz = invBcTexSizes.zwz * r27.zyz;
                      r28.xyz = r31.xyz * r28.xyz;
                      r17.w = materialTexBC3.SampleGrad(samplerLinearClamp_s, r29.xyz, r30.x, r28.x).x;
                    }
                  }
                  r28.xyz = r17.www * float3(0.959999979,0.959999979,0.959999979) + float3(0.0399999991,0.0399999991,0.0399999991);
                  r14.w = saturate(1 + -r17.w);
                  r19.xyz = r19.xyz * r14.www;
                } else {
                  r28.xyz = float3(0.0399999991,0.0399999991,0.0399999991);
                }
                r20.xy = (uint2)r20.zy >> int2(16,16);
                if (r20.x != 0) {
                  r14.w = cmp((int)r20.x == 2);
                  if (r14.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r10.w, r10.w, l(112), t36.xxxx
                  r10.w = samp0[]..swiz;
                    if (4 == 0) r20.z = 0; else if (4+24 < 32) {                     r20.z = (uint)r21.z << (32-(4 + 24)); r20.z = (uint)r20.z >> (32-4);                    } else r20.z = (uint)r21.z >> 24;
                    if (12 == 0) r20.w = 0; else if (12+12 < 32) {                     r20.w = (uint)r21.z << (32-(12 + 12)); r20.w = (uint)r20.w >> (32-12);                    } else r20.w = (uint)r21.z >> 12;
                    r14.w = 1 << (int)r20.z;
                    r15.w = (uint)r21.z >> 28;
                    r15.w = 1 << (int)r15.w;
                    r17.w = (int)r21.z & 4095;
                    r10.w = (int)r10.w & 1023;
                    r29.xz = (uint2)r14.ww;
                    r29.y = (uint)r15.w;
                    r21.yz = r22.xy * r29.zy + float2(0.5,0.5);
                    r22.zw = float2(-0.5,-0.5) + r29.zy;
                    r21.yz = min(r22.zw, r21.yz);
                    r30.x = (uint)r17.w;
                    r30.y = (uint)r20.w;
                    r20.zw = r30.xy + r21.yz;
                    r30.xy = invMaskTexSizes.xy * r20.zw;
                    r30.z = (uint)r10.w;
                    r31.xyz = invMaskTexSizes.xyx * r26.zyz;
                    r31.xyz = r31.xyz * r29.xyz;
                    r32.xyz = invMaskTexSizes.xyx * r27.zyz;
                    r29.xyz = r32.xyz * r29.xyz;
                    r10.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r30.xyz, r31.x, r29.x).x;
                  } else {
                    r14.w = cmp((int)r20.x == 6);
                    r10.w = r14.w ? 0 : 1;
                  }
                } else {
                  r10.w = 1;
                }
                r10.w = saturate(0.764705896 * r10.w);
                if (r20.y != 0) {
                  r14.w = cmp((int)r20.y == 3);
                  if (r14.w != 0) {
                    if (4 == 0) r20.x = 0; else if (4+24 < 32) {                     r20.x = (uint)r21.x << (32-(4 + 24)); r20.x = (uint)r20.x >> (32-4);                    } else r20.x = (uint)r21.x >> 24;
                    if (12 == 0) r20.y = 0; else if (12+12 < 32) {                     r20.y = (uint)r21.x << (32-(12 + 12)); r20.y = (uint)r20.y >> (32-12);                    } else r20.y = (uint)r21.x >> 12;
                    if (10 == 0) r20.z = 0; else if (10+10 < 32) {                     r20.z = (uint)r21.w << (32-(10 + 10)); r20.z = (uint)r20.z >> (32-10);                    } else r20.z = (uint)r21.w >> 10;
                    r14.w = 1 << (int)r20.x;
                    r15.w = (uint)r21.x >> 28;
                    r15.w = 1 << (int)r15.w;
                    r17.w = (int)r21.x & 4095;
                    r21.xz = (uint2)r14.ww;
                    r21.y = (uint)r15.w;
                    r20.xw = r22.xy * r21.xy + float2(0.5,0.5);
                    r22.xy = float2(-0.5,-0.5) + r21.zy;
                    r20.xw = min(r22.xy, r20.xw);
                    r22.x = (uint)r17.w;
                    r22.yz = (uint2)r20.yz;
                    r20.xy = r22.xy + r20.xw;
                    r22.xy = invMaskTexSizes.zw * r20.xy;
                    r20.xyz = invMaskTexSizes.zwz * r26.xyz;
                    r20.xyz = r20.xyz * r21.xyz;
                    r26.xyz = invMaskTexSizes.zwz * r27.xyz;
                    r21.xyz = r26.xyz * r21.xyz;
                    r20.xyz = materialTexBC7linear.SampleGrad(samplerLinearClamp_s, r22.xyz, r20.xyz, r21.xyz).xyz;
                  } else {
                    r20.xyz = float3(0.5,0.5,1);
                  }
                  r20.xy = r20.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
                  r14.w = dot(r20.xy, r20.xy);
                  r14.w = 1 + -r14.w;
                  r14.w = max(0, r14.w);
                  r14.w = sqrt(r14.w);
                  r15.w = r20.z * r20.z;
                  r15.w = 0.333333343 * r15.w;
                  r15.w = min(1, r15.w);
                  r17.w = dot(r25.xyz, r25.xyz);
                  r17.w = rsqrt(r17.w);
                  r21.xyz = r25.xyz * r17.www;
                  r17.w = dot(-r24.xyz, -r24.xyz);
                  r17.w = rsqrt(r17.w);
                  r22.xyz = -r24.xyz * r17.www;
                  r23.xyz = r23.xyz * r3.www;
                  r21.xyz = r21.xyz * r3.www;
                  r22.xyz = r22.xyz * r3.www;
                  r17.w = -17 * r10.w;
                  r17.w = exp2(r17.w);
                  r15.w = r17.w + r15.w;
                  r15.w = log2(r15.w);
                  r15.w = -0.0588235296 * r15.w;
                  r10.w = max(0, r15.w);
                  r20.yzw = r22.xyz * r20.yyy;
                  r20.xyz = r21.xyz * r20.xxx + r20.yzw;
                  r20.xyz = r23.xyz * r14.www + r20.xyz;
                  r14.w = dot(r20.xyz, r20.xyz);
                  r14.w = rsqrt(r14.w);
                  r20.xyz = r20.xyz * r14.www;
                  r14.w = 1 + -r13.w;
                  r17.x = r16.x;
                  r21.xyz = r17.xyz * r14.www;
                  r17.xyz = r20.xyz * r13.www + r21.xyz;
                  r16.x = r17.x;
                }
                r14.w = 1 + -r13.w;
                r20.xyz = r14.www * r11.xyz;
                r11.xyz = r19.xyz * r13.www + r20.xyz;
                r18.z = r16.y;
                r19.xyz = r18.xyz * r14.www;
                r18.xyz = r28.xyz * r13.www + r19.xyz;
                r15.w = r16.w * r14.w;
                r16.z = r16.z * r14.w + r13.w;
                r16.w = r10.w * r13.w + r15.w;
                r16.y = r18.z;
              }
            }
          }
        }
      }
      r12.xyzw = r16.xwyz;
      r13.xyz = r11.xyz;
      r14.yz = r17.yz;
      r15.xy = r18.xy;
      r7.y = (int)r7.y + 32;
    }
    r14.x = r12.x;
    r15.z = r12.z;
    r12.xy = r12.wy;
  } else {
    r13.xyz = float3(0,0,0);
    r14.xyz = float3(0,0,0);
    r15.xyz = float3(0,0,0);
    r12.xy = float2(0,0);
  }
  if (r7.z != 0) {
    r8.xyz = eyeOffset.xyz + v6.xyz;
    r5.w = numDynamicDecals + numStaticDecals;
    r6.w = 0.0078125 * r3.x;
    r6.w = min(15, r6.w);
    r6.w = (uint)r6.w;
    r9.xy = (uint2)r3.yz >> int2(6,6);
    r9.z = (uint)r6.w << 4;
    r6.w = numStaticDecals & -32;
    r7.y = (int)-r6.w + numStaticDecals;
    r7.z = (int)r5.w & -32;
    r7.w = (int)r5.w + (int)-r7.z;
    r10.xy = float2(0,0);
    r11.w = 0;
    r8.w = 1;
    r16.xyz = r13.xyz;
    r17.xyz = r14.xyz;
    r18.xyz = r15.xyz;
    r19.xy = r12.xy;
    r9.w = r6.w;
    while (true) {
      r10.w = cmp((uint)r9.w >= (uint)r5.w);
      if (r10.w != 0) break;
      r10.z = (uint)r9.w >> 5;
      r11.xyz = (int3)r9.xyz + (int3)r10.xyz;
      r10.z = visibleDecals.Load(r11.xyzw).x;
      r10.w = cmp((int)r6.w == (int)r9.w);
      bitmask.x = ((~(-1 << r7.y)) << 0) & 0xffffffff;  r11.x = (((uint)0 << 0) & bitmask.x) | ((uint)r10.z & ~bitmask.x);
      r10.z = r10.w ? r11.x : r10.z;
      r10.w = cmp((int)r7.z == (int)r9.w);
      if (r7.w == 0) r11.x = 0; else if (r7.w+0 < 32) {       r11.x = (uint)r10.z << (32-(r7.w + 0)); r11.x = (uint)r11.x >> (32-r7.w);      } else r11.x = (uint)r10.z >> 0;
      r10.z = r10.w ? r11.x : r10.z;
      r10.w = (int)r4.w + (int)r9.w;
      r11.xyz = r16.xyz;
      r20.xyz = r17.xyz;
      r21.xyz = r18.xyz;
      r12.zw = r19.xy;
      r13.w = r10.z;
      while (true) {
        if (r13.w == 0) break;
        r14.w = firstbitlow((uint)r13.w);
        r15.w = 1 << (int)r14.w;
        r16.w = (int)r13.w & (int)r15.w;
        if (r16.w != 0) {
          r13.w = (int)r13.w ^ (int)r15.w;
          r15.w = (int)r10.w + (int)r14.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r22.xyzw, r15.w, l(0), t11.xyzw
        r22.x = samp0[]..swiz;
        r22.y = samp0[]..swiz;
        r22.z = samp0[]..swiz;
        r22.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r23.xy, r15.w, l(16), t11.xyxx
        r23.x = samp0[]..swiz;
        r23.y = samp0[]..swiz;
          r22.xyz = -v6.xyz + r22.xyz;
          r23.z = r22.w;
          r22.xyz = cmp(abs(r22.xyz) < r23.zxy);
          r15.w = r22.y ? r22.x : 0;
          r15.w = r22.z ? r15.w : 0;
          if (r15.w != 0) {
            r14.w = (int)r9.w + (int)r14.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r22.xyz, r14.w, l(0), t36.xyzx
          r22.x = samp0[]..swiz;
          r22.y = samp0[]..swiz;
          r22.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r23.xyz, r14.w, l(16), t36.xyzx
          r23.x = samp0[]..swiz;
          r23.y = samp0[]..swiz;
          r23.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r24.xyz, r14.w, l(32), t36.xyzx
          r24.x = samp0[]..swiz;
          r24.y = samp0[]..swiz;
          r24.z = samp0[]..swiz;
            r25.x = r22.x;
            r25.y = r23.x;
            r25.z = r24.x;
            r15.w = dot(r25.xyz, r25.xyz);
            r15.w = rsqrt(r15.w);
            r26.xyz = r25.xyz * r15.www;
            r15.w = dot(r26.xyz, v3.xyz);
            r15.w = cmp(r15.w >= 0.5);
            if (r15.w != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r27.xyw, r14.w, l(48), t36.xyxz
            r27.x = samp0[]..swiz;
            r27.y = samp0[]..swiz;
            r27.w = samp0[]..swiz;
              r25.w = r27.x;
              r25.x = dot(r8.xyzw, r25.xyzw);
              r28.x = r22.y;
              r28.y = r23.y;
              r28.z = r24.y;
              r28.w = r27.y;
              r25.y = dot(r8.xyzw, r28.xyzw);
              r27.x = r22.z;
              r27.y = r23.z;
              r27.z = r24.z;
              r25.z = dot(r8.xyzw, r27.xyzw);
              r22.xyz = cmp(abs(r25.xyz) < float3(1,1,1));
              r15.w = r22.y ? r22.x : 0;
              r15.w = r22.z ? r15.w : 0;
              if (r15.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r22.xyzw, r14.w, l(64), t36.xyzw
              r22.x = samp0[]..swiz;
              r22.y = samp0[]..swiz;
              r22.z = samp0[]..swiz;
              r22.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r23.xyzw, r14.w, l(80), t36.xyzw
              r23.x = samp0[]..swiz;
              r23.y = samp0[]..swiz;
              r23.z = samp0[]..swiz;
              r23.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r24.xyzw, r14.w, l(96), t36.xyzw
              r24.x = samp0[]..swiz;
              r24.y = samp0[]..swiz;
              r24.z = samp0[]..swiz;
              r24.w = samp0[]..swiz;
                r29.x = dot(r5.xyz, r28.xyz);
                r29.y = dot(r5.xyz, r27.xyz);
                r30.x = dot(r6.xyz, r28.xyz);
                r30.y = dot(r6.xyz, r27.xyz);
                r19.zw = r25.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                r25.xyz = float3(0.5,-0.5,0.5) * r29.xyx;
                r29.xyz = float3(0.5,-0.5,0.5) * r30.xyx;
                r30.xy = (int2)r23.yz & int2(65535,65535);
                if (r30.x != 0) {
                  if (4 == 0) r15.w = 0; else if (4+24 < 32) {                   r15.w = (uint)r23.w << (32-(4 + 24)); r15.w = (uint)r15.w >> (32-4);                  } else r15.w = (uint)r23.w >> 24;
                  r15.w = 1 << (int)r15.w;
                  r16.w = (uint)r23.w >> 28;
                  r16.w = 1 << (int)r16.w;
                  r17.w = cmp((int)r30.x == 4);
                  if (r17.w != 0) {
                    r17.w = (int)r23.w & 4095;
                    if (12 == 0) r18.w = 0; else if (12+12 < 32) {                     r18.w = (uint)r23.w << (32-(12 + 12)); r18.w = (uint)r18.w >> (32-12);                    } else r18.w = (uint)r23.w >> 12;
                    r20.w = (int)r24.w & 1023;
                    r31.xz = (uint2)r15.ww;
                    r31.y = (uint)r16.w;
                    r30.zw = r19.zw * r31.zy + float2(0.5,0.5);
                    r32.xy = float2(-0.5,-0.5) + r31.zy;
                    r30.zw = min(r32.xy, r30.zw);
                    r32.x = (uint)r17.w;
                    r32.y = (uint)r18.w;
                    r30.zw = r32.xy + r30.zw;
                    r32.xy = invBcTexSizes.xy * r30.zw;
                    r32.z = (uint)r20.w;
                    r33.xyz = invBcTexSizes.xyx * r25.zyz;
                    r33.xyz = r33.xyz * r31.xyz;
                    r34.xyz = invBcTexSizes.xyx * r29.zyz;
                    r31.xyz = r34.xyz * r31.xyz;
                    r31.xyzw = materialTexBC7.SampleGrad(samplerLinearClamp_s, r32.xyz, r33.xyzx, r31.xyzx).xyzw;
                  } else {
                    r17.w = cmp((int)r30.x == 1);
                    if (r17.w != 0) {
                      r17.w = (int)r23.w & 4095;
                      if (12 == 0) r18.w = 0; else if (12+12 < 32) {                       r18.w = (uint)r23.w << (32-(12 + 12)); r18.w = (uint)r18.w >> (32-12);                      } else r18.w = (uint)r23.w >> 12;
                      r20.w = (int)r24.w & 1023;
                      r32.xz = (uint2)r15.ww;
                      r32.y = (uint)r16.w;
                      r30.xz = r19.zw * r32.zy + float2(0.5,0.5);
                      r33.xy = float2(-0.5,-0.5) + r32.zy;
                      r30.xz = min(r33.xy, r30.xz);
                      r33.x = (uint)r17.w;
                      r33.y = (uint)r18.w;
                      r30.xz = r33.xy + r30.xz;
                      r33.xy = invBcTexSizes.zw * r30.xz;
                      r33.z = (uint)r20.w;
                      r30.xzw = invBcTexSizes.zwz * r25.zyz;
                      r30.xzw = r30.xzw * r32.xyz;
                      r34.xyz = invBcTexSizes.zwz * r29.zyz;
                      r32.xyz = r34.xyz * r32.xyz;
                      r31.xyzw = materialTexBC3.SampleGrad(samplerLinearClamp_s, r33.xyz, r30.xzwx, r32.xyzx).xyzw;
                    } else {
                      r31.xyzw = float4(1,1,1,1);
                    }
                  }
                  r22.xyz = r31.xyz * r22.xyz;
                  r15.w = r31.w * r22.w;
                } else {
                  r22.xyz = float3(0,0,0);
                  r15.w = 0;
                }
                r16.w = cmp(r23.x != 1.000000);
                r17.w = r15.w + r22.w;
                r17.w = saturate(r17.w * r23.x + -r23.x);
                r15.w = r16.w ? r17.w : r15.w;
                if (r30.y != 0) {
                  if (4 == 0) r16.w = 0; else if (4+24 < 32) {                   r16.w = (uint)r24.y << (32-(4 + 24)); r16.w = (uint)r16.w >> (32-4);                  } else r16.w = (uint)r24.y >> 24;
                  r16.w = 1 << (int)r16.w;
                  r17.w = (uint)r24.y >> 28;
                  r17.w = 1 << (int)r17.w;
                  r18.w = cmp((int)r30.y == 2);
                  if (r18.w != 0) {
                    r18.w = (int)r24.y & 4095;
                    if (12 == 0) r23.x = 0; else if (12+12 < 32) {                     r23.x = (uint)r24.y << (32-(12 + 12)); r23.x = (uint)r23.x >> (32-12);                    } else r23.x = (uint)r24.y >> 12;
                    if (10 == 0) r23.w = 0; else if (10+20 < 32) {                     r23.w = (uint)r24.w << (32-(10 + 20)); r23.w = (uint)r23.w >> (32-10);                    } else r23.w = (uint)r24.w >> 20;
                    r31.xz = (uint2)r16.ww;
                    r31.y = (uint)r17.w;
                    r30.xz = r19.zw * r31.zy + float2(0.5,0.5);
                    r32.xy = float2(-0.5,-0.5) + r31.zy;
                    r30.xz = min(r32.xy, r30.xz);
                    r32.x = (uint)r18.w;
                    r32.yz = (uint2)r23.xw;
                    r30.xz = r32.xy + r30.xz;
                    r32.xy = invMaskTexSizes.xy * r30.xz;
                    r30.xzw = invMaskTexSizes.xyx * r25.zyz;
                    r30.xzw = r30.xzw * r31.xyz;
                    r33.xyz = invMaskTexSizes.xyx * r29.zyz;
                    r31.xyz = r33.xyz * r31.xyz;
                    r18.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r32.xyz, r30.x, r31.x).x;
                  } else {
                    r20.w = cmp((int)r30.y == 4);
                    if (r20.w != 0) {
                      r20.w = (int)r24.y & 4095;
                      if (12 == 0) r23.x = 0; else if (12+12 < 32) {                       r23.x = (uint)r24.y << (32-(12 + 12)); r23.x = (uint)r23.x >> (32-12);                      } else r23.x = (uint)r24.y >> 12;
                      if (10 == 0) r23.w = 0; else if (10+20 < 32) {                       r23.w = (uint)r24.w << (32-(10 + 20)); r23.w = (uint)r23.w >> (32-10);                      } else r23.w = (uint)r24.w >> 20;
                      r30.xz = (uint2)r16.ww;
                      r30.y = (uint)r17.w;
                      r31.xy = r19.zw * r30.zy + float2(0.5,0.5);
                      r31.zw = float2(-0.5,-0.5) + r30.zy;
                      r31.xy = min(r31.xy, r31.zw);
                      r32.x = (uint)r20.w;
                      r32.y = (uint)r23.x;
                      r31.xy = r32.xy + r31.xy;
                      r31.xy = invBcTexSizes.xy * r31.xy;
                      r31.z = (uint)r23.w;
                      r32.xyz = invBcTexSizes.xyx * r25.zyz;
                      r32.xyz = r32.xyz * r30.xyz;
                      r33.xyz = invBcTexSizes.xyx * r29.zyz;
                      r30.xyz = r33.xyz * r30.xyz;
                      r18.w = materialTexBC7.SampleGrad(samplerLinearClamp_s, r31.xyz, r32.x, r30.x).x;
                    } else {
                      r20.w = (int)r24.y & 4095;
                      if (12 == 0) r23.x = 0; else if (12+12 < 32) {                       r23.x = (uint)r24.y << (32-(12 + 12)); r23.x = (uint)r23.x >> (32-12);                      } else r23.x = (uint)r24.y >> 12;
                      if (10 == 0) r23.w = 0; else if (10+20 < 32) {                       r23.w = (uint)r24.w << (32-(10 + 20)); r23.w = (uint)r23.w >> (32-10);                      } else r23.w = (uint)r24.w >> 20;
                      r30.xz = (uint2)r16.ww;
                      r30.y = (uint)r17.w;
                      r31.xy = r19.zw * r30.zy + float2(0.5,0.5);
                      r31.zw = float2(-0.5,-0.5) + r30.zy;
                      r31.xy = min(r31.xy, r31.zw);
                      r32.x = (uint)r20.w;
                      r32.y = (uint)r23.x;
                      r31.xy = r32.xy + r31.xy;
                      r31.xy = invBcTexSizes.zw * r31.xy;
                      r31.z = (uint)r23.w;
                      r32.xyz = invBcTexSizes.zwz * r25.zyz;
                      r32.xyz = r32.xyz * r30.xyz;
                      r33.xyz = invBcTexSizes.zwz * r29.zyz;
                      r30.xyz = r33.xyz * r30.xyz;
                      r18.w = materialTexBC3.SampleGrad(samplerLinearClamp_s, r31.xyz, r32.x, r30.x).x;
                    }
                  }
                  r30.xyz = r18.www * float3(0.959999979,0.959999979,0.959999979) + float3(0.0399999991,0.0399999991,0.0399999991);
                  r16.w = saturate(1 + -r18.w);
                  r22.xyz = r22.xyz * r16.www;
                } else {
                  r30.xyz = float3(0.0399999991,0.0399999991,0.0399999991);
                }
                r23.xy = (uint2)r23.zy >> int2(16,16);
                if (r23.x != 0) {
                  r16.w = cmp((int)r23.x == 2);
                  if (r16.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=128)(mixed,mixed,mixed,mixed) r14.w, r14.w, l(112), t36.xxxx
                  r14.w = samp0[]..swiz;
                    if (4 == 0) r23.z = 0; else if (4+24 < 32) {                     r23.z = (uint)r24.z << (32-(4 + 24)); r23.z = (uint)r23.z >> (32-4);                    } else r23.z = (uint)r24.z >> 24;
                    if (12 == 0) r23.w = 0; else if (12+12 < 32) {                     r23.w = (uint)r24.z << (32-(12 + 12)); r23.w = (uint)r23.w >> (32-12);                    } else r23.w = (uint)r24.z >> 12;
                    r16.w = 1 << (int)r23.z;
                    r17.w = (uint)r24.z >> 28;
                    r17.w = 1 << (int)r17.w;
                    r18.w = (int)r24.z & 4095;
                    r14.w = (int)r14.w & 1023;
                    r31.xz = (uint2)r16.ww;
                    r31.y = (uint)r17.w;
                    r24.yz = r19.zw * r31.zy + float2(0.5,0.5);
                    r32.xy = float2(-0.5,-0.5) + r31.zy;
                    r24.yz = min(r32.xy, r24.yz);
                    r32.x = (uint)r18.w;
                    r32.y = (uint)r23.w;
                    r23.zw = r32.xy + r24.yz;
                    r32.xy = invMaskTexSizes.xy * r23.zw;
                    r32.z = (uint)r14.w;
                    r33.xyz = invMaskTexSizes.xyx * r25.zyz;
                    r33.xyz = r33.xyz * r31.xyz;
                    r34.xyz = invMaskTexSizes.xyx * r29.zyz;
                    r31.xyz = r34.xyz * r31.xyz;
                    r14.w = materialTexMask.SampleGrad(samplerLinearClamp_s, r32.xyz, r33.x, r31.x).x;
                  } else {
                    r16.w = cmp((int)r23.x == 6);
                    r14.w = r16.w ? 0 : 1;
                  }
                } else {
                  r14.w = 1;
                }
                r14.w = saturate(0.764705896 * r14.w);
                if (r23.y != 0) {
                  r16.w = cmp((int)r23.y == 3);
                  if (r16.w != 0) {
                    if (4 == 0) r23.x = 0; else if (4+24 < 32) {                     r23.x = (uint)r24.x << (32-(4 + 24)); r23.x = (uint)r23.x >> (32-4);                    } else r23.x = (uint)r24.x >> 24;
                    if (12 == 0) r23.y = 0; else if (12+12 < 32) {                     r23.y = (uint)r24.x << (32-(12 + 12)); r23.y = (uint)r23.y >> (32-12);                    } else r23.y = (uint)r24.x >> 12;
                    if (10 == 0) r23.z = 0; else if (10+10 < 32) {                     r23.z = (uint)r24.w << (32-(10 + 10)); r23.z = (uint)r23.z >> (32-10);                    } else r23.z = (uint)r24.w >> 10;
                    r16.w = 1 << (int)r23.x;
                    r17.w = (uint)r24.x >> 28;
                    r17.w = 1 << (int)r17.w;
                    r18.w = (int)r24.x & 4095;
                    r24.xz = (uint2)r16.ww;
                    r24.y = (uint)r17.w;
                    r19.zw = r19.zw * r24.xy + float2(0.5,0.5);
                    r23.xw = float2(-0.5,-0.5) + r24.zy;
                    r19.zw = min(r23.xw, r19.zw);
                    r31.x = (uint)r18.w;
                    r31.yz = (uint2)r23.yz;
                    r19.zw = r31.xy + r19.zw;
                    r31.xy = invMaskTexSizes.zw * r19.zw;
                    r23.xyz = invMaskTexSizes.zwz * r25.xyz;
                    r23.xyz = r23.xyz * r24.xyz;
                    r25.xyz = invMaskTexSizes.zwz * r29.xyz;
                    r24.xyz = r25.xyz * r24.xyz;
                    r23.xyz = materialTexBC7linear.SampleGrad(samplerLinearClamp_s, r31.xyz, r23.xyz, r24.xyz).xyz;
                  } else {
                    r23.xyz = float3(0.5,0.5,1);
                  }
                  r19.zw = r23.xy * float2(1.9921875,1.9921875) + float2(-1,-1);
                  r16.w = dot(r19.zw, r19.zw);
                  r16.w = 1 + -r16.w;
                  r16.w = max(0, r16.w);
                  r16.w = sqrt(r16.w);
                  r17.w = r23.z * r23.z;
                  r17.w = 0.333333343 * r17.w;
                  r17.w = min(1, r17.w);
                  r18.w = dot(r28.xyz, r28.xyz);
                  r18.w = rsqrt(r18.w);
                  r23.xyz = r28.xyz * r18.www;
                  r18.w = dot(-r27.xyz, -r27.xyz);
                  r18.w = rsqrt(r18.w);
                  r24.xyz = -r27.xyz * r18.www;
                  r25.xyz = r26.xyz * r3.www;
                  r23.xyz = r23.xyz * r3.www;
                  r24.xyz = r24.xyz * r3.www;
                  r18.w = -17 * r14.w;
                  r18.w = exp2(r18.w);
                  r17.w = r18.w + r17.w;
                  r17.w = log2(r17.w);
                  r17.w = -0.0588235296 * r17.w;
                  r14.w = max(0, r17.w);
                  r24.xyz = r24.xyz * r19.www;
                  r23.xyz = r23.xyz * r19.zzz + r24.xyz;
                  r23.xyz = r25.xyz * r16.www + r23.xyz;
                  r16.w = dot(r23.xyz, r23.xyz);
                  r16.w = rsqrt(r16.w);
                  r23.xyz = r23.xyz * r16.www;
                  r16.w = 1 + -r15.w;
                  r24.xyz = r20.xyz * r16.www;
                  r20.xyz = r23.xyz * r15.www + r24.xyz;
                }
                r16.w = 1 + -r15.w;
                r23.xyz = r16.www * r11.xyz;
                r11.xyz = r22.xyz * r15.www + r23.xyz;
                r22.xyz = r21.xyz * r16.www;
                r21.xyz = r30.xyz * r15.www + r22.xyz;
                r17.w = r16.w * r12.w;
                r12.z = r12.z * r16.w + r15.w;
                r12.w = r14.w * r15.w + r17.w;
              }
            }
          }
        }
      }
      r16.xyz = r11.xyz;
      r17.xyz = r20.xyz;
      r18.xyz = r21.xyz;
      r19.xy = r12.zw;
      r9.w = (int)r9.w + 32;
    }
    r13.xyz = r16.xyz;
    r14.xyz = r17.xyz;
    r15.xyz = r18.xyz;
    r12.xy = r19.xy;
  }
  r12.x = saturate(r12.x);
  r3.w = 1 + -r12.x;
  r0.xyz = r0.xyz * r3.www + r13.xyz;
  o0.w = r0.w * r3.w + r12.x;
  r2.yzw = r2.yzw * r3.www + r14.xyz;
  r0.w = dot(r2.yzw, r2.yzw);
  r0.w = rsqrt(r0.w);
  r5.xyz = r2.yzw * r0.www;
  r6.y = r1.w * r3.w + r12.y;
  r1.xyz = r1.xyz * r3.www + r15.xyz;
  r0.w = dot(-v6.xyz, -v6.xyz);
  r0.w = rsqrt(r0.w);
  r2.yzw = -v6.xyz * r0.www;
  r8.x = dot(r2.yzw, v4.xyz);
  r8.y = dot(r2.yzw, v5.xyz);
  r1.w = dot(r2.yzw, v3.xyz);
  r3.w = cmp(0 < r1.w);
  r6.zw = -r8.xy / r1.ww;
  r6.zw = r3.ww ? r6.zw : 0;
  r7.yz = float2(-0.5,-0.5) + w1.xy;
  r1.w = 9.99999975e-06 + layerDepth;
  r6.zw = r6.zw * r1.ww;
  r6.zw = scaleUVs.xy * r6.zw;
  r6.zw = r7.yz * scaleUVs.xy + r6.zw;
  r6.zw = offsetUVs.xy * scaleUVs.xy + r6.zw;
  r1.w = rotateUVs.y * gameTime.w + 1;
  r1.w = rotateUVs.x * r1.w;
  r1.w = 0.0174532924 * r1.w;
  sincos(r1.w, r8.x, r9.x);
  r9.y = r9.x;
  r9.z = r8.x;
  r10.x = dot(r9.yz, r6.zw);
  r9.x = -r8.x;
  r10.y = dot(r9.xy, r6.zw);
  r6.zw = gameTime.ww * scrollUVs.xy;
  r6.zw = r6.zw * scaleUVs.xy + r10.xy;
  r1.w = gameTime.w * zoomRate;
  r1.w = cos(r1.w);
  r1.w = r1.w * 0.5 + 0.5;
  r1.w = 1 + -r1.w;
  r3.w = zoomUVs.y + -zoomUVs.x;
  r1.w = r1.w * r3.w + zoomUVs.x;
  r6.zw = r6.zw / r1.ww;
  r6.zw = float2(0.5,0.5) + r6.zw;
  r7.yz = saturate(r6.zw);
  r6.zw = clampU ? r7.yz : r6.zw;
  r1.w = saturate(dot(r2.yzw, r4.xyz));
  r4.xyzw = emissiveMap.Sample(colorSampler_s, r6.zw).xyzw;
  r4.xyz = emissiveTint1.xyz * r4.xyz;
// Known bad code for instruction (needs manual fix):
ld_structured_indexable(structured_buffer, stride=384)(mixed,mixed,mixed,mixed) r3.w, r7.x, l(16), t0.xxxx
r3.w = samp0[]..swiz;
  r6.z = revealMap.Sample(revealSampler_s, w1.xy).x;
  r3.w = saturate(0.998000026 * r3.w);
  r6.w = saturate(-alphaRevealParms.x * 0.5 + r3.w);
  r7.x = -r6.w + r3.w;
  r6.w = r6.z + -r6.w;
  r7.x = 1 / r7.x;
  r6.w = saturate(r7.x * r6.w);
  r7.x = r6.w * -2 + 3;
  r6.w = r6.w * r6.w;
  r6.w = r7.x * r6.w;
  r7.x = saturate(alphaRevealParms.x * 0.5 + r3.w);
  r7.x = r7.x + -r3.w;
  r3.w = r6.z + -r3.w;
  r6.z = 1 / r7.x;
  r3.w = saturate(r6.z * r3.w);
  r6.z = r3.w * -2 + 3;
  r3.w = r3.w * r3.w;
  r3.w = -r6.z * r3.w + 1;
  r3.w = r6.w * r3.w;
  r3.w = log2(r3.w);
  r3.w = alphaRevealParms.y * r3.w;
  r3.w = exp2(r3.w);
  r6.z = 0.200000003 + r3.w;
  r6.z = min(1, r6.z);
  r4.w = r6.z * r4.w;
  r7.xyz = emissiveTint1.xyz * r4.www;
  r4.xyz = saturate(r4.xyz * r3.www + r7.xyz);
  r3.w = log2(r1.w);
  r3.w = emissiveFalloffPower * r3.w;
  r3.w = exp2(r3.w);
  r1.w = 1 + -r1.w;
  r1.w = log2(r1.w);
  r1.w = emissiveFalloffPower * r1.w;
  r1.w = exp2(r1.w);
  r1.w = invertFalloff ? r1.w : r3.w;
  r3.w = cmp(emissiveFalloffPower == 0.000000);
  r1.w = r3.w ? 1 : r1.w;
  r1.w = hdrScale * r1.w;
  r4.xyz = r4.xyz * r1.www;
  r1.w = cmp(isDepthHack != 0);
  r6.zw = (uint2)r3.zy;
  r3.w = dot(r6.wz, float2(0.0671105608,0.00583714992));
  r3.w = frac(r3.w);
  r3.xw = float2(0.0078125,52.9829178) * r3.xw;
  r3.w = frac(r3.w);
  r3.w = r3.w * 6.28318548 + gameTick.w;
  sincos(r3.w, r7.x, r8.x);
  r3.w = dot(r6.zw, float2(0.0671105608,0.00583714992));
  r3.w = frac(r3.w);
  r3.w = 52.9829178 * r3.w;
  r3.w = frac(r3.w);
  r3.w = r3.w * 6.28318548 + gameTick.w;
  sincos(r3.w, r9.x, r10.x);
  r6.x = saturate(dot(r5.xyz, r2.yzw));
  r3.w = dot(-r2.yzw, r5.xyz);
  r3.w = r3.w + r3.w;
  r11.xyz = r5.xyz * -r3.www + -r2.yzw;
  r3.w = 17 * r6.y;
  r3.w = exp2(r3.w);
  r3.w = 2 + r3.w;
  r3.w = 2 / r3.w;
  r4.w = sqrt(r3.w);
  r6.z = sqrt(r4.w);
  r6.z = r6.z * 0.5 + 0.5;
  r6.z = r6.z * r6.z;
  r6.w = 0.5 * r6.z;
  r6.z = -r6.z * 0.5 + 1;
  r7.z = r6.x * r6.z + r6.w;
  r6.zw = r7.zz * r6.zw;
  r7.z = dot(r5.xyz, sunConstants.wldDir.xyz);
  r7.w = saturate(r7.z);
  r8.w = cmp(0 >= r7.w);
  if (r8.w != 0) {
    r9.y = 0;
  }
  if (r8.w == 0) {
    r10.yzw = sunConstants.sstLightingConstants.offToPinTransform._m10_m11_m12 * v6.yyy;
    r10.yzw = v6.xxx * sunConstants.sstLightingConstants.offToPinTransform._m00_m01_m02 + r10.yzw;
    r10.yzw = v6.zzz * sunConstants.sstLightingConstants.offToPinTransform._m20_m21_m22 + r10.yzw;
    r10.yzw = sunConstants.sstLightingConstants.offToPinTransform._m30_m31_m32 + r10.yzw;
    r8.w = -sunConstants.splitDepthOffset + r10.w;
    r8.w = -r8.w * 6.10351563e-05 + 1;
    r9.z = saturate(r8.w);
    r9.z = cmp(r8.w == r9.z);
    if (r9.z != 0) {
      r9.zw = float2(0,0);
      while (true) {
        r12.x = cmp(r9.z >= 3);
        if (r12.x != 0) break;
        r12.x = (uint)r9.z;
        r12.yz = -sunConstants.splitPinTransform[r12.x].xy + r10.yz;
        r12.y = max(abs(r12.y), abs(r12.z));
        r9.w = sunConstants.splitPinTransform[r12.x].z * r12.y;
        r12.x = cmp(r9.w < 1);
        if (r12.x != 0) {
          break;
        }
        r9.z = 1 + r9.z;
        r9.w = 0;
      }
    } else {
      r9.zw = float2(3,0);
    }
    r12.x = cmp(r9.z >= 3);
    if (r12.x != 0) {
      r12.yzw = sunConstants.sstLightingConstants.constants.inchesPerTexel * float3(128,0.5,0.5);
      r13.xz = rcp(r12.yy);
      r13.y = -r13.z;
      r12.yzw = r10.yzy * r13.xyz + r12.zwz;
      r13.xyz = sunConstants.sstLightingConstants.constants.dimensionInTiles.xyx + float3(-0.0078125,-0.0078125,-0.0078125);
      r12.yzw = max(float3(0,0,0), r12.yzw);
      r12.yzw = min(r12.yzw, r13.xyz);
      r13.xy = sunConstants.sstLightingConstants.coordScale * r12.wz;
      r13.xy = floor(r13.xy);
      r13.y = sunConstants.sstLightingConstants.constants.dimensionInTiles.x * r13.y;
      r13.x = r13.y * sunConstants.sstLightingConstants.coordScale + r13.x;
      r13.x = (uint)r13.x;
      r13.x = (int)r13.x + (int10)sunConstants.sstLightingConstants.rootOffset;
    // Known bad code for instruction (needs manual fix):
        ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r13.y, r13.x, l(0), t23.xxxx
    r13.y = samp0[]..swiz;
      r13.z = (int)r13.y & 0x40000000;
      r13.w = (uint)r13.y << 2;
      if (r13.z == 0) {
        r13.z = (int)r13.y & 0x01ffffff;
        r14.x = (int)r13.z + (int)r13.x;
        r13.x = (uint)r13.y >> 25;
        r13.x = (uint)r13.x;
        r12.yzw = r13.xxx * r12.yzw;
        r12.yzw = frac(r12.yzw);
        r12.yzw = float3(128,128,128) * r12.yzw;
        r12.yzw = (uint3)r12.yzw;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.z, r14.x, l(0), t23.xxxx
      r14.z = samp0[]..swiz;
        r13.xy = (uint2)r12.wz >> int2(6,6);
        r13.z = (int)r14.z & 0xc0000000;
        r14.w = (int)r14.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.w, r14.w, l(0), t23.xxxx
      r14.w = samp0[]..swiz;
        r13.y = r13.y ? r14.z : r14.w;
        r14.w = (uint)r13.y >> 13;
        r13.x = r13.x ? r14.w : r13.y;
        r13.x = (int)r13.x & 8191;
        r15.x = (int)r13.x + (int)r14.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r15.z, r15.x, l(0), t23.xxxx
      r15.z = samp0[]..swiz;
        r14.y = 0;
        r15.y = 1;
        r15.xyz = r13.zzz ? r14.xyz : r15.xyz;
        r16.yz = r13.zz ? float2(8.40779079e-45,1.40129846e-45) : float2(7.00649232e-45,2.80259693e-45);
        r13.xy = (uint2)r12.wz >> (uint2)r16.yy;
        r13.xy = (int2)r13.xy & int2(1,1);
        r14.w = (int)r15.z & 0xc0000000;
        r15.w = (int)r15.x + 1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r15.w, r15.w, l(0), t23.xxxx
      r15.w = samp0[]..swiz;
        r13.y = r13.y ? r15.z : r15.w;
        r15.w = (uint)r13.y >> 13;
        r13.x = r13.x ? r15.w : r13.y;
        r13.x = (int)r13.x & 8191;
        r16.x = (int)r13.x + (int)r15.x;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r16.w, r16.x, l(0), t23.xxxx
      r16.w = samp0[]..swiz;
        r15.xyz = r14.www ? r15.xyz : r16.xzw;
        r13.xyz = r13.zzz ? r14.xyz : r15.xyz;
        r14.x = (int)r13.z & 0xc0000000;
        if (r14.x == 0) {
          r14.x = (int)-r13.y + 6;
          r14.xy = (uint2)r12.wz >> (uint2)r14.xx;
          r14.z = (int)r13.z | 0x40000000;
          bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r14.y = (((uint)r14.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r14.x = (((uint)r14.x << 0) & bitmask.x) | ((uint)r14.y & ~bitmask.x);
          r14.x = (int)r14.x * 10;
          r14.x = (uint)r14.z >> (uint)r14.x;
          r14.x = (int)r14.x & 1023;
          r14.x = (int)r13.x + (int)r14.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.w, r14.x, l(0), t23.xxxx
        r14.w = samp0[]..swiz;
          r14.yz = (int2)r13.yy + int2(1,2);
          r13.y = (int)-r14.y + 6;
          r15.xy = (uint2)r12.wz >> (uint2)r13.yy;
          r13.y = (int)r14.w & 0xc0000000;
          r15.z = (int)r14.w | 0x40000000;
          bitmask.y = ((~(-1 << 1)) << 1) & 0xffffffff;  r15.y = (((uint)r15.y << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.x = ((~(-1 << 1)) << 0) & 0xffffffff;  r15.x = (((uint)r15.x << 0) & bitmask.x) | ((uint)r15.y & ~bitmask.x);
          r15.x = (int)r15.x * 10;
          r15.x = (uint)r15.z >> (uint)r15.x;
          r15.x = (int)r15.x & 1023;
          r15.x = (int)r14.x + (int)r15.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r15.z, r15.x, l(0), t23.xxxx
        r15.z = samp0[]..swiz;
          r15.y = r14.z;
          r15.xyz = r13.yyy ? r14.xyw : r15.xyz;
          r14.y = (int)-r15.y + 6;
          r14.yz = (uint2)r12.wz >> (uint2)r14.yy;
          r15.y = (int)r15.z & 0xc0000000;
          r15.w = (int)r15.z | 0x40000000;
          bitmask.z = ((~(-1 << 1)) << 1) & 0xffffffff;  r14.z = (((uint)r14.z << 1) & bitmask.z) | ((uint)0 & ~bitmask.z);
          bitmask.y = ((~(-1 << 1)) << 0) & 0xffffffff;  r14.y = (((uint)r14.y << 0) & bitmask.y) | ((uint)r14.z & ~bitmask.y);
          r14.y = (int)r14.y * 10;
          r14.y = (uint)r15.w >> (uint)r14.y;
          r14.y = (int)r14.y & 1023;
          r16.x = (int)r14.y + (int)r15.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r16.y, r16.x, l(0), t23.xxxx
        r16.y = samp0[]..swiz;
          r14.yz = r15.yy ? r15.xz : r16.xy;
          r13.xz = r13.yy ? r14.xw : r14.yz;
        }
        r13.y = (int)r13.z & 0xc0000000;
        if (r13.y == 0) {
          if (14 == 0) r14.x = 0; else if (14+15 < 32) {           r14.x = (uint)r13.z << (32-(14 + 15)); r14.x = (uint)r14.x >> (32-14);          } else r14.x = (uint)r13.z >> 15;
          r14.x = (uint)r14.x;
          r14.yz = (int2)r13.zz & int2(32767,536870912);
          r14.y = (uint)r14.y;
          r14.xy = sunConstants.sstLightingConstants.constants.spanInInches * r14.xy;
          r14.xy = float2(6.10388815e-05,3.05185094e-05) * r14.xy;
          r14.w = (int)r12.z & 3;
          r14.w = (int)r13.x + (int)r14.w;
          r14.w = (int)r14.w + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r14.w, r14.w, l(0), t23.xxxx
        r14.w = samp0[]..swiz;
          bitmask.x = ((~(-1 << 2)) << 3) & 0xffffffff;  r15.x = (((uint)r12.y << 3) & bitmask.x) | ((uint)0 & ~bitmask.x);
          bitmask.y = ((~(-1 << 2)) << 1) & 0xffffffff;  r15.y = (((uint)r12.z << 1) & bitmask.y) | ((uint)0 & ~bitmask.y);
          bitmask.z = ((~(-1 << 1)) << 4) & 0xffffffff;  r15.z = (((uint)r12.w << 4) & bitmask.z) | ((uint)0 & ~bitmask.z);
          r12.y = (uint)r14.w >> (uint)r15.x;
          r12.y = (int)r12.y & 255;
          r12.y = (uint)r12.y;
          r12.y = r12.y * r14.y;
          r12.y = r12.y * 0.00392156886 + r14.x;
          r12.z = (int)r15.y + 1;
          if (1 == 0) r12.w = 0; else if (1+1 < 32) {           r12.w = (uint)r12.w << (32-(1 + 1)); r12.w = (uint)r12.w >> (32-1);          } else r12.w = (uint)r12.w >> 1;
          r12.z = (int)r12.w + (int)r12.z;
          r12.z = (int)r12.z + (int)r13.x;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r12.z, r12.z, l(0), t23.xxxx
        r12.z = samp0[]..swiz;
          r12.z = (uint)r12.z >> (uint)r15.z;
          r12.z = (int)r12.z & 0x0000ffff;
          r12.z = (uint)r12.z;
          r12.z = r12.z * r14.y;
          r12.z = r12.z * 1.52590219e-05 + r14.x;
          r13.w = r14.z ? r12.y : r12.z;
        } else {
          r12.y = (int)r13.z & 0x80000000;
          r12.z = (int)r13.x + 1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=4)(mixed,mixed,mixed,mixed) r12.z, r12.z, l(0), t23.xxxx
        r12.z = samp0[]..swiz;
          r12.y = r12.y ? r12.z : 0;
          r12.z = (uint)r13.z << 2;
          r12.w = (uint)r12.y >> 16;
          r12.y = (int)r12.y & 0x0000ffff;
          r12.yw = f16tof32(r12.yw);
          r12.z = r10.y * r12.w + r12.z;
          r12.y = r10.z * r12.y + r12.z;
          r13.w = r13.y ? r12.y : r13.w;
        }
      }
      r10.w = -sunConstants.sstLightingConstants.constants.inchesPerTexel * 2 + r10.w;
      r10.w = cmp(r13.w < r10.w);
      r9.y = r10.w ? 0 : 1;
    }
    if (r12.x == 0) {
      if (enableDitheredShadow == 0) {
        r10.w = (uint)r9.z;
        r12.xy = -sunConstants.splitPinTransform[r10.w].xy + r10.yz;
        r12.xy = sunConstants.splitPinTransform[r10.w].zz * r12.xy;
        r12.xy = r12.xy * float2(0.5,-0.5) + float2(0.5,0.5);
        r10.w = (int16)sunConstants.splitArrayOffset;
        r12.z = r10.w + r9.z;
        r10.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r12.xyz, r8.w).x;
        r12.x = gTransShadowmapArray.SampleLevel(samp0_s, r12.xyz, 0).x;
        r10.w = r12.x + r10.w;
        r10.w = saturate(-1 + r10.w);
        r12.x = r10.w * r10.w;
        r9.y = r12.x * r10.w;
      }
      if (enableDitheredShadow != 0) {
        r12.x = -r7.x;
        r10.w = (uint)r9.z;
        r9.z = 1 + r9.z;
        r9.z = min(2, r9.z);
        r9.w = 1 + -r9.w;
        r9.w = 28 * r9.w;
        r9.zw = (uint2)r9.zw;
        r13.xy = -sunConstants.splitPinTransform[r10.w].xy + r10.yz;
        r13.xy = sunConstants.splitPinTransform[r10.w].zz * r13.xy;
        r13.xy = r13.xy * float2(0.5,-0.5) + float2(0.5,0.5);
        r10.yz = -sunConstants.splitPinTransform[r9.z].xy + r10.yz;
        r10.yz = sunConstants.splitPinTransform[r9.z].zz * r10.yz;
        r10.yz = r10.yz * float2(0.5,-0.5) + float2(0.5,0.5);
        r12.y = r8.x;
        r12.z = r7.x;
        r12.w = 0;
        r13.z = 0;
        while (true) {
          r13.w = cmp((uint)r13.z >= 8);
          if (r13.w != 0) break;
          r13.w = cmp((uint)r9.w < (uint)r13.z);
          r14.xy = r13.ww ? r10.yz : r13.xy;
          r14.z = r13.w ? sunConstants.splitPinTransform[r9.z].w : sunConstants.splitPinTransform[r10.w].w;
          r13.w = r13.w ? r9.z : r10.w;
          r15.x = dot(icb[r13.z+0].yx, r12.xy);
          r15.y = dot(icb[r13.z+0].yx, r12.yz);
          r14.xy = r15.xy * r14.zz + r14.xy;
          r13.w = (int)r13.w + (int16)sunConstants.splitArrayOffset;
          r14.z = (uint)r13.w;
          r13.w = gSunShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r14.xyz, r8.w).x;
          r14.x = gTransShadowmapArray.SampleLevel(samp0_s, r14.xyz, 0).x;
          r13.w = r14.x + r13.w;
          r13.w = saturate(-1 + r13.w);
          r12.w = r13.w * 0.125 + r12.w;
          r13.z = (int)r13.z + 1;
        }
        r8.w = r12.w * r12.w;
        r9.y = r8.w * r12.w;
      }
    }
  }
  r8.w = cmp(0 < r7.w);
  if (r8.w != 0) {
    r8.w = cmp(0 < r9.y);
    if (r8.w != 0) {
      r8.w = sunConstants.specScale * r3.w;
      r8.w = r8.w * r7.w;
      if (sunConstants.sunCookieIndex != 0) {
        r12.xyz = eyeOffset.xyz + v6.xyz;
        r12.w = 1;
        r9.z = dot(sunConstants.sunCookieTransform[0].xyzw, r12.xyzw);
        r9.w = dot(sunConstants.sunCookieTransform[1].xyzw, r12.xyzw);
        r12.xy = frac(r9.zw);
        r9.z = -1 + (int14)sunConstants.sunCookieIndex;
        r12.z = (uint)r9.z;
        r10.yzw = gCookieArray.SampleLevel(samplerLinear_s, r12.xyz, 0).xyz;
        r10.yzw = float3(-1,-1,-1) + r10.yzw;
        r10.yzw = sunConstants.sunCookieIntensity * r10.yzw + float3(1,1,1);
        r10.yzw = sunConstants.color.xyz * r10.yzw;
      } else {
        r10.yzw = sunConstants.color.xyz;
      }
      r9.z = viewmodelSunShadowRaw >> 16;
      r9.w = cmp(0 < (uint)r9.z);
      r9.w = r1.w ? r9.w : 0;
      if (r9.w != 0) {
        r9.z = (int)r9.z + numLights;
        r9.z = (int)r9.z + -1;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r9.w, r9.z, l(52), t12.xxxx
      r9.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.xyzw, r9.z, l(68), t12.xyzw
      r12.x = samp0[]..swiz;
      r12.y = samp0[]..swiz;
      r12.z = samp0[]..swiz;
      r12.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.xyzw, r9.z, l(84), t12.xyzw
      r13.x = samp0[]..swiz;
      r13.y = samp0[]..swiz;
      r13.z = samp0[]..swiz;
      r13.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r14.xyzw, r9.z, l(132), t12.xyzw
      r14.x = samp0[]..swiz;
      r14.y = samp0[]..swiz;
      r14.z = samp0[]..swiz;
      r14.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r9.z, l(148), t12.xyzw
      r15.x = samp0[]..swiz;
      r15.y = samp0[]..swiz;
      r15.z = samp0[]..swiz;
      r15.w = samp0[]..swiz;
        r16.x = abs(r7.z) * -0.200000003 + 0.400000006;
        r16.xyz = r5.xyz * r16.xxx + v6.xyz;
        r16.w = 1;
        r12.x = dot(r12.xyzw, r16.xyzw);
        r12.y = dot(r13.xyzw, r16.xyzw);
        r12.xy = r12.xy * float2(0.5,-0.5) + float2(0.5,0.5);
        r12.xy = r12.xy + r15.zw;
        r12.xy = r12.xy * r15.xy;
        r12.zw = r9.ww / r14.xz;
        r13.xy = float2(1,1) + -r12.zw;
        r13.xy = cmp(r12.xy >= r13.xy);
        r12.zw = cmp(r12.zw >= r12.xy);
        r12.zw = (int2)r12.zw | (int2)r13.xy;
        r12.z = (int)r12.w | (int)r12.z;
        if (r12.z == 0) {
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.z, r9.z, l(28), t12.xxxx
        r12.z = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.xyzw, r9.z, l(100), t12.xyzw
        r13.x = samp0[]..swiz;
        r13.y = samp0[]..swiz;
        r13.z = samp0[]..swiz;
        r13.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xy, r9.z, l(164), t12.xyxx
        r15.x = samp0[]..swiz;
        r15.y = samp0[]..swiz;
          r9.z = dot(r13.xyzw, r16.xyzw);
          r12.xy = saturate(r12.xy);
          r13.xy = r12.xy * r14.xz + r14.yw;
          r9.z = r9.z + r15.x;
          r9.z = r9.z / r15.y;
          r9.z = max(6.10351563e-05, r9.z);
          r12.x = r12.z ? 0.000000 : 0;
          if (enableDitheredShadow != 0) {
            r14.x = -r7.x;
            r15.z = (uint)r12.x;
            r14.y = r8.x;
            r14.z = r7.x;
            r12.yz = float2(0,0);
            while (true) {
              r12.w = cmp((int)r12.z >= 8);
              if (r12.w != 0) break;
              r16.x = dot(icb[r12.z+0].yx, r14.xy);
              r16.y = dot(icb[r12.z+0].yx, r14.yz);
              r15.xy = r16.xy * r9.ww + r13.xy;
              r12.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r9.z).x;
              r12.y = r12.w * 0.125 + r12.y;
              r12.z = (int)r12.z + 1;
            }
          } else {
            r13.z = (uint)r12.x;
            r12.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r13.xyz, r9.z).x;
          }
          r9.z = r12.y * r12.y;
          r9.z = r9.z * r12.y;
        } else {
          r9.z = 1;
        }
        r9.y = r9.y * r9.z;
      } else {
        r9.z = viewmodelSunShadowRaw & 0x0000ffff;
        r9.w = cmp(0 < (uint)r9.z);
        r12.x = ~(int)r1.w;
        r9.w = r9.w ? r12.x : 0;
        if (r9.w != 0) {
          r9.z = (int)r9.z + numLights;
          r9.z = (int)r9.z + -1;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r9.w, r9.z, l(52), t12.xxxx
        r9.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.xyzw, r9.z, l(68), t12.xyzw
        r12.x = samp0[]..swiz;
        r12.y = samp0[]..swiz;
        r12.z = samp0[]..swiz;
        r12.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.xyzw, r9.z, l(84), t12.xyzw
        r13.x = samp0[]..swiz;
        r13.y = samp0[]..swiz;
        r13.z = samp0[]..swiz;
        r13.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r14.xyzw, r9.z, l(132), t12.xyzw
        r14.x = samp0[]..swiz;
        r14.y = samp0[]..swiz;
        r14.z = samp0[]..swiz;
        r14.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xyzw, r9.z, l(148), t12.xyzw
        r15.x = samp0[]..swiz;
        r15.y = samp0[]..swiz;
        r15.z = samp0[]..swiz;
        r15.w = samp0[]..swiz;
          r16.x = abs(r7.z) * -0.200000003 + 0.400000006;
          r16.xyz = r5.xyz * r16.xxx + v6.xyz;
          r16.w = 1;
          r12.x = dot(r12.xyzw, r16.xyzw);
          r12.y = dot(r13.xyzw, r16.xyzw);
          r12.xy = r12.xy * float2(0.5,-0.5) + float2(0.5,0.5);
          r12.xy = r12.xy + r15.zw;
          r12.xy = r12.xy * r15.xy;
          r12.zw = r9.ww / r14.xz;
          r13.xy = float2(1,1) + -r12.zw;
          r13.xy = cmp(r12.xy >= r13.xy);
          r12.zw = cmp(r12.zw >= r12.xy);
          r12.zw = (int2)r12.zw | (int2)r13.xy;
          r12.z = (int)r12.w | (int)r12.z;
          if (r12.z == 0) {
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r12.z, r9.z, l(28), t12.xxxx
          r12.z = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r13.xyzw, r9.z, l(100), t12.xyzw
          r13.x = samp0[]..swiz;
          r13.y = samp0[]..swiz;
          r13.z = samp0[]..swiz;
          r13.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r15.xy, r9.z, l(164), t12.xyxx
          r15.x = samp0[]..swiz;
          r15.y = samp0[]..swiz;
            r9.z = dot(r13.xyzw, r16.xyzw);
            r12.xy = saturate(r12.xy);
            r13.xy = r12.xy * r14.xz + r14.yw;
            r9.z = r9.z + r15.x;
            r9.z = r9.z / r15.y;
            r9.z = max(6.10351563e-05, r9.z);
            r12.x = r12.z ? 0.000000 : 0;
            if (enableDitheredShadow != 0) {
              r14.x = -r7.x;
              r15.z = (uint)r12.x;
              r14.y = r8.x;
              r14.z = r7.x;
              r12.yz = float2(0,0);
              while (true) {
                r12.w = cmp((int)r12.z >= 8);
                if (r12.w != 0) break;
                r16.x = dot(icb[r12.z+0].yx, r14.xy);
                r16.y = dot(icb[r12.z+0].yx, r14.yz);
                r15.xy = r16.xy * r9.ww + r13.xy;
                r12.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r15.xyz, r9.z).x;
                r12.y = r12.w * 0.125 + r12.y;
                r12.z = (int)r12.z + 1;
              }
            } else {
              r13.z = (uint)r12.x;
              r12.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r13.xyz, r9.z).x;
            }
            r9.z = r12.y * r12.y;
            r9.z = r9.z * r12.y;
          } else {
            r9.z = 1;
          }
          r9.y = r9.y * r9.z;
        }
      }
      r9.z = -r6.x * 0.5 + 1;
      r9.z = -r7.w * r9.z + 1;
      r9.z = r9.z * r9.z;
      r9.z = -r9.z * 0.620000005 + 0.620000005;
      r9.z = r9.z + -r7.w;
      r9.z = r4.w * r9.z + r7.w;
      r9.z = r9.z * r9.y;
      r12.xyz = r9.zzz * r10.yzw;
      r7.z = cmp(0 < r7.z);
      r13.xyz = -v6.xyz * r0.www + sunConstants.wldDir.xyz;
      r0.w = dot(r13.xyz, r13.xyz);
      r0.w = rsqrt(r0.w);
      r13.xyz = r13.xyz * r0.www;
      r0.w = dot(r13.xyz, r2.yzw);
      r9.z = dot(r5.xyz, r13.xyz);
      r9.w = abs(r9.z) * r3.w + -abs(r9.z);
      r9.z = abs(r9.z) * r9.w + 1;
      r7.w = r7.w * r6.z + r6.w;
      r9.z = r9.z * r9.z;
      r7.w = r9.z * r7.w;
      r7.w = rcp(r7.w);
      r7.w = r7.w * r8.w;
      r7.w = r9.y * r7.w;
      r7.w = 0.25 * r7.w;
      r9.yzw = r7.www * r10.yzw;
      r0.w = saturate(1 + -r0.w);
      r8.w = r0.w * r0.w;
      r8.w = r8.w * r8.w;
      r0.w = r8.w * r0.w;
      r0.w = r7.w * r0.w;
      r13.xyz = r0.www * r10.zwy;
      r12.w = r13.z;
      r12.xyzw = r7.zzzz ? r12.xyzw : 0;
      r13.zw = r9.yz;
      r13.xyzw = r7.zzzz ? r13.xyzw : 0;
      r0.w = r7.z ? r9.w : 0;
    } else {
      r12.xyzw = float4(0,0,0,0);
      r13.xyzw = float4(0,0,0,0);
      r0.w = 0;
    }
  } else {
    r12.xyzw = float4(0,0,0,0);
    r13.xyzw = float4(0,0,0,0);
    r0.w = 0;
  }
  r3.x = min(15, r3.x);
  r3.x = (uint)r3.x;
  r14.xy = (uint2)r3.yz >> int2(6,6);
  r14.z = (uint)r3.x << 4;
  r15.x = -r7.x;
  r3.x = ~(int)r1.w;
  r3.y = -r6.x * 0.5 + 1;
  r16.xy = float2(0,0);
  r17.w = 0;
  r18.xyz = v6.xyz;
  r18.w = 1;
  r19.w = 1;
  r15.z = r7.x;
  r20.w = 1;
  r21.z = 1;
  r22.w = 1;
  r15.yw = r8.xx;
  r7.y = r15.w;
  r23.w = 1;
  r8.y = r15.x;
  r8.z = r7.x;
  r24.w = 1;
  r25.x = r8.x;
  r25.y = r15.x;
  r25.z = r7.x;
  r26.x = r8.x;
  r26.y = r15.x;
  r26.z = r7.x;
  r27.x = r8.x;
  r27.y = r15.x;
  r27.z = r7.x;
  r9.yzw = r12.xyz;
  r28.x = r12.w;
  r28.yz = r13.xy;
  r29.xy = r13.zw;
  r29.z = r0.w;
  r3.z = enableDitheredShadow;
  r7.z = 0;
  while (true) {
    r7.w = cmp((uint)r7.z >= numLights);
    if (r7.w != 0) break;
    r16.z = (uint)r7.z >> 5;
    r17.xyz = (int3)r14.xyz + (int3)r16.xyz;
    r7.w = visibleLights.Load(r17.xyzw).x;
    r10.yzw = r9.yzw;
    r17.xyz = r28.xyz;
    r30.xyz = r29.xyz;
    r8.w = r3.z;
    r14.w = r7.w;
    while (true) {
      if (r14.w == 0) break;
      r16.z = firstbitlow((uint)r14.w);
      r16.w = 1 << (int)r16.z;
      r21.w = (int)r14.w & (int)r16.w;
      if (r21.w != 0) {
        r14.w = (int)r14.w ^ (int)r16.w;
        r16.z = (int)r7.z + (int)r16.z;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r31.xyzw, r16.z, l(0), t11.xyzw
      r31.x = samp0[]..swiz;
      r31.y = samp0[]..swiz;
      r31.z = samp0[]..swiz;
      r31.w = samp0[]..swiz;
      // Known bad code for instruction (needs manual fix):
            ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r32.xy, r16.z, l(16), t11.xyxx
      r32.x = samp0[]..swiz;
      r32.y = samp0[]..swiz;
        r31.xyz = -v6.xyz + r31.xyz;
        r32.z = r31.w;
        r31.xyz = cmp(abs(r31.xyz) < r32.zxy);
        r16.w = r31.y ? r31.x : 0;
        r16.w = r31.z ? r16.w : 0;
        if (r16.w != 0) {
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r31.xyzw, r16.z, l(0), t12.wxyz
        r31.x = samp0[]..swiz;
        r31.y = samp0[]..swiz;
        r31.z = samp0[]..swiz;
        r31.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r32.xyzw, r16.z, l(16), t12.zxyw
        r32.x = samp0[]..swiz;
        r32.y = samp0[]..swiz;
        r32.z = samp0[]..swiz;
        r32.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r33.xyzw, r16.z, l(32), t12.xyzw
        r33.x = samp0[]..swiz;
        r33.y = samp0[]..swiz;
        r33.z = samp0[]..swiz;
        r33.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r34.xyzw, r16.z, l(48), t12.xyzw
        r34.x = samp0[]..swiz;
        r34.y = samp0[]..swiz;
        r34.z = samp0[]..swiz;
        r34.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r16.z, l(64), t12.yzwx
        r35.x = samp0[]..swiz;
        r35.y = samp0[]..swiz;
        r35.z = samp0[]..swiz;
        r35.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.xyzw, r16.z, l(80), t12.xyzw
        r36.x = samp0[]..swiz;
        r36.y = samp0[]..swiz;
        r36.z = samp0[]..swiz;
        r36.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyzw, r16.z, l(96), t12.yzwx
        r37.x = samp0[]..swiz;
        r37.y = samp0[]..swiz;
        r37.z = samp0[]..swiz;
        r37.w = samp0[]..swiz;
          if (3 == 0) r16.w = 0; else if (3+24 < 32) {           r16.w = (uint)r32.w << (32-(3 + 24)); r16.w = (uint)r16.w >> (32-3);          } else r16.w = (uint)r32.w >> 24;
          switch (r16.w) {
            case 4 :            r16.w = cmp(0 < r37.x);
            r38.xy = r36.zw;
            r38.z = r37.w;
            r39.xyz = -r38.xyz * float3(0.5,0.5,0.5) + r31.yzw;
            r40.xyz = -v6.xyz + r39.xyz;
            r21.w = dot(r38.xyz, r40.xyz);
            r25.w = saturate(-r21.w / r37.x);
            r41.xyz = r25.www * r38.xyz + r39.xyz;
            r41.xyz = r16.www ? r41.xyz : r31.yzw;
            r41.xyz = -v6.xyz + r41.xyz;
            r26.w = dot(r41.xyz, r41.xyz);
            r27.w = rsqrt(r26.w);
            r41.xyz = r41.xyz * r27.www;
            r27.w = dot(r5.xyz, r41.xyz);
            r28.w = saturate(r27.w);
            r29.w = cmp(0 < r28.w);
            if (r29.w != 0) {
              r29.w = sqrt(r26.w);
              r30.w = r34.x * r34.x;
              r26.w = r30.w / r26.w;
              r26.w = min(1, r26.w);
              r41.xy = saturate(r29.ww * r33.xz + r33.yw);
              r41.zw = r41.xy * r41.xy;
              r41.xy = r41.xy * float2(-2,-2) + float2(3,3);
              r41.xy = r41.zw * r41.xy;
              r26.w = r41.x * r26.w;
              r26.w = r26.w * r41.y;
              r29.w = cmp(0 < r26.w);
              if (r29.w != 0) {
                if (3 == 0) r29.w = 0; else if (3+27 < 32) {                 r29.w = (uint)r32.w << (32-(3 + 27)); r29.w = (uint)r29.w >> (32-3);                } else r29.w = (uint)r32.w >> 27;
                r29.w = cmp((int)r29.w != 1);
                if (r29.w != 0) {
                  r29.w = abs(r27.w) * -0.200000003 + 0.400000006;
                  r41.xyz = r5.xyz * r29.www + v6.xyz;
                  r41.xyz = r41.xyz + -r35.xyz;
                  r29.w = max(abs(r41.y), abs(r41.z));
                  r29.w = max(abs(r41.x), r29.w);
                  r29.w = r36.x / r29.w;
                  r29.w = r29.w + r36.y;
                  r30.w = dot(r41.xyz, r41.xyz);
                  r30.w = rsqrt(r30.w);
                  r29.w = max(6.10351563e-05, r29.w);
                  r38.w = (int)r32.w & 0x0000ffff;
                  r42.w = (uint)r38.w;
                  r38.w = 0;
                  r39.w = 0;
                  while (true) {
                    r40.w = cmp((int)r39.w >= 8);
                    if (r40.w != 0) break;
                    r43.y = dot(icb[r39.w+0].yx, r15.xy);
                    r43.z = dot(icb[r39.w+0].yx, r15.yz);
                    r43.yz = r43.yz * r34.yy;
                    r43.x = r43.y * r10.x;
                    r43.w = r43.y * r9.x;
                    r42.xyz = r41.xyz * r30.www + r43.xzw;
                    r40.w = gOmniShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r42.xyzw, r29.w).x;
                    r38.w = r40.w * 0.125 + r38.w;
                    r39.w = (int)r39.w + 1;
                  }
                } else {
                  r38.w = 1;
                }
                r26.w = r38.w * r26.w;
                r29.w = cmp(0 < r26.w);
                if (r29.w != 0) {
                  r29.w = r32.x * r3.w;
                  r29.w = 0.25 * r29.w;
                  r30.w = dot(r38.xyz, r11.xyz);
                  r39.w = dot(r11.xyz, r40.xyz);
                  r40.x = -r30.w * r30.w + r37.x;
                  r21.w = r30.w * r39.w + -r21.w;
                  r21.w = saturate(r21.w / r40.x);
                  r30.w = r40.x / r37.x;
                  r30.w = 10 * r30.w;
                  r30.w = min(1, r30.w);
                  r21.w = r21.w + -r25.w;
                  r21.w = r30.w * r21.w + r25.w;
                  r38.xyz = r21.www * r38.xyz + r39.xyz;
                  r38.xyz = r16.www ? r38.xyz : r31.yzw;
                  r38.xyz = -v6.xyz + r38.xyz;
                  r16.w = dot(r38.xyz, r38.xyz);
                  r16.w = rsqrt(r16.w);
                  r39.xyz = r38.xyz * r16.www;
                  if (4 == 0) r21.w = 0; else if (4+16 < 32) {                   r21.w = (uint)r32.w << (32-(4 + 16)); r21.w = (uint)r21.w >> (32-4);                  } else r21.w = (uint)r32.w >> 16;
                  r25.w = cmp(0 < (uint)r21.w);
                  r25.w = r1.w ? r25.w : 0;
                  if (r25.w != 0) {
                    r21.w = (int)r21.w + numLights;
                    r21.w = (int)r21.w + -1;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r25.w, r21.w, l(52), t12.xxxx
                  r25.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r21.w, l(100), t12.xyzw
                  r40.x = samp0[]..swiz;
                  r40.y = samp0[]..swiz;
                  r40.z = samp0[]..swiz;
                  r40.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r21.w, l(116), t12.xyzw
                  r41.x = samp0[]..swiz;
                  r41.y = samp0[]..swiz;
                  r41.z = samp0[]..swiz;
                  r41.w = samp0[]..swiz;
                    r30.w = abs(r27.w) * -0.200000003 + 0.400000006;
                    r19.xyz = r5.xyz * r30.www + v6.xyz;
                    r30.w = dot(r40.xyzw, r19.xyzw);
                    r39.w = dot(r41.xyzw, r19.xyzw);
                    r40.x = cmp(r39.w < r30.w);
                    if (r40.x == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r21.w, l(68), t12.xyzw
                    r41.x = samp0[]..swiz;
                    r41.y = samp0[]..swiz;
                    r41.z = samp0[]..swiz;
                    r41.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r42.xyzw, r21.w, l(84), t12.xyzw
                    r42.x = samp0[]..swiz;
                    r42.y = samp0[]..swiz;
                    r42.z = samp0[]..swiz;
                    r42.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r43.xyzw, r21.w, l(132), t12.xyzw
                    r43.x = samp0[]..swiz;
                    r43.y = samp0[]..swiz;
                    r43.z = samp0[]..swiz;
                    r43.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r21.w, l(148), t12.xyzw
                    r44.x = samp0[]..swiz;
                    r44.y = samp0[]..swiz;
                    r44.z = samp0[]..swiz;
                    r44.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.yz, r21.w, l(164), t12.xxyx
                    r40.y = samp0[]..swiz;
                    r40.z = samp0[]..swiz;
                      r41.x = dot(r41.xyzw, r19.xyzw);
                      r41.y = dot(r42.xyzw, r19.xyzw);
                      r19.xy = r41.xy / r39.ww;
                      r19.xy = r19.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                      r19.xy = r19.xy + r44.zw;
                      r19.xy = r19.xy * r44.xy;
                      r41.xy = r25.ww / r43.xz;
                      r41.zw = float2(1,1) + -r41.xy;
                      r41.zw = cmp(r19.xy >= r41.zw);
                      r41.xy = cmp(r41.xy >= r19.xy);
                      r41.xy = (int2)r41.xy | (int2)r41.zw;
                      r19.z = (int)r41.y | (int)r41.x;
                      r19.xy = saturate(r19.xy);
                      r41.xy = r19.xy * r43.xz + r43.yw;
                      r19.x = r40.z * r39.w;
                      r19.y = r40.y * r39.w + r30.w;
                      r19.x = r19.y / r19.x;
                    } else {
                      r19.z = -1;
                    }
                    r19.y = (int)r40.x | (int)r19.z;
                    if (r19.y == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r19.y, r21.w, l(28), t12.xxxx
                    r19.y = samp0[]..swiz;
                      r19.x = max(6.10351563e-05, r19.x);
                      r19.y = (int)r19.y & 0x0000ffff;
                      if (r8.w != 0) {
                        r40.z = (uint)r19.y;
                        r19.z = 0;
                        r21.w = 0;
                        while (true) {
                          r30.w = cmp((int)r21.w >= 8);
                          if (r30.w != 0) break;
                          r42.x = dot(icb[r21.w+0].yx, r15.xw);
                          r42.y = dot(icb[r21.w+0].xy, r7.xy);
                          r40.xy = r42.xy * r25.ww + r41.xy;
                          r30.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r40.xyz, r19.x).x;
                          r19.z = r30.w * 0.125 + r19.z;
                          r21.w = (int)r21.w + 1;
                        }
                      } else {
                        r41.z = (uint)r19.y;
                        r19.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r41.xyz, r19.x).x;
                      }
                      r19.x = r19.z * r19.z;
                      r19.x = r19.x * r19.z;
                    } else {
                      r19.x = 1;
                    }
                    r26.w = r26.w * r19.x;
                  } else {
                    if (4 == 0) r19.x = 0; else if (4+20 < 32) {                     r19.x = (uint)r32.w << (32-(4 + 20)); r19.x = (uint)r19.x >> (32-4);                    } else r19.x = (uint)r32.w >> 20;
                    r19.y = cmp(0 < (uint)r19.x);
                    r19.y = r19.y ? r3.x : 0;
                    if (r19.y != 0) {
                      r19.x = (int)r19.x + numLights;
                      r19.x = (int)r19.x + -1;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r19.y, r19.x, l(52), t12.xxxx
                    r19.y = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r19.x, l(100), t12.xyzw
                    r40.x = samp0[]..swiz;
                    r40.y = samp0[]..swiz;
                    r40.z = samp0[]..swiz;
                    r40.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r19.x, l(116), t12.xyzw
                    r41.x = samp0[]..swiz;
                    r41.y = samp0[]..swiz;
                    r41.z = samp0[]..swiz;
                    r41.w = samp0[]..swiz;
                      r21.w = abs(r27.w) * -0.200000003 + 0.400000006;
                      r20.xyz = r5.xyz * r21.www + v6.xyz;
                      r21.w = dot(r40.xyzw, r20.xyzw);
                      r25.w = dot(r41.xyzw, r20.xyzw);
                      r30.w = cmp(r25.w < r21.w);
                      if (r30.w == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r19.x, l(68), t12.xyzw
                      r40.x = samp0[]..swiz;
                      r40.y = samp0[]..swiz;
                      r40.z = samp0[]..swiz;
                      r40.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r41.xyzw, r19.x, l(84), t12.xyzw
                      r41.x = samp0[]..swiz;
                      r41.y = samp0[]..swiz;
                      r41.z = samp0[]..swiz;
                      r41.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r42.xyzw, r19.x, l(132), t12.xyzw
                      r42.x = samp0[]..swiz;
                      r42.y = samp0[]..swiz;
                      r42.z = samp0[]..swiz;
                      r42.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r43.xyzw, r19.x, l(148), t12.xyzw
                      r43.x = samp0[]..swiz;
                      r43.y = samp0[]..swiz;
                      r43.z = samp0[]..swiz;
                      r43.w = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xy, r19.x, l(164), t12.xyxx
                      r44.x = samp0[]..swiz;
                      r44.y = samp0[]..swiz;
                        r40.x = dot(r40.xyzw, r20.xyzw);
                        r40.y = dot(r41.xyzw, r20.xyzw);
                        r20.xy = r40.xy / r25.ww;
                        r20.xy = r20.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                        r20.xy = r20.xy + r43.zw;
                        r20.xy = r20.xy * r43.xy;
                        r40.xy = r19.yy / r42.xz;
                        r40.zw = float2(1,1) + -r40.xy;
                        r40.zw = cmp(r20.xy >= r40.zw);
                        r40.xy = cmp(r40.xy >= r20.xy);
                        r40.xy = (int2)r40.xy | (int2)r40.zw;
                        r20.z = (int)r40.y | (int)r40.x;
                        r20.xy = saturate(r20.xy);
                        r40.xy = r20.xy * r42.xz + r42.yw;
                        r20.x = r44.y * r25.w;
                        r20.y = r44.x * r25.w + r21.w;
                        r20.x = r20.y / r20.x;
                      } else {
                        r20.z = -1;
                      }
                      r20.y = (int)r30.w | (int)r20.z;
                      if (r20.y == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r19.x, r19.x, l(28), t12.xxxx
                      r19.x = samp0[]..swiz;
                        r20.x = max(6.10351563e-05, r20.x);
                        r19.x = (int)r19.x & 0x0000ffff;
                        if (r8.w != 0) {
                          r41.z = (uint)r19.x;
                          r20.yz = float2(0,0);
                          while (true) {
                            r21.w = cmp((int)r20.z >= 8);
                            if (r21.w != 0) break;
                            r42.x = dot(icb[r20.z+0].xy, r8.xy);
                            r42.y = dot(icb[r20.z+0].yx, r8.xz);
                            r41.xy = r42.xy * r19.yy + r40.xy;
                            r21.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r41.xyz, r20.x).x;
                            r20.y = r21.w * 0.125 + r20.y;
                            r20.z = (int)r20.z + 1;
                          }
                        } else {
                          r40.z = (uint)r19.x;
                          r20.y = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r40.xyz, r20.x).x;
                        }
                        r19.x = r20.y * r20.y;
                        r19.x = r19.x * r20.y;
                      } else {
                        r19.x = 1;
                      }
                      r26.w = r26.w * r19.x;
                    }
                  }
                  r19.x = -r28.w * r3.y + 1;
                  r19.x = r19.x * r19.x;
                  r19.x = -r19.x * 0.620000005 + 0.620000005;
                  r19.x = r19.x + -r28.w;
                  r19.x = r4.w * r19.x + r28.w;
                  r19.x = r19.x * r26.w;
                  r32.x = r31.x;
                  r19.y = cmp(0 < r27.w);
                  r40.xyz = r19.xxx * r32.xyz + r10.yzw;
                  r19.x = saturate(dot(r5.xyz, r39.xyz));
                  r38.xyz = r38.xyz * r16.www + r2.yzw;
                  r16.w = dot(r38.xyz, r38.xyz);
                  r16.w = rsqrt(r16.w);
                  r38.xyz = r38.xyz * r16.www;
                  r16.w = dot(r38.xyz, r2.yzw);
                  r20.x = dot(r5.xyz, r38.xyz);
                  r20.z = abs(r20.x) * r3.w + -abs(r20.x);
                  r20.x = abs(r20.x) * r20.z + 1;
                  r20.z = r19.x * r6.z + r6.w;
                  r20.x = r20.x * r20.x;
                  r20.x = r20.x * r20.z;
                  r20.x = rcp(r20.x);
                  r19.x = r19.x * r29.w;
                  r19.x = r20.x * r19.x;
                  r19.x = r19.x * r26.w;
                  r38.xyz = r19.xxx * r32.xyz + r30.xyz;
                  r16.w = saturate(1 + -r16.w);
                  r20.x = r16.w * r16.w;
                  r20.x = r20.x * r20.x;
                  r16.w = r20.x * r16.w;
                  r16.w = r19.x * r16.w;
                  r39.xyz = r16.www * r32.xyz + r17.xyz;
                  r10.yzw = r19.yyy ? r40.xyz : r10.yzw;
                  r17.xyz = r19.yyy ? r39.xyz : r17.xyz;
                  r30.xyz = r19.yyy ? r38.xyz : r30.xyz;
                }
              }
            }
            break;
            case 2 :            r38.xy = r34.zw;
            r38.z = r35.w;
            r38.xyz = -v6.xyz + r38.xyz;
            r16.w = dot(r38.xyz, r38.xyz);
            r16.w = rsqrt(r16.w);
            r39.xyz = r38.xyz * r16.www;
            r19.x = dot(r5.xyz, r39.xyz);
            r19.y = saturate(r19.x);
            r20.x = cmp(0 < r19.y);
            if (r20.x != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r39.xyzw, r16.z, l(112), t12.yzwx
            r39.x = samp0[]..swiz;
            r39.y = samp0[]..swiz;
            r39.z = samp0[]..swiz;
            r39.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.xyzw, r16.z, l(128), t12.zxyw
            r40.x = samp0[]..swiz;
            r40.y = samp0[]..swiz;
            r40.z = samp0[]..swiz;
            r40.w = samp0[]..swiz;
              r41.xyz = r39.xyz;
              r41.w = r40.y;
              r20.x = dot(r41.xyzw, r18.xyzw);
              r20.z = cmp(r20.x < 1);
              if (r20.z != 0) {
                r42.xyz = float3(1,1,1);
                r20.z = 0;
              } else {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r43.xyzw, r16.z, l(172), t12.yzwx
              r43.x = samp0[]..swiz;
              r43.y = samp0[]..swiz;
              r43.z = samp0[]..swiz;
              r43.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r44.xyzw, r16.z, l(188), t12.wxyz
              r44.x = samp0[]..swiz;
              r44.y = samp0[]..swiz;
              r44.z = samp0[]..swiz;
              r44.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r45.xyzw, r16.z, l(204), t12.xyzw
              r45.x = samp0[]..swiz;
              r45.y = samp0[]..swiz;
              r45.z = samp0[]..swiz;
              r45.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r21.w, r16.z, l(236), t12.xxxx
              r21.w = samp0[]..swiz;
                r46.xyz = -v6.xyz + r31.yzw;
                r25.w = r34.x * r34.x;
                r26.w = dot(r46.xyz, r46.xyz);
                r25.w = r25.w / r26.w;
                r25.w = min(1, r25.w);
                r33.xy = saturate(r20.xx * r33.xz + r33.yw);
                r33.zw = r33.xy * r33.xy;
                r33.xy = r33.xy * float2(-2,-2) + float2(3,3);
                r33.xy = r33.zw * r33.xy;
                r25.w = r33.x * r25.w;
                r25.w = r25.w * r33.y;
                r35.w = r36.x;
                r33.x = dot(r35.xyzw, r18.xyzw);
                r46.xyz = r36.yzw;
                r46.w = r37.w;
                r33.y = dot(r46.xyzw, r18.xyzw);
                r21.xy = r33.xy / r20.xx;
                r20.x = cmp(r43.w < 0.00048828125);
                if (r20.x != 0) {
                  r44.y = r45.x;
                  r33.xy = saturate(abs(r21.xy) * r44.zw + r44.xy);
                  r33.zw = r33.xy * r33.xy;
                  r33.xy = r33.xy * float2(-2,-2) + float2(3,3);
                  r33.xy = r33.zw * r33.xy;
                  r20.x = r33.x * r33.y;
                } else {
                  r43.w = r44.y;
                  r33.xyzw = saturate(r43.xyzw * abs(r21.yyxx));
                  r33.xyzw = log2(r33.xyzw);
                  r33.xyzw = r44.xxxx * r33.xyzw;
                  r33.xyzw = exp2(r33.xyzw);
                  r33.xy = r33.xy + r33.zw;
                  r33.xy = log2(r33.xy);
                  r33.xy = r45.xx * r33.xy;
                  r33.xy = exp2(r33.xy);
                  r26.w = r44.z * r33.x;
                  r27.w = r44.w * r33.y + -1;
                  r26.w = r44.w * r33.y + -r26.w;
                  r26.w = saturate(r27.w / r26.w);
                  r27.w = r26.w * r26.w;
                  r26.w = r26.w * -2 + 3;
                  r20.x = r27.w * r26.w;
                }
                r20.z = r25.w * r20.x;
                r20.x = (int)r21.w & 255;
                if (r20.x != 0) {
                // Known bad code for instruction (needs manual fix):
                                ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r33.xyz, r16.z, l(220), t12.xyzx
                r33.x = samp0[]..swiz;
                r33.y = samp0[]..swiz;
                r33.z = samp0[]..swiz;
                  r21.w = dot(r45.yzw, r21.xyz);
                  r21.x = dot(r33.xyz, r21.xyz);
                  r33.xy = frac(r21.wx);
                  r20.x = (int)r20.x + -1;
                  r33.z = (uint)r20.x;
                  r42.xyz = gCookieArray.SampleLevel(samplerLinear_s, r33.xyz, 0).xyz;
                } else {
                  r42.xyz = float3(1,1,1);
                }
              }
              r31.yz = r32.yz;
              r21.xyw = r31.xyz * r42.xyz;
              r20.x = cmp(0 < r20.z);
              if (r20.x != 0) {
                if (3 == 0) r20.x = 0; else if (3+27 < 32) {                 r20.x = (uint)r32.w << (32-(3 + 27)); r20.x = (uint)r20.x >> (32-3);                } else r20.x = (uint)r32.w >> 27;
                r20.x = cmp((int)r20.x != 1);
                if (r20.x != 0) {
                  r20.x = abs(r19.x) * -0.200000003 + 0.400000006;
                  r22.xyz = r5.xyz * r20.xxx + v6.xyz;
                  r39.xyz = r37.xyz;
                  r20.x = dot(r39.xyzw, r22.xyzw);
                  r25.w = dot(r41.xyzw, r22.xyzw);
                  r26.w = cmp(r25.w >= r20.x);
                  if (r26.w != 0) {
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r40.y, r16.z, l(144), t12.xxxx
                  r40.y = samp0[]..swiz;
                    r35.w = r36.x;
                    r31.x = dot(r35.xyzw, r22.xyzw);
                    r37.xyz = r36.yzw;
                    r31.y = dot(r37.xyzw, r22.xyzw);
                    r22.xy = r31.xy / r25.ww;
                    r22.xy = saturate(r22.xy * float2(0.5,-0.5) + float2(0.5,0.5));
                    r22.xy = r22.xy * r40.zw + r40.xy;
                    r16.z = r20.x / r25.w;
                    r16.z = max(6.10351563e-05, r16.z);
                    r20.x = (int)r32.w & 0x0000ffff;
                    if (r8.w != 0) {
                      r31.z = (uint)r20.x;
                      r25.w = 0;
                      r26.w = 0;
                      while (true) {
                        r27.w = cmp((int)r26.w >= 8);
                        if (r27.w != 0) break;
                        r33.x = dot(icb[r26.w+0].xy, r25.xy);
                        r33.y = dot(icb[r26.w+0].yx, r25.xz);
                        r31.xy = r33.xy * r34.yy + r22.xy;
                        r27.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r31.xyz, r16.z).x;
                        r25.w = r27.w * 0.125 + r25.w;
                        r26.w = (int)r26.w + 1;
                      }
                    } else {
                      r22.z = (uint)r20.x;
                      r25.w = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r22.xyz, r16.z).x;
                    }
                    r16.z = r25.w * r25.w;
                    r16.z = r16.z * r25.w;
                  } else {
                    r16.z = 1;
                  }
                } else {
                  r16.z = 1;
                }
                r16.z = r20.z * r16.z;
                r20.x = cmp(0 < r16.z);
                if (r20.x != 0) {
                  r20.x = r32.x * r3.w;
                  r20.x = r20.x * r19.y;
                  if (4 == 0) r20.z = 0; else if (4+16 < 32) {                   r20.z = (uint)r32.w << (32-(4 + 16)); r20.z = (uint)r20.z >> (32-4);                  } else r20.z = (uint)r32.w >> 16;
                  r22.x = cmp(0 < (uint)r20.z);
                  r22.x = r1.w ? r22.x : 0;
                  if (r22.x != 0) {
                    r20.z = (int)r20.z + numLights;
                    r20.z = (int)r20.z + -1;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r22.x, r20.z, l(52), t12.xxxx
                  r22.x = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r31.xyzw, r20.z, l(68), t12.xyzw
                  r31.x = samp0[]..swiz;
                  r31.y = samp0[]..swiz;
                  r31.z = samp0[]..swiz;
                  r31.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r33.xyzw, r20.z, l(84), t12.xyzw
                  r33.x = samp0[]..swiz;
                  r33.y = samp0[]..swiz;
                  r33.z = samp0[]..swiz;
                  r33.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r34.xyzw, r20.z, l(100), t12.xyzw
                  r34.x = samp0[]..swiz;
                  r34.y = samp0[]..swiz;
                  r34.z = samp0[]..swiz;
                  r34.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r20.z, l(116), t12.xyzw
                  r35.x = samp0[]..swiz;
                  r35.y = samp0[]..swiz;
                  r35.z = samp0[]..swiz;
                  r35.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.xyzw, r20.z, l(132), t12.xyzw
                  r36.x = samp0[]..swiz;
                  r36.y = samp0[]..swiz;
                  r36.z = samp0[]..swiz;
                  r36.w = samp0[]..swiz;
                  // Known bad code for instruction (needs manual fix):
                                    ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r37.xyzw, r20.z, l(148), t12.xyzw
                  r37.x = samp0[]..swiz;
                  r37.y = samp0[]..swiz;
                  r37.z = samp0[]..swiz;
                  r37.w = samp0[]..swiz;
                    r22.y = abs(r19.x) * -0.200000003 + 0.400000006;
                    r23.xyz = r5.xyz * r22.yyy + v6.xyz;
                    r31.x = dot(r31.xyzw, r23.xyzw);
                    r31.y = dot(r33.xyzw, r23.xyzw);
                    r22.y = dot(r34.xyzw, r23.xyzw);
                    r22.z = dot(r35.xyzw, r23.xyzw);
                    r23.x = cmp(r22.z < r22.y);
                    r23.yz = r31.xy / r22.zz;
                    r23.yz = r23.yz * float2(0.5,-0.5) + float2(0.5,0.5);
                    r23.yz = r23.yz + r37.zw;
                    r23.yz = r23.yz * r37.xy;
                    r31.xy = r22.xx / r36.xz;
                    r31.zw = float2(1,1) + -r31.xy;
                    r31.zw = cmp(r23.yz >= r31.zw);
                    r31.xy = cmp(r31.xy >= r23.yz);
                    r31.xy = (int2)r31.xy | (int2)r31.zw;
                    r26.w = (int)r31.y | (int)r31.x;
                    r23.x = (int)r23.x | (int)r26.w;
                    if (r23.x == 0) {
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r23.x, r20.z, l(28), t12.xxxx
                    r23.x = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r31.xy, r20.z, l(164), t12.xyxx
                    r31.x = samp0[]..swiz;
                    r31.y = samp0[]..swiz;
                      r23.yz = saturate(r23.yz);
                      r32.xy = r23.yz * r36.xz + r36.yw;
                      r20.z = r31.y * r22.z;
                      r22.y = r31.x * r22.z + r22.y;
                      r20.z = r22.y / r20.z;
                      r20.z = max(6.10351563e-05, r20.z);
                      r22.y = r23.x ? 0.000000 : 0;
                      if (r8.w != 0) {
                        r23.z = (uint)r22.y;
                        r22.z = 0;
                        r26.w = 0;
                        while (true) {
                          r27.w = cmp((int)r26.w >= 8);
                          if (r27.w != 0) break;
                          r31.x = dot(icb[r26.w+0].xy, r26.xy);
                          r31.y = dot(icb[r26.w+0].yx, r26.xz);
                          r23.xy = r31.xy * r22.xx + r32.xy;
                          r23.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r23.xyz, r20.z).x;
                          r22.z = r23.x * 0.125 + r22.z;
                          r26.w = (int)r26.w + 1;
                        }
                      } else {
                        r32.z = (uint)r22.y;
                        r22.z = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r32.xyz, r20.z).x;
                      }
                      r20.z = r22.z * r22.z;
                      r20.z = r20.z * r22.z;
                    } else {
                      r20.z = 1;
                    }
                    r16.z = r20.z * r16.z;
                  } else {
                    if (4 == 0) r20.z = 0; else if (4+20 < 32) {                     r20.z = (uint)r32.w << (32-(4 + 20)); r20.z = (uint)r20.z >> (32-4);                    } else r20.z = (uint)r32.w >> 20;
                    r22.x = cmp(0 < (uint)r20.z);
                    r22.x = r22.x ? r3.x : 0;
                    if (r22.x != 0) {
                      r20.z = (int)r20.z + numLights;
                      r20.z = (int)r20.z + -1;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r22.x, r20.z, l(52), t12.xxxx
                    r22.x = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r31.xyzw, r20.z, l(68), t12.xyzw
                    r31.x = samp0[]..swiz;
                    r31.y = samp0[]..swiz;
                    r31.z = samp0[]..swiz;
                    r31.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r32.xyzw, r20.z, l(84), t12.xyzw
                    r32.x = samp0[]..swiz;
                    r32.y = samp0[]..swiz;
                    r32.z = samp0[]..swiz;
                    r32.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r33.xyzw, r20.z, l(100), t12.xyzw
                    r33.x = samp0[]..swiz;
                    r33.y = samp0[]..swiz;
                    r33.z = samp0[]..swiz;
                    r33.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r34.xyzw, r20.z, l(116), t12.xyzw
                    r34.x = samp0[]..swiz;
                    r34.y = samp0[]..swiz;
                    r34.z = samp0[]..swiz;
                    r34.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r35.xyzw, r20.z, l(132), t12.xyzw
                    r35.x = samp0[]..swiz;
                    r35.y = samp0[]..swiz;
                    r35.z = samp0[]..swiz;
                    r35.w = samp0[]..swiz;
                    // Known bad code for instruction (needs manual fix):
                                        ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r36.xyzw, r20.z, l(148), t12.xyzw
                    r36.x = samp0[]..swiz;
                    r36.y = samp0[]..swiz;
                    r36.z = samp0[]..swiz;
                    r36.w = samp0[]..swiz;
                      r22.y = abs(r19.x) * -0.200000003 + 0.400000006;
                      r24.xyz = r5.xyz * r22.yyy + v6.xyz;
                      r23.x = dot(r31.xyzw, r24.xyzw);
                      r23.y = dot(r32.xyzw, r24.xyzw);
                      r22.y = dot(r33.xyzw, r24.xyzw);
                      r23.z = dot(r34.xyzw, r24.xyzw);
                      r24.x = cmp(r23.z < r22.y);
                      r23.xy = r23.xy / r23.zz;
                      r23.xy = r23.xy * float2(0.5,-0.5) + float2(0.5,0.5);
                      r23.xy = r23.xy + r36.zw;
                      r23.xy = r23.xy * r36.xy;
                      r24.yz = r22.xx / r35.xz;
                      r31.xy = float2(1,1) + -r24.yz;
                      r31.xy = cmp(r23.xy >= r31.xy);
                      r24.yz = cmp(r24.yz >= r23.xy);
                      r24.yz = (int2)r24.yz | (int2)r31.xy;
                      r24.y = (int)r24.z | (int)r24.y;
                      r24.x = (int)r24.x | (int)r24.y;
                      if (r24.x == 0) {
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r24.x, r20.z, l(28), t12.xxxx
                      r24.x = samp0[]..swiz;
                      // Known bad code for instruction (needs manual fix):
                                            ld_structured_indexable(structured_buffer, stride=240)(mixed,mixed,mixed,mixed) r24.yz, r20.z, l(164), t12.xxyx
                      r24.y = samp0[]..swiz;
                      r24.z = samp0[]..swiz;
                        r23.xy = saturate(r23.xy);
                        r31.xy = r23.xy * r35.xz + r35.yw;
                        r20.z = r24.z * r23.z;
                        r22.y = r24.y * r23.z + r22.y;
                        r20.z = r22.y / r20.z;
                        r20.z = max(6.10351563e-05, r20.z);
                        r22.y = r24.x ? 0.000000 : 0;
                        if (r8.w != 0) {
                          r23.z = (uint)r22.y;
                          r24.xy = float2(0,0);
                          while (true) {
                            r24.z = cmp((int)r24.y >= 8);
                            if (r24.z != 0) break;
                            r32.x = dot(icb[r24.y+0].xy, r27.xy);
                            r32.y = dot(icb[r24.y+0].yx, r27.xz);
                            r23.xy = r32.xy * r22.xx + r31.xy;
                            r23.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r23.xyz, r20.z).x;
                            r24.x = r23.x * 0.125 + r24.x;
                            r24.y = (int)r24.y + 1;
                          }
                        } else {
                          r31.z = (uint)r22.y;
                          r24.x = gSpotShadowmapArray.SampleCmpLevelZero(ShadowSamplerComparisonState_s, r31.xyz, r20.z).x;
                        }
                        r20.z = r24.x * r24.x;
                        r20.z = r20.z * r24.x;
                      } else {
                        r20.z = 1;
                      }
                      r16.z = r20.z * r16.z;
                    }
                  }
                  r20.z = -r19.y * r3.y + 1;
                  r20.z = r20.z * r20.z;
                  r20.z = -r20.z * 0.620000005 + 0.620000005;
                  r20.z = r20.z + -r19.y;
                  r20.z = r4.w * r20.z + r19.y;
                  r20.z = r20.z * r16.z;
                  r19.x = cmp(0 < r19.x);
                  r23.xyz = r20.zzz * r21.xyw + r10.yzw;
                  r31.xyz = r38.xyz * r16.www + r2.yzw;
                  r16.w = dot(r31.xyz, r31.xyz);
                  r16.w = rsqrt(r16.w);
                  r31.xyz = r31.xyz * r16.www;
                  r16.w = dot(r31.xyz, r2.yzw);
                  r20.z = dot(r5.xyz, r31.xyz);
                  r22.x = abs(r20.z) * r3.w + -abs(r20.z);
                  r20.z = abs(r20.z) * r22.x + 1;
                  r19.y = r19.y * r6.z + r6.w;
                  r20.z = r20.z * r20.z;
                  r19.y = r20.z * r19.y;
                  r19.y = rcp(r19.y);
                  r19.y = r19.y * r20.x;
                  r16.z = r19.y * r16.z;
                  r16.z = 0.25 * r16.z;
                  r31.xyz = r16.zzz * r21.xyw + r30.xyz;
                  r16.w = saturate(1 + -r16.w);
                  r19.y = r16.w * r16.w;
                  r19.y = r19.y * r19.y;
                  r16.w = r19.y * r16.w;
                  r16.z = r16.z * r16.w;
                  r21.xyw = r16.zzz * r21.xyw + r17.xyz;
                  r10.yzw = r19.xxx ? r23.xyz : r10.yzw;
                  r17.xyz = r19.xxx ? r21.xyw : r17.xyz;
                  r30.xyz = r19.xxx ? r31.xyz : r30.xyz;
                }
              }
            }
            break;
            default :
            break;
          }
        }
      }
    }
    r9.yzw = r10.yzw;
    r28.xyz = r17.xyz;
    r29.xyz = r30.xyz;
    r7.z = (int)r7.z + 32;
  }
  r0.w = 1 + -r6.y;
  r1.w = 5 * r0.w;
  r2.y = r0.w * 5 + -2.5;
  r2.y = saturate(0.400000006 * r2.y);
  r2.y = 100 * r2.y;
  r2.zw = -r0.ww * float2(10,5) + float2(6.85740995,7.08500004);
  r2.z = exp2(r2.z);
  r2.z = r6.x * r2.z;
  r3.x = r1.w * r0.w;
  r2.w = -r3.x * 2.0159049 + r2.w;
  r2.w = exp2(r2.w);
  r2.w = r6.x * r2.w;
  r2.zw = float2(9.1368103,9.70808983) * r2.zw;
  r2.z = max(r2.z, r2.w);
  r2.z = max(1.26815999, r2.z);
  r3.xy = float2(0,0);
  r7.w = 0;
  r8.yz = float2(0,1);
  r10.xy = float2(0,0);
  r12.xy = float2(0,0);
  r13.xy = float2(0,0);
  r15.x = 1;
  r16.xyzw = float4(0,0,0,0);
  r17.yzw = float3(0,0,0);
  r2.w = 0;
  r3.w = 0;
  while (true) {
    r4.w = cmp((uint)r3.w >= numRefProbes);
    if (r4.w != 0) break;
    r3.z = (uint)r3.w >> 5;
    r7.xyz = (int3)r3.xyz + (int3)r14.xyz;
    r3.z = visibleProbes.Load(r7.xyzw).x;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r18.xyzw, r3.w, l(0), t15.wxyz
  r18.x = samp0[]..swiz;
  r18.y = samp0[]..swiz;
  r18.z = samp0[]..swiz;
  r18.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r19.xyzw, r3.w, l(16), t15.xyzw
  r19.x = samp0[]..swiz;
  r19.y = samp0[]..swiz;
  r19.z = samp0[]..swiz;
  r19.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r20.xyzw, r3.w, l(32), t15.yxwz
  r20.x = samp0[]..swiz;
  r20.y = samp0[]..swiz;
  r20.z = samp0[]..swiz;
  r20.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r21.xyzw, r3.w, l(48), t15.xyzw
  r21.x = samp0[]..swiz;
  r21.y = samp0[]..swiz;
  r21.z = samp0[]..swiz;
  r21.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r22.xyzw, r3.w, l(64), t15.zwxy
  r22.x = samp0[]..swiz;
  r22.y = samp0[]..swiz;
  r22.z = samp0[]..swiz;
  r22.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r23.xyzw, r3.w, l(80), t15.xyzw
  r23.x = samp0[]..swiz;
  r23.y = samp0[]..swiz;
  r23.z = samp0[]..swiz;
  r23.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r6.zw, r3.w, l(96), t15.xxxy
  r6.z = samp0[]..swiz;
  r6.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r3.w, l(116), t15.xyzw
  r24.x = samp0[]..swiz;
  r24.y = samp0[]..swiz;
  r24.z = samp0[]..swiz;
  r24.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r25.xyzw, r3.w, l(132), t15.zwxy
  r25.x = samp0[]..swiz;
  r25.y = samp0[]..swiz;
  r25.z = samp0[]..swiz;
  r25.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r26.xyzw, r3.w, l(148), t15.zwxy
  r26.x = samp0[]..swiz;
  r26.y = samp0[]..swiz;
  r26.z = samp0[]..swiz;
  r26.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r27.xyzw, r3.w, l(164), t15.zwxy
  r27.x = samp0[]..swiz;
  r27.y = samp0[]..swiz;
  r27.z = samp0[]..swiz;
  r27.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r3.w, l(180), t15.zwxy
  r30.x = samp0[]..swiz;
  r30.y = samp0[]..swiz;
  r30.z = samp0[]..swiz;
  r30.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r3.w, l(196), t15.xyzw
  r31.x = samp0[]..swiz;
  r31.y = samp0[]..swiz;
  r31.z = samp0[]..swiz;
  r31.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xy, r3.w, l(212), t15.xyxx
  r32.x = samp0[]..swiz;
  r32.y = samp0[]..swiz;
    r7.xyz = v6.xyz + -r18.yzw;
    r4.w = dot(r7.xyz, r7.xyz);
    r4.w = sqrt(r4.w);
    r4.w = cmp(probeDebugRadius >= r4.w);
    r8.w = (int)r24.y & 0x0000ffff;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r8.w, l(0), t16.xyzw
  r33.x = samp0[]..swiz;
  r33.y = samp0[]..swiz;
  r33.z = samp0[]..swiz;
  r33.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r8.w, l(16), t16.xyzw
  r34.x = samp0[]..swiz;
  r34.y = samp0[]..swiz;
  r34.z = samp0[]..swiz;
  r34.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r35.xyzw, r8.w, l(32), t16.xyzw
  r35.x = samp0[]..swiz;
  r35.y = samp0[]..swiz;
  r35.z = samp0[]..swiz;
  r35.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r36.xyzw, r8.w, l(48), t16.xyzw
  r36.x = samp0[]..swiz;
  r36.y = samp0[]..swiz;
  r36.z = samp0[]..swiz;
  r36.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r37.xyzw, r8.w, l(64), t16.xyzw
  r37.x = samp0[]..swiz;
  r37.y = samp0[]..swiz;
  r37.z = samp0[]..swiz;
  r37.w = samp0[]..swiz;
  // Known bad code for instruction (needs manual fix):
    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r38.xyzw, r8.w, l(80), t16.xyzw
  r38.x = samp0[]..swiz;
  r38.y = samp0[]..swiz;
  r38.z = samp0[]..swiz;
  r38.w = samp0[]..swiz;
    r9.x = dot(r33.xyz, r7.xyz);
    r9.x = saturate(r9.x + r33.w);
    r10.w = dot(r34.xyz, r7.xyz);
    r10.w = saturate(r10.w + r34.w);
    r9.x = r10.w * r9.x;
    r10.w = dot(r35.xyz, r7.xyz);
    r10.w = saturate(r10.w + r35.w);
    r9.x = r10.w * r9.x;
    r10.w = dot(r36.xyz, r7.xyz);
    r10.w = saturate(r10.w + r36.w);
    r9.x = r10.w * r9.x;
    r10.w = dot(r37.xyz, r7.xyz);
    r10.w = saturate(r10.w + r37.w);
    r9.x = r10.w * r9.x;
    r10.w = dot(r38.xyz, r7.xyz);
    r10.w = saturate(r10.w + r38.w);
    r8.x = r10.w * r9.x;
    r9.x = (int)r6.z & 1;
    r15.zw = r9.xx ? r8.xy : r8.zx;
    r33.xy = r24.zw;
    r33.z = r25.z;
    r8.x = dot(r11.xyz, r33.xyz);
    r9.x = dot(r7.xyz, r33.xyz);
    r9.x = r9.x + -r25.w;
    r10.w = cmp(r9.x >= 0);
    r9.x = max(abs(r9.x), r2.y);
    r9.x = r10.w ? r9.x : -r9.x;
    r8.x = max(1.00000001e-07, -r8.x);
    r8.x = r9.x / r8.x;
    r8.x = min(131072, abs(r8.x));
    r25.z = r26.z;
    r9.x = dot(r11.xyz, r25.xyz);
    r10.w = dot(r7.xyz, r25.xyz);
    r10.w = r10.w + -r26.w;
    r12.w = cmp(r10.w >= 0);
    r10.w = max(abs(r10.w), r2.y);
    r10.w = r12.w ? r10.w : -r10.w;
    r9.x = max(1.00000001e-07, -r9.x);
    r9.x = r10.w / r9.x;
    r8.x = min(abs(r9.x), r8.x);
    r26.z = r27.z;
    r9.x = dot(r11.xyz, r26.xyz);
    r10.w = dot(r7.xyz, r26.xyz);
    r10.w = r10.w + -r27.w;
    r12.w = cmp(r10.w >= 0);
    r10.w = max(abs(r10.w), r2.y);
    r10.w = r12.w ? r10.w : -r10.w;
    r9.x = max(1.00000001e-07, -r9.x);
    r9.x = r10.w / r9.x;
    r8.x = min(abs(r9.x), r8.x);
    r27.z = r30.z;
    r9.x = dot(r11.xyz, r27.xyz);
    r10.w = dot(r7.xyz, r27.xyz);
    r10.w = r10.w + -r30.w;
    r12.w = cmp(r10.w >= 0);
    r10.w = max(abs(r10.w), r2.y);
    r10.w = r12.w ? r10.w : -r10.w;
    r9.x = max(1.00000001e-07, -r9.x);
    r9.x = r10.w / r9.x;
    r8.x = min(abs(r9.x), r8.x);
    r30.z = r31.x;
    r9.x = dot(r11.xyz, r30.xyz);
    r10.w = dot(r7.xyz, r30.xyz);
    r10.w = r10.w + -r31.y;
    r12.w = cmp(r10.w >= 0);
    r10.w = max(abs(r10.w), r2.y);
    r10.w = r12.w ? r10.w : -r10.w;
    r9.x = max(1.00000001e-07, -r9.x);
    r9.x = r10.w / r9.x;
    r8.x = min(abs(r9.x), r8.x);
    r32.zw = r31.zw;
    r9.x = dot(r11.zxy, r32.xzw);
    r10.w = dot(r7.zxy, r32.xzw);
    r10.w = r10.w + -r32.y;
    r12.w = cmp(r10.w >= 0);
    r10.w = max(abs(r10.w), r2.y);
    r10.w = r12.w ? r10.w : -r10.w;
    r9.x = max(1.00000001e-07, -r9.x);
    r9.x = r10.w / r9.x;
    r8.x = min(abs(r9.x), r8.x);
    r25.x = r21.w;
    r25.yz = r22.zw;
    r25.xyz = r25.xyz + r7.xyz;
    r25.xyz = r11.xyz * r8.xxx + r25.xyz;
    r9.x = dot(r25.xyz, r25.xyz);
    r9.x = sqrt(r9.x);
    r8.x = r8.x / r9.x;
    r8.x = r8.x + r8.x;
    r8.x = sqrt(r8.x);
    r8.x = r0.w * 5 + r8.x;
    r8.x = -0.844799995 + r8.x;
    r18.y = r19.z;
    r18.z = r20.x;
    r26.x = dot(r25.xyz, r18.xyz);
    r27.xy = r19.xw;
    r27.z = r20.w;
    r26.y = dot(r25.xyz, r27.xyz);
    r20.x = r19.y;
    r26.z = dot(r25.xyz, r20.xyz);
    if (6 == 0) r19.x = 0; else if (6+25 < 32) {     r19.x = (uint)r24.y << (32-(6 + 25)); r19.x = (uint)r19.x >> (32-6);    } else r19.x = (uint)r24.y >> 25;
    if (9 == 0) r19.y = 0; else if (9+16 < 32) {     r19.y = (uint)r24.y << (32-(9 + 16)); r19.y = (uint)r19.y >> (32-9);    } else r19.y = (uint)r24.y >> 16;
    r26.w = (uint)r19.y;
    r19.yzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r26.xyzw, r8.x).xyz;
    r24.yzw = (int3)r19.yzw & int3(-2147483648,-2147483648,-2147483648);
    r24.yzw = cmp((int3)r24.yzw == int3(2139095040,2139095040,2139095040));
    r8.x = (int)r24.z | (int)r24.y;
    r8.x = (int)r24.w | (int)r8.x;
    r19.yzw = r8.xxx ? float3(1,1,0) : r19.yzw;
    r25.x = dot(r7.xyz, r18.xyz);
    r25.y = dot(r7.xyz, r27.xyz);
    r25.z = dot(r7.xyz, r20.xyz);
    r21.xyz = saturate(r25.xyz * r21.xyz + float3(0.5,0.5,0.5));
    r22.z = r23.x;
    r21.xyz = r21.xyz * r22.xyz + r23.yzw;
    r26.x = dot(r5.xyz, r18.xyz);
    r26.y = dot(r5.xyz, r27.xyz);
    r26.z = dot(r5.xyz, r20.xyz);
    r18.xyz = cmp(float3(0,0,0) < r26.xyz);
    r10.z = r18.x ? 0 : 0.5;
    r20.xyz = r21.xyz + r10.xyz;
    r20.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r20.xyz, 0).xyz;
    r12.z = r18.y ? 0 : 0.5;
    r18.xyw = r21.xyz + r12.xyz;
    r18.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r18.xyw, 0).xyz;
    r13.z = r18.z ? 0 : 0.5;
    r21.xyz = r21.xyz + r13.xyz;
    r21.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r21.xyz, 0).xyz;
    r22.xyz = r26.xyz * r26.xyz;
    r23.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r26.xyzw, 6).xyz;
    r24.yzw = (int3)r23.xyz & int3(-2147483648,-2147483648,-2147483648);
    r24.yzw = cmp((int3)r24.yzw == int3(2139095040,2139095040,2139095040));
    r8.x = (int)r24.z | (int)r24.y;
    r8.x = (int)r24.w | (int)r8.x;
    r23.xyz = r8.xxx ? float3(1,1,0) : r23.xyz;
    r25.xyzw = r16.xyzw;
    r24.yzw = r17.yzw;
    r8.x = r2.w;
    r9.x = r3.z;
    while (true) {
      if (r9.x == 0) break;
      r10.z = firstbitlow((uint)r9.x);
      r10.z = 1 << (int)r10.z;
      r10.w = (int)r9.x & (int)r10.z;
      if (r10.w != 0) {
        r10.z = ~(int)r10.z;
        r9.x = (int)r9.x & (int)r10.z;
        if (r4.w != 0) {
          r10.z = r6.z;
          r26.xy = r15.zw;
          r10.w = 1;
          while (true) {
            r12.z = cmp((int)r10.w >= (int)r19.x);
            if (r12.z != 0) break;
            r12.z = (int)r8.w + (int)r10.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r12.z, l(0), t16.xyzw
          r27.x = samp0[]..swiz;
          r27.y = samp0[]..swiz;
          r27.z = samp0[]..swiz;
          r27.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r12.z, l(16), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r12.z, l(32), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r12.z, l(48), t16.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r12.z, l(64), t16.xyzw
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r12.z, l(80), t16.xyzw
          r34.x = samp0[]..swiz;
          r34.y = samp0[]..swiz;
          r34.z = samp0[]..swiz;
          r34.w = samp0[]..swiz;
            r12.z = dot(r27.xyz, r7.xyz);
            r12.z = saturate(r12.z + r27.w);
            r12.z = r26.x * r12.z;
            r12.w = dot(r30.xyz, r7.xyz);
            r12.w = saturate(r12.w + r30.w);
            r12.z = r12.z * r12.w;
            r12.w = dot(r31.xyz, r7.xyz);
            r12.w = saturate(r12.w + r31.w);
            r12.z = r12.z * r12.w;
            r12.w = dot(r32.xyz, r7.xyz);
            r12.w = saturate(r12.w + r32.w);
            r12.z = r12.z * r12.w;
            r12.w = dot(r33.xyz, r7.xyz);
            r12.w = saturate(r12.w + r33.w);
            r12.z = r12.z * r12.w;
            r12.w = dot(r34.xyz, r7.xyz);
            r12.w = saturate(r12.w + r34.w);
            r26.x = r12.z * r12.w;
            r13.z = (uint)r10.z >> 2;
            if (1 == 0) r13.w = 0; else if (1+2 < 32) {             r13.w = (uint)r10.z << (32-(1 + 2)); r13.w = (uint)r13.w >> (32-1);            } else r13.w = (uint)r10.z >> 2;
            r14.w = (int)r13.z & 2;
            r18.z = max(r26.y, r26.x);
            r12.z = -r12.z * r12.w + 1;
            r12.z = r26.y * r12.z;
            r15.y = r14.w ? r12.z : r18.z;
            r26.xy = r13.ww ? r26.xy : r15.xy;
            r10.w = (int)r10.w + 1;
            r10.z = r13.z;
          }
          r26.y = saturate(r26.y);
          r10.z = r26.y * r6.w;
          r10.w = cmp(0 < r10.z);
          if (r10.w != 0) {
            r27.z = r26.y * r6.w + r24.w;
            r10.z = r10.z * r24.x;
            r26.xzw = r22.xyz * r10.zzz;
            r30.xyz = r26.zzz * r18.xyw;
            r30.xyz = r20.xyz * r26.xxx + r30.xyz;
            r26.xzw = r21.xyz * r26.www + r30.xyz;
            r30.xyz = r26.xzw * r23.xyz;
            r10.w = dot(r30.xyz, float3(0.212599993,0.715200007,0.0722000003));
            r30.xyz = r26.xzw * r23.xyz + r25.xyz;
            r12.z = r10.w * r2.z;
            r26.xzw = r19.yzw * r10.zzz;
            r10.z = dot(r26.xzw, float3(0.212599993,0.715200007,0.0722000003));
            r10.z = r2.z * r10.w + r10.z;
            r10.z = r12.z / r10.z;
            r31.x = r25.w;
            r31.yz = r24.yz;
            r27.xyw = r26.zwx * r10.zzz + r31.yzx;
            r30.w = r27.w;
          } else {
            r30.xyzw = r25.xyzw;
            r27.xyz = r24.yzw;
          }
          r8.x = -1;
          r25.xyzw = r30.xyzw;
          r24.yzw = r27.xyz;
          break;
        }
      }
    }
    if (r8.x != 0) {
      r16.xyzw = r25.xyzw;
      r17.yzw = r24.yzw;
      r2.w = -1;
      break;
    }
    r3.w = (int)r3.w + 32;
    r16.xyzw = r25.xyzw;
    r17.yzw = r24.yzw;
    r2.w = r8.x;
  }
  if (r2.w == 0) {
    r2.w = numRefProbes + -numOverrideProbes;
    r3.x = (int)r2.w & -32;
    r3.y = (int)r2.w + (int)-r3.x;
    r3.z = numRefProbes & -32;
    r3.w = (int)-r3.z + numRefProbes;
    r7.xy = float2(0,0);
    r8.w = 0;
    r10.yz = float2(0,1);
    r12.xy = float2(0,0);
    r13.x = 1;
    r15.xy = float2(0,0);
    r18.xy = float2(0,0);
    r19.xyzw = r16.xyzw;
    r20.yzw = r17.yzw;
    r4.w = r3.x;
    while (true) {
      r6.z = cmp((uint)r4.w >= numRefProbes);
      if (r6.z != 0) break;
      r7.z = (uint)r4.w >> 5;
      r8.xyz = (int3)r7.xyz + (int3)r14.xyz;
      r6.z = visibleProbes.Load(r8.xyzw).x;
      r6.w = cmp((int)r3.x == (int)r4.w);
      bitmask.z = ((~(-1 << r3.y)) << 0) & 0xffffffff;  r7.z = (((uint)0 << 0) & bitmask.z) | ((uint)r6.z & ~bitmask.z);
      r6.z = r6.w ? r7.z : r6.z;
      r6.w = cmp((int)r3.z == (int)r4.w);
      if (r3.w == 0) r7.z = 0; else if (r3.w+0 < 32) {       r7.z = (uint)r6.z << (32-(r3.w + 0)); r7.z = (uint)r7.z >> (32-r3.w);      } else r7.z = (uint)r6.z >> 0;
      r6.z = r6.w ? r7.z : r6.z;
      r6.w = (int)r4.w + numLights;
      r21.xyzw = r19.xyzw;
      r22.xyz = r20.yzw;
      r7.z = r6.z;
      while (true) {
        if (r7.z == 0) break;
        r7.w = firstbitlow((uint)r7.z);
        r8.x = 1 << (int)r7.w;
        r8.y = (int)r7.z & (int)r8.x;
        if (r8.y != 0) {
          r7.z = (int)r7.z ^ (int)r8.x;
          r8.x = (int)r6.w + (int)r7.w;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r23.xyzw, r8.x, l(0), t11.xyzw
        r23.x = samp0[]..swiz;
        r23.y = samp0[]..swiz;
        r23.z = samp0[]..swiz;
        r23.w = samp0[]..swiz;
        // Known bad code for instruction (needs manual fix):
                ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r8.xy, r8.x, l(16), t11.xyxx
        r8.x = samp0[]..swiz;
        r8.y = samp0[]..swiz;
          r23.xyz = -v6.xyz + r23.xyz;
          r8.z = r23.w;
          r8.xyz = cmp(abs(r23.xyz) < r8.zxy);
          r8.x = r8.y ? r8.x : 0;
          r8.x = r8.z ? r8.x : 0;
          if (r8.x != 0) {
            r7.w = (int)r4.w + (int)r7.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r23.xyzw, r7.w, l(0), t15.wxyz
          r23.x = samp0[]..swiz;
          r23.y = samp0[]..swiz;
          r23.z = samp0[]..swiz;
          r23.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r8.xy, r7.w, l(96), t15.xyxx
          r8.x = samp0[]..swiz;
          r8.y = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r7.w, l(116), t15.zwxy
          r24.x = samp0[]..swiz;
          r24.y = samp0[]..swiz;
          r24.z = samp0[]..swiz;
          r24.w = samp0[]..swiz;
            r25.xyz = v6.xyz + -r23.yzw;
            r8.z = (int)r24.w & 0x0000ffff;
            if (6 == 0) r9.x = 0; else if (6+25 < 32) {             r9.x = (uint)r24.w << (32-(6 + 25)); r9.x = (uint)r9.x >> (32-6);            } else r9.x = (uint)r24.w >> 25;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r26.xyzw, r8.z, l(0), t16.xyzw
          r26.x = samp0[]..swiz;
          r26.y = samp0[]..swiz;
          r26.z = samp0[]..swiz;
          r26.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r8.z, l(16), t16.xyzw
          r27.x = samp0[]..swiz;
          r27.y = samp0[]..swiz;
          r27.z = samp0[]..swiz;
          r27.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r8.z, l(32), t16.xyzw
          r30.x = samp0[]..swiz;
          r30.y = samp0[]..swiz;
          r30.z = samp0[]..swiz;
          r30.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r8.z, l(48), t16.xyzw
          r31.x = samp0[]..swiz;
          r31.y = samp0[]..swiz;
          r31.z = samp0[]..swiz;
          r31.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r8.z, l(64), t16.xyzw
          r32.x = samp0[]..swiz;
          r32.y = samp0[]..swiz;
          r32.z = samp0[]..swiz;
          r32.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r8.z, l(80), t16.xyzw
          r33.x = samp0[]..swiz;
          r33.y = samp0[]..swiz;
          r33.z = samp0[]..swiz;
          r33.w = samp0[]..swiz;
            r10.w = dot(r26.xyz, r25.xyz);
            r10.w = saturate(r10.w + r26.w);
            r12.w = dot(r27.xyz, r25.xyz);
            r12.w = saturate(r12.w + r27.w);
            r10.w = r12.w * r10.w;
            r12.w = dot(r30.xyz, r25.xyz);
            r12.w = saturate(r12.w + r30.w);
            r10.w = r12.w * r10.w;
            r12.w = dot(r31.xyz, r25.xyz);
            r12.w = saturate(r12.w + r31.w);
            r10.w = r12.w * r10.w;
            r12.w = dot(r32.xyz, r25.xyz);
            r12.w = saturate(r12.w + r32.w);
            r10.w = r12.w * r10.w;
            r12.w = dot(r33.xyz, r25.xyz);
            r12.w = saturate(r12.w + r33.w);
            r10.x = r12.w * r10.w;
            r10.w = r8.x ? 0.000000 : 0;
            r10.xw = r10.ww ? r10.xy : r10.zx;
            r12.w = r8.x;
            r26.xy = r10.xw;
            r13.z = 1;
            while (true) {
              r13.w = cmp((int)r13.z >= (int)r9.x);
              if (r13.w != 0) break;
              r13.w = (int)r8.z + (int)r13.z;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r13.w, l(0), t16.xyzw
            r27.x = samp0[]..swiz;
            r27.y = samp0[]..swiz;
            r27.z = samp0[]..swiz;
            r27.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r13.w, l(16), t16.xyzw
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r13.w, l(32), t16.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r13.w, l(48), t16.xyzw
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r13.w, l(64), t16.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r13.w, l(80), t16.xyzw
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
              r13.w = dot(r27.xyz, r25.xyz);
              r13.w = saturate(r13.w + r27.w);
              r13.w = r26.x * r13.w;
              r14.w = dot(r30.xyz, r25.xyz);
              r14.w = saturate(r14.w + r30.w);
              r13.w = r14.w * r13.w;
              r14.w = dot(r31.xyz, r25.xyz);
              r14.w = saturate(r14.w + r31.w);
              r13.w = r14.w * r13.w;
              r14.w = dot(r32.xyz, r25.xyz);
              r14.w = saturate(r14.w + r32.w);
              r13.w = r14.w * r13.w;
              r14.w = dot(r33.xyz, r25.xyz);
              r14.w = saturate(r14.w + r33.w);
              r13.w = r14.w * r13.w;
              r14.w = dot(r34.xyz, r25.xyz);
              r14.w = saturate(r14.w + r34.w);
              r26.x = r14.w * r13.w;
              r15.w = (uint)r12.w >> 2;
              if (1 == 0) r18.w = 0; else if (1+2 < 32) {               r18.w = (uint)r12.w << (32-(1 + 2)); r18.w = (uint)r18.w >> (32-1);              } else r18.w = (uint)r12.w >> 2;
              r23.w = (int)r15.w & 2;
              r25.w = max(r26.y, r26.x);
              r13.w = -r13.w * r14.w + 1;
              r13.w = r26.y * r13.w;
              r13.y = r23.w ? r13.w : r25.w;
              r26.xy = r18.ww ? r26.xy : r13.xy;
              r13.z = (int)r13.z + 1;
              r12.w = r15.w;
            }
            r26.y = saturate(r26.y);
            r8.x = r26.y * r8.y;
            r8.z = cmp(0 < r8.x);
            if (r8.z != 0) {
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r27.xyzw, r7.w, l(16), t15.xyzw
            r27.x = samp0[]..swiz;
            r27.y = samp0[]..swiz;
            r27.z = samp0[]..swiz;
            r27.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r30.xyzw, r7.w, l(32), t15.yxwz
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r7.w, l(48), t15.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r7.w, l(64), t15.zwxy
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r7.w, l(80), t15.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r7.w, l(132), t15.zwxy
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r7.w, l(148), t15.zwxy
            r35.x = samp0[]..swiz;
            r35.y = samp0[]..swiz;
            r35.z = samp0[]..swiz;
            r35.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r7.w, l(164), t15.zwxy
            r36.x = samp0[]..swiz;
            r36.y = samp0[]..swiz;
            r36.z = samp0[]..swiz;
            r36.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xyzw, r7.w, l(180), t15.zwxy
            r37.x = samp0[]..swiz;
            r37.y = samp0[]..swiz;
            r37.z = samp0[]..swiz;
            r37.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r38.xyzw, r7.w, l(196), t15.xyzw
            r38.x = samp0[]..swiz;
            r38.y = samp0[]..swiz;
            r38.z = samp0[]..swiz;
            r38.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r39.xy, r7.w, l(212), t15.xyxx
            r39.x = samp0[]..swiz;
            r39.y = samp0[]..swiz;
              r22.z = r26.y * r8.y + r22.z;
              r7.w = r8.x * r24.z;
              r24.z = r34.z;
              r8.x = dot(r11.xyz, r24.xyz);
              r8.y = dot(r25.xyz, r24.xyz);
              r8.y = r8.y + -r34.w;
              r8.z = cmp(r8.y >= 0);
              r8.y = max(abs(r8.y), r2.y);
              r8.y = r8.z ? r8.y : -r8.y;
              r8.x = max(1.00000001e-07, -r8.x);
              r8.x = r8.y / r8.x;
              r8.x = min(131072, abs(r8.x));
              r34.z = r35.z;
              r8.y = dot(r11.xyz, r34.xyz);
              r8.z = dot(r25.xyz, r34.xyz);
              r8.z = r8.z + -r35.w;
              r9.x = cmp(r8.z >= 0);
              r8.z = max(abs(r8.z), r2.y);
              r8.z = r9.x ? r8.z : -r8.z;
              r8.y = max(1.00000001e-07, -r8.y);
              r8.y = r8.z / r8.y;
              r8.x = min(r8.x, abs(r8.y));
              r35.z = r36.z;
              r8.y = dot(r11.xyz, r35.xyz);
              r8.z = dot(r25.xyz, r35.xyz);
              r8.z = r8.z + -r36.w;
              r9.x = cmp(r8.z >= 0);
              r8.z = max(abs(r8.z), r2.y);
              r8.z = r9.x ? r8.z : -r8.z;
              r8.y = max(1.00000001e-07, -r8.y);
              r8.y = r8.z / r8.y;
              r8.x = min(r8.x, abs(r8.y));
              r36.z = r37.z;
              r8.y = dot(r11.xyz, r36.xyz);
              r8.z = dot(r25.xyz, r36.xyz);
              r8.z = r8.z + -r37.w;
              r9.x = cmp(r8.z >= 0);
              r8.z = max(abs(r8.z), r2.y);
              r8.z = r9.x ? r8.z : -r8.z;
              r8.y = max(1.00000001e-07, -r8.y);
              r8.y = r8.z / r8.y;
              r8.x = min(r8.x, abs(r8.y));
              r37.z = r38.x;
              r8.y = dot(r11.xyz, r37.xyz);
              r8.z = dot(r25.xyz, r37.xyz);
              r8.z = r8.z + -r38.y;
              r9.x = cmp(r8.z >= 0);
              r8.z = max(abs(r8.z), r2.y);
              r8.z = r9.x ? r8.z : -r8.z;
              r8.y = max(1.00000001e-07, -r8.y);
              r8.y = r8.z / r8.y;
              r8.x = min(r8.x, abs(r8.y));
              r39.zw = r38.zw;
              r8.y = dot(r11.zxy, r39.xzw);
              r8.z = dot(r25.zxy, r39.xzw);
              r8.z = r8.z + -r39.y;
              r9.x = cmp(r8.z >= 0);
              r8.z = max(abs(r8.z), r2.y);
              r8.z = r9.x ? r8.z : -r8.z;
              r8.y = max(1.00000001e-07, -r8.y);
              r8.y = r8.z / r8.y;
              r8.x = min(r8.x, abs(r8.y));
              r24.x = r31.w;
              r24.yz = r32.zw;
              r13.yzw = r24.xyz + r25.xyz;
              r13.yzw = r11.xyz * r8.xxx + r13.yzw;
              r8.y = dot(r13.yzw, r13.yzw);
              r8.y = sqrt(r8.y);
              r8.x = r8.x / r8.y;
              r8.x = r8.x + r8.x;
              r8.x = sqrt(r8.x);
              r8.x = r0.w * 5 + r8.x;
              r8.x = -0.844799995 + r8.x;
              r23.y = r27.z;
              r23.z = r30.x;
              r34.x = dot(r13.yzw, r23.xyz);
              r24.xy = r27.xw;
              r24.z = r30.w;
              r34.y = dot(r13.yzw, r24.xyz);
              r30.x = r27.y;
              r34.z = dot(r13.yzw, r30.xyz);
              if (9 == 0) r8.y = 0; else if (9+16 < 32) {               r8.y = (uint)r24.w << (32-(9 + 16)); r8.y = (uint)r8.y >> (32-9);              } else r8.y = (uint)r24.w >> 16;
              r34.w = (uint)r8.y;
              r8.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r34.xyzw, r8.x).xyz;
              r13.yzw = (int3)r8.xyz & int3(-2147483648,-2147483648,-2147483648);
              r13.yzw = cmp((int3)r13.yzw == int3(2139095040,2139095040,2139095040));
              r9.x = (int)r13.z | (int)r13.y;
              r9.x = (int)r13.w | (int)r9.x;
              r8.xyz = r9.xxx ? float3(1,1,0) : r8.xyz;
              r27.x = dot(r25.xyz, r23.xyz);
              r27.y = dot(r25.xyz, r24.xyz);
              r27.z = dot(r25.xyz, r30.xyz);
              r13.yzw = saturate(r27.xyz * r31.xyz + float3(0.5,0.5,0.5));
              r32.z = r33.x;
              r13.yzw = r13.yzw * r32.xyz + r33.yzw;
              r34.x = dot(r5.xyz, r23.xyz);
              r34.y = dot(r5.xyz, r24.xyz);
              r34.z = dot(r5.xyz, r30.xyz);
              r23.xyz = cmp(float3(0,0,0) < r34.xyz);
              r12.z = r23.x ? 0 : 0.5;
              r24.xyz = r13.yzw + r12.xyz;
              r24.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r24.xyz, 0).xyz;
              r25.xyz = r34.xyz * r34.xyz;
              r25.xyz = r25.xyz * r7.www;
              r15.z = r23.y ? 0 : 0.5;
              r23.xyw = r15.xyz + r13.yzw;
              r23.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r23.xyw, 0).xyz;
              r23.xyw = r23.xyw * r25.yyy;
              r23.xyw = r24.xyz * r25.xxx + r23.xyw;
              r18.z = r23.z ? 0 : 0.5;
              r13.yzw = r18.xyz + r13.yzw;
              r13.yzw = gProbeZArray.SampleLevel(samplerLinearClamp_s, r13.yzw, 0).xyz;
              r13.yzw = r13.yzw * r25.zzz + r23.xyw;
              r23.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r34.xyzw, 6).xyz;
              r24.xyz = (int3)r23.xyz & int3(-2147483648,-2147483648,-2147483648);
              r24.xyz = cmp((int3)r24.xyz == int3(2139095040,2139095040,2139095040));
              r9.x = (int)r24.y | (int)r24.x;
              r9.x = (int)r24.z | (int)r9.x;
              r23.xyz = r9.xxx ? float3(1,1,0) : r23.xyz;
              r24.xyz = r23.xyz * r13.yzw;
              r9.x = dot(r24.xyz, float3(0.212599993,0.715200007,0.0722000003));
              r21.xyz = r13.yzw * r23.xyz + r21.xyz;
              r10.x = r9.x * r2.z;
              r8.xyz = r8.xyz * r7.www;
              r7.w = dot(r8.xyz, float3(0.212599993,0.715200007,0.0722000003));
              r7.w = r2.z * r9.x + r7.w;
              r7.w = r10.x / r7.w;
              r23.x = r21.w;
              r23.yz = r22.xy;
              r22.xyw = r8.yzx * r7.www + r23.yzx;
              r21.w = r22.w;
            }
          }
        }
      }
      r19.xyzw = r21.xyzw;
      r20.yzw = r22.xyz;
      r4.w = (int)r4.w + 32;
    }
    r3.z = cmp(r20.w < 1);
    if (r3.z != 0) {
      r7.xy = float2(0,0);
      r8.w = 0;
      r10.yz = float2(0,1);
      r12.xy = float2(0,0);
      r13.x = 1;
      r15.xy = float2(0,0);
      r18.xy = float2(0,0);
      r21.x = r19.w;
      r21.yzw = r20.yzw;
      r22.xyz = r19.xyz;
      r3.z = r20.w;
      r3.w = 0;
      while (true) {
        r4.w = cmp((uint)r3.w >= (uint)r2.w);
        if (r4.w != 0) break;
        r7.z = (uint)r3.w >> 5;
        r8.xyz = (int3)r7.xyz + (int3)r14.xyz;
        r4.w = visibleProbes.Load(r8.xyzw).x;
        r6.z = cmp((int)r3.x == (int)r3.w);
        if (r3.y == 0) r6.w = 0; else if (r3.y+0 < 32) {         r6.w = (uint)r4.w << (32-(r3.y + 0)); r6.w = (uint)r6.w >> (32-r3.y);        } else r6.w = (uint)r4.w >> 0;
        r4.w = r6.z ? r6.w : r4.w;
        r6.z = (int)r3.w + numLights;
        r23.xyzw = r21.xyzw;
        r8.xyz = r22.xyz;
        r6.w = r3.z;
        r7.z = r4.w;
        while (true) {
          if (r7.z == 0) break;
          r7.w = firstbitlow((uint)r7.z);
          r9.x = 1 << (int)r7.w;
          r10.w = (int)r7.z & (int)r9.x;
          if (r10.w != 0) {
            r7.z = (int)r7.z ^ (int)r9.x;
            r9.x = (int)r6.z + (int)r7.w;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r24.xyzw, r9.x, l(0), t11.xyzw
          r24.x = samp0[]..swiz;
          r24.y = samp0[]..swiz;
          r24.z = samp0[]..swiz;
          r24.w = samp0[]..swiz;
          // Known bad code for instruction (needs manual fix):
                    ld_structured_indexable(structured_buffer, stride=80)(mixed,mixed,mixed,mixed) r25.xy, r9.x, l(16), t11.xyxx
          r25.x = samp0[]..swiz;
          r25.y = samp0[]..swiz;
            r24.xyz = -v6.xyz + r24.xyz;
            r25.z = r24.w;
            r24.xyz = cmp(abs(r24.xyz) < r25.zxy);
            r9.x = r24.y ? r24.x : 0;
            r9.x = r24.z ? r9.x : 0;
            if (r9.x != 0) {
              r7.w = (int)r3.w + (int)r7.w;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r24.xyzw, r7.w, l(0), t15.wxyz
            r24.x = samp0[]..swiz;
            r24.y = samp0[]..swiz;
            r24.z = samp0[]..swiz;
            r24.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r13.zw, r7.w, l(96), t15.xxxy
            r13.z = samp0[]..swiz;
            r13.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r25.xyzw, r7.w, l(116), t15.zwxy
            r25.x = samp0[]..swiz;
            r25.y = samp0[]..swiz;
            r25.z = samp0[]..swiz;
            r25.w = samp0[]..swiz;
              r26.xyz = v6.xyz + -r24.yzw;
              r9.x = (int)r25.w & 0x0000ffff;
              if (6 == 0) r10.w = 0; else if (6+25 < 32) {               r10.w = (uint)r25.w << (32-(6 + 25)); r10.w = (uint)r10.w >> (32-6);              } else r10.w = (uint)r25.w >> 25;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r27.xyzw, r9.x, l(0), t16.xyzw
            r27.x = samp0[]..swiz;
            r27.y = samp0[]..swiz;
            r27.z = samp0[]..swiz;
            r27.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r30.xyzw, r9.x, l(16), t16.xyzw
            r30.x = samp0[]..swiz;
            r30.y = samp0[]..swiz;
            r30.z = samp0[]..swiz;
            r30.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r9.x, l(32), t16.xyzw
            r31.x = samp0[]..swiz;
            r31.y = samp0[]..swiz;
            r31.z = samp0[]..swiz;
            r31.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r9.x, l(48), t16.xyzw
            r32.x = samp0[]..swiz;
            r32.y = samp0[]..swiz;
            r32.z = samp0[]..swiz;
            r32.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r9.x, l(64), t16.xyzw
            r33.x = samp0[]..swiz;
            r33.y = samp0[]..swiz;
            r33.z = samp0[]..swiz;
            r33.w = samp0[]..swiz;
            // Known bad code for instruction (needs manual fix):
                        ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r9.x, l(80), t16.xyzw
            r34.x = samp0[]..swiz;
            r34.y = samp0[]..swiz;
            r34.z = samp0[]..swiz;
            r34.w = samp0[]..swiz;
              r12.w = dot(r27.xyz, r26.xyz);
              r12.w = saturate(r12.w + r27.w);
              r14.w = dot(r30.xyz, r26.xyz);
              r14.w = saturate(r14.w + r30.w);
              r12.w = r14.w * r12.w;
              r14.w = dot(r31.xyz, r26.xyz);
              r14.w = saturate(r14.w + r31.w);
              r12.w = r14.w * r12.w;
              r14.w = dot(r32.xyz, r26.xyz);
              r14.w = saturate(r14.w + r32.w);
              r12.w = r14.w * r12.w;
              r14.w = dot(r33.xyz, r26.xyz);
              r14.w = saturate(r14.w + r33.w);
              r12.w = r14.w * r12.w;
              r14.w = dot(r34.xyz, r26.xyz);
              r14.w = saturate(r14.w + r34.w);
              r10.x = r14.w * r12.w;
              r12.w = (int)r13.z & 1;
              r27.xy = r12.ww ? r10.xy : r10.zx;
              r10.x = r13.z;
              r30.xy = r27.xy;
              r12.w = 1;
              while (true) {
                r14.w = cmp((int)r12.w >= (int)r10.w);
                if (r14.w != 0) break;
                r14.w = (int)r9.x + (int)r12.w;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r31.xyzw, r14.w, l(0), t16.xyzw
              r31.x = samp0[]..swiz;
              r31.y = samp0[]..swiz;
              r31.z = samp0[]..swiz;
              r31.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r32.xyzw, r14.w, l(16), t16.xyzw
              r32.x = samp0[]..swiz;
              r32.y = samp0[]..swiz;
              r32.z = samp0[]..swiz;
              r32.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r33.xyzw, r14.w, l(32), t16.xyzw
              r33.x = samp0[]..swiz;
              r33.y = samp0[]..swiz;
              r33.z = samp0[]..swiz;
              r33.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r34.xyzw, r14.w, l(48), t16.xyzw
              r34.x = samp0[]..swiz;
              r34.y = samp0[]..swiz;
              r34.z = samp0[]..swiz;
              r34.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r35.xyzw, r14.w, l(64), t16.xyzw
              r35.x = samp0[]..swiz;
              r35.y = samp0[]..swiz;
              r35.z = samp0[]..swiz;
              r35.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=96)(mixed,mixed,mixed,mixed) r36.xyzw, r14.w, l(80), t16.xyzw
              r36.x = samp0[]..swiz;
              r36.y = samp0[]..swiz;
              r36.z = samp0[]..swiz;
              r36.w = samp0[]..swiz;
                r14.w = dot(r31.xyz, r26.xyz);
                r14.w = saturate(r14.w + r31.w);
                r14.w = r30.x * r14.w;
                r15.w = dot(r32.xyz, r26.xyz);
                r15.w = saturate(r15.w + r32.w);
                r14.w = r15.w * r14.w;
                r15.w = dot(r33.xyz, r26.xyz);
                r15.w = saturate(r15.w + r33.w);
                r14.w = r15.w * r14.w;
                r15.w = dot(r34.xyz, r26.xyz);
                r15.w = saturate(r15.w + r34.w);
                r14.w = r15.w * r14.w;
                r15.w = dot(r35.xyz, r26.xyz);
                r15.w = saturate(r15.w + r35.w);
                r14.w = r15.w * r14.w;
                r15.w = dot(r36.xyz, r26.xyz);
                r15.w = saturate(r15.w + r36.w);
                r30.x = r15.w * r14.w;
                r17.w = (uint)r10.x >> 2;
                if (1 == 0) r18.w = 0; else if (1+2 < 32) {                 r18.w = (uint)r10.x << (32-(1 + 2)); r18.w = (uint)r18.w >> (32-1);                } else r18.w = (uint)r10.x >> 2;
                r22.w = (int)r17.w & 2;
                r24.w = max(r30.y, r30.x);
                r14.w = -r14.w * r15.w + 1;
                r14.w = r30.y * r14.w;
                r13.y = r22.w ? r14.w : r24.w;
                r30.xy = r18.ww ? r30.xy : r13.xy;
                r12.w = (int)r12.w + 1;
                r10.x = r17.w;
              }
              r9.x = saturate(r30.y + -r6.w);
              r10.x = r9.x * r13.w;
              r10.w = cmp(0 < r10.x);
              if (r10.w != 0) {
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r27.xyzw, r7.w, l(16), t15.xyzw
              r27.x = samp0[]..swiz;
              r27.y = samp0[]..swiz;
              r27.z = samp0[]..swiz;
              r27.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r31.xyzw, r7.w, l(32), t15.yxwz
              r31.x = samp0[]..swiz;
              r31.y = samp0[]..swiz;
              r31.z = samp0[]..swiz;
              r31.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r32.xyzw, r7.w, l(48), t15.xyzw
              r32.x = samp0[]..swiz;
              r32.y = samp0[]..swiz;
              r32.z = samp0[]..swiz;
              r32.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r33.xyzw, r7.w, l(64), t15.zwxy
              r33.x = samp0[]..swiz;
              r33.y = samp0[]..swiz;
              r33.z = samp0[]..swiz;
              r33.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r34.xyzw, r7.w, l(80), t15.xyzw
              r34.x = samp0[]..swiz;
              r34.y = samp0[]..swiz;
              r34.z = samp0[]..swiz;
              r34.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r35.xyzw, r7.w, l(132), t15.zwxy
              r35.x = samp0[]..swiz;
              r35.y = samp0[]..swiz;
              r35.z = samp0[]..swiz;
              r35.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r36.xyzw, r7.w, l(148), t15.zwxy
              r36.x = samp0[]..swiz;
              r36.y = samp0[]..swiz;
              r36.z = samp0[]..swiz;
              r36.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r37.xyzw, r7.w, l(164), t15.zwxy
              r37.x = samp0[]..swiz;
              r37.y = samp0[]..swiz;
              r37.z = samp0[]..swiz;
              r37.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r38.xyzw, r7.w, l(180), t15.zwxy
              r38.x = samp0[]..swiz;
              r38.y = samp0[]..swiz;
              r38.z = samp0[]..swiz;
              r38.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r39.xyzw, r7.w, l(196), t15.xyzw
              r39.x = samp0[]..swiz;
              r39.y = samp0[]..swiz;
              r39.z = samp0[]..swiz;
              r39.w = samp0[]..swiz;
              // Known bad code for instruction (needs manual fix):
                            ld_structured_indexable(structured_buffer, stride=224)(mixed,mixed,mixed,mixed) r40.xy, r7.w, l(212), t15.xyxx
              r40.x = samp0[]..swiz;
              r40.y = samp0[]..swiz;
                r23.w = r9.x * r13.w + r23.w;
                r7.w = r10.x * r25.z;
                r25.z = r35.z;
                r9.x = dot(r11.xyz, r25.xyz);
                r10.x = dot(r26.xyz, r25.xyz);
                r10.x = r10.x + -r35.w;
                r10.w = cmp(r10.x >= 0);
                r10.x = max(abs(r10.x), r2.y);
                r10.x = r10.w ? r10.x : -r10.x;
                r9.x = max(1.00000001e-07, -r9.x);
                r9.x = r10.x / r9.x;
                r9.x = min(131072, abs(r9.x));
                r35.z = r36.z;
                r10.x = dot(r11.xyz, r35.xyz);
                r10.w = dot(r26.xyz, r35.xyz);
                r10.w = r10.w + -r36.w;
                r12.w = cmp(r10.w >= 0);
                r10.w = max(abs(r10.w), r2.y);
                r10.w = r12.w ? r10.w : -r10.w;
                r10.x = max(1.00000001e-07, -r10.x);
                r10.x = r10.w / r10.x;
                r9.x = min(abs(r10.x), r9.x);
                r36.z = r37.z;
                r10.x = dot(r11.xyz, r36.xyz);
                r10.w = dot(r26.xyz, r36.xyz);
                r10.w = r10.w + -r37.w;
                r12.w = cmp(r10.w >= 0);
                r10.w = max(abs(r10.w), r2.y);
                r10.w = r12.w ? r10.w : -r10.w;
                r10.x = max(1.00000001e-07, -r10.x);
                r10.x = r10.w / r10.x;
                r9.x = min(abs(r10.x), r9.x);
                r37.z = r38.z;
                r10.x = dot(r11.xyz, r37.xyz);
                r10.w = dot(r26.xyz, r37.xyz);
                r10.w = r10.w + -r38.w;
                r12.w = cmp(r10.w >= 0);
                r10.w = max(abs(r10.w), r2.y);
                r10.w = r12.w ? r10.w : -r10.w;
                r10.x = max(1.00000001e-07, -r10.x);
                r10.x = r10.w / r10.x;
                r9.x = min(abs(r10.x), r9.x);
                r38.z = r39.x;
                r10.x = dot(r11.xyz, r38.xyz);
                r10.w = dot(r26.xyz, r38.xyz);
                r10.w = r10.w + -r39.y;
                r12.w = cmp(r10.w >= 0);
                r10.w = max(abs(r10.w), r2.y);
                r10.w = r12.w ? r10.w : -r10.w;
                r10.x = max(1.00000001e-07, -r10.x);
                r10.x = r10.w / r10.x;
                r9.x = min(abs(r10.x), r9.x);
                r40.zw = r39.zw;
                r10.x = dot(r11.zxy, r40.xzw);
                r10.w = dot(r26.zxy, r40.xzw);
                r10.w = r10.w + -r40.y;
                r12.w = cmp(r10.w >= 0);
                r10.w = max(abs(r10.w), r2.y);
                r10.w = r12.w ? r10.w : -r10.w;
                r10.x = max(1.00000001e-07, -r10.x);
                r10.x = r10.w / r10.x;
                r9.x = min(abs(r10.x), r9.x);
                r25.x = r32.w;
                r25.yz = r33.zw;
                r13.yzw = r25.xyz + r26.xyz;
                r13.yzw = r11.xyz * r9.xxx + r13.yzw;
                r10.x = dot(r13.yzw, r13.yzw);
                r10.x = sqrt(r10.x);
                r9.x = r9.x / r10.x;
                r9.x = r9.x + r9.x;
                r9.x = sqrt(r9.x);
                r9.x = r0.w * 5 + r9.x;
                r9.x = -0.844799995 + r9.x;
                r24.y = r27.z;
                r24.z = r31.x;
                r35.x = dot(r13.yzw, r24.xyz);
                r25.xy = r27.xw;
                r25.z = r31.w;
                r35.y = dot(r13.yzw, r25.xyz);
                r31.x = r27.y;
                r35.z = dot(r13.yzw, r31.xyz);
                if (9 == 0) r10.x = 0; else if (9+16 < 32) {                 r10.x = (uint)r25.w << (32-(9 + 16)); r10.x = (uint)r10.x >> (32-9);                } else r10.x = (uint)r25.w >> 16;
                r35.w = (uint)r10.x;
                r13.yzw = gReflectionProbeArray.SampleLevel(samplerLinear_s, r35.xyzw, r9.x).xyz;
                r27.xyz = (int3)r13.yzw & int3(-2147483648,-2147483648,-2147483648);
                r27.xyz = cmp((int3)r27.xyz == int3(2139095040,2139095040,2139095040));
                r9.x = (int)r27.y | (int)r27.x;
                r9.x = (int)r27.z | (int)r9.x;
                r13.yzw = r9.xxx ? float3(1,1,0) : r13.yzw;
                r27.x = dot(r26.xyz, r24.xyz);
                r27.y = dot(r26.xyz, r25.xyz);
                r27.z = dot(r26.xyz, r31.xyz);
                r26.xyz = saturate(r27.xyz * r32.xyz + float3(0.5,0.5,0.5));
                r33.z = r34.x;
                r26.xyz = r26.xyz * r33.xyz + r34.yzw;
                r35.x = dot(r5.xyz, r24.xyz);
                r35.y = dot(r5.xyz, r25.xyz);
                r35.z = dot(r5.xyz, r31.xyz);
                r24.xyz = cmp(float3(0,0,0) < r35.xyz);
                r12.z = r24.x ? 0 : 0.5;
                r25.xyz = r26.xyz + r12.xyz;
                r25.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r25.xyz, 0).xyz;
                r27.xyz = r35.xyz * r35.xyz;
                r27.xyz = r27.xyz * r7.www;
                r15.z = r24.y ? 0 : 0.5;
                r24.xyw = r26.xyz + r15.xyz;
                r24.xyw = gProbeYArray.SampleLevel(samplerLinearClamp_s, r24.xyw, 0).xyz;
                r24.xyw = r24.xyw * r27.yyy;
                r24.xyw = r25.xyz * r27.xxx + r24.xyw;
                r18.z = r24.z ? 0 : 0.5;
                r25.xyz = r26.xyz + r18.xyz;
                r25.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r25.xyz, 0).xyz;
                r24.xyz = r25.xyz * r27.zzz + r24.xyw;
                r25.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r35.xyzw, 6).xyz;
                r26.xyz = (int3)r25.xyz & int3(-2147483648,-2147483648,-2147483648);
                r26.xyz = cmp((int3)r26.xyz == int3(2139095040,2139095040,2139095040));
                r9.x = (int)r26.y | (int)r26.x;
                r9.x = (int)r26.z | (int)r9.x;
                r25.xyz = r9.xxx ? float3(1,1,0) : r25.xyz;
                r26.xyz = r25.xyz * r24.xyz;
                r9.x = dot(r26.xyz, float3(0.212599993,0.715200007,0.0722000003));
                r8.xyz = r24.xyz * r25.xyz + r8.xyz;
                r10.x = r9.x * r2.z;
                r13.yzw = r13.yzw * r7.www;
                r7.w = dot(r13.yzw, float3(0.212599993,0.715200007,0.0722000003));
                r7.w = r2.z * r9.x + r7.w;
                r7.w = r10.x / r7.w;
                r23.xyz = r13.yzw * r7.www + r23.xyz;
              }
            }
          }
        }
        r21.xyzw = r23.xyzw;
        r22.xyz = r8.xyz;
        r3.w = (int)r3.w + 32;
      }
      r20.xyzw = r21.zxyw;
      r19.xyz = r22.xyz;
      r20.xyzw = r20.yzxw;
    } else {
      r20.x = r19.w;
    }
    r0.w = max(1, r20.w);
    r0.w = rcp(r0.w);
    r20.w = saturate(r20.w);
    r16.xyz = r19.xyz * r0.www;
    r17.xyz = r20.xyz * r0.www;
    r2.y = cmp(r20.w < 0.99000001);
    if (r2.y != 0) {
      r2.y = 1 + -r20.w;
      r2.z = sunConstants.globalProbeExposure * r2.y;
      r3.xyz = -globalProbeConstants.data[0].xyz + v6.xyz;
      r7.x = globalProbeConstants.data[0].w * r3.x;
      r7.yz = globalProbeConstants.data[1].xy * r3.yz;
      r3.xyz = saturate(float3(0.5,0.5,0.5) + r7.xyz);
      r7.xy = globalProbeConstants.data[1].zw * r3.xy;
      r7.z = globalProbeConstants.data[2].x * r3.z;
      r3.xyz = globalProbeConstants.data[2].yzw + r7.xyz;
      r7.xyz = cmp(float3(0,0,0) < r5.xyz);
      r7.xyz = r7.xyz ? float3(0,0,0) : float3(0.5,0.5,0.5);
      r7.w = 0;
      r8.xyz = r7.wwx + r3.xyz;
      r8.xyz = gProbeXArray.SampleLevel(samplerLinearClamp_s, r8.xyz, 0).xyz;
      r10.xyz = r5.xyz * r5.xyz;
      r10.xyz = r10.xyz * r2.zzz;
      r12.xyz = r7.wwy + r3.xyz;
      r12.xyz = gProbeYArray.SampleLevel(samplerLinearClamp_s, r12.xyz, 0).xyz;
      r12.xyz = r12.xyz * r10.yyy;
      r8.xyz = r8.xyz * r10.xxx + r12.xyz;
      r3.xyz = r7.wwz + r3.xyz;
      r3.xyz = gProbeZArray.SampleLevel(samplerLinearClamp_s, r3.xyz, 0).xyz;
      r3.xyz = r3.xyz * r10.zzz + r8.xyz;
      r5.w = 0;
      r5.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r5.xyzw, 6).xyz;
      r3.xyz = r5.xyz * r3.xyz;
      r2.z = dot(r3.xyz, float3(0.212599993,0.715200007,0.0722000003));
      r16.xyz = r19.xyz * r0.www + r3.xyz;
      r11.w = 0;
      r3.xyz = gReflectionProbeArray.SampleLevel(samplerLinear_s, r11.xyzw, r1.w).xyz;
      r0.w = sunConstants.globalProbeExposure * r2.y + -r2.z;
      r0.w = r6.y * r0.w + r2.z;
      r17.xyz = r3.xyz * r0.www + r17.xyz;
    }
  } else {
    r17.x = r16.w;
  }
  r2.yzw = r16.xyz * r2.xxx + r9.yzw;
  r3.xy = r6.xy * float2(0.984375,0.984375) + float2(0.0078125,0.0078125);
  r3.xy = gEnvBRDFGeneric.SampleLevel(samplerLinear_s, r3.xy, 0).xy;
  r5.xyz = r17.xyz * r2.xxx;
  r3.xzw = r5.xyz * r3.xxx + r28.xyz;
  r5.xyz = r5.xyz * r3.yyy + r29.xyz;
  r6.xyz = float3(1,1,1) + -r1.xyz;
  r3.xyz = r6.xyz * r3.xzw;
  r1.xyz = r5.xyz * r1.xyz + r3.xyz;
  r0.xyz = r2.yzw * r0.xyz + r1.xyz;
  r0.w = dot(r0.xyz, float3(0.298999995,0.587000012,0.114));
  r0.w = max(9.99999975e-05, r0.w);
  r0.w = max(relHDRExposure.x, r0.w);
  r0.xyz = r4.xyz * r0.www + r0.xyz;
  r0.w = cmp(0 < fogConstants.atmosphereExtinctionIntensity);
  if (r0.w != 0) {
    r0.w = dot(v6.xyz, v6.xyz);
    r1.x = rsqrt(r0.w);
    r1.xyz = v6.xyz * r1.xxx;
    r0.w = sqrt(r0.w);
    r1.w = cmp(0 < fogConstants.blendAmount);
    if (r1.w != 0) {
      r2.xy = r0.ww * fogConstants.atmospherefogdistancedensityscale.xy + fogConstants.atmospherefogdistanceoffset.xy;
      r2.xy = fogConstants.atmospherefogdensityatcamera.xy * r2.xy;
      r1.w = cmp(0.00999999978 < abs(v6.z));
      r2.zw = fogConstants.atmospherefogheightdensityscale.xy * v6.zz;
      r3.xy = float2(-1.44269502,-1.44269502) * r2.zw;
      r3.xy = exp2(r3.xy);
      r3.xy = float2(1,1) + -r3.xy;
      r2.zw = r3.xy / r2.zw;
      r2.zw = r2.xy * r2.zw;
      r2.xy = r1.ww ? r2.zw : r2.xy;
      r2.xzw = fogConstants.atmosphereTotalDensity.xyz * r2.xxx;
      r2.xzw = exp2(r2.xzw);
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r2.yyy;
      r3.xyz = exp2(r3.xyz);
      r3.xyz = r3.xyz + -r2.xzw;
      r2.xyz = fogConstants.blendAmount * r3.xyz + r2.xzw;
    } else {
      r1.w = r0.w * fogConstants.atmospherefogdistancedensityscale.x + fogConstants.atmospherefogdistanceoffset.x;
      r1.w = fogConstants.atmospherefogdensityatcamera.x * r1.w;
      r2.w = cmp(0.00999999978 < abs(v6.z));
      r3.x = fogConstants.atmospherefogheightdensityscale.x * v6.z;
      r3.y = -1.44269502 * r3.x;
      r3.y = exp2(r3.y);
      r3.y = 1 + -r3.y;
      r3.x = r3.y / r3.x;
      r3.x = r3.x * r1.w;
      r1.w = r2.w ? r3.x : r1.w;
      r3.xyz = fogConstants.atmosphereTotalDensity.xyz * r1.www;
      r2.xyz = exp2(r3.xyz);
    }
    r2.xyz = r2.xyz * fogConstants.atmosphereExtinctionIntensity + float3(1,1,1);
    r2.xyz = saturate(-fogConstants.atmosphereExtinctionIntensity + r2.xyz);
    r1.x = dot(fogConstants.wldSunFogDir.xyz, -r1.xyz);
    r1.y = -fogConstants.atmosphereMieSchlickK * fogConstants.atmosphereMieSchlickK + 1;
    r1.z = fogConstants.atmosphereMieSchlickK * -r1.x + 1;
    r1.z = r1.z * r1.z;
    r1.z = 12.566371 * r1.z;
    r1.y = r1.y / r1.z;
    r0.w = -fogConstants.atmospherehazebasedist + r0.w;
    r0.w = saturate(fogConstants.atmospherehazefadedist * r0.w);
    r0.w = r1.y * r0.w;
    r1.x = saturate(r1.x);
    r1.x = r1.x * r1.x + 1;
    r1.x = r1.x * 0.0596831031 + -1;
    r1.x = fogConstants.atmospheresunstrength * r1.x + 1;
    r1.yzw = fogConstants.atmosphereMieDensity.xyz * r0.www;
    r1.xyz = r1.xxx * fogConstants.atmosphereRayleighDensity.xyz + r1.yzw;
    r1.xyz = fogConstants.atmosphereInScatterIntensity * r1.xyz;
    r3.xyz = float3(1,1,1) + -r2.xyz;
    r1.xyz = r3.xyz * r1.xyz;
    r1.xyz = r0.xyz * r2.xyz + r1.xyz;
  } else {
    r0.w = fogConstants.heightFalloff * v6.z;
    r1.w = fogConstants.heightFalloff * v6.z + fogConstants.K0;
    r2.x = cmp(abs(r0.w) < 9.99999975e-05);
    r2.y = min(64, r1.w);
    r2.y = 1.44269502 * r2.y;
    r2.y = exp2(r2.y);
    r2.z = saturate(fogConstants.K0b);
    r2.w = cmp(r1.w < 0);
    r1.w = 1 + r1.w;
    r1.w = r2.w ? r2.y : r1.w;
    r1.w = -fogConstants.K0b + r1.w;
    r0.w = r2.x ? 1 : r0.w;
    r0.w = r1.w / r0.w;
    r0.w = r2.x ? r2.z : r0.w;
    r0.w = fogConstants.expMul * r0.w;
    r1.w = dot(v6.xyz, v6.xyz);
    r2.x = sqrt(r1.w);
    r0.w = r0.w * r2.x + fogConstants.expAdd;
    r0.w = exp2(r0.w);
    r0.w = min(1, r0.w);
    r0.w = 1 + -r0.w;
    r1.w = rsqrt(r1.w);
    r2.xyz = v6.xyz * r1.www;
    r1.w = dot(fogConstants.wldSunFogDir.xyz, r2.xyz);
    r1.w = saturate(fogConstants.sunFogAngles.y * r1.w + fogConstants.sunFogAngles.x);
    r2.xyzw = -fogConstants.sunFogColor.xyzw + fogConstants.fogColor.xyzw;
    r2.xyzw = r1.wwww * r2.xyzw + fogConstants.sunFogColor.xyzw;
    r0.w = r2.w * r0.w;
    r2.xyz = r2.xyz + -r0.xyz;
    r1.xyz = r0.www * r2.xyz + r0.xyz;
  }
  r0.xyz = relHDRExposure.yyy * r1.xyz;
  r1.xyz = cmp(r0.xyz >= float3(6.10351563e-05,6.10351563e-05,6.10351563e-05));
  r0.xyz = r1.xyz ? r0.xyz : 0;
  o0.xyz = min(float3(65024,65024,64512), r0.xyz);
  return;
}