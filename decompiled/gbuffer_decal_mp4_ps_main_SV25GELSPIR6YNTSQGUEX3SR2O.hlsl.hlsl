// ---- Created with 3Dmigoto v1.2.45 on Thu Jul 15 15:27:47 2021
...
Texture2D<float4> yTexture : register(t7);
Texture2D<float4> cbcrTexture : register(t9);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.w = yTexture.Sample(ySampler_s, r0.yz).x;
  r1.x = -0.0625 + r0.w;
  r0.yz = cbcrTexture.Sample(cbcrSampler_s, r0.yz).xy;
  r1.yz = float2(-0.5,-0.5) + r0.yz;
  r2.x = dot(float2(1.16439998,1.79270005), r1.xz);
  r2.y = dot(float3(1.16439998,-0.213300005,-0.532899976), r1.xyz);
  r2.z = dot(float2(1.16439998,2.11240005), r1.xy);
  r0.yzw = cmp(float3(0.0404499993,0.0404499993,0.0404499993) >= r2.xyz);
  r1.xyz = float3(0.0773993805,0.0773993805,0.0773993805) * r2.xyz;
