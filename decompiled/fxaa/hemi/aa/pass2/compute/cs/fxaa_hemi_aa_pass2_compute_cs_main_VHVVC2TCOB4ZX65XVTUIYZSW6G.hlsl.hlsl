...
r0.x = LinearSampler[]..swiz;
  r0.yz = (uint2)r0.xx >> int2(8,20);
  r1.xy = (int2)r0.yz & int2(4095,4095);
  r0.yz = (int2)r0.xx & int2(254,1);
  r2.xy = (uint2)r0.yz;
  r0.y = 0.00196850393 * r2.x;
  bitmask.x = ((~(-1 << 1)) << 1) & 0xffffffff;  r0.x = (((uint)r0.x << 1) & bitmask.x) | ((uint)0 & ~bitmask.x);
  r1.z = (int)r1.y + (int)r0.x;
  r3.xy = (int2)r1.xz + int2(0,-1);
  r3.zw = float2(0,0);
  ...
  r1.z = (int)r1.z;
  r0.x = 0.25 * abs(r0.x);
  r2.x = postFxControl0.x;
  r2.y = 0;
  r3.xy = float2(22.5,0);
  r4.xy = float2(22.5,0);
  r2.z = 0;
  while (true) {
    r2.w = cmp((uint)r2.z >= 7);
    if (r2.w != 0) break;
    r3.zw = icb[r2.z+0].xx * r2.xy;
    r4.zw = r0.zw * postFxControl0.xy + -r3.zw;
    r3.zw = r0.zw * postFxControl0.xy + r3.zw;
    r2.w = Luma.SampleLevel(LinearSampler_s, r4.zw, 0).x;
    r2.w = -r1.w * 0.5 + r2.w;
    r4.z = cmp(abs(r2.w) >= r0.x);
    r4.w = cmp(icb[r2.z+0].x < r3.x);
    r4.z = r4.w ? r4.z : 0;
    r4.w = cmp(0 < r2.w);
    r2.w = cmp(r2.w < 0);
    r2.w = (int)-r4.w + (int)r2.w;
    r2.w = (int)r2.w;
    r5.y = cmp(r1.z == r2.w);
    r5.xz = icb[r2.z+0].xx;
    r3.xy = r4.zz ? r5.xy : r3.xy;
    r2.w = Luma.SampleLevel(LinearSampler_s, r3.zw, 0).x;
    r2.w = -r1.w * 0.5 + r2.w;
    r3.z = cmp(abs(r2.w) >= r0.x);
    r3.w = cmp(icb[r2.z+0].x < r4.x);
    r3.z = r3.w ? r3.z : 0;
    r3.w = cmp(0 < r2.w);
    r2.w = cmp(r2.w < 0);
  ...
  if (r0.y != 0) {
    r0.y = -r0.x * 2 + 1;
    r2.xw = float2(65536,65536) * r0.xx;
    r2.y = 32768 * r0.y;
    r2.z = 0;
