...
Texture2D<float4> blurTexture : register(t6);
Texture2D<float4> fireTexture : register(t7);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  float4 x0[16];
  x0[0].xy = float2(0,0);
  x0[1].xy = float2(0.25,0);
  x0[2].xy = float2(0.5,0);
  x0[3].xy = float2(0.75,0);
  x0[4].xy = float2(0,0.25);
  x0[5].xy = float2(0.25,0.25);
  x0[6].xy = float2(0.5,0.25);
  x0[7].xy = float2(0.75,0.25);
  x0[8].xy = float2(0,0.5);
  x0[9].xy = float2(0.25,0.5);
  x0[10].xy = float2(0.5,0.5);
  x0[11].xy = float2(0.75,0.5);
  x0[12].xy = float2(0,0.75);
  x0[13].xy = float2(0.25,0.75);
  x0[14].xy = float2(0.5,0.75);
  x0[15].xy = float2(0.75,0.75);
  r0.x = gameTime.w * postFxControl0.y;
  r0.x = frac(r0.x);
  r0.x = 16 * r0.x;
  r0.x = (int)r0.x;
  r0.xy = x0[r0.x+0].xy;
  r0.xy = v1.xy * float2(0.25,0.25) + r0.xy;
  r0.xyzw = fireTexture.Sample(bilinearClamp_s, r0.xy).xyzw;
  r1.xy = float2(-0.5,-0.5) + v1.xy;
  r1.xy = float2(2,1.125) * r1.xy;
  r1.x = dot(r1.xy, r1.xy);
  r1.x = sqrt(r1.x);
  r1.x = saturate(-postFxControl0.x * r1.x + 1);
  r2.xyzw = pixelTexture.Sample(bilinearClamp_s, v1.xy).xyzw;
  r3.xyzw = blurTexture.Sample(bilinearClamp_s, v1.xy).xyzw;
  r3.xyzw = float4(3.05175781e-05,3.05175781e-05,3.05175781e-05,1) * r3.xyzw;
  r2.xyzw = r2.xyzw * float4(3.05175781e-05,3.05175781e-05,3.05175781e-05,1) + -r3.xyzw;
  r1.xyzw = r1.xxxx * r2.xyzw + r3.xyzw;
  r2.x = 4 * postFxControl0.z;
