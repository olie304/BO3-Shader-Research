//
// Generated by Microsoft (R) HLSL Shader Compiler 9.30.9200.16384
//
//   using 3Dmigoto v1.2.45 on Thu Jul 15 15:29:58 2021
//
//
// Buffer Definitions: 
//
// cbuffer PerSceneConsts
// {
//
//   row_major float4x4 projectionMatrix;// Offset:    0 Size:    64 [unused]
//   row_major float4x4 viewMatrix;     // Offset:   64 Size:    64 [unused]
//   row_major float4x4 viewProjectionMatrix;// Offset:  128 Size:    64
//   row_major float4x4 inverseProjectionMatrix;// Offset:  192 Size:    64 [unused]
//   row_major float4x4 inverseViewMatrix;// Offset:  256 Size:    64 [unused]
//   row_major float4x4 inverseViewProjectionMatrix;// Offset:  320 Size:    64 [unused]
//   float4 eyeOffset;                  // Offset:  384 Size:    16
//   float4 adsZScale;                  // Offset:  400 Size:    16 [unused]
//   float4 hdrControl0;                // Offset:  416 Size:    16 [unused]
//   float4 hdrControl1;                // Offset:  432 Size:    16 [unused]
//   float4 fogColor;                   // Offset:  448 Size:    16 [unused]
//   float4 fogConsts;                  // Offset:  464 Size:    16 [unused]
//   float4 fogConsts2;                 // Offset:  480 Size:    16 [unused]
//   float4 fogConsts3;                 // Offset:  496 Size:    16 [unused]
//   float4 fogConsts4;                 // Offset:  512 Size:    16 [unused]
//   float4 fogConsts5;                 // Offset:  528 Size:    16 [unused]
//   float4 fogConsts6;                 // Offset:  544 Size:    16 [unused]
//   float4 fogConsts7;                 // Offset:  560 Size:    16 [unused]
//   float4 fogConsts8;                 // Offset:  576 Size:    16 [unused]
//   float4 fogConsts9;                 // Offset:  592 Size:    16 [unused]
//   float3 sunFogDir;                  // Offset:  608 Size:    12 [unused]
//   float4 sunFogColor;                // Offset:  624 Size:    16 [unused]
//   float2 sunFog;                     // Offset:  640 Size:     8 [unused]
//   float4 zNear;                      // Offset:  656 Size:    16 [unused]
//   float3 clothPrimaryTint;           // Offset:  672 Size:    12 [unused]
//   float3 clothSecondaryTint;         // Offset:  688 Size:    12 [unused]
//   float4 renderTargetSize;           // Offset:  704 Size:    16 [unused]
//   float4 upscaledTargetSize;         // Offset:  720 Size:    16 [unused]
//   float4 materialColor;              // Offset:  736 Size:    16 [unused]
//   float4 cameraUp;                   // Offset:  752 Size:    16 [unused]
//   float4 cameraLook;                 // Offset:  768 Size:    16 [unused]
//   float4 cameraSide;                 // Offset:  784 Size:    16 [unused]
//   float4 cameraVelocity;             // Offset:  800 Size:    16 [unused]
//   float4 skyMxR;                     // Offset:  816 Size:    16 [unused]
//   float4 skyMxG;                     // Offset:  832 Size:    16 [unused]
//   float4 skyMxB;                     // Offset:  848 Size:    16 [unused]
//   float4 sunMxR;                     // Offset:  864 Size:    16 [unused]
//   float4 sunMxG;                     // Offset:  880 Size:    16 [unused]
//   float4 sunMxB;                     // Offset:  896 Size:    16 [unused]
//   float4 skyRotationTransition;      // Offset:  912 Size:    16 [unused]
//   float4 debugColorOverride;         // Offset:  928 Size:    16 [unused]
//   float4 debugAlphaOverride;         // Offset:  944 Size:    16 [unused]
//   float4 debugNormalOverride;        // Offset:  960 Size:    16 [unused]
//   float4 debugSpecularOverride;      // Offset:  976 Size:    16 [unused]
//   float4 debugGlossOverride;         // Offset:  992 Size:    16 [unused]
//   float4 debugOcclusionOverride;     // Offset: 1008 Size:    16 [unused]
//   float4 debugStreamerControl;       // Offset: 1024 Size:    16 [unused]
//   float4 emblemLUTSelector;          // Offset: 1040 Size:    16 [unused]
//   float4 colorMatrixR;               // Offset: 1056 Size:    16 [unused]
//   float4 colorMatrixG;               // Offset: 1072 Size:    16 [unused]
//   float4 colorMatrixB;               // Offset: 1088 Size:    16 [unused]
//   float4 gameTime;                   // Offset: 1104 Size:    16 [unused]
//   float4 gameTick;                   // Offset: 1120 Size:    16 [unused]
//   float4 subpixelOffset;             // Offset: 1136 Size:    16 [unused]
//   float4 viewportDimensions;         // Offset: 1152 Size:    16 [unused]
//   float4 viewSpaceScaleBias;         // Offset: 1168 Size:    16 [unused]
//   float4 ui3dUVSetup0;               // Offset: 1184 Size:    16 [unused]
//   float4 ui3dUVSetup1;               // Offset: 1200 Size:    16 [unused]
//   float4 ui3dUVSetup2;               // Offset: 1216 Size:    16 [unused]
//   float4 ui3dUVSetup3;               // Offset: 1232 Size:    16 [unused]
//   float4 ui3dUVSetup4;               // Offset: 1248 Size:    16 [unused]
//   float4 ui3dUVSetup5;               // Offset: 1264 Size:    16 [unused]
//   float4 clipSpaceLookupScale;       // Offset: 1280 Size:    16 [unused]
//   float4 clipSpaceLookupOffset;      // Offset: 1296 Size:    16 [unused]
//   uint4 computeSpriteControl;        // Offset: 1312 Size:    16 [unused]
//   float4 invBcTexSizes;              // Offset: 1328 Size:    16 [unused]
//   float4 invMaskTexSizes;            // Offset: 1344 Size:    16 [unused]
//   float4 relHDRExposure;             // Offset: 1360 Size:    16 [unused]
//   uint4 triDensityFlags;             // Offset: 1376 Size:    16 [unused]
//   float4 triDensityParams;           // Offset: 1392 Size:    16 [unused]
//   float4 voldecalRevealTextureInfo;  // Offset: 1408 Size:    16 [unused]
//   float4 extraClipPlane0;            // Offset: 1424 Size:    16 [unused]
//   float4 extraClipPlane1;            // Offset: 1440 Size:    16 [unused]
//   float4 shaderDebug;                // Offset: 1456 Size:    16 [unused]
//   uint isDepthHack;                  // Offset: 1472 Size:     4 [unused]
//
// }
//
// cbuffer MotionVectorParams
// {
//
//   struct MotionVectorConstants
//   {
//       
//       row_major float4x4 prevFrameOffToClpMatrixFull;// Offset:    0
//       row_major float4x4 prevFrameOffToClpMatrixPartial;// Offset:   64
//       row_major float4x4 curFrameCamToOffMatrix;// Offset:  128
//       float3 prevFrameEyeOffset;     // Offset:  192
//       float motionStrength;          // Offset:  204
//       float3 eyeOffsetFrameDelta;    // Offset:  208
//       float _padding_23;             // Offset:  220
//       float2 curFrameHalfNearPlaneSize;// Offset:  224
//       float2 _padding_26;            // Offset:  232
//
//   } motionVectorConstants;           // Offset:    0 Size:   240
//
// }
//
// Resource bind info for shaderConstantSetBuffer
// {
//
//   struct GpuShaderConstantSet
//   {
//       
//       float4 scriptVector0;          // Offset:    0
//       float4 scriptVector1;          // Offset:   16
//       float4 scriptVector2;          // Offset:   32
//       float4 scriptVector3;          // Offset:   48
//       float4 scriptVector4;          // Offset:   64
//       float4 scriptVector5;          // Offset:   80
//       float4 scriptVector6;          // Offset:   96
//       float4 scriptVector7;          // Offset:  112
//       float4 weaponParam0;           // Offset:  128
//       float4 weaponParam1;           // Offset:  144
//       float4 weaponParam2;           // Offset:  160
//       float4 weaponParam3;           // Offset:  176
//       float4 weaponParam4;           // Offset:  192
//       float4 weaponParam5;           // Offset:  208
//       float4 weaponParam6;           // Offset:  224
//       float4 weaponParam7;           // Offset:  240
//       float4 characterParam0;        // Offset:  256
//       float4 characterParam1;        // Offset:  272
//       float4 characterParam2;        // Offset:  288
//       float4 characterParam3;        // Offset:  304
//       float4 characterParam4;        // Offset:  320
//       float4 characterParam5;        // Offset:  336
//       float4 characterParam6;        // Offset:  352
//       float4 characterParam7;        // Offset:  368
//
//   } $Element;                        // Offset:    0 Size:   384
//
// }
//
// Resource bind info for modelInstanceBuffer
// {
//
//   struct ModelInstanceData
//   {
//       
//       uint boneArrayIndex;           // Offset:    0
//       uint shaderConstantSet;        // Offset:    4
//       uint flagsAndPrevFrameIndex;   // Offset:    8
//       uint worldMatrix;              // Offset:   12
//       uint siegeAnimStateOffset;     // Offset:   16
//       uint prevFrameSiegeAnimStateOffset;// Offset:   20
//
//   } $Element;                        // Offset:    0 Size:    24
//
// }
//
// Resource bind info for boneMatrixBuffer
// {
//
//   struct GpuBoneData
//   {
//       
//       float4 mat[3];                 // Offset:    0
//       float4 extra;                  // Offset:   48
//
//   } $Element;                        // Offset:    0 Size:    64
//
// }
//
// Resource bind info for boneMatrixBufferPrevFrame
// {
//
//   struct GpuBoneData
//   {
//       
//       float4 mat[3];                 // Offset:    0
//       float4 extra;                  // Offset:   48
//
//   } $Element;                        // Offset:    0 Size:    64
//
// }
//
//
// Resource Bindings:
//
// Name                                 Type  Format         Dim      HLSL Bind  Count
// ------------------------------ ---------- ------- ----------- -------------- ------
// shaderConstantSetBuffer           texture  struct         r/o             t0      1 
// gpuSkinBase                       texture  float4          2d             t1      1 
// gpuSkinQuat                       texture  float4          2d             t2      1 
// gpuSkinPos                        texture  float4          2d             t3      1 
// modelInstanceBuffer               texture  struct         r/o             t4      1 
// boneMatrixBuffer                  texture  struct         r/o             t5      1 
// boneMatrixBufferPrevFrame         texture  struct         r/o             t8      1 
// PerSceneConsts                    cbuffer      NA          NA            cb1      1 
// MotionVectorParams                cbuffer      NA          NA            cb7      1 
//
//
//
// Input signature:
//
// Name                 Index   Mask Register SysValue  Format   Used
// -------------------- ----- ------ -------- -------- ------- ------
// POSITION                 0   xyz         0     NONE   float   xyz 
// TEXCOORD                 0   xy          1     NONE   float   xy  
// NORMAL                   0   xyz         2     NONE   float   xyz 
// TANGENT                  0   xyzw        3     NONE   float   xyzw
// BLENDWEIGHT              0   xyzw        4     NONE   float   xyzw
// BLENDINDICES             0   xyzw        5     NONE    uint   xyzw
// TEXCOORD                15   x           6     NONE    uint   x   
//
//
// Output signature:
//
// Name                 Index   Mask Register SysValue  Format   Used
// -------------------- ----- ------ -------- -------- ------- ------
// SV_POSITION              0   xyzw        0      POS   float   xyzw
// COLOR                    1   x           1     NONE   float   x   
// TEXCOORD                 0    yz         1     NONE   float    yz 
// TEXCOORD                 1   xyz         2     NONE   float   xyz 
// TEXCOORD                 2   xyz         3     NONE   float   xyz 
// TEXCOORD                 3   xyz         4     NONE   float   xyz 
// TEXCOORD                 7   xyzw        5     NONE   float   xyzw
//
vs_5_0
dcl_globalFlags refactoringAllowed
dcl_constantbuffer CB1[25], immediateIndexed
dcl_constantbuffer CB7[13], immediateIndexed
dcl_resource_structured t0, 384
dcl_resource_texture2d (float,float,float,float) t1
dcl_resource_texture2d (float,float,float,float) t2
dcl_resource_texture2d (float,float,float,float) t3
dcl_resource_structured t4, 24
dcl_resource_structured t5, 64
dcl_resource_structured t8, 64
dcl_input v0.xyz
dcl_input v1.xy
dcl_input v2.xyz
dcl_input v3.xyzw
dcl_input v4.xyzw
dcl_input v5.xyzw
dcl_input v6.x
dcl_output_siv o0.xyzw, position
dcl_output o1.x
dcl_output o1.yz
dcl_output o2.xyz
dcl_output o3.xyz
dcl_output o4.xyz
dcl_output o5.xyzw
dcl_temps 16
mad r0.xyz, v2.xyzx, l(2.001957, 2.001957, 2.001957, 0.000000), l(-1.00195694, -1.00195694, -1.00195694, 0.000000)
mad r1.xyz, v3.xyzx, l(2.001957, 2.001957, 2.001957, 0.000000), l(-1.00195694, -1.00195694, -1.00195694, 0.000000)
mad r0.w, v3.w, l(2.000000), l(-1.000000)
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r2.xyzw, v6.x, l(4), t4.xyzw
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r2.w, l(48), t5.xyzw
movc r3.xyzw, r2.wwww, r3.xyzw, l(0,0,0,0)
ne r1.w, l(0.000000, 0.000000, 0.000000, 0.000000), r3.w
if_nz r1.w
  ftou r1.w, r3.w
  if_nz r1.w
    iadd r2.w, r1.w, v5.x
    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r2.w, l(0), t5.xyzw
    dp3 r3.w, r4.xyzx, v0.xyzx
    add r3.w, r4.w, r3.w
    mul r5.x, r3.w, v4.x
    dp3 r3.w, r4.xyzx, r0.xyzx
    mul r6.x, r3.w, v4.x
    dp3 r3.w, r4.xyzx, r1.xyzx
    mul r4.x, r3.w, v4.x
    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(16), t5.xyzw
    dp3 r3.w, r7.xyzx, v0.xyzx
    add r3.w, r7.w, r3.w
    mul r5.y, r3.w, v4.x
    dp3 r3.w, r7.xyzx, r0.xyzx
    mul r6.y, r3.w, v4.x
    dp3 r3.w, r7.xyzx, r1.xyzx
    mul r4.y, r3.w, v4.x
    ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(32), t5.xyzw
    dp3 r2.w, r7.xyzx, v0.xyzx
    add r2.w, r7.w, r2.w
    mul r5.z, r2.w, v4.x
    dp3 r2.w, r7.xyzx, r0.xyzx
    mul r6.z, r2.w, v4.x
    dp3 r2.w, r7.xyzx, r1.xyzx
    mul r4.z, r2.w, v4.x
    ne r2.w, v4.y, l(0.000000)
    if_nz r2.w
      iadd r2.w, r1.w, v5.y
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(0), t5.xyzw
      dp3 r3.w, r7.xyzx, v0.xyzx
      add r3.w, r7.w, r3.w
      mad r5.x, r3.w, v4.y, r5.x
      dp3 r3.w, r7.xyzx, r0.xyzx
      mad r6.x, r3.w, v4.y, r6.x
      dp3 r3.w, r7.xyzx, r1.xyzx
      mad r4.x, r3.w, v4.y, r4.x
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(16), t5.xyzw
      dp3 r3.w, r7.xyzx, v0.xyzx
      add r3.w, r7.w, r3.w
      mad r5.y, r3.w, v4.y, r5.y
      dp3 r3.w, r7.xyzx, r0.xyzx
      mad r6.y, r3.w, v4.y, r6.y
      dp3 r3.w, r7.xyzx, r1.xyzx
      mad r4.y, r3.w, v4.y, r4.y
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(32), t5.xyzw
      dp3 r2.w, r7.xyzx, v0.xyzx
      add r2.w, r7.w, r2.w
      mad r5.z, r2.w, v4.y, r5.z
      dp3 r2.w, r7.xyzx, r0.xyzx
      mad r6.z, r2.w, v4.y, r6.z
      dp3 r2.w, r7.xyzx, r1.xyzx
      mad r4.z, r2.w, v4.y, r4.z
      ne r2.w, v4.z, l(0.000000)
      if_nz r2.w
        iadd r2.w, r1.w, v5.z
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(0), t5.xyzw
        dp3 r3.w, r7.xyzx, v0.xyzx
        add r3.w, r7.w, r3.w
        mad r5.x, r3.w, v4.z, r5.x
        dp3 r3.w, r7.xyzx, r0.xyzx
        mad r6.x, r3.w, v4.z, r6.x
        dp3 r3.w, r7.xyzx, r1.xyzx
        mad r4.x, r3.w, v4.z, r4.x
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(16), t5.xyzw
        dp3 r3.w, r7.xyzx, v0.xyzx
        add r3.w, r7.w, r3.w
        mad r5.y, r3.w, v4.z, r5.y
        dp3 r3.w, r7.xyzx, r0.xyzx
        mad r6.y, r3.w, v4.z, r6.y
        dp3 r3.w, r7.xyzx, r1.xyzx
        mad r4.y, r3.w, v4.z, r4.y
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r2.w, l(32), t5.xyzw
        dp3 r2.w, r7.xyzx, v0.xyzx
        add r2.w, r7.w, r2.w
        mad r5.z, r2.w, v4.z, r5.z
        dp3 r2.w, r7.xyzx, r0.xyzx
        mad r6.z, r2.w, v4.z, r6.z
        dp3 r2.w, r7.xyzx, r1.xyzx
        mad r4.z, r2.w, v4.z, r4.z
        ne r2.w, v4.w, l(0.000000)
        if_nz r2.w
          iadd r1.w, r1.w, v5.w
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r1.w, l(0), t5.xyzw
          dp3 r2.w, r7.xyzx, v0.xyzx
          add r2.w, r7.w, r2.w
          mad r5.x, r2.w, v4.w, r5.x
          dp3 r2.w, r7.xyzx, r0.xyzx
          mad r6.x, r2.w, v4.w, r6.x
          dp3 r2.w, r7.xyzx, r1.xyzx
          mad r4.x, r2.w, v4.w, r4.x
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r1.w, l(16), t5.xyzw
          dp3 r2.w, r7.xyzx, v0.xyzx
          add r2.w, r7.w, r2.w
          mad r5.y, r2.w, v4.w, r5.y
          dp3 r2.w, r7.xyzx, r0.xyzx
          mad r6.y, r2.w, v4.w, r6.y
          dp3 r2.w, r7.xyzx, r1.xyzx
          mad r4.y, r2.w, v4.w, r4.y
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r1.w, l(32), t5.xyzw
          dp3 r1.w, r7.xyzx, v0.xyzx
          add r1.w, r7.w, r1.w
          mad r5.z, r1.w, v4.w, r5.z
          dp3 r1.w, r7.xyzx, r0.xyzx
          mad r6.z, r1.w, v4.w, r6.z
          dp3 r1.w, r7.xyzx, r1.xyzx
          mad r4.z, r1.w, v4.w, r4.z
        endif 
      endif 
    endif 
    dp3 r1.w, r6.xyzx, r6.xyzx
    rsq r1.w, r1.w
    mul r0.xyz, r1.wwww, r6.xyzx
    dp3 r1.w, r4.xyzx, r4.xyzx
    rsq r1.w, r1.w
    mul r1.xyz, r1.wwww, r4.xyzx
  else 
    mov r5.xyz, v0.xyzx
  endif 
else 
  ftou r4.yw, r3.xxxy
  mov r4.x, v5.x
  mov r4.z, l(0)
  ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xyzz, t2.xyzw
  ld_indexable(texture2d)(float,float,float,float) r7.xyzw, r4.xwzz, t2.xyzw
  ld_indexable(texture2d)(float,float,float,float) r3.xyw, r4.xyzz, t3.xywz
  ld_indexable(texture2d)(float,float,float,float) r8.xyz, r4.xwzz, t3.xyzw
  ld_indexable(texture2d)(float,float,float,float) r9.xyz, r4.xzzz, t1.xyzw
  add r7.xyzw, -r6.xyzw, r7.xyzw
  mad r6.xyzw, r3.zzzz, r7.xyzw, r6.xyzw
  add r7.xyz, -r3.xywx, r8.xyzx
  mad r3.xyw, r3.zzzz, r7.xyxz, r3.xyxw
  add r7.xyz, -r9.xyzx, v0.xyzx
  dp4 r1.w, r6.xyzw, r6.xyzw
  rsq r1.w, r1.w
  mul r6.xyzw, r1.wwww, r6.xyzw
  add r8.xyz, r6.xyzx, r6.xyzx
  mul r10.xyz, r6.xzwx, r8.xxxx
  mul r1.w, r6.w, r8.y
  mul r8.zw, r6.wwwz, r8.zzzz
  mad r2.w, r6.y, r8.y, r8.w
  add r11.x, -r2.w, l(1.000000)
  mad r12.x, r6.z, r8.x, -r1.w
  mad r11.y, r6.y, r8.x, -r8.z
  mad r13.xy, r6.yxyy, r8.xxxx, r8.zwzz
  mad r13.z, r6.z, r8.y, -r10.z
  mad r6.xyz, r6.zwyz, r8.yyyy, r10.zyxz
  mov r11.z, r6.y
  dp3 r8.x, r7.xyzx, r11.xyzx
  add r13.w, -r13.y, l(1.000000)
  dp3 r8.y, r7.xzyx, r13.xzwx
  mad r12.yz, r6.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
  dp3 r8.z, r7.xyzx, r12.xyzx
  dp3 r6.x, r0.xyzx, r11.xyzx
  dp3 r6.y, r0.xzyx, r13.xzwx
  dp3 r6.z, r0.xyzx, r12.xyzx
  dp3 r7.x, r1.xyzx, r11.xyzx
  dp3 r7.y, r1.xzyx, r13.xzwx
  dp3 r7.z, r1.xyzx, r12.xyzx
  add r8.xyz, r9.xyzx, r8.xyzx
  add r3.xyw, r3.xyxw, r8.xyxz
  mul r5.xyz, r3.xywx, v4.xxxx
  mul r3.xyw, r6.xyxz, v4.xxxx
  mul r6.xyz, r7.xyzx, v4.xxxx
  ne r1.w, v4.y, l(0.000000)
  if_nz r1.w
    mov r4.x, v5.y
    ld_indexable(texture2d)(float,float,float,float) r7.xyzw, r4.xyzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r8.xyzw, r4.xwzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r9.xyz, r4.xyzz, t3.xyzw
    ld_indexable(texture2d)(float,float,float,float) r10.xyz, r4.xwzz, t3.xyzw
    ld_indexable(texture2d)(float,float,float,float) r11.xyz, r4.xzzz, t1.xyzw
    add r8.xyzw, -r7.xyzw, r8.xyzw
    mad r7.xyzw, r3.zzzz, r8.xyzw, r7.xyzw
    add r8.xyz, -r9.xyzx, r10.xyzx
    mad r8.xyz, r3.zzzz, r8.xyzx, r9.xyzx
    add r9.xyz, -r11.xyzx, v0.xyzx
    dp4 r1.w, r7.xyzw, r7.xyzw
    rsq r1.w, r1.w
    mul r7.xyzw, r1.wwww, r7.xyzw
    add r10.xyz, r7.xyzx, r7.xyzx
    mul r12.xyz, r7.xzwx, r10.xxxx
    mul r1.w, r7.w, r10.y
    mul r10.zw, r7.wwwz, r10.zzzz
    mad r2.w, r7.y, r10.y, r10.w
    add r13.x, -r2.w, l(1.000000)
    mad r14.x, r7.z, r10.x, -r1.w
    mad r13.y, r7.y, r10.x, -r10.z
    mad r15.xy, r7.yxyy, r10.xxxx, r10.zwzz
    mad r15.z, r7.z, r10.y, -r12.z
    mad r7.xyz, r7.zwyz, r10.yyyy, r12.zyxz
    mov r13.z, r7.y
    dp3 r10.x, r9.xyzx, r13.xyzx
    add r15.w, -r15.y, l(1.000000)
    dp3 r10.y, r9.xzyx, r15.xzwx
    mad r14.yz, r7.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
    dp3 r10.z, r9.xyzx, r14.xyzx
    dp3 r7.x, r0.xyzx, r13.xyzx
    dp3 r7.y, r0.xzyx, r15.xzwx
    dp3 r7.z, r0.xyzx, r14.xyzx
    dp3 r9.x, r1.xyzx, r13.xyzx
    dp3 r9.y, r1.xzyx, r15.xzwx
    dp3 r9.z, r1.xyzx, r14.xyzx
    add r10.xyz, r11.xyzx, r10.xyzx
    add r8.xyz, r8.xyzx, r10.xyzx
    mad r5.xyz, r8.xyzx, v4.yyyy, r5.xyzx
    mad r3.xyw, r7.xyxz, v4.yyyy, r3.xyxw
    mad r6.xyz, r9.xyzx, v4.yyyy, r6.xyzx
    ne r1.w, v4.z, l(0.000000)
    if_nz r1.w
      mov r4.x, v5.z
      ld_indexable(texture2d)(float,float,float,float) r7.xyzw, r4.xyzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r8.xyzw, r4.xwzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r9.xyz, r4.xyzz, t3.xyzw
      ld_indexable(texture2d)(float,float,float,float) r10.xyz, r4.xwzz, t3.xyzw
      ld_indexable(texture2d)(float,float,float,float) r11.xyz, r4.xzzz, t1.xyzw
      add r8.xyzw, -r7.xyzw, r8.xyzw
      mad r7.xyzw, r3.zzzz, r8.xyzw, r7.xyzw
      add r8.xyz, -r9.xyzx, r10.xyzx
      mad r8.xyz, r3.zzzz, r8.xyzx, r9.xyzx
      add r9.xyz, -r11.xyzx, v0.xyzx
      dp4 r1.w, r7.xyzw, r7.xyzw
      rsq r1.w, r1.w
      mul r7.xyzw, r1.wwww, r7.xyzw
      add r10.xyz, r7.xyzx, r7.xyzx
      mul r12.xyz, r7.xzwx, r10.xxxx
      mul r1.w, r7.w, r10.y
      mul r10.zw, r7.wwwz, r10.zzzz
      mad r2.w, r7.y, r10.y, r10.w
      add r13.x, -r2.w, l(1.000000)
      mad r14.x, r7.z, r10.x, -r1.w
      mad r13.y, r7.y, r10.x, -r10.z
      mad r15.xy, r7.yxyy, r10.xxxx, r10.zwzz
      mad r15.z, r7.z, r10.y, -r12.z
      mad r7.xyz, r7.zwyz, r10.yyyy, r12.zyxz
      mov r13.z, r7.y
      dp3 r10.x, r9.xyzx, r13.xyzx
      add r15.w, -r15.y, l(1.000000)
      dp3 r10.y, r9.xzyx, r15.xzwx
      mad r14.yz, r7.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
      dp3 r10.z, r9.xyzx, r14.xyzx
      dp3 r7.x, r0.xyzx, r13.xyzx
      dp3 r7.y, r0.xzyx, r15.xzwx
      dp3 r7.z, r0.xyzx, r14.xyzx
      dp3 r9.x, r1.xyzx, r13.xyzx
      dp3 r9.y, r1.xzyx, r15.xzwx
      dp3 r9.z, r1.xyzx, r14.xyzx
      add r10.xyz, r11.xyzx, r10.xyzx
      add r8.xyz, r8.xyzx, r10.xyzx
      mad r5.xyz, r8.xyzx, v4.zzzz, r5.xyzx
      mad r3.xyw, r7.xyxz, v4.zzzz, r3.xyxw
      mad r6.xyz, r9.xyzx, v4.zzzz, r6.xyzx
      ne r1.w, v4.w, l(0.000000)
      if_nz r1.w
        mov r4.x, v5.w
        ld_indexable(texture2d)(float,float,float,float) r7.xyzw, r4.xyzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r8.xyzw, r4.xwzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r9.xyz, r4.xyzz, t3.xyzw
        ld_indexable(texture2d)(float,float,float,float) r10.xyz, r4.xwzz, t3.xyzw
        ld_indexable(texture2d)(float,float,float,float) r4.xyz, r4.xzzz, t1.xyzw
        add r8.xyzw, -r7.xyzw, r8.xyzw
        mad r7.xyzw, r3.zzzz, r8.xyzw, r7.xyzw
        add r8.xyz, -r9.xyzx, r10.xyzx
        mad r8.xyz, r3.zzzz, r8.xyzx, r9.xyzx
        add r9.xyz, -r4.xyzx, v0.xyzx
        dp4 r1.w, r7.xyzw, r7.xyzw
        rsq r1.w, r1.w
        mul r7.xyzw, r1.wwww, r7.xyzw
        add r10.xyz, r7.xyzx, r7.xyzx
        mul r11.xyz, r7.xzwx, r10.xxxx
        mul r1.w, r7.w, r10.y
        mul r10.zw, r7.wwwz, r10.zzzz
        mad r2.w, r7.y, r10.y, r10.w
        add r12.x, -r2.w, l(1.000000)
        mad r13.x, r7.z, r10.x, -r1.w
        mad r12.y, r7.y, r10.x, -r10.z
        mad r14.xy, r7.yxyy, r10.xxxx, r10.zwzz
        mad r14.z, r7.z, r10.y, -r11.z
        mad r7.xyz, r7.zwyz, r10.yyyy, r11.zyxz
        mov r12.z, r7.y
        dp3 r10.x, r9.xyzx, r12.xyzx
        add r14.w, -r14.y, l(1.000000)
        dp3 r10.y, r9.xzyx, r14.xzwx
        mad r13.yz, r7.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
        dp3 r10.z, r9.xyzx, r13.xyzx
        dp3 r7.x, r0.xyzx, r12.xyzx
        dp3 r7.y, r0.xzyx, r14.xzwx
        dp3 r7.z, r0.xyzx, r13.xyzx
        dp3 r9.x, r1.xyzx, r12.xyzx
        dp3 r9.y, r1.xzyx, r14.xzwx
        dp3 r9.z, r1.xyzx, r13.xyzx
        add r4.xyz, r4.xyzx, r10.xyzx
        add r4.xyz, r8.xyzx, r4.xyzx
        mad r5.xyz, r4.xyzx, v4.wwww, r5.xyzx
        mad r3.xyw, r7.xyxz, v4.wwww, r3.xyxw
        mad r6.xyz, r9.xyzx, v4.wwww, r6.xyzx
      endif 
    endif 
  endif 
  dp3 r1.w, r3.xywx, r3.xywx
  rsq r1.w, r1.w
  mul r0.xyz, r1.wwww, r3.xywx
  dp3 r1.w, r6.xyzx, r6.xyzx
  rsq r1.w, r1.w
  mul r1.xyz, r1.wwww, r6.xyzx
endif 
mul r3.xyz, r0.zxyz, r1.yzxy
mad r3.xyz, r0.yzxy, r1.zxyz, -r3.xyzx
mul r3.xyz, r0.wwww, r3.xyzx
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r0.w, v6.x, l(0), t4.xxxx
iadd r1.w, r0.w, r2.z
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r1.w, l(0), t5.xyzw
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r6.xyzw, r1.w, l(16), t5.xyzw
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r7.xyzw, r1.w, l(32), t5.xyzw
mov r8.x, r4.w
mov r8.y, r6.w
mov r8.z, r7.w
add r8.xyw, r8.xyxz, -cb1[24].xyxz
mov r5.w, l(1.000000)
mov r4.w, r8.x
dp4 r1.w, r5.xyzw, r4.xyzw
mov r6.w, r8.y
dp4 r2.z, r5.xyzw, r6.xyzw
mov r8.xyz, r7.xyzx
dp4 r2.w, r5.xyzw, r8.xyzw
dp3 r5.x, r0.xyzx, r4.xyzx
dp3 r5.y, r0.xyzx, r6.xyzx
dp3 r5.z, r0.xyzx, r8.xyzx
dp3 r0.x, r5.xyzx, r5.xyzx
rsq r0.x, r0.x
mul o2.xyz, r0.xxxx, r5.xyzx
dp3 r0.x, r1.xyzx, r4.xyzx
dp3 r0.y, r1.xyzx, r6.xyzx
dp3 r0.z, r1.xyzx, r8.xyzx
dp3 r1.x, r0.xyzx, r0.xyzx
rsq r1.x, r1.x
mul o3.xyz, r0.xyzx, r1.xxxx
dp3 r0.x, r3.xyzx, r4.xyzx
dp3 r0.y, r3.xyzx, r6.xyzx
dp3 r0.z, r3.xyzx, r8.xyzx
dp3 r1.x, r0.xyzx, r0.xyzx
rsq r1.x, r1.x
mul o4.xyz, r0.xyzx, r1.xxxx
mul r3.xyzw, r2.zzzz, cb1[9].xyzw
mad r1.xyzw, r1.wwww, cb1[8].xyzw, r3.xyzw
mad r1.xyzw, r2.wwww, cb1[10].xyzw, r1.xyzw
add o0.xyzw, r1.xyzw, cb1[11].xyzw
and r0.x, r2.y, l(0x0fffffff)
if_nz r0.x
  ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r0.xy, v6.x, l(16), t4.xyxx
  ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyzw, r0.x, l(48), t5.xyzw
  movc r1.xyzw, r0.xxxx, r1.xyzw, l(0,0,0,0)
  ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.y, l(48), t8.xyzw
  movc r1.xyzw, r0.yyyy, r3.xyzw, r1.xyzw
  ne r0.x, l(0.000000, 0.000000, 0.000000, 0.000000), r1.w
  if_nz r0.x
    ftou r0.x, r1.w
    if_nz r0.x
      iadd r0.y, r0.x, v5.x
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.y, l(0), t8.xyzw
      dp3 r0.z, r3.xyzx, v0.xyzx
      add r0.z, r3.w, r0.z
      mul r3.x, r0.z, v4.x
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t8.xyzw
      dp3 r0.z, r4.xyzx, v0.xyzx
      add r0.z, r4.w, r0.z
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t8.xyzw
      dp3 r0.y, r4.xyzx, v0.xyzx
      add r0.y, r4.w, r0.y
      mul r3.yz, r0.zzyz, v4.xxxx
      ne r0.y, v4.y, l(0.000000)
      if_nz r0.y
        iadd r0.y, r0.x, v5.y
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(0), t8.xyzw
        dp3 r0.z, r4.xyzx, v0.xyzx
        add r0.z, r4.w, r0.z
        mad r3.x, r0.z, v4.y, r3.x
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t8.xyzw
        dp3 r0.z, r4.xyzx, v0.xyzx
        add r0.z, r4.w, r0.z
        mad r3.y, r0.z, v4.y, r3.y
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t8.xyzw
        dp3 r0.y, r4.xyzx, v0.xyzx
        add r0.y, r4.w, r0.y
        mad r3.z, r0.y, v4.y, r3.z
        ne r0.y, v4.z, l(0.000000)
        if_nz r0.y
          iadd r0.y, r0.x, v5.z
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(0), t8.xyzw
          dp3 r0.z, r4.xyzx, v0.xyzx
          add r0.z, r4.w, r0.z
          mad r3.x, r0.z, v4.z, r3.x
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t8.xyzw
          dp3 r0.z, r4.xyzx, v0.xyzx
          add r0.z, r4.w, r0.z
          mad r3.y, r0.z, v4.z, r3.y
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t8.xyzw
          dp3 r0.y, r4.xyzx, v0.xyzx
          add r0.y, r4.w, r0.y
          mad r3.z, r0.y, v4.z, r3.z
          ne r0.y, v4.w, l(0.000000)
          if_nz r0.y
            iadd r0.x, r0.x, v5.w
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(0), t8.xyzw
            dp3 r0.y, r4.xyzx, v0.xyzx
            add r0.y, r4.w, r0.y
            mad r3.x, r0.y, v4.w, r3.x
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(16), t8.xyzw
            dp3 r0.y, r4.xyzx, v0.xyzx
            add r0.y, r4.w, r0.y
            mad r3.y, r0.y, v4.w, r3.y
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(32), t8.xyzw
            dp3 r0.x, r4.xyzx, v0.xyzx
            add r0.x, r4.w, r0.x
            mad r3.z, r0.x, v4.w, r3.z
          endif 
        endif 
      endif 
    else 
      mov r3.xyz, v0.xyzx
    endif 
  else 
    ftou r4.yw, r1.xxxy
    mov r4.x, v5.x
    mov r4.z, l(0)
    ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
    ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
    ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
    add r6.xyzw, -r5.xyzw, r6.xyzw
    mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
    add r1.xyw, -r0.xyxz, r1.xyxw
    mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
    add r1.xyw, -r2.yzyw, v0.xyxz
    dp4 r6.x, r5.xyzw, r5.xyzw
    rsq r6.x, r6.x
    mul r5.xyzw, r5.xyzw, r6.xxxx
    add r6.xyz, r5.xyzx, r5.xyzx
    mul r7.xyz, r5.xzwx, r6.xxxx
    mul r6.w, r5.w, r6.y
    mul r8.xy, r5.wzww, r6.zzzz
    mad r6.z, r5.y, r6.y, r8.y
    add r9.x, -r6.z, l(1.000000)
    mad r10.x, r5.z, r6.x, -r6.w
    mad r9.y, r5.y, r6.x, -r8.x
    mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
    mad r8.z, r5.z, r6.y, -r7.z
    mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
    mov r9.z, r5.y
    dp3 r6.x, r1.xywx, r9.xyzx
    add r8.w, -r8.y, l(1.000000)
    dp3 r6.y, r1.xwyx, r8.xzwx
    mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
    dp3 r6.z, r1.xywx, r10.xyzx
    add r1.xyw, r2.yzyw, r6.xyxz
    add r0.xyz, r0.xyzx, r1.xywx
    mul r3.xyz, r0.xyzx, v4.xxxx
    ne r0.x, v4.y, l(0.000000)
    if_nz r0.x
      mov r4.x, v5.y
      ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
      ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
      ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
      add r6.xyzw, -r5.xyzw, r6.xyzw
      mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
      add r1.xyw, -r0.xyxz, r1.xyxw
      mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
      add r1.xyw, -r2.yzyw, v0.xyxz
      dp4 r6.x, r5.xyzw, r5.xyzw
      rsq r6.x, r6.x
      mul r5.xyzw, r5.xyzw, r6.xxxx
      add r6.xyz, r5.xyzx, r5.xyzx
      mul r7.xyz, r5.xzwx, r6.xxxx
      mul r6.w, r5.w, r6.y
      mul r8.xy, r5.wzww, r6.zzzz
      mad r6.z, r5.y, r6.y, r8.y
      add r9.x, -r6.z, l(1.000000)
      mad r10.x, r5.z, r6.x, -r6.w
      mad r9.y, r5.y, r6.x, -r8.x
      mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
      mad r8.z, r5.z, r6.y, -r7.z
      mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
      mov r9.z, r5.y
      dp3 r6.x, r1.xywx, r9.xyzx
      add r8.w, -r8.y, l(1.000000)
      dp3 r6.y, r1.xwyx, r8.xzwx
      mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
      dp3 r6.z, r1.xywx, r10.xyzx
      add r1.xyw, r2.yzyw, r6.xyxz
      add r0.xyz, r0.xyzx, r1.xywx
      mad r3.xyz, r0.xyzx, v4.yyyy, r3.xyzx
      ne r0.x, v4.z, l(0.000000)
      if_nz r0.x
        mov r4.x, v5.z
        ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
        ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
        ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
        add r6.xyzw, -r5.xyzw, r6.xyzw
        mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
        add r1.xyw, -r0.xyxz, r1.xyxw
        mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
        add r1.xyw, -r2.yzyw, v0.xyxz
        dp4 r6.x, r5.xyzw, r5.xyzw
        rsq r6.x, r6.x
        mul r5.xyzw, r5.xyzw, r6.xxxx
        add r6.xyz, r5.xyzx, r5.xyzx
        mul r7.xyz, r5.xzwx, r6.xxxx
        mul r6.w, r5.w, r6.y
        mul r8.xy, r5.wzww, r6.zzzz
        mad r6.z, r5.y, r6.y, r8.y
        add r9.x, -r6.z, l(1.000000)
        mad r10.x, r5.z, r6.x, -r6.w
        mad r9.y, r5.y, r6.x, -r8.x
        mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
        mad r8.z, r5.z, r6.y, -r7.z
        mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
        mov r9.z, r5.y
        dp3 r6.x, r1.xywx, r9.xyzx
        add r8.w, -r8.y, l(1.000000)
        dp3 r6.y, r1.xwyx, r8.xzwx
        mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
        dp3 r6.z, r1.xywx, r10.xyzx
        add r1.xyw, r2.yzyw, r6.xyxz
        add r0.xyz, r0.xyzx, r1.xywx
        mad r3.xyz, r0.xyzx, v4.zzzz, r3.xyzx
        ne r0.x, v4.w, l(0.000000)
        if_nz r0.x
          mov r4.x, v5.w
          ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
          ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
          ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
          ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
          ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
          add r4.xyzw, -r5.xyzw, r6.xyzw
          mad r4.xyzw, r1.zzzz, r4.xyzw, r5.xyzw
          add r1.xyw, -r0.xyxz, r1.xyxw
          mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
          add r1.xyz, -r2.yzwy, v0.xyzx
          dp4 r1.w, r4.xyzw, r4.xyzw
          rsq r1.w, r1.w
          mul r4.xyzw, r1.wwww, r4.xyzw
          add r5.xyz, r4.xyzx, r4.xyzx
          mul r6.xyz, r4.xzwx, r5.xxxx
          mul r1.w, r4.w, r5.y
          mul r5.zw, r4.wwwz, r5.zzzz
          mad r6.w, r4.y, r5.y, r5.w
          add r7.x, -r6.w, l(1.000000)
          mad r8.x, r4.z, r5.x, -r1.w
          mad r7.y, r4.y, r5.x, -r5.z
          mad r9.xy, r4.yxyy, r5.xxxx, r5.zwzz
          mad r9.z, r4.z, r5.y, -r6.z
          mad r4.xyz, r4.zwyz, r5.yyyy, r6.zyxz
          mov r7.z, r4.y
          dp3 r5.x, r1.xyzx, r7.xyzx
          add r9.w, -r9.y, l(1.000000)
          dp3 r5.y, r1.xzyx, r9.xzwx
          mad r8.yz, r4.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
          dp3 r5.z, r1.xyzx, r8.xyzx
          add r1.xyz, r2.yzwy, r5.xyzx
          add r0.xyz, r0.xyzx, r1.xyzx
          mad r3.xyz, r0.xyzx, v4.wwww, r3.xyzx
        endif 
      endif 
    endif 
  endif 
else 
  ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r0.x, v6.x, l(16), t4.xxxx
  ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyzw, r0.x, l(48), t5.xyzw
  movc r1.xyzw, r0.xxxx, r1.xyzw, l(0,0,0,0)
  ne r0.x, l(0.000000, 0.000000, 0.000000, 0.000000), r1.w
  if_nz r0.x
    ftou r0.x, r1.w
    if_nz r0.x
      iadd r0.y, r0.x, v5.x
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(0), t5.xyzw
      dp3 r0.z, r4.xyzx, v0.xyzx
      add r0.z, r4.w, r0.z
      mul r3.x, r0.z, v4.x
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t5.xyzw
      dp3 r0.z, r4.xyzx, v0.xyzx
      add r0.z, r4.w, r0.z
      ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t5.xyzw
      dp3 r0.y, r4.xyzx, v0.xyzx
      add r0.y, r4.w, r0.y
      mul r3.yz, r0.zzyz, v4.xxxx
      ne r0.y, v4.y, l(0.000000)
      if_nz r0.y
        iadd r0.y, r0.x, v5.y
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(0), t5.xyzw
        dp3 r0.z, r4.xyzx, v0.xyzx
        add r0.z, r4.w, r0.z
        mad r3.x, r0.z, v4.y, r3.x
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t5.xyzw
        dp3 r0.z, r4.xyzx, v0.xyzx
        add r0.z, r4.w, r0.z
        mad r3.y, r0.z, v4.y, r3.y
        ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t5.xyzw
        dp3 r0.y, r4.xyzx, v0.xyzx
        add r0.y, r4.w, r0.y
        mad r3.z, r0.y, v4.y, r3.z
        ne r0.y, v4.z, l(0.000000)
        if_nz r0.y
          iadd r0.y, r0.x, v5.z
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(0), t5.xyzw
          dp3 r0.z, r4.xyzx, v0.xyzx
          add r0.z, r4.w, r0.z
          mad r3.x, r0.z, v4.z, r3.x
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(16), t5.xyzw
          dp3 r0.z, r4.xyzx, v0.xyzx
          add r0.z, r4.w, r0.z
          mad r3.y, r0.z, v4.z, r3.y
          ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.y, l(32), t5.xyzw
          dp3 r0.y, r4.xyzx, v0.xyzx
          add r0.y, r4.w, r0.y
          mad r3.z, r0.y, v4.z, r3.z
          ne r0.y, v4.w, l(0.000000)
          if_nz r0.y
            iadd r0.x, r0.x, v5.w
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(0), t5.xyzw
            dp3 r0.y, r4.xyzx, v0.xyzx
            add r0.y, r4.w, r0.y
            mad r3.x, r0.y, v4.w, r3.x
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(16), t5.xyzw
            dp3 r0.y, r4.xyzx, v0.xyzx
            add r0.y, r4.w, r0.y
            mad r3.y, r0.y, v4.w, r3.y
            ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(32), t5.xyzw
            dp3 r0.x, r4.xyzx, v0.xyzx
            add r0.x, r4.w, r0.x
            mad r3.z, r0.x, v4.w, r3.z
          endif 
        endif 
      endif 
    else 
      mov r3.xyz, v0.xyzx
    endif 
  else 
    ftou r4.yw, r1.xxxy
    mov r4.x, v5.x
    mov r4.z, l(0)
    ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
    ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
    ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
    ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
    add r6.xyzw, -r5.xyzw, r6.xyzw
    mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
    add r1.xyw, -r0.xyxz, r1.xyxw
    mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
    add r1.xyw, -r2.yzyw, v0.xyxz
    dp4 r6.x, r5.xyzw, r5.xyzw
    rsq r6.x, r6.x
    mul r5.xyzw, r5.xyzw, r6.xxxx
    add r6.xyz, r5.xyzx, r5.xyzx
    mul r7.xyz, r5.xzwx, r6.xxxx
    mul r6.w, r5.w, r6.y
    mul r8.xy, r5.wzww, r6.zzzz
    mad r6.z, r5.y, r6.y, r8.y
    add r9.x, -r6.z, l(1.000000)
    mad r10.x, r5.z, r6.x, -r6.w
    mad r9.y, r5.y, r6.x, -r8.x
    mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
    mad r8.z, r5.z, r6.y, -r7.z
    mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
    mov r9.z, r5.y
    dp3 r6.x, r1.xywx, r9.xyzx
    add r8.w, -r8.y, l(1.000000)
    dp3 r6.y, r1.xwyx, r8.xzwx
    mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
    dp3 r6.z, r1.xywx, r10.xyzx
    add r1.xyw, r2.yzyw, r6.xyxz
    add r0.xyz, r0.xyzx, r1.xywx
    mul r3.xyz, r0.xyzx, v4.xxxx
    ne r0.x, v4.y, l(0.000000)
    if_nz r0.x
      mov r4.x, v5.y
      ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
      ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
      ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
      ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
      add r6.xyzw, -r5.xyzw, r6.xyzw
      mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
      add r1.xyw, -r0.xyxz, r1.xyxw
      mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
      add r1.xyw, -r2.yzyw, v0.xyxz
      dp4 r6.x, r5.xyzw, r5.xyzw
      rsq r6.x, r6.x
      mul r5.xyzw, r5.xyzw, r6.xxxx
      add r6.xyz, r5.xyzx, r5.xyzx
      mul r7.xyz, r5.xzwx, r6.xxxx
      mul r6.w, r5.w, r6.y
      mul r8.xy, r5.wzww, r6.zzzz
      mad r6.z, r5.y, r6.y, r8.y
      add r9.x, -r6.z, l(1.000000)
      mad r10.x, r5.z, r6.x, -r6.w
      mad r9.y, r5.y, r6.x, -r8.x
      mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
      mad r8.z, r5.z, r6.y, -r7.z
      mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
      mov r9.z, r5.y
      dp3 r6.x, r1.xywx, r9.xyzx
      add r8.w, -r8.y, l(1.000000)
      dp3 r6.y, r1.xwyx, r8.xzwx
      mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
      dp3 r6.z, r1.xywx, r10.xyzx
      add r1.xyw, r2.yzyw, r6.xyxz
      add r0.xyz, r0.xyzx, r1.xywx
      mad r3.xyz, r0.xyzx, v4.yyyy, r3.xyzx
      ne r0.x, v4.z, l(0.000000)
      if_nz r0.x
        mov r4.x, v5.z
        ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
        ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
        ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
        ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
        add r6.xyzw, -r5.xyzw, r6.xyzw
        mad r5.xyzw, r1.zzzz, r6.xyzw, r5.xyzw
        add r1.xyw, -r0.xyxz, r1.xyxw
        mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
        add r1.xyw, -r2.yzyw, v0.xyxz
        dp4 r6.x, r5.xyzw, r5.xyzw
        rsq r6.x, r6.x
        mul r5.xyzw, r5.xyzw, r6.xxxx
        add r6.xyz, r5.xyzx, r5.xyzx
        mul r7.xyz, r5.xzwx, r6.xxxx
        mul r6.w, r5.w, r6.y
        mul r8.xy, r5.wzww, r6.zzzz
        mad r6.z, r5.y, r6.y, r8.y
        add r9.x, -r6.z, l(1.000000)
        mad r10.x, r5.z, r6.x, -r6.w
        mad r9.y, r5.y, r6.x, -r8.x
        mad r8.xy, r5.yxyy, r6.xxxx, r8.xyxx
        mad r8.z, r5.z, r6.y, -r7.z
        mad r5.xyz, r5.zwyz, r6.yyyy, r7.zyxz
        mov r9.z, r5.y
        dp3 r6.x, r1.xywx, r9.xyzx
        add r8.w, -r8.y, l(1.000000)
        dp3 r6.y, r1.xwyx, r8.xzwx
        mad r10.yz, r5.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
        dp3 r6.z, r1.xywx, r10.xyzx
        add r1.xyw, r2.yzyw, r6.xyxz
        add r0.xyz, r0.xyzx, r1.xywx
        mad r3.xyz, r0.xyzx, v4.zzzz, r3.xyzx
        ne r0.x, v4.w, l(0.000000)
        if_nz r0.x
          mov r4.x, v5.w
          ld_indexable(texture2d)(float,float,float,float) r5.xyzw, r4.xyzz, t2.xyzw
          ld_indexable(texture2d)(float,float,float,float) r6.xyzw, r4.xwzz, t2.xyzw
          ld_indexable(texture2d)(float,float,float,float) r0.xyz, r4.xyzz, t3.xyzw
          ld_indexable(texture2d)(float,float,float,float) r1.xyw, r4.xwzz, t3.xywz
          ld_indexable(texture2d)(float,float,float,float) r2.yzw, r4.xzzz, t1.wxyz
          add r4.xyzw, -r5.xyzw, r6.xyzw
          mad r4.xyzw, r1.zzzz, r4.xyzw, r5.xyzw
          add r1.xyw, -r0.xyxz, r1.xyxw
          mad r0.xyz, r1.zzzz, r1.xywx, r0.xyzx
          add r1.xyz, -r2.yzwy, v0.xyzx
          dp4 r1.w, r4.xyzw, r4.xyzw
          rsq r1.w, r1.w
          mul r4.xyzw, r1.wwww, r4.xyzw
          add r5.xyz, r4.xyzx, r4.xyzx
          mul r6.xyz, r4.xzwx, r5.xxxx
          mul r1.w, r4.w, r5.y
          mul r5.zw, r4.wwwz, r5.zzzz
          mad r6.w, r4.y, r5.y, r5.w
          add r7.x, -r6.w, l(1.000000)
          mad r8.x, r4.z, r5.x, -r1.w
          mad r7.y, r4.y, r5.x, -r5.z
          mad r9.xy, r4.yxyy, r5.xxxx, r5.zwzz
          mad r9.z, r4.z, r5.y, -r6.z
          mad r4.xyz, r4.zwyz, r5.yyyy, r6.zyxz
          mov r7.z, r4.y
          dp3 r5.x, r1.xyzx, r7.xyzx
          add r9.w, -r9.y, l(1.000000)
          dp3 r5.y, r1.xzyx, r9.xzwx
          mad r8.yz, r4.xxzx, l(0.000000, 1.000000, -1.000000, 0.000000), l(0.000000, 0.000000, 1.000000, 0.000000)
          dp3 r5.z, r1.xyzx, r8.xyzx
          add r1.xyz, r2.yzwy, r5.xyzx
          add r0.xyz, r0.xyzx, r1.xyzx
          mad r3.xyz, r0.xyzx, v4.wwww, r3.xyzx
        endif 
      endif 
    endif 
  endif 
endif 
ld_structured_indexable(structured_buffer, stride=24)(mixed,mixed,mixed,mixed) r0.xy, v6.x, l(8), t4.xyxx
and r0.x, r0.x, l(0x0fffffff)
ine r0.z, r0.y, l(0)
ieq r0.w, r0.w, l(0)
and r0.z, r0.w, r0.z
ine r0.w, r0.x, l(0)
and r0.z, r0.w, r0.z
movc r0.y, r0.z, r0.x, r0.y
iadd r0.x, r0.x, r0.y
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyzw, r0.x, l(0), t8.xyzw
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r4.xyzw, r0.x, l(16), t8.xyzw
ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.xyzw, r0.x, l(32), t8.xyzw
mov r5.x, r1.w
mov r5.y, r4.w
mov r5.z, r0.w
add r5.xyw, r5.xyxz, -cb7[12].xyxz
mov r3.w, l(1.000000)
mov r1.w, r5.x
dp4 r0.w, r3.xyzw, r1.xyzw
mov r4.w, r5.y
dp4 r1.x, r3.xyzw, r4.xyzw
mov r5.xyz, r0.xyzx
dp4 r0.x, r3.xyzw, r5.xyzw
mul r1.xyzw, r1.xxxx, cb7[1].xyzw
mad r1.xyzw, r0.wwww, cb7[0].xyzw, r1.xyzw
mad r0.xyzw, r0.xxxx, cb7[2].xyzw, r1.xyzw
add o5.xyzw, r0.xyzw, cb7[3].xyzw
ld_structured_indexable(structured_buffer, stride=384)(mixed,mixed,mixed,mixed) o1.x, r2.x, l(0), t0.xxxx
mov o1.yz, v1.xxyx
ret 
// Approximately 830 instruction slots used
