...
float3 scriptControl;
float3 tint1;
float3 tint2;
float3 tint3;
bool relativeHDR;
bool invertFalloff;
float uvMotionToggle1;
float4 scanlineConsts;
...
void ps_main( 
  ...
  float4 v4 : TEXCOORD1,  // Normal?
  float4 v5 : TEXCOORD2,  // Tangent?
  float3 v6 : TEXCOORD3,  // BiTangent?
  ...
  out float4 output : SV_TARGET0)
{
  ...
  r0.x = dot(-offposition.xyz, -offposition.xyz);
  r0.x = rsqrt(r0.x);
  r0.xyz = -offposition.xyz * r0.xxx;  // ObjSpaceViewDir?
  ...
  r1.xy = r1.xy * r0.ww;
  //r1.zw = float2(-0.5,-0.5) + iuv.xy;
  r1.xy = iuv.xy + r1.xy;
  
  
  // -------- USE_SCROLL_ROTATE_UVS --------
  ...
  r0.w = 1;  // Hard code exposure coef to 1 instead of using flicker
  
  
  // -------- USE_EMISSIVE_FLICKER --------
  /*r0.w = v3.x + v3.y;
  r0.w = v3.z + r0.w;
